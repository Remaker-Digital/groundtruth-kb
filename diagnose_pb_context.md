...
Generated: 2026-06-22T18:37:25Z
...
- GT-KB open MemBase work items: 25
- GT-KB dashboard-scoped bridge/contention entries, non-authoritative for queue state: 97
...
- Harness parity: pass (harness=all, role=loyal-opposition, PASS=120)
...
- Current work items: 3536; non-terminal: 354; active projects: 50; ungrouped non-terminal: 133.
- Status counts: deferred=1, not_a_defect=7, open=353, resolved=2946, retired=104, verified=64, wont_fix=61.
- Active project states:
  ...
  - `GTKB-RELIABILITY-FIXES`: 38 non-terminal (open=38); top: `WI-3454` - Auth-gate Requirement Sufficiency parser escapes pre-impl review surfaces [open, P2].
  - `May29 Hygiene`: 13 non-terminal (open=13); top: `WI-4738` - Bound workstream-focus startup relay dashboard summary read [open, P1].
  - `LO Advisory Routing`: 12 non-terminal (open=12); top: `WI-3330` - Route LO advisory: INSIGHTS-2026-05-15-14-35-skill-usage-advisory.md [open, P3].
  - `Activity-Envelope Disposition and Autonomous Dispatch`: 11 non-terminal (open=11); top: `WI-4683` - Reconcile the activity-vocabulary drift to one canonical closed set (re-admit ops) [open, P1].
  - `GTKB-BRIDGE-PROTOCOL-RELIABILITY`: 8 non-terminal (open=8); top: `WI-4680` - Make LO VERIFIED verdict recording atomic with final commit [open, P0].
  - `GTKB-LO-ADVISORY-INTAKE`: 7 non-terminal (open=7); top: `WI-4403` - Compact Summary Router Mode [open, P2].
  - `Harness Testing and Quality Benchmarking 1`: 7 non-terminal (open=7); top: `WI-4580` - Build isolated GT-KB benchmark fixture corpus with seeded flaws [open, P1].
  - `Backlog Triage and Hygiene`: 6 non-terminal (open=6); top: `WI-4741` - Build auto-retire-on-VERIFIED project automation: wire retirement actuation into VERIFIED-finalization + fix completion-detector archival-blindness [open, P2].
  ...
  - `Agent Disposition and Protocol Enforcement`: 2 non-terminal (open=2); top: `WI-4589` - Gate cloud deployment and external-service mutations behind owner-visible bridge authorization [open, P1].
  ...
  - `GTKB-DETERMINISTIC-SERVICES-001`: 2 non-terminal (open=2); top: `WI-3326` - Resolve phantom spec citations in SessionStart hook payload (ADR-SESSION-START-INIT-KEYWORD-CONTRACT-001, DCL-SESSION-START-INIT-KEYWORD-MATCHING-001) [open, P3].
  ...
  - `GTKB-ADOPTER-EXPERIENCE`: 1 non-terminal (open=1); top: `WI-3352` - Canonical end-to-end GT-KB lifecycle reference for new-agent orientation [open, P3].
  ...
  - `GTKB-GOVERNANCE-HARDENING`: 1 non-terminal (open=1); top: `WI-3268` - GTKB-CODEX-FEEDBACK-PATTERN-LINTS - pre-filing lint catching recurrent NO-GO mechanical patterns [open, P2].
