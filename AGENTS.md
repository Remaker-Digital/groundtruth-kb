...
do not substitute a recursive shell deletion. The native binding remains intact.

Apply owner choices to the affected canonical source or requested action. Session
logs retain the conversation for later harvest. Interpret requirements in the task's context and
apply material changes to current formal sources before dependent implementation.
Use the harness's supported question mechanism only for a material choice that
remains unresolved. Existing owner direction does not need per-artifact approval.
Read back the canonical result; do not create a classification, decision or
permissions archive for each message. Report a failed write with its recovery route.
