...
- Fresh-session startup must load `.agent/rules/canonical-terminology.md`
...
(per `DELIB-S324-PB-INTERROGATION-DIRECTIVE` and `.agent/rules/operating-model.md` §1)
(per `GOV-SOURCE-OF-TRUTH-FRESHNESS-001` v2 clause (a) and `.agent/rules/sot-read-discipline.md`)
...
mechanically enforced by `.agent/hooks/owner-decision-tracker.py` per `bridge/gtkb-gov-askuserquestion-enforcement-stack-slice-a-hook-reenable-014.md` VERIFIED.)
