...
# Project root resolution: prefer the ANTIGRAVITY_PROJECT_DIR env var (the harness
# sets this for hooks); fall back to walking up from this file's location.
PROJECT_ROOT = Path(os.environ.get("ANTIGRAVITY_PROJECT_DIR") or Path(__file__).resolve().parents[2]).resolve()
...
def _auto_archive_if_enabled(entry: DecisionEntry, session_hint: str = "") -> None:
    ...
    ``ANTIGRAVITY_PROJECT_DIR`` at hook startup), never ``Path.cwd()``. Failures
        ...
        if not existing:
            parts.extend(
                [
                    ...
                    "",
                    "This dated sidecar is written by .agent/hooks/owner-decision-tracker.py.",
...
This file is owned by .agent/hooks/owner-decision-tracker.py.
...
def _session_hint() -> str:
    ...
    Read from the ANTIGRAVITY_SESSION_ID env var if set; otherwise empty. The
    field is informational; no logic depends on it being populated.
    """
    return os.environ.get("ANTIGRAVITY_SESSION_ID", "")
