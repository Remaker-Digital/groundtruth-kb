...
deny record to ``.agent/hooks/scanner-safe-writer.log`` for the metrics
collector (Tier A #6).
...
``.agent/hooks/scanner-safe-writer.log``. Fields (``schema_version`` first):
...
DENY_LOG_PATH = Path(".agent/hooks/scanner-safe-writer.log")
SCHEMA_VERSION = 1
...
def main() -> None:
    ...
    reason = (
        ...
        "values before retrying. See .agent/hooks/scanner-safe-writer.log "
