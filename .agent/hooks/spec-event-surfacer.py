...
Per-session ledger at ``.agent/session/spec-events-seen.jsonl`` prevents
...
LEDGER_REL_PATH = ".agent/session/spec-events-seen.jsonl"
...
SESSION_START_REL_PATH = ".agent/session/session-start.json"
...
def _resolve_session_started_at(cwd: Path) -> tuple[str, bool]:
    ...
    Primary source: ``.agent/session/session-start.json`` (written by the
