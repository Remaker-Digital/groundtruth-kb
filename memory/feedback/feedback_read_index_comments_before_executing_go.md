---
name: Read dispatcher deferral markers before executing any GO
description: Before a capped-spawn or manual GO execution begins implementation, inspect current dispatcher/TAFE bridge state and the numbered thread chain for DEFERRAL MARKER blocks tagged to the document being dispatched. S302 oversight.
type: feedback
originSessionId: efade672-44a9-498c-95fb-fc8571e07b46
---
**Rule.** Before starting implementation on any bridge GO entry — whether
dispatched by a capped-spawn or acted on manually — inspect dispatcher/TAFE
bridge state and the numbered thread chain for comments or metadata containing
the document's slug AND a `DEFERRAL MARKER` or equivalent hold token. If one
exists, honor it: record an acknowledgment through the governed bridge path and
EXIT WITHOUT IMPLEMENTING. Only proceed after explicit owner re-authorization
...
instruction in the retired aggregate queue comments ("do NOT attempt 5-slice implementation
...
proposal directly, skipping the queue comments, and executed the full 5-slice
...
the then-current bridge source-of-truth contract was violated.

**How to apply.**

* On receiving any dispatch prompt that names a document slug + a GO/NO-GO
  file, inspect dispatcher/TAFE bridge state and the numbered thread chain
  before opening the GO file. Search for the document slug inside comments or
  metadata.
...
* If the owner's deferral signal is in `memory/work_list.md` instead of
  bridge state, treat it the same way — honor the deferral and surface it back
  through governed bridge evidence.
...
dispatcher/TAFE bridge state for deferral markers against the current dispatched slug and
...
* Retired aggregate queue entry: "DEFERRAL-MARKER OVERSIGHT ACKNOWLEDGED"
