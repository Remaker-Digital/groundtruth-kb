...
| 16 | `GENERATOR-HARDENING-001` | not-yet-filed (added 2026-04-27 S314) | Discovered by `GTKB-ISOLATION-016` Wave 2 Slice 11 (VERIFIED at `bridge/gtkb-isolation-016-phase8-wave2-slice11-016.md`). The dashboard generator `scripts/session_self_initialization.py` currently reads project state via the module-level `PROJECT_ROOT` global rather than threading the `--project-root` argument through every call site. Slice 11's audit-hook lane SURFACES the leaks; this hardening EFFICIENTLY ELIMINATES them. **Not blocking Wave 2 closure** (Slice 11 verified that the lane correctly reports `status='error'` when leaks exist) and **not blocking ISOLATION-018 cutover** (cutover can use a manually-prepared dashboard render until the generator is hardened, or skip dashboard regen on first cutover and harden as follow-on). Empirical evidence in commits `7c938058` (impl) + `7d621449` (REVISED-1) + `ceadd07b` (VERIFIED): `-013` log-only smoke captured 17 distinct violations; `-015` fail-closed smoke captured 1 (the first; subsequent 16 reads prevented by `os._exit(99)` termination). | Source-verified leak sites in `scripts/session_self_initialization.py`: lines 89-90 (`DEFAULT_DASHBOARD_DIR` / `DEFAULT_HISTORY_PATH` module constants bypass `--project-root`), line 646 (`.env.local` read in `_local_env_values()`), lines 1161/1182 (`git ls-remote` uses `PROJECT_ROOT` cwd — the live FAIL-CLOSED first-violation), line 3434 (`_load_pending_owner_decisions(PROJECT_ROOT)`), line 5232 argparse default. Plus user-home reads at `C:\Users\micha\.codex\agent-red-hooks\session-startup-preferences.json`, `operating-role.md`, `session-lifecycle-guard.json`, and matching `.claude\agent-red-hooks\`. Plus `C:\Temp\<sample_render>\.claude\session\work-subject.json` and `.claude\hooks\.workstream-focus-state.json` (workstream_focus state files written to subprocess CWD instead of `--project-root`). **Next:** file scoping bridge `bridge/generator-hardening-001-001.md` proposing per-call-site parameterization (replace literal `PROJECT_ROOT` with `project_root` arg threaded through the call chain; harness-config paths use a `--harness-config-root` arg with default to current home; subprocess `cwd=` paths use `project_root` not `PROJECT_ROOT`). Verification gate: re-run Slice 11 `--phase dashboard --execute` and expect `status='ok'` with `violations_count=0` and no quarantine. **Implementation owner:** if `session_self_initialization.py` is upstream-owned in `groundtruth-kb`, file scoping bridge there; if owned in Agent Red, file locally. Source-verify by checking whether the file is under `scripts/` (Agent Red) or `groundtruth_kb/` (framework) at scoping time. |
