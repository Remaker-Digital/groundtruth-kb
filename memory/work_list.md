...
| 18 | `GENERATOR-HARDENING-CROSS-REPO` | not-yet-filed (S315 round 5 surfaced as new program) | Codex NO-GO `bridge/generator-hardening-001-008.md` requires the remaining Slice 11 cross-repo subprocess violation (`_git_checkout_info` inspecting upstream `groundtruth-kb` checkout) to be covered by a dedicated, accepted bridge before GH-001 can be VERIFIED. Codex `-002` of GH-002 identified this as a co-design problem requiring BOTH the generator AND the audit-hook runner (`scripts/rehearse/_dashboard_regen_runner.py`) to share an allowlist contract. Single-layer fixes don't work because the runner enforces its own `build_is_allowed(legacy_root, sandbox_root)` policy independently. | File scoping bridge `bridge/generator-hardening-cross-repo-001.md` proposing: (a) `--allowed-cross-repo-roots` argparse arg added to BOTH the generator AND the dashboard-regen runner; (b) shared resolution logic so generator and runner agree on the same paths; (c) updated `_git_checkout_info` to skip-and-degrade when path is outside resolved allowlist; (d) Slice 11 lane proof showing both layers accept the same root. Required to unblock GH-001 close per Codex `-008`. |
| 17 | `GENERATOR-HARDENING-002` | scoping in flight; REVISED-1 (§B-only) NO-GO at `-004` (S315 round 5) | Continuation of GH-001 (commit `80e16ba8` reduced Slice 11 audit-hook violations 17→1). **§A removed from GH-002** per Codex `-002` recommendation (split B into own bridge); §A tracked separately as row 18 (GH-CROSS-REPO). REVISED-1 at `bridge/generator-hardening-002-003.md` scoped to §B only (Type F harness-home reads parameterization: 8 `Path.home()` sites at lines 94/107-108/111-112/1037-1038/1059). Codex `-004` NO-GO on REVISED-1: §B split is correct, but negative test contract too weak (monkeypatch logs+returns instead of fail-hard; fixed-line source-grep can shift after edits). | File REVISED-2 `bridge/generator-hardening-002-005.md` tightening test contract per Codex `-004` Required Revision: (1) Path.home() monkey-patch raises hard during `--harness-config-root <tmp>` runs except documented setup paths; (2) replace fixed-line source-grep with semantic AST-check or symbol-level grep that no Path.home() remains in the 3 builders + `_discover_skill_files` + `_plugin_inventory`; (3) positive proof asserts sentinel role-record path/content used, not just filename match. |
| 16 | `GENERATOR-HARDENING-001` | post-impl REVISED-1 NO-GO at `-008` (S315 round 5) — re-revision required | Thread: scoping `-001` → NO-GO `-002` → REVISED-1 `-003` → GO `-004` → impl commit `80e16ba8` → post-impl `-005` NEW → NO-GO `-006` → REVISED-1 of post-impl `-007` (gate-amendment) → **NO-GO `-008`**. Codex `-008`: gate amendment cited GH-002 `-003` as the follow-on for the remaining cross-repo violation, but GH-002 `-003` explicitly REMOVED §A from scope. The follow-on is now parked, not accepted — verification semantics of the amended gate fail. **Slice 11 audit-hook lane: still 17 → 1 violations (94% reduction)** but lane reports `status: error` not `status: ok`. Code at commit `80e16ba8` is correct; this is purely about verification status + follow-on traceability. | Three options per Codex `-008` Required Revision: (1) **File row-18 GH-CROSS-REPO bridge first**, get GO, then re-file GH-001 REVISED-2 of post-impl (the cleanest path because it produces the accepted follow-on the gate amendment requires); OR (2) implement cross-repo remediation now so Slice 11 reaches `violations: 0` (folds row 18 work into GH-001); OR (3) re-file GH-001 as partial-completion record explicitly NOT VERIFIED. **Recommendation: option 1**, treating row 18 as prerequisite for GH-001 close. |
