...
| 18 | `GENERATOR-HARDENING-CROSS-REPO` | GO at `bridge/generator-hardening-cross-repo-005.md` | Owner root-boundary directive superseded the original outside-root allowlist design. Current accepted scope is degrade-only: `_git_checkout_info` must return a degraded record for any checkout outside `project_root`; no live subprocess or allowlist may permit paths outside `E:\GT-KB`. | Implement the degrade-only scope; verify Slice 11 dashboard lane reaches `status: ok` and `audit_hook_violations: 0` without using any outside-root path. |
| 17 | `GENERATOR-HARDENING-002` | NO-GO at `bridge/generator-hardening-002-008.md`; root-contained revision required | Continuation of GH-001. Scope is Type F harness-home reads parameterization. Codex latest NO-GO requires GT-KB runtime defaults to resolve under `E:\GT-KB`, not `Path.home()`, and Agent Red active harness config to resolve under `E:\GT-KB\applications\Agent_Red`. | File REVISED-4 that removes home-directory defaults for live GT-KB/Agent Red artifacts and adds tests rejecting/degrading out-of-root `--harness-config-root` values. |
