...
Updated: 2026-04-27 (S314) — added GENERATOR-HARDENING-001 row from GTKB-ISOLATION-016 Wave 2 Slice 11 VERIFIED at `bridge/gtkb-isolation-016-phase8-wave2-slice11-016.md`; the audit-hook lane empirically surfaces ~17 PROJECT_ROOT-bound legacy reads in `scripts/session_self_initialization.py`. Earlier S314 entries: P1 of GTKB-STARTUP-ENHANCEMENTS VERIFIED at `bridge/gtkb-startup-enhancements-p1-006.md` (S309); GTKB-WRAPUP-ENHANCEMENTS row from session-wrap-up effectiveness evaluation.
...
| 14 | `GTKB-BRIDGE-POLLER-001` | scoping in flight; restructured into 2 threads at S315 (REVISED-3 of umbrella + standalone P1) | **S315 update:** Codex NO-GO at `-005` raised 5 required-revision items (split P1, add verification spike, add concurrency/isolation, preserve governance gates, rework cost analysis). Restructured: (a) standalone P1 detector/parser/checkpoint bridge filed at `bridge/gtkb-bridge-poller-p1-detector-001.md` NEW; (b) umbrella REVISED-3 at `bridge/gtkb-bridge-poller-001-smart-poller-006.md` adds §3 verification spike (proves `claude -p --bare` and `codex exec` semantics in disposable repo), §5 concurrency/isolation contract (file-locking, branch/worktree policy, collision handling), §6 governance preservation contract (mandatory hooks, credential safety, role contract), §7 reworked cost analysis (per-spawn startup tax separated from work cost; v1 with-hooks default ~1-3M tokens/day vs OLD halted poller's ~12.5M and v2 MCP's ~50-200k). Original goal unchanged: zero-LLM-cost detector + automatic harness invocation without owner-in-loop. Cost/benefit analysis embedded per `.claude/rules/bridge-essential.md` "Re-Enabling Pollers" gate. | Two parallel actionable threads. (1) Await Codex GO/NO-GO on standalone P1; on GO, file P1 implementation bridge upstream in `groundtruth-kb`. (2) Await Codex GO/NO-GO on umbrella REVISED-3; on GO, file P2 registry bridge + P2.5 verification-spike bridge in parallel; P3 invoker design gated on P2.5 evidence. Each subsequent phase as separate bridge. P7-P8 Agent Red adoption fires after upstream framework phases VERIFIED. |
...
| 16 | `GENERATOR-HARDENING-001` | scoping in flight (added 2026-04-27 S314; REVISED-1 filed S315) | Discovered by `GTKB-ISOLATION-016` Wave 2 Slice 11 (VERIFIED at `bridge/gtkb-isolation-016-phase8-wave2-slice11-016.md`). The dashboard generator `scripts/session_self_initialization.py` currently reads project state via the module-level `PROJECT_ROOT` global rather than threading the `--project-root` argument through every call site. Slice 11's audit-hook lane SURFACES the leaks; this hardening EFFICIENTLY ELIMINATES them. **Not blocking Wave 2 closure** (Slice 11 verified that the lane correctly reports `status='error'` when leaks exist) and **not blocking ISOLATION-018 cutover** (cutover can use a manually-prepared dashboard render until the generator is hardened, or skip dashboard regen on first cutover and harden as follow-on). Empirical evidence in commits `7c938058` (impl) + `7d621449` (REVISED-1) + `ceadd07b` (VERIFIED): `-013` log-only smoke captured 17 distinct violations; `-015` fail-closed smoke captured 1 (the first; subsequent 16 reads prevented by `os._exit(99)` termination). | Source-verified leak sites in `scripts/session_self_initialization.py`: lines 89-90 (`DEFAULT_DASHBOARD_DIR` / `DEFAULT_HISTORY_PATH` module constants bypass `--project-root`), line 646 (`.env.local` read in `_local_env_values()`), lines 1161/1182 (`git ls-remote` uses `PROJECT_ROOT` cwd — the live FAIL-CLOSED first-violation), line 3434 (`_load_pending_owner_decisions(PROJECT_ROOT)`), line 5232 argparse default. Plus user-home reads at `C:\Users\micha\.codex\agent-red-hooks\session-startup-preferences.json`, `operating-role.md`, `session-lifecycle-guard.json`, and matching `.claude\agent-red-hooks\`. Plus `C:\Temp\<sample_render>\.claude\session\work-subject.json` and `.claude\hooks\.workstream-focus-state.json` (workstream_focus state files written to subprocess CWD instead of `--project-root`). **S315 update:** scoping bridge `bridge/generator-hardening-001-001.md` filed NEW (commit `eb2cf3c2`); Codex NO-GO at `-002`; REVISED-1 filed at `bridge/generator-hardening-001-003.md` (commit `a1bd0c9a`); **Codex GO at `-004` (commit `3483e4b3`).** Implementation now unblocked. **Implementation owner: Agent Red local** (source-verified — no `groundtruth_kb/` package at this root). **Next session — IMPLEMENTATION QUEUED:** file `bridge/generator-hardening-001-impl-001.md` covering: (a) Type C signature changes (7 functions: lines 152, 157, 177, 196, 2483, 2533, 4862 of `scripts/session_self_initialization.py`) — remove `= PROJECT_ROOT` defaults, thread `project_root` through callers; (b) Type B parameterization (`_local_env_values()`, two `git ls-remote` cwd sites, plus `_local_env_value()` wrapper at line 659 per Codex Finding 2); drop `_LOCAL_ENV_CACHE`; (c) Type D fix at line 3434 (`render_report` → use model's project_root); (d) §4.6 argparse defaults change (`--dashboard-dir`/`--history-path` to `None`; post-parse derive from resolved `--project-root`); remove `DEFAULT_DASHBOARD_DIR`/`DEFAULT_HISTORY_PATH` module constants. Codex `-004` implementation constraints: (i) `PROJECT_ROOT` only as CLI fallback for `--project-root`, never internal; (ii) include public-CLI partial-arg regression test; (iii) source-verify external callers via `grep -rn "from session_self_initialization import\|from scripts.session_self_initialization import"` before removing Type C defaults. Verification gate: Slice 11 `python scripts/rehearse_isolation.py --phase dashboard --execute` returns `status=ok, violations_count=0` AND new test `test_main_with_only_project_root_writes_under_that_root` passes. Estimated work: ~60-90 min. |
