...
**S324 PRIORITY ELEVATION (2026-04-30 owner directive):** The GTKB-ISOLATION program is elevated to TOP priority. Sequence: **(1) ISOLATION-016** — Agent Red non-destructive migration rehearsal (actionable now; UNBLOCKED per Phase 7 Slice 1 VERIFIED; recommends fresh session); **(2) ISOLATION-017** — adopter packaging + clean-adopter validation (after 016 VERIFIED); **(3) ISOLATION-018** — Agent Red child-directory cutover (after 017 VERIFIED; requires explicit owner approval for migration window); **(4) ISOLATION-019** — program closeout + cleanup (after 018 VERIFIED). Parallel future work: **ISOLATION-015 Slice 2** (typed `work_subject.set/rollback` control-plane operations) does NOT block the 016→019 sequence. All other actionable items below (ADR-eval program S0–S6, helper-parity REVISED-2, MEMBASE Slice A, Prime-classification refinement, etc.) defer beneath the ISOLATION program.
