...
| 17 | `GENERATOR-HARDENING-002` | scoping in flight (filed 2026-04-27 S315) | Continuation of GH-001 (commit `80e16ba8` reduced Slice 11 audit-hook violations 17→1). Two independent sub-features bundled in `bridge/generator-hardening-002-001.md`: §A cross-repo subprocess sandbox awareness (the remaining 1 violation from `_git_checkout_info` inspecting upstream `groundtruth-kb` checkout) — proposed fix: `--allowed-cross-repo-roots` argparse + sandbox-aware degradation; §B Type F harness-home reads parameterization (8 source-verified `Path.home()` sites: lines 94, 107-108, 111-112, 1037-1038, 1059) — proposed fix: `--harness-config-root` argparse arg + module constants → functions. Verification gate: Slice 11 dashboard lane reports `violations_count: 0` AND new tests `test_upgrade_posture_skips_checkout_outside_allowlist` + `test_main_with_harness_config_root_uses_that_root_not_home` pass. Codex may GO §A and §B independently. **Implementation owner: Agent Red local** (same file as GH-001). | Awaiting Codex GO/NO-GO. On GO: implement per scoping (~30-45 min); file post-impl with Slice 11 final-zero-violations evidence; followup adopter commit updates `.claude/settings.json` to add `--allowed-cross-repo-roots` to SessionStart hook command. |
| 16 | `GENERATOR-HARDENING-001` | **DONE — post-impl filed at `-005` 2026-04-27 S315; awaits Codex VERIFIED** | Discovered by Slice 11 audit-hook lane (VERIFIED at `bridge/gtkb-isolation-016-phase8-wave2-slice11-016.md`). Thread: scoping `-001` NEW → Codex NO-GO `-002` → REVISED-1 `-003` → Codex GO `-004` → implementation commit `80e16ba8` → post-impl `-005` NEW. **Slice 11 audit-hook lane: 17 → 1 violations** (94% reduction). The remaining 1 is a different leak class (cross-repo subprocess) — covered by row 17 (GH-002 §A). 34/34 focused tests pass; lint clean; format applied. New regression test `test_main_with_only_project_root_writes_under_that_root` would have caught the original Codex `-002` finding. **Implementation owner: Agent Red local** (source-verified; no `groundtruth_kb/` package at this root). | When Codex VERIFIED at `-006`: move row to "Completed during current session" at next session-wrap. Type F harness-home reads + cross-repo subprocess remediation tracked separately as row 17 (GH-002). |
