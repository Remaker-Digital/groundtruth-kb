...
1. ~~**ISOLATION-017 Slice 4** — `gt project upgrade` isolation + migration/rollback behavior.~~ **DONE — VERIFIED 2026-05-03 S328** at `bridge/gtkb-isolation-017-slice4-upgrade-2026-05-02-012.md`. 4-NO-GO/REVISED cycle (`-002`/`-004`/`-006`/`-010`) closing terminally with reclassification of check #5 (`isolation:hooks-point-to-wrappers`) from auto-fixable → needs-adopter-input. Final partition: 1 hard-refuse + 4 auto-fixable + 4 needs-adopter-input = 9. IPR-SLICE4-UPGRADE-ISOLATION-001 v1 + CVR-SLICE4-UPGRADE-ISOLATION-001 v1 inserted to MemBase per GOV-20.
2. **ISOLATION-017 Slice 5** — clean-adopter test suite + fixtures. The real productization proof: a fresh install behaves correctly from an adopter root without GT-KB product leakage. **GO'd at `bridge/gtkb-isolation-017-slice5-clean-adopter-tests-2026-05-03-004.md` (Codex unconditional GO 2026-05-03 S328).** REVISED-1 carries owner-approved partial scope: stale-detection retained; refresh + disposability deferred to Slice 5.5 (row 31) per `DELIB-S328-ISOLATION-017-SLICE5-OVERLAY-SCOPE-REVISION-OWNER-DIRECTIVE` v1. **IMPLEMENTATION IN-FLIGHT; PAUSED AT WRAP S328.** Resume point: `groundtruth-kb/tests/adopter/conftest.py` is written + import-verified clean (exports `_clean_adopter_factory`, `_existing_adopter_factory`, `_setup_git`, `_make_synthetic_product_root`, `clean_adopter` pytest fixture). Next-session work: (a) implement 10 named tests under `tests/adopter/test_*.py` per Phase 9 §5 spec-to-test table in `-003`; (b) capture 3 migration fixtures under `tests/fixtures/adopter/{pre_isolation_minimal,pre_isolation_with_managed_drift}/` (the `pre_isolation_under_product_root` is constructed inline by the conftest); (c) `test_golden_fixture_diff_per_version.py`; (d) `test_existing_adopter_migration_kit.py`; (e) `test_overlay_stale_detection.py` (F1-fix wrap of Slice 1 check #9); (f) CI documentation comment in `groundtruth-kb/.github/workflows/ci.yml`; (g) run `uv run pytest groundtruth-kb/tests/adopter/` + `python -m pytest groundtruth-kb/tests/` + ruff; (h) insert IPR-SLICE5-CLEAN-ADOPTER-TESTS-001; (i) file post-impl at `-005`. Estimated remaining envelope: ~950 LOC tests + 3 fixture trees + ~10 LOC CI comment.
