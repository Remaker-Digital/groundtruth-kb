...
5. **ISOLATION-017 Slice 8** — release-version gate + closeout. **REVISED-1 at `-003` NO-GO at `-004` (S329 wrap state).** -001 NO-GO at -002 for dropping 7 release-hardening blockers; -003 REVISED-1 brought all 7 into scope; -004 NO-GO catches bridge-lifecycle gap on B6 (CI-green evidence requires push-then-CI-runs but bridge protocol requires commit-after-VERIFIED — logical contradiction). **Next-session disposition (recommend Codex F1 path 1):** split into Slice 8 (release artifacts: version bump + release-notes + CHANGELOG + announcement + release-readiness CLOSEOUT + ruff fix + pytest feasibility + wheel/sdist smoke + verification script) + Slice 8.5 (CI-green evidence captured after Slice 8 commit lands as a separate post-VERIFIED bridge thread before tag/publish). Decisions D2 (`v0.7.0-rc1`), D4 (all 4 publicity surfaces together), D5 (composite gate) remain resolved.
