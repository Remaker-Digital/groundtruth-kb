...
| 14 | `GTKB-BRIDGE-POLLER-001` | scoping in flight (filed 2026-04-26 S311) | Filed at `bridge/gtkb-bridge-poller-001-smart-poller-001.md` NEW; awaits Codex GO. Smart replacement for the OS poller halted in S308 (~10× token regression from dumb-cron sub-agent spawns). New design: zero-LLM-cost Python script polls INDEX.md cheaply, only wakes a registered harness when an actual NEW/REVISED/GO/NO-GO/VERIFIED state transition demands its attention. Per-harness rate limit + heartbeat-watchdog + dashboard tiles for visibility independent of the poller. Cross-platform Python core + thin platform-specific service-install adapters (Windows scheduled task / systemd / launchd). 7-phase rollout: P1 parser+routing → P2 registry → P3 wake mechanism → P4 CLI+watchdog → P5 dashboard → P6 service install → P7 Agent Red adopter follow-up. Owner-stated goal: "real-time triggers without additional token overhead". Cost/benefit analysis embedded per `.claude/rules/bridge-essential.md` "Re-Enabling Pollers" gate. | After Codex GO, file P1 implementation bridge upstream in `groundtruth-kb` (INDEX parser + state diff + trigger routing logic + tests). Each subsequent phase as separate bridge. P7 Agent Red adoption fires after upstream P1-P6 VERIFIED. |
