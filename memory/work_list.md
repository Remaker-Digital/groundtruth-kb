...
- S330/S331 advisory/governance captures (do not advance unless owner explicitly elevates): `GTKB-AI-ASSISTED-DELIVERY-MATURITY-MODEL`, `GTKB-ENV-INVENTORY-001`, `GTKB-SYSTEMS-TERMINOLOGY-MAP-001`, and `GTKB-RESOURCE-REFERENCE-DISAMBIGUATION-001`. These are recorded so Prime Builder can comment and later route them through the standard bridge lifecycle without losing the decisions from these sessions.
...
| 13 | `GTKB-DB-BACKUP-001` | scoping in flight (filed 2026-04-26 S311) | Filed at `bridge/gtkb-db-backup-001-snapshot-daemon-001.md` NEW; awaits Codex GO. Capability: `gt db snapshot` framework command that runs `VACUUM INTO <output>/groundtruth-{ts}.db` + integrity_check, writes audit manifest, rotates with retention policy. Adopter-configured cadence (cron / SyncBackSE pre-script / hook) — not a framework-scheduled daemon. Motivated by S311 KB recovery: both Drive + SyncBackSE faithfully copy corrupted state, so the safety nets amplify failure rather than catch it. Snapshot+integrity-check pipeline catches live-DB corruption at snapshot time and prevents downstream backups from inheriting it. | After Codex GO, file implementation bridge upstream in `groundtruth-kb` (`groundtruth_kb/cli/db_snapshot.py` + tests + docs). After upstream VERIFIED, Agent Red adopts via `gt project upgrade` + adopter-side follow-up to point SyncBackSE `CLAUDE-COPY` profile at the snapshot dir instead of live `E:\GT-KB\`. |
...
| 29 | `GTKB-OWNER-DECISION-TRACKER-REGEX-TIGHTENING` | **owner-directed 2026-05-02 S328; deferred under feature freeze** | Owner directive S328 (after observed pattern): the `prose:awaiting_input` and `prose:offering_or_choice` patterns in `.claude/hooks/owner-decision-tracker.py` (`PROSE_DECISION_PATTERNS`) match too liberally on factual status statements (e.g., the word "Awaiting" in "Awaiting Codex re-review" — a factual report — triggers the same detector as "Awaiting your direction" — a genuine owner question). Empirical evidence S328: 3 false positives in one session (DECISION-0319, DECISION-0321, DECISION-0327), plus one recursive Stop-hook firing where the resolution prose itself re-triggered the detector when discussing the false positive. **Proposed direction:** tighten the regex to require an interrogative marker or 2nd-person object after `Awaiting` (e.g., require `your direction\?` or `for your`); add a guard that the detector does NOT match against text describing the detector itself (e.g., paragraphs whose surrounding context mentions "regex" or "PROSE_DECISION_PATTERNS"). The existing T14 false-positive guard infrastructure is the right insertion point. **Sequencing:** **DEFERRED under freeze** per `memory/work_list.md` TOP directive ("No new governance scope work until ISOLATION-017 Slice 8 VERIFIED"). Hook tuning is governance-scope (changes mechanical enforcement contract). For the remainder of S328, the Stop hook block is suppressed via `GTKB_BLOCK_ON_PROSE_DECISION_ASK=0` in `.claude/settings.local.json::env`; detection + durable-file writes continue. Re-enables next session by default. Workaround discipline captured in `memory/feedback_avoid_quoting_decision_tracker_fragments.md`. | When unblocked: file scoping bridge `bridge/gtkb-owner-decision-tracker-regex-tightening-001.md` proposing the tightened regex + T14-class guard extension + tests proving (a) factual "Awaiting X" status statements no longer match, (b) genuine owner-facing prose questions still match, (c) recursive self-triggering on agent text describing the detector is suppressed. After Codex GO + impl + VERIFIED, remove the `GTKB_BLOCK_ON_PROSE_DECISION_ASK=0` env-var override from `.claude/settings.local.json`. No spec promotion at row creation; spec promotion (e.g., extending `GOV-OWNER-DECISION-SURFACING-001`) happens at implementation-bridge filing per `GOV-CHAT-DERIVED-SPEC-APPROVAL-001`. |
...
| 34 | `GTKB-SYSTEMS-TERMINOLOGY-MAP-001` (Canonical artifact/interface names and startup operating surface map) | **owner-directed 2026-05-03 S330; backlog addition approved** | Owner identified recurring agent confusion around terms that name specific systems or artifacts, especially ambiguous terms like "backlog." Existing backlog coverage is partial: `GTKB-GOV-BACKLOG-SOURCE-OF-TRUTH` handles backlog consolidation, `GTKB-STARTUP-REFRACTOR-001` handles startup mechanics, and `GTKB-ENV-INVENTORY-001` handles harness/dev-environment inventory. This item fills the cross-cutting gap: a canonical map of routine GT-KB artifacts/interfaces, their accepted alternate names, authority, locations, access methods, mutation permissions, generated-vs-authoritative status, and startup presentation. **First reconciliation case:** resolve the current "backlog" contradiction between `memory/work_list.md` as live operational backlog and future MemBase `backlog_items` authority. | Next when unblocked or elevated: file `bridge/gtkb-systems-terminology-map-001.md` proposing glossary extensions, a compact startup "GT-KB Systems and Tools" section for PB/LO, doctor/tests proving common owner terms resolve to correct artifacts, and integration with backlog source-of-truth and startup-refactor work. |
| 39 | `GTKB-RESOURCE-REFERENCE-DISAMBIGUATION-001` (External resource identity registry and confusion audit) | **owner-directed 2026-05-04 S331; advisory captured as input to further deliberation; backlog addition approved** | Owner directed that all external resources be mapped to URLs and/or identities so AI agents reliably interpret user input and historical references, avoiding duplicate or erroneous resources becoming entangled. Codex Loyal Opposition created seed registry `.claude/rules/project-resource-aliases.toml`, companion `memory/project_external_resource_registry.md`, and advisory report `independent-progress-assessments/CODEX-INSIGHT-DROPBOX/RESOURCE-REFERENCE-CONFUSION-CANDIDATES-2026-05-04.md`. Report identifies top confusion candidates: "the GitHub"/"repo", CI/GitHub Actions, README badges/wiki/issues, Azure/subscription/production, SonarCloud/quality gate, PyPI/package, project/application/platform, KB/MemBase, docs site/wiki/issues, and upstream/local source. **Sequencing:** deliberation-input track; do not implement during v0.7.0-rc1 release path unless owner elevates. **Coupling:** overlaps `GTKB-SYSTEMS-TERMINOLOGY-MAP-001`, `GTKB-ENV-INVENTORY-001`, startup refactor, release-readiness gates, and resource URL/identity doctor checks. | Next when unblocked or elevated: conduct a formal deliberation using the Codex report as input; decide whether to promote `.claude/rules/project-resource-aliases.toml` into a governed GT-KB artifact, add doctor/startup checks, scan bridge/release/README surfaces for unqualified resource terms, verify/pin SonarCloud and docs-site identities, and supersede or repair stale Agent Red-bound GT-KB release references. |
| 35 | `AGENT-RED-RUFF-CLEANUP-001` (Application-side ruff resolution; Agent Red product code) | **deferred from Slice 8 B2 narrowing per `DELIB-S330-ISOLATION-017-SLICE8-B2-RUFF-SCOPE-CHOICE` (2026-05-03 S330)** | Slice 8's B2 (full-repo ruff resolution) was narrowed to `groundtruth-kb/` only because the ruff baseline probe found 1,943 issues in Agent Red product code (`tests/`: 1,076; `scripts/`: 824; `src/`: 15; misc: 28) versus 29 in `groundtruth-kb/`. Per the operating-model platform/application distinction, Agent Red application cleanup belongs to Agent Red's own release-hardening arc, not v0.7.0-rc1's GT-KB platform release. **Distribution by rule (top 10):** I001 (886, unsorted-imports, auto-fixable), UP017 (264, datetime-timezone-utc, auto-fixable), F541 (241, f-string-missing-placeholders, auto-fixable), F401 (104, unused-import, auto-fixable), SIM117 (88, multiple-with-statements, auto-fixable), E402 (87, module-import-not-at-top-of-file, manual), B007 (55, unused-loop-control-variable, manual), UP015 (45, redundant-open-modes, auto-fixable), F841 (36, unused-variable, manual), E401 (22, multiple-imports-on-one-line, auto-fixable). 1,653 of 1,943 (85%) auto-fixable; 290 require manual judgment. **Sequencing:** non-blocking for v0.7.0-rc1; appropriate to file as part of Agent Red's release readiness work, not GT-KB platform releases. **Risk:** Some auto-fixes (esp. F541 f-string removal in test files) may have unintended behavioral side-effects in product test code; needs review of auto-fix diff before commit. | Next when picked up: file `bridge/agent-red-ruff-cleanup-001-001.md` proposing the cleanup scope. Two-phase approach recommended: Phase A — `ruff check . --fix` to apply 1,653 mechanical fixes in a single commit (review diff carefully before commit, esp. F541 changes); Phase B — manual triage of 290 remaining issues (E402, B007, F841, etc.) with per-file judgment calls. After Codex GO + impl + VERIFIED, the GT-KB-side B2 verifier scope can be expanded back to full-repo if desired. |
...
| **P7 Decision-tracker false-positive guard tightening** | Quotation-aware + code-fence-aware guards. Closes the DECISION-0001/0002 false-positive class: doc-paragraphs describing the detector should not trigger the detector. | reliability | low |
