...
**Default idle work directive 2026-05-06:** when Mike has not supplied a different session task, both Prime Builder and Loyal Opposition default to this revised release plan. Loyal Opposition reviews or verifies the highest-priority `NEW`/`REVISED` bridge entry in this plan; Prime Builder revises `NO-GO` entries or implements `GO` entries in the same order. Order: (1) `GTKB-SECRETS-PURGE-AND-COMMIT-ENFORCEMENT`; (2) `GTKB-ISOLATION-017` Slice 8.6, then Slice 8.5, then the `v0.7.0-rc1` release lane; (3) `GTKB-ISOLATION-018`; (4) `GTKB-ISOLATION-019`; (5) post-rc/GA release hardening such as platform CI coverage, evaluation-module waiver retirement, and pip-installed adopter UX unless Mike explicitly elevates another item.
