...
- S330 advisory/governance captures (do not advance unless owner explicitly elevates): `GTKB-AI-ASSISTED-DELIVERY-MATURITY-MODEL`, `GTKB-ENV-INVENTORY-001`, and `GTKB-SYSTEMS-TERMINOLOGY-MAP-001`. These are recorded so Prime Builder can comment and later route them through the standard bridge lifecycle without losing the decisions from this session.
