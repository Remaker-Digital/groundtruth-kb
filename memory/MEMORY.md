...
- **S330 NEXT SESSION QUEUE (3 bridge threads to file):** (1) `bridge/gtkb-isolation-018-agent-red-file-migration-001.md` — actual move of Agent Red files from GT-KB root to `applications/Agent_Red/`, Agent Red repo migration to `mike-remakerdigital/agent-red`; (2) `bridge/gtkb-cross-cutting-requirements-registry-001.md` — implements DCL-CROSS-CUTTING-REQUIREMENTS-REGISTRY-001 + the registry-aware PreToolUse hook + Codex review extension; (3) `bridge/gtkb-requirements-collection-hook-001.md` — implements IPR-REQUIREMENTS-COLLECTION-HOOK-001 (UserPromptSubmit hook + LLM classifier + tiered retrieval + 3-option presentation + DELIB capture). Each spawns its own approval packets and downstream CVRs at VERIFIED time.
