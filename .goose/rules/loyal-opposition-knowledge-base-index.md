...
## Active Rules under `.goose/rules/`
...
- `.goose/rules/canonical-terminology.md` — core primer subset only at startup;
  full corpus and activity-specific terms load on `::open <activity>`.
- `.goose/rules/file-bridge-protocol.md` — bridge statuses, gates, claim/preflight.
...
`.goose/skills/` adapters are generated from the canonical baseline skill source; no separate adapter
