...
This rule is auto-loaded via `.goose/rules/` convention.
...
(per `.goose/rules/deliberation-protocol.md`)
...
2. **Prime reads the advisory** as part of the standard Loyal-Opposition-insight review path defined in `.goose/rules/way-of-working.md`.
...
3. What owner decisions must be durable, recorded via `AskUserQuestion`
   per `.goose/rules/prime-builder-role.md` § "AskUserQuestion as the
...
Decisions / Input` section (per `.goose/rules/file-bridge-protocol.md`
...
protected paths (e.g., `.goose/rules/*.md`, `AGENTS.md`, the root harness instruction file)
