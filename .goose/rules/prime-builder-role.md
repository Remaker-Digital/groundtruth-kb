...
- Fresh-session startup must load `.goose/rules/canonical-terminology.md`
...
(per `DELIB-S324-PB-INTERROGATION-DIRECTIVE` and `.goose/rules/operating-model.md` §1)
(per `GOV-SOURCE-OF-TRUTH-FRESHNESS-001` v2 clause (a) and `.goose/rules/sot-read-discipline.md`)
...
mechanically enforced by `.goose/hooks/owner-decision-tracker.py` per `bridge/gtkb-gov-askuserquestion-enforcement-stack-slice-a-hook-reenable-014.md` VERIFIED.)
