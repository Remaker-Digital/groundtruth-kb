...
(Active per Sub-slice C of GTKB-GOV-AUQ-ENFORCEMENT-STACK; mechanically enforced by `.goose/hooks/bridge-compliance-gate.py`.)
...
The bridge-propose helper at scripts/skill-helpers/bridge-propose/write_bridge.py pre-populates this section by default via pre_populate_prior_deliberations: glossary-source seeding from .goose/rules/canonical-terminology.md plus optional semantic search. Authors review and prune the pre-populated entries.
