...
This rule auto-loads via `.goose/rules/` convention and is TRACKED in git
(negated from the `.goose/` blanket ignore). Do not remove it.
...
Loyal Opposition bridge scans are similarly owner-triggered in the reviewing harness.

The 2026-04-25 OS-poller halt was made after the former OS poller
(activated ~2026-04-23) was found to fire on a fixed interval regardless
of bridge activity: 173 capped-spawns/day at peak for one harness plus 92 counterpart
...
- AXIS 1 (both directions between counterpart harnesses when no interactive session is
...
- AXIS 2 prompt-driven surface: the `.goose/hooks/bridge-axis-2-surface.py`
...
- Skip the GO/NO-GO discipline for any code change beyond the explicit
  exemptions in `.goose/rules/counterpart-review-gate.md`
...
- **S292**: Session-start briefs for 7+ sessions silently omitted the in-session
  ...
  in `memory/*.md` are not enforceable; hooks and `.goose` hook
  registration are.
- **S294**: Discovered `.goose/` was blanket-ignored in `.gitignore`, so the
