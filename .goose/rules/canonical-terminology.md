...
authoritative source. Sources include `.goose/rules/operating-model.md` §2
...
templates (CLAUDE.md, MEMORY.md, `.goose/rules/canonical-terminology.md`,
...
`.goose/rules/file-bridge-protocol.md`.
...
`.goose/rules/file-bridge-protocol.md` Mandatory Specification-Derived
Verification Gate.
...
**Source:** bridge-state surfaces; `.goose/rules/file-bridge-protocol.md`
...
`.goose/rules/canonical-terminology.md` by
...
narrative authority `.goose/rules/sot-read-discipline.md`;

**Implementation pointer:** `.goose/hooks/sot-read-discipline.py` (canonical hook) plus per-harness command-verb adapters registered in each harness's hook surface; doctor `_check_sot_read_discipline`; owner-authorized bypass `GTKB_SOT_READ_DISCIPLINE_BYPASS`.
...
**Implementation pointer:** `config/registry/sot-artifacts.toml` (`forbidden_substitutes` entries); enforced by `.goose/hooks/sot-read-discipline.py`.
...
(`.goose/hooks/bridge-compliance-gate.py`) that fails the Write of bridge
...
`.goose/hooks/credential-scan.py` (general-purpose, all Write/Edit targets)
and `.goose/hooks/scanner-safe-writer.py` (bridge-scoped, Write-only for
...
(`.goose/hooks/owner-decision-tracker.py`) that detects prose decision-ask
...
`.goose/rules/operating-model.md`. Carries rule-cited soft authority:
cited by `.goose/rules/loyal-opposition.md` and `AGENTS.md` as the
...
work subject is recorded in `.goose/session/work-subject.json` and is set
...
`.goose/session/active-session-role.json` and per-session `role-*.json` files
...
purged); `.goose/rules/bridge-essential.md` § Operational Mode.
...
without writing `.goose/session/active-session-role.json`.
...
**Implementation pointer:** `.goose/hooks/*` init/ wrap/ topic-router surfaces;
...
protected narrative artifacts at `.goose/rules/*.md`, `AGENTS.md`,
...
`.goose/rules/canonical-terminology.toml` for the required-terms matrix.
