...
the canonical `.claude/skills/` tree and be projected into Codex through the
...
- Codex consumes generated adapters under `.codex/skills/<skill-slug>/SKILL.md`.
...
- `.codex/skills/MANIFEST.json` is generated metadata for Codex adapter load and
  drift checks.
...
2. Create or update `.claude/skills/<skill-slug>/SKILL.md` with valid
...
6. Ensure `.codex/skills/MANIFEST.json` includes the new adapter record with the
...
- `.codex/skills/<slug>/SKILL.md` carries the
  `GTKB-CODEX-SKILL-ADAPTER` generated block and records the same source path
  and SHA as the manifest and registry.
- `.codex/skills/MANIFEST.json` has exactly one adapter entry for the capability
  id.
...
Generated at: content-addressed e30a2541f404
Canonical source: .harness-baseline-configuration/skills/gtkb-skill-governance-lifecycle/SKILL.md
