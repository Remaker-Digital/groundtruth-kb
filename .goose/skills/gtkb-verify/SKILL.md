...
Verification Gate** in `.goose/rules/file-bridge-protocol.md`. It scaffolds
...
6. Run a deliberation search per `.goose/rules/deliberation-protocol.md`
...
11. For a positive `VERIFIED` verdict, run the reviewed final body through the
    ...
    ```text
    python scripts/skill-helpers/gtkb-verify/write_verdict.py --slug <slug> --body-file <reviewed-verdict-body> --finalize-verified --no-prepopulate --commit-message "<type(scope): message>" --include <verified-path> [--include <verified-path> ...]
    ```
...
- `## Findings` — for a `NO-GO` verdict only; each finding structured per
  `.goose/rules/report-depth.md` and
  `.goose/rules/report-depth-prime-builder-context.md` (observation,
...
Generated at: content-addressed 0c8b939bcfca
Canonical source: .harness-baseline-configuration/skills/gtkb-verify/SKILL.md
Canonical source sha256: 0c8b939bcfca967888c33f96f9a2d5b57dd122beffad00b46d99fe1c2eafaced
