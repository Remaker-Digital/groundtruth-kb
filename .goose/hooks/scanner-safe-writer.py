...
deny record to ``.goose/hooks/scanner-safe-writer.log`` for the metrics
collector (Tier A #6).
...
``.goose/hooks/scanner-safe-writer.log``. Fields (``schema_version`` first):
...
DENY_LOG_PATH = Path(".goose/hooks/scanner-safe-writer.log")
SCHEMA_VERSION = 1
...
def main() -> None:
    ...
    reason = (
        ...
        "values before retrying. See .goose/hooks/scanner-safe-writer.log "
