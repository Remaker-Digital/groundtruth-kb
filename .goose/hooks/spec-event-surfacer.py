...
Per-session ledger at ``.goose/session/spec-events-seen.jsonl`` prevents
...
LEDGER_REL_PATH = ".goose/session/spec-events-seen.jsonl"
...
SESSION_START_REL_PATH = ".goose/session/session-start.json"
...
def _resolve_session_started_at(cwd: Path) -> tuple[str, bool]:
    ...
    Primary source: ``.goose/session/session-start.json`` (written by the
