...
deny record to ``.codex/hooks/scanner-safe-writer.log`` for the metrics
collector (Tier A #6).
...
``.codex/hooks/scanner-safe-writer.log``. Fields (``schema_version`` first):
...
DENY_LOG_PATH = Path(".codex/hooks/scanner-safe-writer.log")
SCHEMA_VERSION = 1
...
def main() -> None:
    ...
    reason = (
        ...
        "values before retrying. See .codex/hooks/scanner-safe-writer.log "
