...
SPEC_LINK_TOKEN_RE = re.compile(
    ...
    r"|(?:^|[`(\s])(?:.codex/rules|groundtruth-kb/docs|docs|bridge)/[^\s`)]+",
...
# .codex/rules/file-bridge-protocol.md section "Mandatory Implementation-Start
...
AUDIT_OUTPUT_RELATIVE_PATH = Path(".codex/hooks") / "last-bridge-audit.json"
...
def _record_gate_denial(pattern_id: str, subject: str, reason: str, *, root: Path | None = None) -> None:
    ...
    base = root or Path(os.environ.get("CODEX_PROJECT_DIR") or os.getcwd()).resolve()
...
def _ancestor_or_self(root: Path, cwd_path: Path) -> bool:
    ...
    under ``<canonical>/.codex/worktrees/``. A resolver result that is neither
...
def _is_under_harness_worktrees(path: Path) -> bool:
    ...
    return any(left == ".codex" and right == "worktrees" for left, right in zip(parts, parts[1:], strict=False))
...
def _canonical_project_root(cwd_path: Path) -> Path:
    ...
    root. A session running inside a ``.codex/worktrees/*`` linked worktree has
...
def _bridge_work_intent_deny_reason(*, cwd_path: Path, file_path: str, payload: dict) -> str | None:
    ...
    if holder is None:
        return (
            ...
            "Per .codex/rules/file-bridge-protocol.md 'Mandatory Pre-Drafting Claim Step', run: "
...
# reliable routing signal. See .codex/rules/file-bridge-protocol.md
...
def _requirement_sufficiency_section_gap(content: str) -> str | None:
    ...
    Operative states per .codex/rules/file-bridge-protocol.md section "Mandatory
...
def _verdict_preflight_freshness_deny_reason(
    *,
    ...
    # DELIB-HARNESS-NO-ACTION-LO-RESPONSE-SET-20260702, .codex/rules/counterpart-review-gate.md
...
def _no_action_prior_verdict_deny(file_path: str, content: str) -> str | None:
    ...
    return (
        ...
        "DCL-NO-ACTION-STATUS-SEMANTICS-001; see .codex/rules/file-bridge-protocol.md section "
        "'NO-ACTION Status'.)"
    )
    ...
    if _is_bridge_markdown_file(file_path) and content:
        if _body_status_token_violation(file_path, content):
            return (
                ...
                "body-status-token rule; see .codex/rules/file-bridge-protocol.md "
        ...
        if (
            ...
            return (
                ...
                ".codex/rules/counterpart-review-gate.md.)"
            )
        ...
        if (
            ...
            if req_suff_gap:
                return (
                    ...
                    "'New or revised requirement required before implementation'). "
                    f"Gap: {req_suff_gap}. "
                    "(Hard-block per .codex/rules/file-bridge-protocol.md "
