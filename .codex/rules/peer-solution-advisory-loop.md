...
This rule is auto-loaded via `.codex/rules/` convention.
...
(per `.codex/rules/deliberation-protocol.md`)
...
2. **Prime reads the advisory** as part of the standard Loyal-Opposition-insight review path defined in `.codex/rules/way-of-working.md`.
...
3. What owner decisions must be durable, recorded via `AskUserQuestion`
   per `.codex/rules/prime-builder-role.md` § "AskUserQuestion as the
...
Decisions / Input` section (per `.codex/rules/file-bridge-protocol.md`
...
protected paths (e.g., `.codex/rules/*.md`, `AGENTS.md`, the root harness instruction file)
