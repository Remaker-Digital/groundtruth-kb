...
comply with `.codex/rules/project-root-boundary.md`: all active GT-KB files and
...
`.codex/hooks/bridge-compliance-gate.py` per the companion bridge thread
...
The rule is mechanically enforced by `.codex/hooks/bridge-compliance-gate.py`
...
The bridge-compliance-gate hook (`.codex/hooks/bridge-compliance-gate.py`) mechanically enforces this requirement at Write time.
