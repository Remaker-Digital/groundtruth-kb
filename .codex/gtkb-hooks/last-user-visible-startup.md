...
Generated: 2026-05-16T05:46:04Z
...
- GT-KB open MemBase work items: 3
- GT-KB dashboard-scoped bridge/contention entries, non-authoritative for queue state: 7
- GT-KB drift changed paths: 1
- GT-KB Testing/tool rollup: 0 failing, 6 manual, 13 ready/passing
- Bridge role slot: `loyal-opposition` (prime-builder, loyal-opposition, or shared)
...
- GT-KB dev environment inventory: present; generated 2026-05-08T19:04:30Z; redaction pass
- Harness parity: pass (harness=codex, role=loyal-opposition, PASS=22)

### Project State Rollup

- Source: MemBase table: current_work_items grouped by `project_name`.
- Current work items: 2098; non-terminal: 153; active projects: 44; ungrouped non-terminal: 41.
- Status counts: deferred=1, in_progress=1, new=1, not_a_defect=7, open=150, resolved=1776, retired=58, verified=45, wont_fix=59.
- Active project states:
  - `AGENT-RED-TEST-COVERAGE-GAPS`: 38 non-terminal (open=38); top: `WI-3185` - Test coverage gap: Testable Element Dimension Taxonomy [open, low].
  - `AGENT-RED-SPEC-HYGIENE`: 7 non-terminal (open=7); top: `WI-3183` - KB integrity -- SPA cluster test-ID investigation closure: 10 SPA specs have no current test linkage [open, medium].
  - `PROJECT-GTKB-RELIABILITY-FIXES`: 6 non-terminal (open=6); top: `WI-3332` - Suppress already-known startup relay matches in owner-decision tracker [open, P1].
  - `GTKB-DETERMINISTIC-SERVICES-001`: 5 non-terminal (open=5); top: `WI-3265` - Cross-harness trigger fires unreliably in codex exec sessions (dispatch-state refresh lag) [open, P1, order 1].
  ...
  - `GTKB-BRIDGE-TOOLING-ENHANCEMENTS`: 3 non-terminal (open=3); top: `WI-3274` - Bridge protocol: parallel-session collision protection (work-intent registry or per-thread single-writer election) [open, medium].
  ...
  - `GTKB-RELIABILITY-FIXES`: 2 non-terminal (open=2); top: `WI-3320` - Fix flaky bridge-compliance-audit test (shared audit-file race) [open, P3].
  ...
  - `GTKB-BRIDGE-POLLER-001`: 1 non-terminal (open=1); top: `GTKB-BRIDGE-POLLER-001` - GTKB-BRIDGE-POLLER-001 [open, order 14].
  - `GTKB-BRIDGE-POLLER-COMPLEXITY-REFACTOR`: 1 non-terminal (open=1); top: `GTKB-BRIDGE-POLLER-COMPLEXITY-REFACTOR` - GTKB-BRIDGE-POLLER-COMPLEXITY-REFACTOR [open, order 23].
  - `GTKB-BRIDGE-POLLER-PRIME-CLASSIFICATION-REFINEMENT`: 1 non-terminal (open=1); top: `GTKB-BRIDGE-POLLER-PRIME-CLASSIFICATION-REFINEMENT` - GTKB-BRIDGE-POLLER-PRIME-CLASSIFICATION-REFINEMENT [open, order 22].
  ...
  - `GTKB-BRIDGE-WORK-FRONT-DRAIN-001`: 1 non-terminal (open=1); top: `WI-3256` - Reconsider mechanism for gtkb-claude-code-bridge-status-thread-automation (Codex NO-GO@004; 4 NO-GOs across 4 different mechanisms) [open, P2, order 8].
  ...
  - `GTKB-GOV-CODE-QUALITY-BASELINE`: 1 non-terminal (open=1); top: `GTKB-GOV-CODE-QUALITY-BASELINE` - GTKB-GOV-CODE-QUALITY-BASELINE [open, order 7].
  ...
  - `PROJECT-GTKB-BRIDGE-PROTOCOL-RELIABILITY`: 1 non-terminal (open=1); top: `GTKB-IMPL-AUTH-VERIFICATION-HEADING-GATE-ALIGNMENT` - Align implementation-start gate verification-plan heading recognition with bridge clause-preflight [open, medium].
