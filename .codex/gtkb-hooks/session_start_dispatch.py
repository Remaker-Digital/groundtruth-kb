"""Codex SessionStart hook dispatcher (thin delegating wrapper).
...
``scripts/session_start_dispatch_core.py``. This wrapper sets the Codex-harness
...
shared core. Behavior is identical to the Claude wrapper except ``HARNESS_NAME``
and ``OUT_DIR``.
...
# Codex-harness configuration (the only real per-harness difference).
HARNESS_NAME = "codex"
