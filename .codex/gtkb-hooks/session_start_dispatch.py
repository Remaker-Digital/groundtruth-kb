"""Codex SessionStart hook dispatcher.

Mirrors `.claude/hooks/session_start_dispatch.py` so the Codex harness
emits a properly-shaped SessionStart `hookSpecificOutput` envelope and
applies the same IP-4 canonical-init-keyword recognition.
...
def _read_first_prompt_line() -> str | None:
    ...
    ``scripts/cross_harness_bridge_trigger.py::_spawn_harness``). Codex's
