...
@pytest.fixture
def gated_db(tmp_path):
    """Create a temporary database with transport evidence gate active."""
    ...
    database = KnowledgeDB(tmp_path / "test.db", gate_registry=registry)
    yield database
    database.close()
    ...
    def test_all_views_exist(self, db):
        ...
        expected = sorted(
            [
                ...
                "current_work_items",
                "kpi_backlog_churn",
                "kpi_deliberation_provenance",
                "kpi_spec_test_mapping",
            ]
        )
...
class TestTransportGovernanceGate:
    ...
    def test_nontransport_spec_not_gated(self, gated_db):
