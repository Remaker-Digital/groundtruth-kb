"""Platform tests for benchmark: assertion_signal_noise.
...
from scripts.benchmarks import assertion_signal_noise as bm
...
def test_assertion_signal_noise_basic_run():
...
def test_assertion_signal_noise_idempotency_dimensions():
...
def test_assertion_signal_noise_expected_dimension_keys():
    result = bm.run(PAST, FUTURE, REPO)
    for k in ("genuine_drift", "chronic_noise", "flaky", "healthy", "sample_size"):
        assert k in result.dimensions


def test_assertion_signal_noise_empty_window_graceful():
    result = bm.run(EMPTY_START, EMPTY_END, REPO)
    assert isinstance(result, BenchmarkResult)
    assert isinstance(result.value, (int, float))


def test_assertion_signal_noise_output_writing(tmp_path):
