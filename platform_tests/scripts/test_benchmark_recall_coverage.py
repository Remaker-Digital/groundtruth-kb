"""Platform tests for benchmark: recall_coverage.
...
from scripts.benchmarks import recall_coverage as bm
...
def test_recall_coverage_basic_run():
...
def test_recall_coverage_idempotency_dimensions():
...
def test_recall_coverage_expected_dimension_keys():
    result = bm.run(PAST, FUTURE, REPO)
    for k in ("specifications", "work_items", "tests"):
        assert k in result.dimensions


def test_recall_coverage_empty_window_graceful():
    result = bm.run(EMPTY_START, EMPTY_END, REPO)
    assert isinstance(result, BenchmarkResult)
    assert isinstance(result.value, (int, float))


def test_recall_coverage_output_writing(tmp_path):
