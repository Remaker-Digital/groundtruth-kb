...
"""Provider filtering in the Ollama adapter's own routing configuration.

The actual loader validates local model inventory only for its provider's model
rows. A model row with an omitted provider uses the local provider default;
explicit peer-provider rows do not enter the local inventory check. These
controlled inputs exercise loader behavior, not installed model capability.
"""
...
def make_root(tmp_path: Path, routing_text: str) -> Path:
    ...
    routing_path = root / oh.ROUTING_CONFIG_PATH
    routing_path.parent.mkdir(parents=True)
    routing_path.write_text(routing_text.strip() + "\n", encoding="utf-8")
    return root
