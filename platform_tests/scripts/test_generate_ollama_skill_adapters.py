...
---
name: {skill_name}
description: Example skill description for Ollama adapter tests.
---
...
def test_generate_writes_compact_adapter_and_manifest(tmp_path: Path) -> None:
    ...
    assert adapter_paths == [".ollama/skills/example/SKILL.md"]
    assert changed == [".ollama/skills/example/SKILL.md", ".ollama/skills/MANIFEST.json"]
    adapter_text = (tmp_path / ".ollama" / "skills" / "example" / "SKILL.md").read_text(encoding="utf-8")
    assert "GTKB-OLLAMA-SKILL-ADAPTER" in adapter_text
    ...
    manifest = json.loads((tmp_path / ".ollama" / "skills" / "MANIFEST.json").read_text(encoding="utf-8"))
    assert manifest["generated_by"] == "scripts/generate_ollama_skill_adapters.py"
    assert manifest["adapters"][0]["adapter_relative_path"] == ".ollama/skills/example/SKILL.md"
...
def test_frontmatter_validation_fails_closed(tmp_path: Path) -> None:
    ...
    with pytest.raises(gen.OllamaSkillAdapterError, match="description"):
