...
def test_deliberation_recall_basic_run():
...
def test_deliberation_recall_idempotency_dimensions():
...
def test_deliberation_recall_expected_dimension_keys():
    result = bm.run(PAST, FUTURE, REPO)
    for k in ("sample_size", "hits_at_3", "search_failure_rate"):
        assert k in result.dimensions


def test_deliberation_recall_empty_window_graceful():
