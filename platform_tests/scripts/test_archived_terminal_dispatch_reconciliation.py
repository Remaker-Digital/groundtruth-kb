...
def test_newer_committed_archive_suppresses_live_slug(tmp_path: Path) -> None:
    """A newer committed terminal archive verdict is trusted and the
    archive-aware index suppresses the corresponding slug."""
    ...
    (bridge_dir / "test-slug-001.md").write_text("GO\n\nProposal.\n", encoding="utf-8")

    # Committed archive (newer version)
    archive_dir = project_root / "archive" / "bridge-terminal-verdicts"
    archive_dir.mkdir(parents=True)
    (archive_dir / "test-slug-003.md").write_text(
        "VERIFIED\n\nDocument: test-slug\nVersion: 003\nVerdict.\n",
        encoding="utf-8",
    )
    ...
    assert "test-slug" in verdicts
    assert verdicts["test-slug"].version == 3

    # Now test the archive-aware index via subprocess to get a clean import.
    # We build a tiny script that exercises the function in the tmp repo.
    script = f'''
import sys
sys.path.insert(0, r"{SCRIPTS}")
sys.path.insert(0, r"{GTKB_SRC}")
from bridge_thread_files import index_bridge_thread_files_archive_aware
from pathlib import Path
aware = index_bridge_thread_files_archive_aware(Path(r"{project_root}"))
print("SUPPRESSED" if "test-slug" not in aware else "VISIBLE")
'''
...
def test_equal_version_archive_does_not_suppress(tmp_path: Path) -> None:
    """Archive version == live version keeps the slug visible."""
    project_root = tmp_path / "repo"
    project_root.mkdir()
    _init_git_repo(project_root)
    ...
    bridge_dir = project_root / "bridge"
    bridge_dir.mkdir(parents=True)
    (bridge_dir / "eq-ver-005.md").write_text("GO\n\nProposal.\n", encoding="utf-8")

    archive_dir = project_root / "archive" / "bridge-terminal-verdicts"
    archive_dir.mkdir(parents=True)
    (archive_dir / "eq-ver-005.md").write_text(
        "VERIFIED\n\nDocument: eq-ver\nVersion: 005\nVerdict.\n",
        encoding="utf-8",
    )
...
from pathlib import Path
aware = index_bridge_thread_files_archive_aware(Path(r"{project_root}"))
print("SUPPRESSED" if "eq-ver" not in aware else "VISIBLE")
'''
    result = subprocess.run(
        [sys.executable, "-c", script],
        capture_output=True,
...
def test_state_report_excludes_archived_slugs(tmp_path: Path) -> None:
    """Archive-aware index excludes archived slug, keeps unrelated work."""
    project_root = tmp_path / "repo"
    project_root.mkdir()
    _init_git_repo(project_root)
    ...
    bridge_dir = project_root / "bridge"
    bridge_dir.mkdir(parents=True)
    (bridge_dir / "arch-slug-001.md").write_text("GO\n\nLive proposal.\n", encoding="utf-8")
    (bridge_dir / "stay-slug-002.md").write_text("NEW\n\nUnrelated work.\n", encoding="utf-8")

    archive_dir = project_root / "archive" / "bridge-terminal-verdicts"
    archive_dir.mkdir(parents=True)
    (archive_dir / "arch-slug-005.md").write_text(
        "VERIFIED\n\nDocument: arch-slug\nVersion: 005\nVerdict.\n",
        encoding="utf-8",
    )
...
aware = index_bridge_thread_files_archive_aware(Path(r"{project_root}"))
parts = []
for slug in sorted(aware):
    parts.append(slug)
print("ARCH-SUPPRESSED" if "arch-slug" not in aware else "ARCH-VISIBLE")
print("STAY-VISIBLE" if "stay-slug" in aware else "STAY-MISSING")
'''
    result = subprocess.run(
        [sys.executable, "-c", script],
        capture_output=True,
    ...
    assert "ARCH-SUPPRESSED" in result.stdout
    assert "STAY-VISIBLE" in result.stdout
