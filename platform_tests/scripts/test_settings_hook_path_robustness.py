...
references ``.harness-baseline-configuration/hooks/<name>.py`` resolves through the project-root-
absolute ``$CLAUDE_PROJECT_DIR`` form rather than the working-directory-
relative ``python .harness-baseline-configuration/hooks/<name>.py`` form.
...
# ``python `` followed directly by ``.harness-baseline-configuration/hooks/`` — i.e., the path is
# resolved relative to the harness's current working directory rather than
# the project root.
RELATIVE_HOOK_COMMAND_RE = re.compile(r"^\s*python\s+\.harness-baseline-configuration/hooks/")
...
def test_no_relative_python_hook_commands_in_claude_settings() -> None:
    ...
    assert not offenders, (
        ...
        "`python .harness-baseline-configuration/hooks/<name>.py` to "
        '`python "$CLAUDE_PROJECT_DIR/.harness-baseline-configuration/hooks/<name>.py"`. '
        f"Offenders: {offenders}"
    )


def test_every_claude_hook_path_resolves_through_project_root() -> None:
    """Positive assertion: every `.harness-baseline-configuration/hooks/<name>.py` reference uses
    ...
    hook_path_commands = [
        (event, matcher, command)
        for event, matcher, command in _iter_hook_commands(settings)
        if ".harness-baseline-configuration/hooks/" in command and _launches_python(command)
    ]
    assert hook_path_commands, (
        "No python .harness-baseline-configuration/hooks/ registrations found; test fixture stale."
    )
    missing_project_dir = [
        (event, matcher, command)
    ...
    assert not missing_project_dir, (
        "Found python `.harness-baseline-configuration/hooks/` registrations that do not anchor through "
