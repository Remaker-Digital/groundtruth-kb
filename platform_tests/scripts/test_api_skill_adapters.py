...
MANIFEST_PATH = REPO_ROOT / ".api-harness" / "skills" / "MANIFEST.json"
...
def test_manifest_matches_generated_adapter_files() -> None:
    ...
    for adapter in manifest["adapters"]:
        ...
        assert "GTKB-API-SKILL-ADAPTER" in text
...
def test_bridge_adapter_is_compact_pointer_not_full_skill_copy() -> None:
    adapter_text = (REPO_ROOT / ".api-harness" / "skills" / "bridge" / "SKILL.md").read_text(encoding="utf-8")

    assert "# API Harness Skill Adapter: gtkb-bridge" in adapter_text
...
def test_generator_check_passes_for_repository() -> None:
    completed = subprocess.run(
        [sys.executable, "scripts/generate_api_skill_adapters.py", "--check"],
        cwd=REPO_ROOT,
        text=True,
        capture_output=True,
        check=False,
    ...
    assert "API skill adapters: PASS" in completed.stdout
...
def test_ollama_registry_declares_adapter_support() -> None:
    ...
    assert ollama["skill_adapter_generator"] == "scripts/generate_api_skill_adapters.py"
