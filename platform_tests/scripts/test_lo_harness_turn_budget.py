...
def test_lo_harness_routing_uses_owner_approved_generous_envelope() -> None:
    ...
    # R6 (ii): every provider's section lives in the one authored routing source; nothing is projected.
    routing = tomllib.loads((root / ".harness-baseline-configuration" / "routing.toml").read_text(encoding="utf-8"))[
        "routing"
    ]
