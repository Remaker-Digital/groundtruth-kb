"""Platform tests for benchmark: versions_per_landed_change."""
...
from scripts.benchmarks import versions_per_landed_change as bm
...
def test_versions_per_landed_change_basic_run():
...
def test_versions_per_landed_change_idempotency_dimensions():
...
def test_versions_per_landed_change_expected_dimension_keys():
    result = bm.run(PAST, FUTURE, REPO)
    for k in ("total_versions", "unique_documents"):
        assert k in result.dimensions


def test_versions_per_landed_change_empty_window_graceful():
...
def test_versions_per_landed_change_output_writing(tmp_path):
