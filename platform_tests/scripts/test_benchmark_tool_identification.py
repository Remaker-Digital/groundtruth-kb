"""Platform tests for benchmark: tool_identification.
...
from scripts.benchmarks import tool_identification as bm
...
def test_tool_identification_basic_run():
...
def test_tool_identification_idempotency_dimensions():
...
def test_tool_identification_expected_dimension_keys():
    result = bm.run(PAST, FUTURE, REPO)
    for k in ("specifications", "work_items", "deliberations"):
        assert k in result.dimensions


def test_tool_identification_empty_window_graceful():
    result = bm.run(EMPTY_START, EMPTY_END, REPO)
    assert isinstance(result, BenchmarkResult)
    assert isinstance(result.value, (int, float))


def test_tool_identification_output_writing(tmp_path):
