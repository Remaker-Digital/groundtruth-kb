...
def _seed_registry(
    root: Path, harnesses: dict[str, tuple[str, list[str]] | tuple[str, list[str], str]]
) -> None:
...
def _load_hook_module():
    spec = importlib.util.spec_from_file_location(
        "workstream_focus_hook_wi5580", HOOK_PATH
    )
    assert spec and spec.loader
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
        ...
        json.dumps(
            {
                "schema_version": 1,
                "harnesses": {
                    name: {"id": value} for name, value in identities.items()
                },
            }
        )
        + "\n",
...
def _run_hook(
    payload: dict, state_path: Path, *, guard_path: Path | None = None
) -> dict:
    env = {
        **dict(os.environ),
...
def test_default_work_subject_is_gtkb_and_startup_lines_explain_commands(
...
def test_canonical_state_file_written_under_claude_session(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    ...
    module.save_state(
        module.FOCUS_GTKB_INFRASTRUCTURE, REPO_ROOT, updated_by="owner_prompt"
    )
...
def test_legacy_state_migrates_on_load_when_canonical_absent(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
...
def test_work_subject_application_command_sets_canonical_state(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    canonical, _ = _isolate_state(monkeypatch, tmp_path)
...
def test_work_subject_gtkb_command_sets_canonical_state(tmp_path, monkeypatch) -> None:
    ...
    assert (
        "Current work subject set to GT-KB Infrastructure Focus"
...
def test_hook_payload_accepts_claude_prompt_field_for_user_promptsubmit(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    canonical, _ = _isolate_state(monkeypatch, tmp_path)
    ...
    assert "Current work subject set to Application Focus" in response["systemMessage"]
    assert (
        json.loads(canonical.read_text(encoding="utf-8"))["current_subject"]
        == module.SUBJECT_APPLICATION
    )


def test_hook_payload_accepts_claude_prompt_field_for_startup_gate(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
...
def test_sessionstart_plus_daemon_dispatch_prompt_without_marker_processes_bridge_task(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
...
def test_startup_gate_canonical_application_subject_sets_application_scope(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    canonical, _ = _isolate_state(monkeypatch, tmp_path)
...
def test_startup_gate_emits_bounded_pointer_not_inlined_disclosure(
    tmp_path, monkeypatch
) -> None:
    ...
    assert len(context.encode("utf-8")) < 4096, (
        "relay additionalContext must stay bounded"
    )
    ...
    assert "disclosure body line" not in context, (
        "full disclosure body must not be inlined"
    )
    assert "## Cached User-Visible Startup Message" not in context


def test_startup_gate_message_authorizes_one_read_only_read(
    tmp_path, monkeypatch
) -> None:
    ...
    assert "do not use tools" not in lowered, (
        "the contradictory blanket tool prohibition must be gone"
    )


def test_lo_default_startup_gate_continues_to_harness_action(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
    ...
    assert (
        "process actionable latest `NEW` / `REVISED` bridge entries oldest-to-newest by default"
        in context
    )
    ...
    assert (
        module.guard_tool_use(
            {
                "tool_name": "Bash",
                "tool_input": {"command": "gt bridge state-report --markdown"},
            },
            tmp_path,
        )
        == {}
    ...
    assert (
        module.guard_tool_use(
            {
                "tool_name": "Write",
                "tool_input": {"file_path": "bridge/gtkb-wi5186-example-002.md"},
            },
            tmp_path,
        )
        == {}
...
def test_lo_advisory_startup_gate_asks_before_auto_process(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
    ...
    assert (
        "Do not write verdict files or auto-process bridge entries in advisory mode"
        in context
    )
    assert "stop and wait for the next owner message" not in context
    guard_state = json.loads((tmp_path / "guard.json").read_text(encoding="utf-8"))
...
def test_prime_builder_startup_gate_still_waits_after_disclosure(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
...
def test_startup_gate_does_not_consult_shared_dashboard_report(
    tmp_path, monkeypatch
) -> None:
    ...
    assert "STARTUP RELAY FAILURE" in context, (
        "absent harness-scoped cache must fail visibly"
    )


def test_startup_gate_fails_visibly_on_inconsistent_cache(
    tmp_path, monkeypatch
) -> None:
...
def test_user_promptsubmit_clears_stale_startup_gate_after_startup_stop(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    canonical, _ = _isolate_state(monkeypatch, tmp_path)
...
def test_prompt_hook_accepts_bom_prefixed_stdin_from_windows_pipeline(tmp_path) -> None:
    ...
    result = subprocess.run(
        ...
        input=(
            "\ufeff"
            + json.dumps(
                {"prompt": "::init gtkb pb", "hook_event_name": "UserPromptSubmit"}
            )
        ).encode("utf-8"),
        capture_output=True,
        timeout=30,
    ...
    assert (
        json.loads(guard_path.read_text(encoding="utf-8"))["startup_response_pending"]
        is True
    )


@pytest.mark.skip(
    reason="workstream-focus.py intentionally retired S304/S305; see REVISED-5 BN-3"
)
def test_prompt_hook_switches_focus_with_standalone_commands(tmp_path) -> None:
    ...
    assert (
        json.loads(state_path.read_text(encoding="utf-8"))["current_focus"]
        == "gtkb_infrastructure"
    )
...
def test_prompt_hook_toggles_next_session_role_with_simple_phrase(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    ...
    assert (
        "Next fresh-session operating mode set to Prime Builder"
        in response["systemMessage"]
    )
    ...
    assert (
        "Next fresh-session operating mode set to Loyal Opposition"
        in response["systemMessage"]
    )
...
def test_prompt_hook_toggles_dashboard_auto_launch(tmp_path, monkeypatch) -> None:
    ...
    assert (
        json.loads(preferences_path.read_text(encoding="utf-8"))[
            "open_dashboard_on_session_start"
        ]
        is True
    )
...
def test_prompt_hook_discards_first_fresh_session_message_when_startup_gate_is_armed(
    ...
    response = _run_hook(
        {"user_prompt": "Please resume."}, state_path, guard_path=guard_path
    )
    ...
    assert response["hookSpecificOutput"]["hookEventName"] == "UserPromptSubmit"
    assert (
        "startup disclosure already generated for this session"
...
def test_startup_response_pending_clears_on_next_owner_prompt_and_allows_normal_processing(
    ...
    response = _run_hook(
        {"user_prompt": "GT-KB mode"}, state_path, guard_path=guard_path
    )
...
def test_classify_root_4_categories(tmp_path, monkeypatch) -> None:
    ...
    assert (
        module.classify_root("src/example.py", REPO_ROOT)
        == module.ROOT_APPLICATION_PRODUCT
    )
    assert (
        module.classify_root(".claude/rules/new-rule.md", REPO_ROOT)
        == module.ROOT_CURRENT_REPO_BRIDGE_OR_GOVERNANCE
    )
    assert (
        module.classify_root("bridge/some-proposal-001.md", REPO_ROOT)
        == module.ROOT_CURRENT_REPO_BRIDGE_OR_GOVERNANCE
    )
    assert (
        module.classify_root("AGENTS.md", REPO_ROOT)
        == module.ROOT_CURRENT_REPO_BRIDGE_OR_GOVERNANCE
    )
    assert module.classify_root("README.md", REPO_ROOT) == module.ROOT_NEUTRAL


# Empty today: GT-KB's live config/ tree has no genuine application-owned
# subdirectory. A new application config dir must be named here or carved out.
_EXPECTED_APPLICATION_CONFIG_SUBDIRS: frozenset[str] = frozenset()

_CONFIG_CARVEOUT_REMEDY = (
    "add it to CURRENT_REPO_BRIDGE_OR_GOVERNANCE_PREFIXES, or declare it "
    "application config in _EXPECTED_APPLICATION_CONFIG_SUBDIRS"
)


def _config_subdir_drift_failures(module, config_root) -> list[str]:
    failures: list[str] = []
    for child in sorted(config_root.iterdir()):
        if not child.is_dir():
            continue
        sample = f"config/{child.name}/probe"
        classified = module.classify_root(sample, REPO_ROOT)
        if child.name in _EXPECTED_APPLICATION_CONFIG_SUBDIRS:
            if classified != module.ROOT_APPLICATION_PRODUCT:
                failures.append(
                    f"config/{child.name}/ classified {classified}; expected "
                    f"application_product because it is on the application allowlist"
                )
            continue
        if classified != module.ROOT_CURRENT_REPO_BRIDGE_OR_GOVERNANCE:
            failures.append(
                f"config/{child.name}/ classified {classified}; {_CONFIG_CARVEOUT_REMEDY}"
            )
    return failures


def test_classify_root_config_platform_carveout() -> None:
    # WI-5100 / WI-5970: platform config/ subdirs are governance surfaces.
    # C2 walks the live tree so a new uncarved config/<dir>/ fails the suite.
    module = _load_module()
    config_root = REPO_ROOT / "config"
    failures = _config_subdir_drift_failures(module, config_root)
    assert failures == [], "\n".join(failures)
    assert (
    ...
    assert (
        module.classify_root("config/app-settings.toml", REPO_ROOT)
        == module.ROOT_APPLICATION_PRODUCT
    )
    assert (
        module.classify_root("src/example.py", REPO_ROOT)
...
def test_config_carveout_drift_guard_fails_on_synthetic_uncarved_subdir(
    tmp_path,
) -> None:
    module = _load_module()
    synthetic = tmp_path / "config" / "synthetic-uncarved-wi5970"
    synthetic.mkdir(parents=True)
    failures = _config_subdir_drift_failures(module, tmp_path / "config")
    assert failures, "drift guard must fail for an uncarved config subdirectory"
    joined = "\n".join(failures)
...
def test_application_subject_blocks_current_repo_bridge_or_governance_write(
...
def test_application_subject_allows_bridge_advisory_write(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
...
def test_application_subject_blocks_non_advisory_bridge_write(
...
def test_gtkb_subject_allows_current_repo_bridge_or_governance_write(
    tmp_path, monkeypatch
) -> None:
    ...
    response = module.guard_tool_use(
        {
            "tool_name": "Write",
            "tool_input": {"file_path": ".claude/rules/new-rule.md"},
        },
        REPO_ROOT,
    )
...
def test_startup_response_pending_blocks_tool_use_until_next_owner_prompt(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    guard_path = tmp_path / "guard.json"
    ...
    assert "startup_prompt_preview" not in json.loads(
        guard_path.read_text(encoding="utf-8")
    )


def test_auq_acknowledgement_clears_only_matching_pending_session(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    guard_path = tmp_path / "guard.json"
    ...
    assert guard_state["startup_response_pending"] is False
    assert (
        guard_state["startup_input_gate_clear_reason"] == "ask_user_question_completed"
    )
    assert "startup_prompt_preview" not in guard_state
    assert (
        module.guard_tool_use(
...
def test_stale_startup_response_pending_does_not_block_later_tool_use(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    guard_path = tmp_path / "guard.json"
...
def test_bash_guard_blocks_mutating_gtkb_and_governance_commands(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
    ...
    read_response = module.guard_tool_use(
        {
            "tool_name": "Bash",
            "tool_input": {
                "command": "Get-Content .claude/rules/prime-builder-role.md"
            },
        },
        REPO_ROOT,
    )
...
def test_apply_patch_guard_blocks_mutating_governance_file(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
...
def test_apply_patch_guard_blocks_gtkb_subject_application_file(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
...
def test_overlay_startup_note_subject_mismatch_is_warning() -> None:
    module = _load_module()
    note = module.overlay_startup_note(
        {"overlay_present": True, "subject_mismatch": True}
    )
    assert note["level"] == "warning"
...
def test_overlay_startup_note_projection_diff_is_warning() -> None:
    module = _load_module()
    note = module.overlay_startup_note(
        {"overlay_present": True, "projection_diff": True}
    )
    assert note["level"] == "warning"
...
def test_detect_counterpart_state_no_counterpart_files_no_warning(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
...
def test_detect_counterpart_state_same_role_warns(tmp_path, monkeypatch) -> None:
    ...
    assert any(
        "prime-builder" in msg and "collide" in msg for msg in result["warnings"]
    )
...
def test_detect_counterpart_state_different_role_warns(tmp_path, monkeypatch) -> None:
    ...
    assert result["counterpart_present"] is True
    assert any(
        "prime-builder" in msg and "loyal-opposition" in msg
...
def test_detect_counterpart_state_subject_mismatch_warns(tmp_path, monkeypatch) -> None:
    ...
    claude_guard = (
        tmp_path / "harness-state" / "claude" / "session-lifecycle-guard.json"
    )
...
def test_detect_counterpart_state_subject_match_no_warning(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    canonical, _ = _isolate_state(monkeypatch, tmp_path)
...
def test_detect_counterpart_state_missing_counterpart_no_crash(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    monkeypatch.setenv("GTKB_HARNESS_NAME", "claude")
...
def test_assert_readiness_subject_scope_hard_rejects_unlabeled_combined_green() -> None:
    module = _load_module()
    with pytest.raises(
        module.SubjectScopeError, match="combined application \\+ GT-KB"
    ):
...
def test_harness_state_records_for_project_returns_sandbox_relative_paths(
    tmp_path,
) -> None:
    module = _load_module()
    sandbox = tmp_path / "sandbox"
    role_assignment_path, lifecycle_guards = module._harness_state_records_for_project(
        sandbox
    )

    assert role_assignment_path == sandbox / "harness-state" / "harness-registry.json"
    assert (
        lifecycle_guards["codex"]
        == sandbox / "harness-state" / "codex" / "session-lifecycle-guard.json"
    )
    assert (
        lifecycle_guards["claude"]
        == sandbox / "harness-state" / "claude" / "session-lifecycle-guard.json"
    )
    assert (
        role_assignment_path
        != module.PROJECT_ROOT / "harness-state" / "harness-registry.json"
    )
    assert lifecycle_guards["codex"] != module.HARNESS_LIFECYCLE_GUARDS["codex"]


def test_detect_counterpart_state_uses_project_root_paths_when_provided(
    tmp_path, monkeypatch
) -> None:
    ...
    def _fake_load(project_root: Path, assignment_path: Path | None = None) -> dict:
        recorded_paths.append(
            assignment_path or project_root / "harness-state" / "harness-registry.json"
        )
        return {"harnesses": {}}
...
def test_detect_counterpart_state_falls_back_to_canonical_when_project_root_omitted(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    recorded_paths: list[Path] = []
...
def test_startup_gate_self_heals_freshness_stale_cache(tmp_path, monkeypatch) -> None:
    ...
    assert (
        "STARTUP RELAY FAILURE"
        not in response["hookSpecificOutput"]["additionalContext"]
    )


def test_startup_gate_default_refresh_budget_allows_local_render(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
...
def test_refresh_records_timeout_abandonment(tmp_path, monkeypatch) -> None:
    ...
    _mock_dispatch_core_render_and_write(
        monkeypatch, module, render=_slow_render, write=lambda *a, **k: None
    )

    result = module._refresh_startup_relay_cache_bounded(
        tmp_path, role_mode="pb", meta={"role_mode": "pb"}
    )
    ...
    diag = (
        ...
        / ".codex"
        / "gtkb-hooks"
        / module.STARTUP_RELAY_REFRESH_DIAGNOSTIC_NAME
    )
...
def test_refresh_records_completion(tmp_path, monkeypatch) -> None:
    ...
    _mock_dispatch_core_render_and_write(
        monkeypatch, module, render=_fast_render, write=lambda *a, **k: None
    )
...
def test_stale_but_intact_cache_reports_staleness_not_corruption(
    tmp_path, monkeypatch
) -> None:
    ...
    monkeypatch.setattr(module, "_resolved_harness_id", lambda root: "A")
    monkeypatch.setattr(
        module, "_refresh_startup_relay_cache_bounded", lambda *a, **k: False
    )
...
def test_startup_gate_self_heals_rederivable_content_drift(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
    ...
    assert called
    assert cache_file.read_text(encoding="utf-8").strip().endswith("Self-healed body")
    healed_meta = json.loads(meta_file.read_text(encoding="utf-8"))
    healed_body = cache_file.read_text(encoding="utf-8")
    assert (
        healed_meta["sha256"] == hashlib.sha256(healed_body.encode("utf-8")).hexdigest()
    )
...
def test_startup_gate_refresh_timeout_fails_visibly_without_late_cache_write(
    tmp_path, monkeypatch
) -> None:
    ...
    assert elapsed < 0.5
    assert (
        "STARTUP RELAY FAILURE" in response["hookSpecificOutput"]["additionalContext"]
    )
    time.sleep(0.25)
    assert cache_file.read_text(encoding="utf-8").strip() == body_text


def test_startup_gate_no_self_heal_on_non_recoverable_inconsistency(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
...
def test_startup_gate_no_self_heal_on_fresh_consistent_cache(
    tmp_path, monkeypatch
) -> None:
    module = _load_module()
    _isolate_state(monkeypatch, tmp_path)
...
def test_continuation_armed_gate_does_not_block_tool_use(tmp_path, monkeypatch) -> None:
    ...
    assert guard_state["stale_startup_response_pending_cleared"] is True
    assert (
        guard_state["stale_startup_response_pending_cleared_reason"]
        == "session_continuation_armed_source"
    )
...
def test_claude_adapter_refuses_foreign_harness_override_without_writing(
    tmp_path, monkeypatch
) -> None:
...
def test_claude_adapter_writes_only_its_durable_exact_session_document(
    tmp_path, monkeypatch
) -> None:
    hook = _load_hook_module()
    _seed_harness_identities(tmp_path, {"codex": "A", "claude": "B"})
