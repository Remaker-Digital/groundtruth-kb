...
def _seed_harness_identities(root: Path, identities: dict[str, str]) -> None:
    ...
    path.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "harnesses": {name: {"id": value} for name, value in identities.items()},
            }
        )
        + "\n",
        encoding="utf-8",
...
def _stub_disclosure_render(monkeypatch, module, body: str) -> None:
    """Seed the real-time disclosure render with a fixed body (WI-7318).

    The relay no longer reads a cache file, so tests supply the rendered
    document directly instead of writing one to disk.
    """
    monkeypatch.setattr(
        module,
        "_render_startup_disclosure_bounded",
        lambda root, *, role_mode=None: body,
    )
...
def test_lo_default_startup_gate_continues_to_harness_action(tmp_path, monkeypatch) -> None:
    ...
    _stub_disclosure_render(
        monkeypatch, module, "# GroundTruth-KB Fresh Session Startup\n\n## Startup Disclosure\n\nlo body"
    )
...
def _config_subdir_drift_failures(module, config_root) -> list[str]:
    ...
    for child in sorted(config_root.iterdir()):
        ...
        if classified != module.ROOT_CURRENT_REPO_BRIDGE_OR_GOVERNANCE:
            failures.append(f"config/{child.name}/ classified {classified}; {_CONFIG_CARVEOUT_REMEDY}")
    return failures
...
def test_bash_guard_blocks_mutating_gtkb_and_governance_commands(tmp_path, monkeypatch) -> None:
    ...
    read_response = module.guard_tool_use(
        {
            "tool_name": "Bash",
            "tool_input": {"command": "Get-Content .claude/rules/prime-builder-role.md"},
        },
        REPO_ROOT,
    )
...
#
# The prior implementation tested ``known_path.rstrip("/") in command``, a bare
# substring match. ``config/`` therefore matched the WORD ``configuration``, so a
# read-only command touching ``.harness-baseline-configuration/...`` was refused
# as an application-product write. These cases pin the boundary semantics.


def test_known_path_not_matched_inside_a_longer_word() -> None:
    """The reported false positive: ``config`` inside ``.harness-baseline-configuration``."""
    module = _load_module()
    command = "ls -d .harness-baseline-configuration/rules/.claude"
    assert module._known_path_mentioned("config/", command) is False


def test_known_path_not_matched_in_unrelated_prose() -> None:
    module = _load_module()
    assert module._known_path_mentioned("config/", "echo reconfigure") is False
    assert module._known_path_mentioned("src/", "git add baseline-src-notes") is False


def test_directory_prefix_requires_a_separator() -> None:
    """``widget/`` must not match ``widgets/``, which is a different segment."""
    module = _load_module()
    assert module._known_path_mentioned("widget/", "cat widgets/readme") is False


def test_genuine_mentions_still_match() -> None:
    module = _load_module()
    assert module._known_path_mentioned("config/", "rm config/app.toml") is True
    assert module._known_path_mentioned("src/", "git add src/mod.py") is True


def test_genuine_mentions_match_through_common_shell_decoration() -> None:
    """Dot-relative, quoted, and flag-attached forms are real mentions."""
    module = _load_module()
    for command in (
        "rm ./config/app.toml",
        'rm "config/app.toml"',
        "rm 'config/app.toml'",
        "tar --exclude=config/app.toml -cf out.tar .",
        "rm a.txt;config/b.txt",
    ):
        assert module._known_path_mentioned("config/", command) is True, command


def test_boundary_fix_does_not_suppress_the_guard_end_to_end() -> None:
    """A real application-product write is still extracted for classification."""
    module = _load_module()
    mentions = module._path_mentions_from_command("rm config/app.toml")
    assert any(m == "config/" or m.startswith("config/") for m in mentions)


def test_boundary_fix_removes_the_false_mention_end_to_end() -> None:
    """The baseline-configuration read no longer yields a ``config/`` mention."""
    module = _load_module()
    mentions = module._path_mentions_from_command("ls -d .harness-baseline-configuration/rules/.claude")
    assert "config/" not in mentions
