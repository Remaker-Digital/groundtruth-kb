...
Generated: 2026-08-06T17:40:56Z
...
- Interactive role source: startup disclosure cache role mode pb; authoritative only when selected by the owner transcript/init-keyword path
...
- GT-KB open MemBase work items: 11 (subject-scoped; 646 across all subjects)
- GT-KB dashboard-scoped bridge/contention entries, non-authoritative for queue state: 313
...
- Harness parity: warn (phase-1 catalog parity; operational_readiness=not evaluated; run phase-2 readiness and hook discovery diff; harness=cursor, role=prime-builder, DEGRADED=41, EXTRA=1, PASS=4, UNSUPPORTED=18)
...
- Harness: id E from harness-state/harness-identities.json; topology `multi_harness`.
- Dashboard opening: enabled; mode `harness_browser`; package 0.7.0rc1.

### Operating State
- Release blockers: 0.
- Testing/tools: 0 failing, 0 manual, 17 ready/passing (queried repo: unknown).
- Dev environment inventory: stale; generated 2026-05-08T19:04:30Z; redaction pass
- Harness parity: warn (phase-1 catalog parity; operational_readiness=not evaluated; run phase-2 readiness and hook discovery diff; harness=cursor, role=prime-builder, DEGRADED=41, EXTRA=1, PASS=4, UNSUPPORTED=18)
- Data freshness: 2026-08-06T17:40:56Z.