#!/usr/bin/env python3
"""Codex Bash adapter for the bridge proposal WI-ID collision gate."""
...
CANONICAL_HOOK = PROJECT_ROOT / ".claude" / "hooks" / "bridge-proposal-wi-id-collision-gate.py"
SKIPPED_DIAGNOSTIC = PROJECT_ROOT / ".codex" / "gtkb-hooks" / "last-wi-id-collision-skipped.json"
...
BRIDGE_FILE_REFERENCE_RE = re.compile(
    r"(?:[A-Za-z]:)?(?:[\\/]|[\w .-]+[\\/])*bridge[\\/][A-Za-z0-9_.-]+-\d{3}\.md",
    re.IGNORECASE,
)
...
def synthetic_claude_payload(payload: dict[str, Any], file_path: str, content: str) -> dict[str, Any]:
    return {
        ...
        "session_id": payload.get("session_id", "codex-wi-id-collision"),
