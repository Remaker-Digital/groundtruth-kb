...
Per-session ledger at ``{{HARNESS_CONFIG_DIR}}/session/spec-events-seen.jsonl`` prevents
...
LEDGER_REL_PATH = "{{HARNESS_CONFIG_DIR}}/session/spec-events-seen.jsonl"
...
SESSION_START_REL_PATH = "{{HARNESS_CONFIG_DIR}}/session/session-start.json"
...
def _resolve_session_started_at(cwd: Path) -> tuple[str, bool]:
    ...
    Primary source: ``{{HARNESS_CONFIG_DIR}}/session/session-start.json`` (written by the
