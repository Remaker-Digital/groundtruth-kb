...
SPEC_LINK_TOKEN_RE = re.compile(
    ...
    r"|(?:^|[`(\s])(?:{{HARNESS_RULES_DIR}}|groundtruth-kb/docs|docs|bridge)/[^\s`)]+",
...
# {{HARNESS_RULES_DIR}}/file-bridge-protocol.md section "Mandatory Implementation-Start
...
AUDIT_OUTPUT_RELATIVE_PATH = Path("{{HARNESS_HOOKS_DIR}}") / "last-bridge-audit.json"
...
def _record_gate_denial(pattern_id: str, subject: str, reason: str, *, root: Path | None = None) -> None:
    ...
    base = root or Path(os.environ.get("{{HARNESS_PROJECT_DIR_VAR}}") or os.getcwd()).resolve()
...
def _ancestor_or_self(root: Path, cwd_path: Path) -> bool:
    ...
    under ``<canonical>/{{HARNESS_CONFIG_DIR}}/worktrees/``. A resolver result that is neither
...
def _canonical_project_root(cwd_path: Path) -> Path:
    ...
    root. A session running inside a ``{{HARNESS_CONFIG_DIR}}/worktrees/*`` linked worktree has
...
def _bridge_work_intent_deny_reason(*, cwd_path: Path, file_path: str, payload: dict) -> str | None:
    ...
    if holder is None:
        return (
            ...
            "Per {{HARNESS_RULES_DIR}}/file-bridge-protocol.md 'Mandatory Pre-Drafting Claim Step', run: "
...
# reliable routing signal. See {{HARNESS_RULES_DIR}}/file-bridge-protocol.md
...
def _requirement_sufficiency_section_gap(content: str) -> str | None:
    ...
    Operative states per {{HARNESS_RULES_DIR}}/file-bridge-protocol.md section "Mandatory
...
def _verdict_preflight_freshness_deny_reason(
    *,
    ...
    # DELIB-HARNESS-NO-ACTION-LO-RESPONSE-SET-20260702, {{HARNESS_RULES_DIR}}/counterpart-review-gate.md
...
def _no_action_prior_verdict_deny(file_path: str, content: str) -> str | None:
    ...
    return (
        ...
        "DCL-NO-ACTION-STATUS-SEMANTICS-001; see {{HARNESS_RULES_DIR}}/file-bridge-protocol.md section "
        "'NO-ACTION Status'.)"
    )
    ...
    if _is_bridge_markdown_file(file_path) and content:
        ...
        if (
            ...
            return (
                ...
                "{{HARNESS_RULES_DIR}}/counterpart-review-gate.md.)"
            )
        ...
        if (
            ...
            if req_suff_gap:
                return (
                    ...
                    "'New or revised requirement required before implementation'). "
                    f"Gap: {req_suff_gap}. "
                    "(Hard-block per {{HARNESS_RULES_DIR}}/file-bridge-protocol.md "
