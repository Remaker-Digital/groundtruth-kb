...
# Project root resolution: prefer the {{HARNESS_PROJECT_DIR_VAR}} env var (the harness
# sets this for hooks); fall back to walking up from this file's location.
PROJECT_ROOT = Path(os.environ.get("{{HARNESS_PROJECT_DIR_VAR}}") or Path(__file__).resolve().parents[2]).resolve()
...
def _auto_archive_if_enabled(entry: DecisionEntry, session_hint: str = "") -> None:
    ...
    ``{{HARNESS_PROJECT_DIR_VAR}}`` at hook startup), never ``Path.cwd()``. Failures
        ...
        if not existing:
            parts.extend(
                [
                    ...
                    "",
                    "This dated sidecar is written by {{HARNESS_HOOKS_DIR}}/owner-decision-tracker.py.",
...
This file is owned by {{HARNESS_HOOKS_DIR}}/owner-decision-tracker.py.
...
def _session_hint() -> str:
    ...
    Read from the {{HARNESS_SESSION_ID_VAR}} env var if set; otherwise empty. The
    field is informational; no logic depends on it being populated.
    """
    return os.environ.get("{{HARNESS_SESSION_ID_VAR}}", "")
