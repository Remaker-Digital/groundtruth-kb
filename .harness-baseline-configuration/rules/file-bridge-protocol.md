...
comply with `{{HARNESS_RULES_DIR}}/project-root-boundary.md`: all active GT-KB files and
...
`{{HARNESS_HOOKS_DIR}}/bridge-compliance-gate.py` per the companion bridge thread
...
The rule is mechanically enforced by `{{HARNESS_HOOKS_DIR}}/bridge-compliance-gate.py`
...
The bridge-compliance-gate hook (`{{HARNESS_HOOKS_DIR}}/bridge-compliance-gate.py`) mechanically enforces this requirement at Write time.
