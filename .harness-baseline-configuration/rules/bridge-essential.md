...
This rule auto-loads via `{{HARNESS_RULES_DIR}}/` convention and is TRACKED in git
(negated from the `{{HARNESS_CONFIG_DIR}}/` blanket ignore). Do not remove it.
...
- Skip the GO/NO-GO discipline for any code change beyond the explicit
  exemptions in `{{HARNESS_RULES_DIR}}/counterpart-review-gate.md`
...
- **S292**: Session-start briefs for 7+ sessions silently omitted the in-session
  ...
  in `memory/*.md` are not enforceable; hooks and `{{HARNESS_CONFIG_DIR}}` hook
  registration are.
- **S294**: Discovered `{{HARNESS_CONFIG_DIR}}/` was blanket-ignored in `.gitignore`, so the
