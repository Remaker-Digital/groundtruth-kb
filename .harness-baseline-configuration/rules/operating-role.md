...
- Registered harnesses hold workstation-unique single-letter IDs recorded in `harness-state/harness-identities.json` (illustrative values removed; the identity artifact is authoritative)
...
`{{HARNESS_CONFIG_DIR}}/session/active-session-role.json` may cache the resolved role for hook
