...
authoritative source. Sources include `{{HARNESS_RULES_DIR}}/operating-model.md` §2
...
templates (CLAUDE.md, MEMORY.md, `{{HARNESS_RULES_DIR}}/canonical-terminology.md`,
...
`{{HARNESS_RULES_DIR}}/file-bridge-protocol.md` Mandatory Specification-Derived
Verification Gate.
...
`{{HARNESS_RULES_DIR}}/canonical-terminology.md` by
...
narrative authority `{{HARNESS_RULES_DIR}}/sot-read-discipline.md`;

**Implementation pointer:** `{{HARNESS_HOOKS_DIR}}/sot-read-discipline.py` (canonical hook) plus per-harness command-verb adapters registered in each harness's hook surface; doctor `_check_sot_read_discipline`; owner-authorized bypass `GTKB_SOT_READ_DISCIPLINE_BYPASS`.
...
**Implementation pointer:** `config/registry/sot-artifacts.toml` (`forbidden_substitutes` entries); enforced by `{{HARNESS_HOOKS_DIR}}/sot-read-discipline.py`.
...
(`{{HARNESS_HOOKS_DIR}}/bridge-compliance-gate.py`) that fails the Write of bridge
...
`{{HARNESS_HOOKS_DIR}}/credential-scan.py` (general-purpose, all Write/Edit targets)
and `{{HARNESS_HOOKS_DIR}}/scanner-safe-writer.py` (bridge-scoped, Write-only for
...
(`{{HARNESS_HOOKS_DIR}}/owner-decision-tracker.py`) that detects prose decision-ask
...
`{{HARNESS_RULES_DIR}}/operating-model.md`. Carries rule-cited soft authority:
cited by `{{HARNESS_RULES_DIR}}/loyal-opposition.md` and `AGENTS.md` as the
...
work subject is recorded in `{{HARNESS_CONFIG_DIR}}/session/work-subject.json` and is set
...
`{{HARNESS_CONFIG_DIR}}/session/active-session-role.json` and per-session `role-*.json` files
...
without writing `{{HARNESS_CONFIG_DIR}}/session/active-session-role.json`.
...
**Implementation pointer:** `{{HARNESS_HOOKS_DIR}}/*` init/ wrap/ topic-router surfaces;
...
protected narrative artifacts at `{{HARNESS_RULES_DIR}}/*.md`, `AGENTS.md`,
...
`{{HARNESS_RULES_DIR}}/canonical-terminology.toml` for the required-terms matrix.
