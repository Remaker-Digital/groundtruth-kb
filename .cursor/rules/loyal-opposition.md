...
Canonical operating-model reference: `.cursor/rules/operating-model.md` (rule-cited soft authority).
Canonical glossary load: `.cursor/rules/canonical-terminology.md` must be read
...
the canonical operating-model artifact at `.cursor/rules/operating-model.md` §1
...
Per Sub-slice C of GTKB-GOV-AUQ-ENFORCEMENT-STACK and `.cursor/rules/file-bridge-protocol.md` "Mandatory Owner Decisions / Input Section Gate":
