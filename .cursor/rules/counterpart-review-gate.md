...
This rule auto-loads via `.cursor/rules/` convention and is TRACKED in git.
...
(Active per Sub-slice C of GTKB-GOV-AUQ-ENFORCEMENT-STACK; mechanically enforced by `.cursor/hooks/bridge-compliance-gate.py`.)
...
The bridge-propose helper at scripts/skill-helpers/bridge-propose/write_bridge.py pre-populates this section by default via pre_populate_prior_deliberations: glossary-source seeding from .cursor/rules/canonical-terminology.md plus optional semantic search. Authors review and prune the pre-populated entries.
