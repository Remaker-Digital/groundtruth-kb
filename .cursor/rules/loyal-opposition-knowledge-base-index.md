...
## Active Rules under `.cursor/rules/`
...
- `.cursor/rules/canonical-terminology.md` — core primer subset only at startup;
  full corpus and activity-specific terms load on `::open <activity>`.
- `.cursor/rules/file-bridge-protocol.md` — bridge statuses, gates, claim/preflight.
...
`.cursor/skills/` adapters are generated from the canonical baseline skill source; no separate adapter
