...
This rule is auto-loaded via `.cursor/rules/` convention.
...
(per `.cursor/rules/deliberation-protocol.md`)
...
2. **Prime reads the advisory** as part of the standard Loyal-Opposition-insight review path defined in `.cursor/rules/way-of-working.md`.
...
3. What owner decisions must be durable, recorded via `AskUserQuestion`
   per `.cursor/rules/prime-builder-role.md` § "AskUserQuestion as the
...
Decisions / Input` section (per `.cursor/rules/file-bridge-protocol.md`
...
protected paths (e.g., `.cursor/rules/*.md`, `AGENTS.md`, the root harness instruction file)
