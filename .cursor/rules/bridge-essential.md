...
(negated from the `.cursor/` blanket ignore). Do not remove it.
...
`.cursor/hooks/bridge-axis-2-surface.py`, registered via
...
- Skip the GO/NO-GO discipline for any code change beyond the explicit
  exemptions in `.cursor/rules/counterpart-review-gate.md`
...
- **S292**: Session-start briefs for 7+ sessions silently omitted the in-session
  ...
  in `memory/*.md` are not enforceable; hooks and `.cursor` hook
  registration are.
- **S294**: Discovered `.cursor/` was blanket-ignored in `.gitignore`, so the
