...
authoritative source. Sources include `.cursor/rules/operating-model.md` §2
...
templates (CLAUDE.md, MEMORY.md, `.cursor/rules/canonical-terminology.md`,
...
`.cursor/rules/file-bridge-protocol.md`.
...
`.cursor/rules/file-bridge-protocol.md` Mandatory Specification-Derived
Verification Gate.
...
**Source:** bridge-state surfaces; `.cursor/rules/file-bridge-protocol.md`
...
`.cursor/rules/canonical-terminology.md` by
...
narrative authority `.cursor/rules/sot-read-discipline.md`;

**Implementation pointer:** `.cursor/hooks/sot-read-discipline.py` (canonical hook) plus per-harness command-verb adapters registered in each harness's hook surface; doctor `_check_sot_read_discipline`; owner-authorized bypass `GTKB_SOT_READ_DISCIPLINE_BYPASS`.
...
**Implementation pointer:** `config/registry/sot-artifacts.toml` (`forbidden_substitutes` entries); enforced by `.cursor/hooks/sot-read-discipline.py`.
...
(`.cursor/hooks/bridge-compliance-gate.py`) that fails the Write of bridge
...
`.cursor/hooks/credential-scan.py` (general-purpose, all Write/Edit targets)
and `.cursor/hooks/scanner-safe-writer.py` (bridge-scoped, Write-only for
...
(`.cursor/hooks/owner-decision-tracker.py`) that detects prose decision-ask
...
`.cursor/rules/operating-model.md`. Carries rule-cited soft authority:
cited by `.cursor/rules/loyal-opposition.md` and `AGENTS.md` as the
...
work subject is recorded in `.cursor/session/work-subject.json` and is set
...
`.cursor/session/active-session-role.json` and per-session `role-*.json` files
...
purged); `.cursor/rules/bridge-essential.md` § Operational Mode.
...
without writing `.cursor/session/active-session-role.json`.
...
**Implementation pointer:** `.cursor/hooks/*` init/ wrap/ topic-router surfaces;
...
protected narrative artifacts at `.cursor/rules/*.md`, `AGENTS.md`,
...
`.cursor/rules/canonical-terminology.toml` for the required-terms matrix.
