...
comply with `.cursor/rules/project-root-boundary.md`: all active GT-KB files and
...
`.cursor/hooks/bridge-compliance-gate.py` per the companion bridge thread
...
The rule is mechanically enforced by `.cursor/hooks/bridge-compliance-gate.py`
...
The bridge-compliance-gate hook (`.cursor/hooks/bridge-compliance-gate.py`) mechanically enforces this requirement at Write time.
