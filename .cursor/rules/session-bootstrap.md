...
- Both Prime Builder and Loyal Opposition startup procedures load
  `.cursor/rules/canonical-terminology.md` so the live glossary is available
...
7. In Loyal Opposition mode, process latest `NEW` or `REVISED` entries from
   oldest to newest using `.cursor/rules/file-bridge-protocol.md`.
...
10. Confirm **global baseline** startup surfaces loaded (see
   ...
   - `.cursor/rules/operating-role.md` guidance
   - `.cursor/rules/canonical-terminology.md` (core primer subset only)
...
12. For substantial LO review work inside an opened activity envelope, use:
   - `.cursor/rules/review-checklists.md`
   - `.cursor/rules/template-code-review.md`
   - `.cursor/rules/template-decision-memo.md`
...
.cursor/rules/canonical-terminology.md (core primer subset),
.cursor/rules/file-bridge-protocol.md,
...
- The `.cursor` skills and hook changes are local-only because `.cursor` is git-ignored in this repo.
