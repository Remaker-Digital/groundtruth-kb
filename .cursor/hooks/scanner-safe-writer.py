...
deny record to ``.cursor/hooks/scanner-safe-writer.log`` for the metrics
collector (Tier A #6).
...
``.cursor/hooks/scanner-safe-writer.log``. Fields (``schema_version`` first):
...
DENY_LOG_PATH = Path(".cursor/hooks/scanner-safe-writer.log")
SCHEMA_VERSION = 1
...
def main() -> None:
    ...
    reason = (
        ...
        "values before retrying. See .cursor/hooks/scanner-safe-writer.log "
