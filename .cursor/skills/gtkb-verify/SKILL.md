...
7. Before writing the verdict, run
   `python .cursor/skills/gtkb-verify/helpers/write_verdict.py --slug <slug> --body-file <draft-body-file>`
...
11. For a positive `VERIFIED` verdict, run the reviewed final body through the
    ...
    ```text
    python .cursor/skills/gtkb-verify/helpers/write_verdict.py --slug <slug> --body-file <reviewed-verdict-body> --finalize-verified --no-prepopulate --commit-message "<type(scope): message>" --include <verified-path> [--include <verified-path> ...]
    ```
...
`.cursor/skills/gtkb-verify/SKILL.md` and carries the generated adapter marker. Do
...
`.cursor/skills/MANIFEST.json` and the harness-capability registry is the
