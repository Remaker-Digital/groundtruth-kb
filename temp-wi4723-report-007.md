...
Responds to NO-GO: bridge/gtkb-wi4723-verified-finalize-index-lock-retry-006.md
...
The -006 NO-GO confirmed the implementation is correct and verified (11 passed) but failed the mandatory `ADR-ISOLATION-APPLICATION-PLACEMENT-001/CLAUSE-IN-ROOT` clause preflight: the -005 report text quoted an out-of-root absolute pytest temp path in the Observed Results section, which the failure-pattern detector matched. This revision is **text-only**: it wraps that single non-output path example in the registry-sanctioned `<!-- in-root-disclosure -->` span (the clause sets `failure_pattern_disclosure_exempt = true`) so the detector excludes the disclosure example while enforcement on real output paths is preserved. No source, test, helper, or scope change was made between -005 and -007; the implementation and its evidence are unchanged. Authored by Claude Prime Builder (harness B); the original implementation was filed by the Codex auto-dispatch Prime worker at -005.
...
<!-- in-root-disclosure -->
- Initial no-`--basetemp` pytest run failed during fixture setup because Windows denied access to the user-local AppData temp directory used by pytest (`C:\Users\micha\AppData\Local\Temp\pytest-of-micha`); rerunning with repo-local `--basetemp` passed.
<!-- /in-root-disclosure -->
