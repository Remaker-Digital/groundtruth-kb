...
def _discover_running_worker(project_dir: Path, agent: str) -> dict | None:
    ...
    healthy, health_state = resident_worker.resident_worker_is_healthy(agent)
    snapshot = resident_worker.resident_worker_health_snapshot(agent) or {}
...
def _start_detached(
    ...
    *,
    ...
    if os.name == "nt":
        ...
        script = str((project_dir / "bridge_resident_worker.py").resolve())
        arg_parts = [
            f'"{script}"',
            "--agent", agent,
    ...
    cmd = [
        sys.executable,
        str(project_dir / "bridge_resident_worker.py"),
        "--agent",
        agent,
        "--cadence-minutes",
        str(cadence_minutes),
