...
Per ``bridge/gtkb-bridge-poller-p1-detector-003.md`` section 3.6, these tests
cover append-only event emission, round-trip read, and the typed-event
helpers for bootstrap and transition events.
"""
...
def test_emit_audit_event_appends_jsonl_line(tmp_path: Path) -> None:
    emit_audit_event(tmp_path, "test_kind", {"foo": "bar"})
    contents = (tmp_path / AUDIT_FILENAME).read_text(encoding="utf-8")
...
def test_emit_multiple_events_produces_multiple_lines(tmp_path: Path) -> None:
    emit_audit_event(tmp_path, "a", {})
    emit_audit_event(tmp_path, "b", {})
    emit_audit_event(tmp_path, "c", {})
    lines = (tmp_path / AUDIT_FILENAME).read_text(encoding="utf-8").splitlines()
    assert len(lines) == 3


def test_read_audit_log_round_trip(tmp_path: Path) -> None:
    emit_audit_event(tmp_path, "kind_a", {"k1": "v1"})
    emit_audit_event(tmp_path, "kind_b", {"k2": 42})
    events = read_audit_log(tmp_path)
    assert len(events) == 2
...
def test_read_audit_log_returns_empty_for_missing_file(tmp_path: Path) -> None:
    assert read_audit_log(tmp_path) == ()


def test_read_audit_log_skips_malformed_lines(tmp_path: Path) -> None:
    audit_file = tmp_path / AUDIT_FILENAME
...
def test_emit_bootstrap_event_records_documents_seen(tmp_path: Path) -> None:
    emit_bootstrap_event(tmp_path, documents_seen=42)
    [event] = read_audit_log(tmp_path)
