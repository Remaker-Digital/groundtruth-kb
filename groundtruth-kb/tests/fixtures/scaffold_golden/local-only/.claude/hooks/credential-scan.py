...
``groundtruth_kb.governance.credential_patterns`` when the canonical package
is importable. When the hook runs in a standalone environment that cannot
...
with the canonical module by the ``test_inline_fallback_catalog_matches_canonical``
parity test in ``tests/test_credential_patterns.py`` — drift fails the build.
...
try:
    ...
    # and ``bash_output_pattern_list()`` via
    # ``test_inline_fallback_catalog_matches_canonical`` in
    # ``tests/test_credential_patterns.py``. Drift fails the build.
    CREDENTIAL_PATTERNS = [
