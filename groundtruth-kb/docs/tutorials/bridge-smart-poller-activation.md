...
`scripts/gtkb_dispatcher_daemon.py`. There is no installation
procedure analogous to `install_smart_poller_task.ps1`; the daemon is
activated by daemon configuration in `.claude/settings.json`
...
To verify the daemon is active:

```text
python -c "from groundtruth_kb.cli import main; main(['project','doctor','--dir','.'], standalone_mode=False)"
```

The doctor reports the dispatcher-daemon status via
`_check_dispatcher_daemon` (PASS/WARN/FAIL covering daemon script
presence, both daemon configuration, and dispatch-state freshness) and
