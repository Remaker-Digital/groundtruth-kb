        ...
        field_config={
            "defaults": {
                ...
                "mappings": [
                    {
                        "type": "special",
                        "options": {"match": "null", "result": {"text": "Unavailable", "color": "yellow"}},
                    }
                ],
...
def _metric_query(metric_key: str) -> str:
    if not isinstance(metric_key, str) or metric_key not in (
        ...
        raise ValueError("unsupported_dashboard_metric_key")
...
def build_dashboard() -> dict[str, Any]:
        ...
        _text_panel(
            ...
            "\n".join(
                [
                    ...
                    (
                        "Primary setup: `gt dashboard init` → `gt dashboard install` → `gt "
                        "dashboard start` → `gt dashboard refresh`."
                    ),
                    (
                        "[Manual refresh](http://127.0.0.1:8766/control) binds to loopback only; copy "
                        "local paths from Shortcuts (do not use Open for workspace files)."
                    ),
                ]
            ),
        )
    ...
    panels.extend(
        [
            ...
            _stat_panel(
                ...
                _grid(16, 52, 8, 4),
                _metric_query("native_authority_findings"),
            ),
            _stat_panel(
                builder,
    ...
    detail_panels = [
        _table_panel(
            ...
            _grid(0, 0, 24, 8),
            (
                "SELECT sort_order, section, title, instruction, command, link_label, "
                "link_url FROM setup_steps ORDER BY sort_order;"
            ),
            links={"link_url": "Open"},
        ),
        _table_panel(
    ...
    delivery_details = [
        _table_panel(
            ...
            _grid(0, 0, 24, 10),
            """
            SELECT sort_order, date_label, stage_label, event, version, commit_sha, branch,
                   result, test_results, source, url
    ...
    kpi_details = [
        _table_panel(
            ...
            _grid(0, 0, 24, 7),
            (
                "SELECT metric_label AS metric, value, upper(status) AS status, description "
                "FROM current_metrics ORDER BY metric_key;"
            ),
            hidden_columns=(),
        ),
        _time_series_panel(
    ...
    # collapsed detail row so the existing
