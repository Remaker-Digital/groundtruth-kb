...
The dispatcher daemon does not require static harness registration; it reads
current bridge dispatch state and numbered bridge-file status on each tick. This
module is retained for compatibility with adopters that still consume the
``register`` CLI subcommand from a SessionStart hook.
