...
class TestWidgetInstallation:
    ...
    def test_widget_key_field_present(self, live_widget_page: Page):
        """Widget key field exists (may show key or 'No widget key found')."""
        live_widget_page.wait_for_timeout(1000)
        main_text = live_widget_page.text_content("main") or ""
        has_key_section = (
    ...
    def test_api_url_field_present(self, live_widget_page: Page):
        """API URL field exists on the widget page."""
        main_text = live_widget_page.text_content("main") or ""
        assert "API URL" in main_text, "API URL field not found on widget page"

    def test_widget_key_or_activation_message(self, live_widget_page: Page):
        """Widget key shows a real key OR an activation prompt."""
        live_widget_page.wait_for_timeout(1000)
        main_text = live_widget_page.text_content("main") or ""
        input_values = _get_all_input_values(live_widget_page)
    ...
    def test_installation_section_has_content(self, live_widget_page: Page):
    ...
    def test_embed_code_or_activation_message(self, live_widget_page: Page):
...
class TestWidgetAppearance:
    ...
    def test_color_picker_has_real_value(self, live_widget_page: Page):
...
class TestWidgetContent:
    ...
    def test_header_title_populated(self, live_widget_page: Page):
    ...
    def test_agent_display_name_populated(self, live_widget_page: Page):
...
class TestWidgetPreview:
    ...
    def test_preview_widget_is_interactive(self, live_widget_page: Page):
        ...
        launcher = live_widget_page.locator("text=Powered by Agent Red").first
        ...
        chat_input = live_widget_page.locator(
            "iframe[src*='widget'], "
