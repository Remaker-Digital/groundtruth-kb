...
class TestComplianceTitle:
    ...
    def test_page_title(self, shared_compliance_page: Page):
        """Page shows 'Compliance' in title."""
        text = _main_text(shared_compliance_page)
        assert "compliance" in text.lower()

    def test_pii_scrubbing_card(self, shared_compliance_page: Page):
        """Shows 'PII Scrubbing Enabled' summary card."""
        if _is_rate_limited(shared_compliance_page):
            pytest.skip("Rate limited")
        text = _main_text(shared_compliance_page).lower()
...
class TestComplianceTable:
    ...
    def test_table_present(self, shared_compliance_page: Page):
        """Compliance table exists."""
        if _is_rate_limited(shared_compliance_page):
            pytest.skip("Rate limited")
        table = shared_compliance_page.locator("main table")
        assert table.count() > 0, "Compliance table must exist"

    def test_dsar_column(self, shared_compliance_page: Page):
...
class TestSecretPostureTitle:
    ...
    def test_page_title(self, shared_secrets_page: Page):
        """Page shows 'Secret Posture' title."""
        text = _main_text(shared_secrets_page)
        assert "secret" in text.lower()

    def test_total_secrets_card(self, shared_secrets_page: Page):
        """Shows 'Total Secrets' summary card or 'No secrets' empty state."""
        if _is_rate_limited(shared_secrets_page):
            pytest.skip("Rate limited")
        text = _main_text(shared_secrets_page).lower()
...
class TestSecretPostureTable:
    ...
    def test_table_present(self, shared_secrets_page: Page):
        """Secret posture table exists or empty state shown (no secrets on fresh staging)."""
        if _is_rate_limited(shared_secrets_page):
            pytest.skip("Rate limited")
        table = shared_secrets_page.locator("main table")
        if table.count() > 0:
            return  # Table exists
    ...
    def test_secret_type_badges(self, shared_secrets_page: Page):
...
class TestBillingHealthTitle:
    ...
    def test_page_title(self, shared_billing_health_page: Page):
        """Page shows 'Billing Health' title."""
        text = _main_text(shared_billing_health_page)
        assert "billing" in text.lower()

    def test_summary_cards(self, shared_billing_health_page: Page):
        """Shows summary cards (Total Tenants, Needing Review, Webhook Rate)."""
        if _is_rate_limited(shared_billing_health_page):
            pytest.skip("Rate limited")
...
class TestBillingHealthTable:
    ...
    def test_table_present(self, shared_billing_health_page: Page):
        """Billing health table exists."""
        if _is_rate_limited(shared_billing_health_page):
            pytest.skip("Rate limited")
        table = shared_billing_health_page.locator("main table")
        assert table.count() > 0, "Billing health table must exist"

    def test_status_badges(self, shared_billing_health_page: Page):
...
class TestCostAnalyticsTitle:
    ...
    def test_page_title(self, shared_cost_analytics_page: Page):
        """Page shows 'Cost Analytics' title."""
        text = _main_text(shared_cost_analytics_page)
        assert "cost" in text.lower()

    def test_period_selector(self, shared_cost_analytics_page: Page):
        """Period selector (7d/30d/90d/365d) is present."""
        if _is_rate_limited(shared_cost_analytics_page):
            pytest.skip("Rate limited")
        text = _main_text(shared_cost_analytics_page).lower()
        has_period = any(p in text for p in ["7 day", "30 day", "90 day", "365 day", "last 7", "last 30"])
        if not has_period:
            # Select dropdown may contain period options
            selects = shared_cost_analytics_page.locator("main input[role='searchbox'], main [class*='Select'] input")
...
class TestCostAnalyticsCards:
    ...
    def test_platform_cost_card(self, shared_cost_analytics_page: Page):
    ...
    def test_cost_table(self, shared_cost_analytics_page: Page):
        """Per-tenant cost table exists."""
        if _is_rate_limited(shared_cost_analytics_page):
            pytest.skip("Rate limited")
        table = shared_cost_analytics_page.locator("main table")
...
class TestSLATrendsTitle:
    ...
    def test_page_title(self, shared_sla_trends_page: Page):
        """Page shows 'SLA Trends' title."""
        text = _main_text(shared_sla_trends_page)
        assert "sla" in text.lower()

    def test_range_selector(self, shared_sla_trends_page: Page):
        ...
        if not has_range:
            ...
            buttons = shared_sla_trends_page.locator("main [role='radio'], main input[type='radio']")
...
class TestSLATrendsCharts:
    ...
    def test_uptime_content(self, shared_sla_trends_page: Page):
        """Shows uptime-related content (% or chart)."""
        if _is_rate_limited(shared_sla_trends_page):
            pytest.skip("Rate limited")
        text = _main_text(shared_sla_trends_page).lower()
        has_uptime = "uptime" in text or "%" in _main_text(shared_sla_trends_page)
        assert has_uptime, "Must show uptime data"

    def test_latency_content(self, shared_sla_trends_page: Page):
    ...
    def test_recharts_svg(self, shared_sla_trends_page: Page):
        ...
        if _is_rate_limited(shared_sla_trends_page):
            pytest.skip("Rate limited")
        svgs = shared_sla_trends_page.locator("main svg[class*='recharts'], main .recharts-wrapper svg")
        if svgs.count() == 0:
            # Recharts may use different class naming
            all_svgs = shared_sla_trends_page.locator("main svg")
            assert all_svgs.count() >= 1, "Charts should render as SVG elements"

    def test_error_budget_gauges(self, shared_sla_trends_page: Page):
...
class TestAbuseDetectionTitle:
    ...
    def test_page_title(self, shared_abuse_detection_page: Page):
        """Page shows 'Abuse Detection' title."""
        text = _main_text(shared_abuse_detection_page)
        assert "abuse" in text.lower()

    def test_summary_cards(self, shared_abuse_detection_page: Page):
        """Shows summary cards (Tenants Scanned, Flagged, High-Risk) or empty state."""
        if _is_rate_limited(shared_abuse_detection_page):
            pytest.skip("Rate limited")
...
class TestAbuseDetectionSignals:
    ...
    def test_signal_types(self, shared_abuse_detection_page: Page):
    ...
    def test_risk_table_or_empty(self, shared_abuse_detection_page: Page):
        """Shows high-risk tenants table or empty state."""
        if _is_rate_limited(shared_abuse_detection_page):
            pytest.skip("Rate limited")
        text = _main_text(shared_abuse_detection_page).lower()
        has_table = shared_abuse_detection_page.locator("main table").count() > 0
...
class TestMfaSettingsTitle:
    ...
    def test_page_title(self, shared_mfa_settings_page: Page):
        """Page shows 'MFA' in title."""
        text = _main_text(shared_mfa_settings_page)
        assert "mfa" in text.lower() or "two-factor" in text.lower() or "multi-factor" in text.lower()

    def test_enrollment_or_status(self, shared_mfa_settings_page: Page):
        """Shows either enrollment flow or enabled status card."""
        if _is_rate_limited(shared_mfa_settings_page):
            pytest.skip("Rate limited")
        text = _main_text(shared_mfa_settings_page).lower()
...
class TestMfaSettingsInteraction:
    ...
    def test_action_button(self, shared_mfa_settings_page: Page):
        """Has an action button (Enable MFA or Disable MFA)."""
        if _is_rate_limited(shared_mfa_settings_page):
            pytest.skip("Rate limited")
        page = shared_mfa_settings_page
    ...
    def test_backup_codes_or_qr(self, shared_mfa_settings_page: Page):
