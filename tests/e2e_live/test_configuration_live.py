...
class TestPageHeader:
    ...
    def test_page_title(self, shared_config_page: Page):
        ...
        text = _text(shared_config_page)
        assert re.search(r"config", text, re.I), (
            f"Page title not found. Text: {text[:200]}"
        )

    def test_page_subtitle(self, shared_config_page: Page):
...
class TestSavedConfigurations:
    ...
    def test_saved_configs_section_visible(self, shared_config_page: Page):
        """[EL-config-003/A] 'Saved configurations' section or version history visible."""
        text = _wait_for_config_data(shared_config_page)
        if _is_rate_limited(shared_config_page):
            pytest.skip("Rate limited")
    ...
    def test_save_current_as_button(self, shared_config_page: Page):
        """[EL-config-004/A] 'Save current as' button exists."""
        from playwright.sync_api import expect
        _wait_for_config_data(shared_config_page)
        if _is_rate_limited(shared_config_page):
            pytest.skip("Rate limited")
        btn = shared_config_page.get_by_role(
            "button", name=re.compile(r"Save current", re.I)
        )
        expect(btn.first).to_be_visible(timeout=5_000)

    def test_saved_configs_table_or_list(self, shared_config_page: Page):
        ...
        has_configs = (
            ...
            or shared_config_page.locator("table, [role='list'], [role='listbox']").count() > 0
        )
        assert has_configs, "No saved configs section found"

    def test_active_badge_indicator(self, shared_config_page: Page):
    ...
    def test_config_timestamp_displayed(self, shared_config_page: Page):
    ...
    def test_field_count_badge(self, shared_config_page: Page):
        ...
        if not has_count:
            ...
            badges = shared_config_page.locator(
...
class TestSavedConfigCrud:
    ...
    def test_save_config_opens_modal(self, shared_config_page: Page):
        ...
        assert _click_save_current_as(shared_config_page), (
            "'Save current as…' button not found"
        )

        # Modal should show dialog with name input
        dialog = shared_config_page.locator("[role='dialog']")
        expect(dialog.first).to_be_visible(timeout=3_000)

        # Cancel/close the modal
        cancel = shared_config_page.get_by_role("button", name="Cancel")
        if cancel.count() > 0:
            cancel.first.click()
            shared_config_page.wait_for_timeout(500)

    def test_save_config_modal_has_name_input(self, shared_config_page: Page):
        ...
        name_input = shared_config_page.get_by_label("Configuration name")
    ...
    def test_save_modal_cancel_closes(self, shared_config_page: Page):
    ...
    def test_save_and_delete_named_config(self, shared_config_page: Page):
        ...
        submit = shared_config_page.locator(
        ...
        if submit.first.is_disabled():
            ...
            _fill_and_dirty(name_input, config_name)
            shared_config_page.wait_for_timeout(1000)
        submit.first.click()
        shared_config_page.wait_for_timeout(3000)

        # Verify the config appears
        shared_config_page.reload(wait_until="load")
        text = _wait_for_config_data(shared_config_page)
        ...
        if config_created:
            ...
            del_btns = shared_config_page.locator(
            ...
            for i in range(del_btns.count()):
                ...
                parent_text = shared_config_page.evaluate(
...
class TestBrandToneFields:
    ...
    def test_brand_name_input(self, shared_config_page: Page):
    ...
    def test_brand_voice_textarea(self, shared_config_page: Page):
        ...
        has_field = (
            "brand voice" in text
            or "voice" in text
            or shared_config_page.locator("textarea").count() > 0
        )
        assert has_field, "Brand voice textarea not found"

    def test_formality_dropdown(self, shared_config_page: Page):
        ...
        if not has_formality:
            selects = shared_config_page.locator("select, [role='combobox']")
    ...
    def test_response_length_dropdown(self, shared_config_page: Page):
    ...
    def test_return_window_input(self, shared_config_page: Page):
    ...
    def test_refund_policy_textarea(self, shared_config_page: Page):
...
class TestBehaviorControls:
    ...
    def test_escalation_threshold_slider(self, shared_config_page: Page):
    ...
    def test_idle_timeout_input(self, shared_config_page: Page):
    ...
    def test_max_ai_turns_input(self, shared_config_page: Page):
...
class TestAiAndLanguage:
    ...
    def test_custom_instructions_textarea(self, shared_config_page: Page):
        ...
        assert has_instructions or shared_config_page.locator("textarea").count() > 0, (
            "Custom instructions section not found"
        )

    def test_primary_language_dropdown(self, shared_config_page: Page):
    ...
    def test_supported_languages_multi_select(self, shared_config_page: Page):
...
class TestAiSuggestions:
    ...
    def test_ai_suggestion_badges_exist(self, shared_config_page: Page):
        """[EL-config-038..041/A] AI suggestion badges visible near at least one field."""
        _wait_for_config_data(shared_config_page)
        if _is_rate_limited(shared_config_page):
            pytest.skip("Rate limited")
        _wait_for_form_inputs(shared_config_page)
        text = _text(shared_config_page).lower()
        ...
        if not has_suggestion:
            ...
            brand_input = _find_field_by_label(shared_config_page, "Brand name")
            if brand_input:
                brand_input.fill("")
                shared_config_page.wait_for_timeout(3000)
                has_suggestion = shared_config_page.locator(
                    ".mantine-Badge-root:has-text('Suggested'), "
                    "[class*='Badge']:has-text('Suggested')"
                ).count() > 0
        if not has_suggestion:
...
class TestLoadingAndStatus:
    ...
    def test_page_loads_without_error(self, shared_config_page: Page):
    ...
    def test_config_content_loaded(self, shared_config_page: Page):
    ...
    def test_draft_auto_save_indicator(self, shared_config_page: Page):
...
class TestDraftSaveRoundTrip:
    ...
    def test_edit_draft_and_save(self, shared_config_page: Page):
        ...
        target_input = _find_field_by_label(shared_config_page, "Brand name")
        if target_input is None:
            target_input = _find_field_by_label(shared_config_page, "Brand voice")
        if target_input is None:
            # Fallback to first visible textarea
            textareas = shared_config_page.locator("textarea")
        ...
        saved = _save_and_wait(shared_config_page)
        if not saved:
            pytest.skip(
        ...
        # Reload and verify
        shared_config_page.reload(wait_until="load")
        shared_config_page.wait_for_selector("text=Configuration", timeout=15_000)
        shared_config_page.wait_for_timeout(3000)

        if _is_rate_limited(shared_config_page):
            pytest.skip("Rate limited after reload")

        page_text = shared_config_page.text_content("main") or ""
    ...
    def test_save_button_triggers_api_call(self, shared_config_page: Page):
        ...
        shared_config_page.route("**/api/config**", _capture_api)

        # Use brand name field with _fill_and_dirty for reliable Mantine state
        target = _find_field_by_label(shared_config_page, "Brand name")
        if target is None:
            target = shared_config_page.locator("input[type='text']").first
        ...
        saved = _save_and_wait(shared_config_page)
        shared_config_page.unroute("**/api/config**")
...
class TestNegativeConfigInputs:
    ...
    def test_xss_in_brand_name_sanitized(self, shared_config_page: Page):
    ...
    def test_xss_in_textarea_sanitized(self, shared_config_page: Page):
        ...
        _save_and_wait(shared_config_page)

        shared_config_page.reload(wait_until="load")
        text = _wait_for_config_data(shared_config_page)
        assert "config" in text.lower(), "Page crashed after XSS in textarea"

    def test_sql_injection_in_brand_name(self, shared_config_page: Page):
    ...
    def test_overlong_brand_name_handled(self, shared_config_page: Page):
        ...
        _save_and_wait(shared_config_page)  # OK if save was blocked

        text = _text(shared_config_page).lower()
        assert "config" in text or "brand" in text, "Page crashed after overlong brand name"

    def test_overlong_textarea_handled(self, shared_config_page: Page):
    ...
    def test_empty_brand_name_accepted_or_validated(self, shared_config_page: Page):
        ...
        _save_and_wait(shared_config_page)  # OK if save blocked (empty required field)

        text = _text(shared_config_page).lower()
        assert "config" in text or "brand" in text, "Page crashed after emptying brand name"

    def test_special_characters_in_fields(self, shared_config_page: Page):
    ...
    def test_rapid_save_cycle_stability(self, shared_config_page: Page):
        ...
        for i in range(3):
            ...
            shared_config_page.wait_for_timeout(300)
            if _save_and_wait(shared_config_page):
                any_saved = True
    ...
    def test_save_config_with_empty_name(self, shared_config_page: Page):
        ...
        if submit.count() > 0:
            ...
            if not is_blocked:
                try:
                    ...
                    is_blocked = True
            shared_config_page.wait_for_timeout(1500)
    ...
    def test_save_config_with_xss_name(self, shared_config_page: Page):
