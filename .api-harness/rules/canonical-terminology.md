...
authoritative source. Sources include `.api-harness/rules/operating-model.md` §2
...
templates (CLAUDE.md, MEMORY.md, `.api-harness/rules/canonical-terminology.md`,
...
`.api-harness/rules/file-bridge-protocol.md`.
...
`.api-harness/rules/file-bridge-protocol.md` Mandatory Specification-Derived
Verification Gate.
...
**Source:** bridge-state surfaces; `.api-harness/rules/file-bridge-protocol.md`
...
`.api-harness/rules/canonical-terminology.md` by
...
narrative authority `.api-harness/rules/sot-read-discipline.md`;

**Implementation pointer:** `.api-harness/hooks/sot-read-discipline.py` (canonical hook) plus per-harness command-verb adapters registered in each harness's hook surface; doctor `_check_sot_read_discipline`; owner-authorized bypass `GTKB_SOT_READ_DISCIPLINE_BYPASS`.
...
**Implementation pointer:** `config/registry/sot-artifacts.toml` (`forbidden_substitutes` entries); enforced by `.api-harness/hooks/sot-read-discipline.py`.
...
(`.api-harness/hooks/bridge-compliance-gate.py`) that fails the Write of bridge
...
`.api-harness/hooks/credential-scan.py` (general-purpose, all Write/Edit targets)
and `.api-harness/hooks/scanner-safe-writer.py` (bridge-scoped, Write-only for
...
(`.api-harness/hooks/owner-decision-tracker.py`) that detects prose decision-ask
...
`.api-harness/rules/operating-model.md`. Carries rule-cited soft authority:
cited by `.api-harness/rules/loyal-opposition.md` and `AGENTS.md` as the
...
work subject is recorded in `.api-harness/session/work-subject.json` and is set
...
`.api-harness/session/active-session-role.json` and per-session `role-*.json` files
...
purged); `.api-harness/rules/bridge-essential.md` § Operational Mode.
...
without writing `.api-harness/session/active-session-role.json`.
...
**Implementation pointer:** `.api-harness/hooks/*` init/ wrap/ topic-router surfaces;
...
protected narrative artifacts at `.api-harness/rules/*.md`, `AGENTS.md`,
...
`.api-harness/rules/canonical-terminology.toml` for the required-terms matrix.
