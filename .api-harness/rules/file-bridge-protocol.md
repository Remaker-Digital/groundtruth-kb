...
comply with `.api-harness/rules/project-root-boundary.md`: all active GT-KB files and
...
`.api-harness/hooks/bridge-compliance-gate.py` per the companion bridge thread
...
The rule is mechanically enforced by `.api-harness/hooks/bridge-compliance-gate.py`
...
The bridge-compliance-gate hook (`.api-harness/hooks/bridge-compliance-gate.py`) mechanically enforces this requirement at Write time.
