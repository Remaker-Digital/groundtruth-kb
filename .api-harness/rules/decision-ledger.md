...
- decision:
  ...
  Codex must first examine bridge state and examine the state
  of all active sub-agents.
- impact:
  Status updates should report current bridge state from bridge state bridge
...
- impact:
  ...
  Loyal Opposition review/verification work. Codex should use bridge state
...
- decision:
  The project adopts dedicated Codex review rules under `.api-harness/rules/`:
...
- impact:
  Future review and investigation work should use these rules as the default operating scaffold.
- status:
  Active
