...
- Both Prime Builder and Loyal Opposition startup procedures load
  `.api-harness/rules/canonical-terminology.md` so the live glossary is available
...
- The bridge dispatcher and bridge state must be checked at startup
  in both Prime Builder and Loyal Opposition roles.
- bridge state is the authoritative source for bridge queue state;
...
1. Read bridge state and run dispatcher status/health when topology,
   target eligibility, or dispatch health is in question.
2. Treat bridge state as authoritative; do not use
...
7. In Loyal Opposition mode, process latest `NEW` or `REVISED` entries from
   oldest to newest using `.api-harness/rules/file-bridge-protocol.md`.
...
10. Confirm **global baseline** startup surfaces loaded (see
   ...
   - `.api-harness/rules/operating-role.md` guidance
   - `.api-harness/rules/canonical-terminology.md` (core primer subset only)
...
12. For substantial LO review work inside an opened activity envelope, use:
   - `.api-harness/rules/review-checklists.md`
   - `.api-harness/rules/template-code-review.md`
   - `.api-harness/rules/template-decision-memo.md`
13. In Loyal Opposition mode, include the standard project-state startup
    ...
    - bridge latest-status counts and latest `NEW`/`REVISED`
      actionability
...
.api-harness/rules/canonical-terminology.md (core primer subset),
.api-harness/rules/file-bridge-protocol.md,
...
bridge state and dispatcher status/health for bridge work. Prime
...
- The `.api-harness` skills and hook changes are local-only because `.api-harness` is git-ignored in this repo.
