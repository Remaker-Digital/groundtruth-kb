...
(negated from the `.api-harness/` blanket ignore). Do not remove it.
...
`.api-harness/hooks/bridge-axis-2-surface.py`, registered via
...
- Skip the GO/NO-GO discipline for any code change beyond the explicit
  exemptions in `.api-harness/rules/counterpart-review-gate.md`
...
- **S292**: Session-start briefs for 7+ sessions silently omitted the in-session
  ...
  in `memory/*.md` are not enforceable; hooks and `.api-harness` hook
  registration are.
- **S294**: Discovered `.api-harness/` was blanket-ignored in `.gitignore`, so the
