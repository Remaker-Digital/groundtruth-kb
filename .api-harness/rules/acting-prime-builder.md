...
(refresh deliberation) establish the current state: `.api-harness/config.toml` and
`.api-harness/hooks.json` ARE a live Codex interception boundary on Windows for
...
relying on `.api-harness/hooks.json` as a live interception boundary, and must use
...
(Active per S331 owner directive; mechanically enforced by `.api-harness/hooks/owner-decision-tracker.py` 
