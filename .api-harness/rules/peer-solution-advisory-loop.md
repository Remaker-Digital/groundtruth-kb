...
This rule is auto-loaded via `.api-harness/rules/` convention.
...
(per `.api-harness/rules/deliberation-protocol.md`)
...
2. **Prime reads the advisory** as part of the standard Loyal-Opposition-insight review path defined in `.api-harness/rules/way-of-working.md`.
...
3. What owner decisions must be durable, recorded via `AskUserQuestion`
   per `.api-harness/rules/prime-builder-role.md` § "AskUserQuestion as the
...
Decisions / Input` section (per `.api-harness/rules/file-bridge-protocol.md`
...
protected paths (e.g., `.api-harness/rules/*.md`, `AGENTS.md`, the root harness instruction file)
