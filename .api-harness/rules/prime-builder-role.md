...
- Fresh-session startup must load `.api-harness/rules/canonical-terminology.md`
...
(per `DELIB-S324-PB-INTERROGATION-DIRECTIVE` and `.api-harness/rules/operating-model.md` §1)
(per `GOV-SOURCE-OF-TRUTH-FRESHNESS-001` v2 clause (a) and `.api-harness/rules/sot-read-discipline.md`)
...
mechanically enforced by `.api-harness/hooks/owner-decision-tracker.py` per `bridge/gtkb-gov-askuserquestion-enforcement-stack-slice-a-hook-reenable-014.md` VERIFIED.)
