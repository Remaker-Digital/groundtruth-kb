...
2. **Patch content is limited to exactly the four declared shared files** (`.agent/skills/MANIFEST.json`, `.api-harness/skills/MANIFEST.json`, `.api-harness/skills/MANIFEST.json`, `config/agent-control/harness-capability-registry.toml`), and the numstat inside the patch (1/1, 2/2, 1/1, 2/2 respectively) matches the report's declared numstat exactly.
