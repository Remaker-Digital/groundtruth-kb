...
1. **Byte-parity claim independently recomputed — confirmed exact.** Direct
   ...
   `.api-harness/hooks/bridge-compliance-gate.py` and
...
6. **Independently proven: the `[template]`-side failures are NEW, not
   ...
   same grep against `git show HEAD:.api-harness/hooks/bridge-compliance-gate.py`
...
- declared_target_paths: [".api-harness/hooks/bridge-compliance-gate.py", "groundtruth-kb/templates/hooks/bridge-compliance-gate.py", "platform_tests/hooks/test_bridge_compliance_gate_envelope_head.py", "platform_tests/scripts/test_bridge_compliance_gate_disposition.py"]
