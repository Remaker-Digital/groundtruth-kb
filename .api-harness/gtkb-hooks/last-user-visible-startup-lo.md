...
Generated: 2026-08-13T00:50:59Z
...
- GT-KB open MemBase work items: 13 (subject-scoped; 887 across all subjects)
- GT-KB dashboard-scoped bridge/contention entries, non-authoritative for queue state: 347
- GT-KB drift changed paths: 3
...
- Harness parity: warn (phase-1 catalog parity; operational_readiness=not evaluated; run phase-2 readiness and hook discovery diff; harness=codex, role=loyal-opposition, DEGRADED=3, EXTRA=1, PASS=42, UNSUPPORTED=11, WARN=1)
...
- Current work items: 4981; non-terminal: 887; active projects: 59; ungrouped non-terminal: 134.
- Status counts: deferred=1, not_a_defect=8, open=886, resolved=3513, retired=446, verified=67, wont_fix=60.
- Active project states:
  - `GTKB-BRIDGE-PROTOCOL-RELIABILITY`: 161 non-terminal (open=161); top: `WI-5686` - ENVELOPE_RESPONDER_BY_STATUS is inverted: dispatchable bridge artifacts carry the author role in the artifact-head ::init line instead of the responder role [open, P0].
  ...
  - `PROJECT-GTKB-SESSION-ENVELOPE`: 11 non-terminal (open=11); top: `WI-5749` - Shared .api-harness/session/envelope.json is clobbered by concurrent harnesses: non-deterministic session identity, provenance and authorization-pointer drift [open, P0].
  ...
  - `GT-KB Get Healthy Recovery Program`: 4 non-terminal (open=4); top: `WI-6184` - Qualify the Get Healthy baseline and produce the Release Re-planning Baseline [open, P0].
