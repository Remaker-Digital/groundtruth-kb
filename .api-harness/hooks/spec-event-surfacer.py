...
Per-session ledger at ``.api-harness/session/spec-events-seen.jsonl`` prevents
...
LEDGER_REL_PATH = ".api-harness/session/spec-events-seen.jsonl"
...
SESSION_START_REL_PATH = ".api-harness/session/session-start.json"
...
def _resolve_session_started_at(cwd: Path) -> tuple[str, bool]:
    ...
    Primary source: ``.api-harness/session/session-start.json`` (written by the
