...
- **backlog** — the unified view of all known work for an application or platform, including all work items and the project/sub-project groupings that organize those work items. Known work should converge into one MemBase source of truth, with generated views such as `memory/work_list.md` used only for human-readable compatibility once convergence is implemented. Allowed synonyms: "work_list", "the work list", "standing backlog". Forbidden: using "backlog" as a synonym for "ignore list" or "deprecated"; using "backlog item" as a separate conceptual class distinct from work item.
