...
def fig1_radar():
    ...
    for i, (angle, val) in enumerate(zip(angles[:-1], values)):
        offset = 0.8
        ax.text(
            angle, val + offset, str(val), ha="center", va="center", fontsize=11, fontweight="bold", color=BRAND_BLUE
        )

    ax.set_title("OrbaTech Engineering Maturity Assessment", fontsize=14, fontweight="bold", color=BRAND_BLUE, pad=20)
...
def fig2_architecture():
    ...
    def box(x, y, w, h, label, color, sublabel=None):
        rect = mpatches.FancyBboxPatch(
            (x, y), w, h, boxstyle="round,pad=0.1", facecolor=color, edgecolor=BRAND_BLUE, linewidth=1.5
        )
        ax.add_patch(rect)
        ax.text(
            x + w / 2,
        ...
        if sublabel:
            ax.text(x + w / 2, y + h / 2 - 0.18, sublabel, ha="center", va="center", fontsize=7.5, color="#E0E0E0")
    ...
    rect = mpatches.FancyBboxPatch(
        (1.5, 1.2), 7, 0.6, boxstyle="round,pad=0.1", facecolor="#FFF3CD", edgecolor=BRAND_ORANGE, linewidth=1.5
    )
    ax.add_patch(rect)
    ax.text(
        5,
        1.5,
        "TENANT ISOLATION: TenantId column + EF Core Global Query Filters\n"
    ...
    for y_from, y_to in [(6.0, 5.2), (4.7, 3.9)]:
        ax.annotate("", xy=(5, y_to), xytext=(5, y_from), arrowprops=dict(arrowstyle="->", color=BRAND_GRAY, lw=1.5))

    ax.set_title("OrbaTech Application Architecture", fontsize=14, fontweight="bold", color=BRAND_BLUE, pad=10)
...
def fig3_security():
    ...
    colors = [
        BRAND_RED if s <= 2 else BRAND_ORANGE if s <= 4 else BRAND_YELLOW if s <= 6 else BRAND_GREEN for s in scores
    ]
...
def fig5_azure():
    ...
    def azure_box(x, y, w, h, label, sublabel, color):
        rect = mpatches.FancyBboxPatch(
            (x, y), w, h, boxstyle="round,pad=0.08", facecolor=color, edgecolor="#2C3E50", linewidth=1.2
        )
        ax.add_patch(rect)
        ax.text(
            x + w / 2, y + h / 2 + 0.12, label, ha="center", va="center", fontsize=9, fontweight="bold", color="white"
        )
        ax.text(x + w / 2, y + h / 2 - 0.12, sublabel, ha="center", va="center", fontsize=7, color="#E0E0E0")

    # Title
    ax.text(
        6,
        7.7,
        "Recommended Azure Architecture — OrbaTech CRM",
        ha="center",
        fontsize=14,
        fontweight="bold",
        color=BRAND_BLUE,
    ...
    ax.text(
        6,
        7.1,
        "Azure Canada Central (Primary) / East US (Secondary)",
        ha="center",
        fontsize=10,
        color=BRAND_BLUE,
        fontstyle="italic",
    ...
    rect2 = mpatches.FancyBboxPatch(
        (1.0, 0.8), 8.5, 0.8, boxstyle="round,pad=0.08", facecolor="#E8F5E9", edgecolor=BRAND_GREEN, linewidth=1.2
    )
    ax.add_patch(rect2)
    ax.text(
        5.25,
        1.2,
        "Private Endpoints  |  Managed Identities  |  SQL RLS  |  Azure RBAC",
        ha="center",
        va="center",
        fontsize=9,
        fontweight="bold",
    ...
    # Arrows
    ax.annotate("", xy=(6, 5.6), xytext=(6, 6.2), arrowprops=dict(arrowstyle="->", color=BRAND_GRAY, lw=1.5))
...
# ══════════════════════════════════════════════════════════════════════════
def fig6_costs():
    components = ["Compute", "SQL DB", "Storage/CDN", "Identity", "Monitoring", "Networking", "Email", "Backup/DR"]
    ...
    ax.set_title("Estimated Monthly Azure Costs by Scaling Tier", fontsize=14, fontweight="bold", color=BRAND_BLUE)
...
def fig7_roadmap():
    ...
    for i, (label, prio, start, dur, color) in enumerate(tasks):
        ...
        ax.text(start + dur / 2, y, f"{label}", ha="center", va="center", fontsize=8, fontweight="bold", color="white")
        ax.text(start - 0.3, y, prio, ha="right", va="center", fontsize=8, fontweight="bold", color=color)
    ...
    ax.set_yticks([])
    ax.set_title("Recommended Technical Priority Roadmap", fontsize=14, fontweight="bold", color=BRAND_BLUE)
...
# ══════════════════════════════════════════════════════════════════════════
def fig8_comparison():
    categories = ["Compute", "Database", "Identity", "Tenant Isolation", "CI/CD", "Monitoring", "Secrets", "Email/SMS"]
    orbatech = [
        "App Service\n(PaaS)",
        "SQL Server",
        "Entra ID B2C\n(recommended)",
    ...
    # Headers
    ax.text(3, len(categories) + 0.5, "OrbaTech CRM", ha="center", fontsize=12, fontweight="bold", color=BRAND_ORANGE)
    ax.text(6, len(categories) + 0.5, "Dimension", ha="center", fontsize=12, fontweight="bold", color=BRAND_BLUE)
    ax.text(9, len(categories) + 0.5, "Agent Red", ha="center", fontsize=12, fontweight="bold", color=BRAND_GREEN)
    ...
    for i, (cat, orb, ar) in enumerate(zip(categories, orbatech, agentred)):
        ...
        if i % 2 == 0:
            rect = mpatches.FancyBboxPatch(
                (0.5, y - 0.4), 11, 0.8, boxstyle="round,pad=0.02", facecolor="#F0F4F8", edgecolor="none"
            )
            ax.add_patch(rect)
    ...
    ax.set_title(
        "OrbaTech vs Agent Red — Azure Deployment Comparison", fontsize=14, fontweight="bold", color=BRAND_BLUE, pad=20
    )
...
def fig9_integration():
    ...
    rect1 = mpatches.FancyBboxPatch(
        (0.5, 1), 4.5, 4, boxstyle="round,pad=0.15", facecolor="#E8F4FD", edgecolor=BRAND_BLUE, linewidth=2
    )
    ax.add_patch(rect1)
    ax.text(2.75, 4.7, "OrbaTech CRM", ha="center", fontsize=12, fontweight="bold", color=BRAND_BLUE)

    orb_items = ["Contacts & Profiles", "Deal Pipeline", "Email History", "Activity Timeline", "Custom Fields"]
    for i, item in enumerate(orb_items):
        ax.text(2.75, 4.0 - i * 0.6, f"• {item}", ha="center", fontsize=9, color="#2C3E50")

    # Agent Red box
    rect2 = mpatches.FancyBboxPatch(
        (7, 1), 4.5, 4, boxstyle="round,pad=0.15", facecolor="#E8F5E9", edgecolor=BRAND_GREEN, linewidth=2
    )
    ax.add_patch(rect2)
    ax.text(9.25, 4.7, "Agent Red AI", ha="center", fontsize=12, fontweight="bold", color=BRAND_GREEN)

    ar_items = ["AI Conversations", "Sentiment Analysis", "Quality Scores", "Lead Qualification", "Summaries"]
    for i, item in enumerate(ar_items):
        ax.text(9.25, 4.0 - i * 0.6, f"• {item}", ha="center", fontsize=9, color="#2C3E50")

    # Arrows
    ax.annotate("", xy=(7, 3.8), xytext=(5, 3.8), arrowprops=dict(arrowstyle="->", color=BRAND_BLUE, lw=2.5))
    ax.text(6, 4.1, "Contact Context\nOpportunity Status\nActivity History", ha="center", fontsize=8, color=BRAND_BLUE)

    ax.annotate("", xy=(5, 2.2), xytext=(7, 2.2), arrowprops=dict(arrowstyle="->", color=BRAND_GREEN, lw=2.5))
    ax.text(6, 1.6, "Summaries\nSentiment\nQuality Metrics", ha="center", fontsize=8, color=BRAND_GREEN)

    # Integration layer
    rect3 = mpatches.FancyBboxPatch(
        (4.5, 0.3), 3, 0.5, boxstyle="round,pad=0.08", facecolor="#FFF3CD", edgecolor=BRAND_ORANGE, linewidth=1.5
    )
    ax.add_patch(rect3)
    ax.text(6, 0.55, "OAuth 2.0  |  REST  |  Webhooks", ha="center", fontsize=9, fontweight="bold", color="#856404")

    ax.set_title(
        "OrbaTech + Agent Red — Bidirectional Integration", fontsize=14, fontweight="bold", color=BRAND_BLUE, pad=10
    )
...
def fig10_summary_matrix():
    ...
    for i in range(len(platforms)):
        for j in range(len(dimensions)):
            ...
            ax1.text(j, i, str(data[i, j]), ha="center", va="center", fontsize=11, fontweight="bold", color=color)

    ax1.set_title("Enterprise Readiness Scores (1-5)", fontsize=13, fontweight="bold", color=BRAND_BLUE)
