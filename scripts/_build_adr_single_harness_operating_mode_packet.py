...
ADR_BODY = (
    ...
    "dispatch was provided by the dispatcher daemon (scripts/gtkb_dispatcher_daemon.py) "
    ...
    "dispatcher daemon active; single-harness dispatcher NOT active.\n"
    "- Single-harness (first-class): single harness installed with multi-element role set "
    "{prime-builder, loyal-opposition}; dispatcher daemon registered but spawns nothing (no counterpart); "
    "single-harness dispatcher active via Desktop scheduled task (per DCL-SINGLE-HARNESS-DISPATCHER-DESKTOP-TASK-001).\n"
    "\n"
    "Role records (harness-state/harness-registry.json) carry role as a JSON list (the wire representation of a "
    ...
    "retired in Slice 4 (2026-05-09) and the dispatcher daemon is the live multi-harness "
    ...
    "owner answer S341).\n"
    "- harness-state/harness-registry.json role wire format is list-of-strings drawn from "
    ...
    "scripts/session_self_initialization.py, scripts/gtkb_dispatcher_daemon.py migrate from scalar "
    ...
    "Dispatch substrates:\n"
    "\n"
    "- Multi-harness: dispatcher daemon (existing, unchanged).\n"
