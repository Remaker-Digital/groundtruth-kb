...
Optional flags:
    --session-id <id>     Override CLAUDE_SESSION_ID env var
...
def _resolve_session_id(arg_value: str | None) -> str:
    """Return the session_id from --session-id or CLAUDE_SESSION_ID env."""
    if arg_value:
        return arg_value
    env_value = os.environ.get(SESSION_ENV_VAR)
    if env_value:
        return env_value
    raise SystemExit(f"session_id required: pass --session-id or set {SESSION_ENV_VAR} env var")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    common_kwargs = dict(help="(see module docstring for details)")
    ...
    p_claim.add_argument("--session-id", help="Override CLAUDE_SESSION_ID env var.")
    ...
    p_release.add_argument("--session-id", help="Override CLAUDE_SESSION_ID env var.")
