...
# WI-6228 Changes B and C — single source for the harness-baseline skills prefix.
#
# This value appears in three roles that must agree: the projection-path safety
# check applied to `canonical_source` (Change B — a stale prefix here rejected
# every registry entry as an "Unsafe projection path"), the content-rewrite regex
# that repoints helper/reference paths inside projected bodies, and the manifest's
# declared `source_of_truth` (Change C). Previously all three carried the retired
# baseline's prefix literal independently, so they could drift apart silently.
BASELINE_SKILLS_PREFIX = ".harness-baseline-configuration/skills/"

_CANONICAL_RESOURCE_RE = re.compile(
    r"(?P<prefix>"
    + re.escape(BASELINE_SKILLS_PREFIX)
    + r"|\.claude/skills/)(?P<skill>[A-Za-z0-9._-]+)/(?P<resource>helpers|references)/"
)
...
def _strip_all_adapter_blocks(text: str) -> str:
    return ADAPTER_BLOCK_RE.sub("", text)
...
def _safe_relative(value: str, *, prefix: str | tuple[str, ...], suffix: str) -> str:
...
def render_adapter(source_text: str, adapter: CursorSkillAdapter, *, generated_at: str) -> str:
    renderer = _codex_generator()
    clean_source = _strip_all_adapter_blocks(source_text)
    ...
    rendered = renderer.render_adapter(clean_source, renderer_adapter, generated_at=generated_at)
    ...
    rendered = re.sub(
        r"\.(?:codex|claude|agents|agent|goose|api-harness)(/skills/[A-Za-z0-9._-]+/)", r".cursor\1", rendered
    )
    rendered = re.sub(
        r"\.(?:codex|claude|agents|agent|goose|api-harness)(\\skills\\[A-Za-z0-9._-]+\\)", r".cursor\1", rendered
    )
    # Resource references map to local Cursor skills directory:
    rendered = _CANONICAL_RESOURCE_RE.sub(
        lambda match: f".cursor/skills/{match.group('skill')}/{match.group('resource')}/",
        rendered,
    )
    return rendered
...
def _manifest_content(adapters: list[CursorSkillAdapter]) -> bytes:
    payload = {
        ...
        "source_of_truth": BASELINE_SKILLS_PREFIX + "*/SKILL.md",
