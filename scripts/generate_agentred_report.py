...
OUTPUT_PATH = os.path.join(
    BASE, "applications", "Agent_Red", "pdf-tooling", "AgentRed-Technical-Evaluation-Report.docx"
)
...
def chart_maturity_radar():
    ...
    ax.set_title("Agent Red Engineering Maturity Assessment", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=30)
...
def chart_competitive_comparison():
    ...
    ax.set_title("AI Customer Service Feature Comparison", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15)
...
def chart_azure_costs():
    ...
    colors = ["#1565C0", "#2196F3", "#FF6F00", "#4CAF50", "#B71C1C", "#E65100", "#64B5F6", "#00695C"]

    for i, (label, vals) in enumerate(costs.items()):
        ax.bar(x, vals, width, label=label, bottom=bottom, color=colors[i], edgecolor="white", linewidth=0.5)
        bottom += np.array(vals)

    totals = [sum(v[i] for v in costs.values()) for i in range(3)]
    for i, total in enumerate(totals):
        ax.text(
            i, total + 50, f"${total:,}/mo", ha="center", va="bottom", fontsize=12, fontweight="bold", color=DARK_GRAY
        )
    ...
    ax.set_title(
        "Agent Red Estimated Azure Monthly Costs by Scale Tier", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15
    )
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def chart_architecture_current():
    ...
    ax.set_ylim(0, 10)
    ax.axis("off")
    ax.set_title("Agent Red Multi-Agent Pipeline Architecture", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15)

    def draw_box(x, y, w, h, label, color, sublabel=None, fontsize=10):
        rect = mpatches.FancyBboxPatch(
            (x, y), w, h, boxstyle="round,pad=0.15", facecolor=color, edgecolor="#333333", linewidth=1.5, alpha=0.85
        )
        ax.add_patch(rect)
        ...
        if sublabel:
            ax.text(
                ...
                va="center",
                fontsize=8,
                color=MID_GRAY if is_light else "#DDDDDD",
            )
    ...
    draw_box(
        ...
        REMAKER_RED,
        "52 Routers | SSE Streaming | JWT + API Key Auth | Rate Limiting",
        fontsize=12,
    )
    ...
    draw_box(
        ...
        "#37474F",
        "Per-Tenant Isolation | Durable Subscriptions",
        fontsize=11,
    )
    ...
    # Data tier
    draw_box(0.5, 0.5, 3.5, 1.3, "Azure Cosmos DB", "#1A237E", "20+ Collections | Tenant Partitions")
    draw_box(4.5, 0.5, 3, 1.3, "Azure Key Vault", "#2E7D32", "Envelope Encryption | HSM KEK")
...
def chart_azure_deployment():
    ...
    ax.set_ylim(0, 9)
    ax.axis("off")
    ax.set_title("Agent Red Current Azure Deployment", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15)
    ...
    draw_box(
        ...
        13,
        1.0,
        "Azure Container Apps Ingress + Custom Domains",
        "#0D47A1",
        "SSL Termination | CORS | Health Probes",
        fontsize=12,
    )
    ...
    # IaC + CI/CD
    draw_box(0.5, 0.8, 6, 1.0, "Terraform IaC (7 .tf files) | GitHub Actions (8 workflows)", "#E8F5E9", fontsize=10)
    draw_box(7, 0.8, 6.5, 1.0, "ACR Registry | 3 Dockerfiles | Managed Identity RBAC", "#FBE9E7", fontsize=10)
...
def chart_priority_roadmap():
    ...
    for i, (name, start, end, color, priority) in enumerate(tasks):
        ax.barh(i, end - start, left=start, height=0.6, color=color, alpha=0.8, edgecolor="white", linewidth=0.5)
    ...
    ax.set_title(
        "Agent Red Recommended Technical Priority Roadmap", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15
    )
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.set_xlim(0, 13)
...
def chart_security_heatmap():
    ...
    ax.barh(
        ...
        scores,
        color=[colors_map[s] for s in scores],
        height=0.6,
        edgecolor="white",
        linewidth=1,
    )
    ...
    ax.set_title("Security Posture Assessment", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def chart_integration_architecture():
    ...
    ax.axis("off")
    ax.set_title(
        "Bidirectional Integration: Agent Red AI ↔ OrbaTech CRM",
        fontsize=14,
        fontweight="bold",
        color=DARK_GRAY,
        pad=15,
    ...
    def draw_box(x, y, w, h, label, color, sublabel=None, fontsize=10):
        ...
        if sublabel:
            ax.text(x + w / 2, y + h / 2 - 0.3, sublabel, ha="center", va="center", fontsize=8, color="#DDDDDD")

    # Agent Red side (LEFT — subject of this report)
    draw_box(0.3, 5.0, 5, 1.3, "Agent Red AI", REMAKER_RED, "Python | Azure | Cosmos DB", fontsize=13)
    ...
    draw_box(7.7, 5.0, 5, 1.3, "OrbaTech CRM", "#1565C0", ".NET 8 | Blazor | SQL Server", fontsize=13)
    ...
    ax.text(6.5, 6.5, "AI Insights → CRM Records", fontsize=10, ha="center", color=REMAKER_RED, fontweight="bold")
    ax.text(6.5, 0.7, "CRM Data → AI Context", fontsize=10, ha="center", color="#1565C0", fontweight="bold")
...
def chart_annex_comparison():
    ...
    ax.axis("off")
    ax.set_title(
        "Annex A: Agent Red Azure Deployment vs OrbaTech Recommended Configuration",
        fontsize=13,
        fontweight="bold",
        color=DARK_GRAY,
        pad=15,
    ...
    table = ax.table(cellText=rows, colLabels=columns, cellLoc="center", loc="center", colWidths=[0.25, 0.35, 0.35])
...
def chart_test_coverage():
    ...
    ax.set_title(
        f"Test Suite Distribution (Total: {sum(counts):,} tests)",
        fontsize=14,
        fontweight="bold",
        color=DARK_GRAY,
        pad=15,
...
def build_report():
    ...
    add_figure_caption(
        doc,
        1,
        "Engineering maturity across eight dimensions (scale of 1-10). "
    ...
    add_figure_caption(
        doc,
        2,
        "Agent Red multi-agent pipeline architecture showing the 6 core agents, "
    ...
    stack_rows = [
        (
            "API Framework",
            "FastAPI + Uvicorn\n(4 workers, tini PID 1)",
            "High-performance async Python framework. Automatic OpenAPI docs. Pydantic v2 validation. Production-ready with tini signal handling.",
        ),
        (
            "AI/ML",
            "Azure OpenAI\n(GPT-4o, GPT-4o-mini)",
            "Enterprise-grade AI models with Azure-managed scaling. Langfuse observability for prompt monitoring and cost tracking.",
        ),
        (
            "Database",
            "Azure Cosmos DB\n(20+ collections)",
            "Globally distributed NoSQL with native partition-key tenant isolation. Serverless + provisioned throughput options. Continuous 7-day backup.",
        ),
        (
            "Message Bus",
            "NATS JetStream",
            "High-performance agent-to-agent communication with per-tenant isolation, durable subscriptions, and at-least-once delivery.",
        ),
        (
            "Frontend (Admin)",
            "React 18.3 + Mantine\n(3 SPAs)",
            "Three tailored admin interfaces: Shopify embedded, standalone, and platform operator. Shared component library.",
        ),
        (
            "Frontend (Widget)",
            "Preact 10.25\n(Shadow DOM + iframe)",
            "Lightweight embeddable widget with CSS isolation (Shadow DOM launcher) and full DOM isolation (iframe panel). 8-language support.",
        ),
        (
            "Encryption",
            "AES-256-GCM\nAzure Key Vault HSM KEK",
            "Per-tenant envelope encryption with HSM-backed master key. Auto-rotation. No plaintext sensitive data at rest.",
        ),
        (
            "Infrastructure",
            "Terraform + Docker\nAzure Container Apps",
            "9 independently scaled containers. Terraform IaC for environment management. 3 Dockerfiles with non-root user.",
        ),
        (
            "CI/CD",
            "GitHub Actions\n(8 workflows)",
            "Build, test (5-shard parallel), lint, deploy pipelines. Dependency vulnerability scanning. OpenAPI compatibility checks.",
        ),
    ]
    ...
    maturity_rows = [
        (
            "Tech Stack",
            "9/10",
            "Python 3.12, FastAPI, Cosmos DB, NATS, React, Preact. All current versions. Cloud-native choices.",
        ),
        (
            "Architecture",
            "9/10",
            "Multi-agent pipeline with mixin orchestrator. Plugin system. 3 SPAs + widget. Repository pattern with tenant-scoped base class.",
        ),
        (
            "Code Quality",
            "8/10",
            "Ruff linting (CI-enforced), import cycle detection, dead code analysis, cyclomatic complexity monitoring. ESLint for TypeScript.",
        ),
        (
            "Testing",
            "8/10",
            "5,400+ tests across 10 test types. 5-shard parallel CI. Coverage gate. API fuzzing, mutation testing, property-based testing.",
        ),
        (
            "CI/CD",
            "8/10",
            "8 GitHub Actions workflows. Automated build/test/deploy pipeline. Dependency vulnerability scanning. OpenAPI compatibility.",
        ),
        (
            "Security",
            "9/10",
            "Envelope encryption, HSM-backed keys, managed identities, input sanitization, pre-auth rate limiting, audit logging, PII tokenization.",
        ),
        (
            "Documentation",
            "7/10",
            "Extensive internal docs (CLAUDE.md system, knowledge DB with 2,092 specs). Admin guide site. Legal drafts. Could improve public API docs.",
        ),
        (
            "Deployment",
            "9/10",
            "9 Azure Container Apps with Terraform IaC. 3 Dockerfiles. Staging + production. Managed identity RBAC. Health probes.",
        ),
    ]
    ...
    add_figure_caption(
        doc,
        3,
        "Test suite distribution by type. Integration tests form the largest category, "
    ...
    sec_details = [
        (
            ...
            "Triple authentication model: Shopify JWT session tokens, SHA-256 hashed API keys via Key Vault, and publishable widget keys with pk_live_ prefix scoping. Additional support for MFA/TOTP, magic link auth, and Argon2 password hashing.",
        ),
        (
            "Authorization (Adequate)",
            "Per-tenant RBAC with TeamMemberRole enum. Platform admin separated via PLATFORM_ADMIN_TENANT_ID sentinel. Dedicated admin console. Area for improvement: more granular permission model.",
        ),
        (
            "Data Encryption (Strong)",
            "AES-256-GCM envelope encryption with Azure Key Vault HSM-backed master KEK. Per-tenant Data Encryption Keys (DEKs). Auto-rotation via Key Vault. All sensitive data encrypted at rest.",
        ),
        (
            "Secrets Management (Strong)",
            "All secrets in Azure Key Vault with managed identity access. No hardcoded keys policy enforced by guardrail regex (SPEC-1845). API key rotation endpoint. CMK auto-rotation.",
        ),
        (
            "Tenant Isolation (Strong)",
            "Cosmos DB partition keys enforce physical data separation at the database level. TenantScopedRepository base class structurally prevents cross-tenant access. Per-tenant NATS isolation for message routing.",
        ),
        (
            "Audit Logging (Adequate)",
            "Append-only audit log with 12 event types and 1-year retention. GDPR compliance with PII scrubbing, data export, cascading deletion, and consent management.",
        ),
    ]
    ...
    isolation_layers = [
        (
            "Database Layer",
            "Cosmos DB partition keys use tenant_id as the partition key on all 20+ collections. Cross-partition queries are structurally impossible without explicit tenant_id specification.",
        ),
        (
            "Application Layer",
            "TenantScopedRepository base class enforces mandatory tenant_id on every database operation. All repositories inherit from this base class, making tenant isolation a structural guarantee rather than a convention.",
        ),
        (
            "Message Bus Layer",
            "NATS JetStream uses per-tenant subject namespacing for agent-to-agent communication, preventing message leakage between tenants.",
        ),
        (
            "Encryption Layer",
            "Per-tenant Data Encryption Keys (DEKs) ensure that even with database-level access, one tenant's data cannot be decrypted with another tenant's key.",
        ),
        (
            "Observability Layer",
            "Per-tenant OpenTelemetry correlation (tenant_id as span attribute) enables isolated monitoring and debugging without cross-tenant data exposure.",
        ),
    ]
    ...
    add_figure_caption(
        doc,
        5,
        "Feature capability comparison across twelve AI customer service dimensions. "
    ...
    competitors = [
        (
            ...
            "$25-$500/user/mo",
            "Market leader with Einstein AI. Comprehensive but complex and expensive. Strong enterprise adoption. AI features are add-ons to existing CRM.",
        ),
        (
            "Zendesk AI",
            "$55-$169/agent/mo",
            "Established support platform with AI agent capabilities. Strong ticket management and knowledge base. AI features expanding rapidly.",
        ),
        (
            "Intercom Fin",
            "$29-$132/seat/mo",
            "Leading conversational support platform. Fin AI agent handles common queries. Strong in-app messaging and product tours. Growing AI capabilities.",
        ),
        (
            "Tidio",
            "$0-$59/mo",
            "Budget-friendly chatbot platform for small businesses. Limited AI sophistication. Strong Shopify integration. Growing market share in micro-business segment.",
        ),
    ]
    ...
    rel_items = [
        (
            ...
            "All 9 containers have health check endpoints. Container Apps automatically restarts unhealthy instances and routes traffic away during failures.",
        ),
        (
            "Auto-scaling",
            "HTTP-triggered scaling (2-8 replicas for gateway, 2-10 for response generator) and NATS-triggered scaling for agent containers ensures capacity matches demand.",
        ),
        (
            "Database resilience",
            "Cosmos DB continuous backup with 7-day point-in-time restore (PITR). Serverless + provisioned throughput options prevent throttling under load.",
        ),
        (
            "Message durability",
            "NATS JetStream durable subscriptions provide at-least-once delivery guarantees for agent-to-agent communication.",
        ),
        (
            "Critic-retraction",
            "The critic agent can retract an AI response mid-stream if quality thresholds are not met, preventing low-quality responses from reaching customers.",
        ),
        (
            "Areas for improvement",
            "Multi-region disaster recovery is not yet implemented. Load testing at 1,000+ tenant scale has not been validated. Circuit breaker patterns could be more comprehensive.",
        ),
    ]
    ...
    svc_items = [
        (
            "Deployment process",
            "Automated build/deploy scripts (build.py, deploy.py). GitHub Actions triggers Docker build, pushes to ACR, deploys to Container Apps. UI-only overlay deploys in 30-60 seconds.",
        ),
        (
            "Monitoring",
            "Azure Monitor + Application Insights for infrastructure. Langfuse for AI-specific observability (prompt monitoring, token usage, cost tracking). Custom observability pipeline.",
        ),
        (
            "Logging",
            "Structured logging with Azure Log Analytics workspace. Per-tenant correlation via OpenTelemetry span attributes.",
        ),
        (
            "Diagnostics",
            "Health check endpoints on all containers. Pre-flight check scripts for staging and production environments. Test host containers for automated verification.",
        ),
        (
            "Database management",
            "Schema changes managed through application code (Cosmos DB is schema-flexible). Migration scripts in scripts/ directory with safety gates.",
        ),
    ]
    ...
    perf_items = [
        (
            "AI Pipeline Latency",
            "The multi-agent pipeline (intent classification, knowledge retrieval, response generation, critic review) introduces sequential latency. SSE streaming mitigates perceived latency by delivering tokens incrementally. Measured at approximately 5 seconds end-to-end, 69-80 tokens per response.",
        ),
        (
            "Cosmos DB Performance",
            "Partition key-based queries are O(1) at the database level. Cross-partition queries are avoided by design. Serverless mode handles bursty workloads; provisioned throughput prevents throttling at scale.",
        ),
        (
            "Rate Limiting",
            "Per-tenant rate limiting at 300 RPM (derived from ramp-to-overload testing: 1,380 RPM safe capacity per replica) provides noisy-neighbor protection.",
        ),
        (
            "Container Scaling",
            "HTTP-triggered and NATS-triggered auto-scaling independently scales each agent based on actual demand. Response Generator (most compute-intensive) scales up to 10 replicas.",
        ),
        (
            "Caching",
            "Redis caching for rate limiting and tenant configuration. Langfuse caching for AI observability. Additional caching opportunities exist for knowledge base embeddings.",
        ),
    ]
    ...
    add_figure_caption(
        doc,
        6,
        "Current Agent Red Azure deployment showing 9 container apps, "
    ...
    containers = [
        ...
        (
            ...
            "2-6 replicas",
            "NATS scaling",
            "GPT-4o-mini. Classifies customer intent.",
        ),
        ...
        (
            ...
            "2-10 replicas",
            "NATS scaling",
            "GPT-4o. Generates customer responses.",
        ),
    ...
    add_figure_caption(
        doc,
        7,
        "Estimated monthly Azure costs by scale tier. Azure OpenAI becomes the "
        ...
        (
            ...
            [
                (
                    ...
                    "Extend Azure Monitor alerting rules. Add SLO dashboards (availability, latency p99, error rate). Implement on-call rotation alerting.",
                ),
                (
                    "Load/Stress Testing at Scale",
                    "Validate 100-tenant and 1,000-tenant performance using Locust load testing. Identify bottlenecks in NATS, Cosmos DB throughput, and OpenAI rate limits.",
                ),
                (
                    "SOC 2 Type II Preparation",
                    "Document security controls. Implement evidence collection automation. Engage third-party auditor. Target 6-month audit window.",
                ),
                (
                    "Public API Documentation",
                    "Publish OpenAPI-based developer documentation. Create integration quickstart guides. Enable self-serve API key provisioning.",
                ),
            ],
        ),
        (
            ...
            [
                (
                    ...
                    "Deploy secondary region (Canada East or East US 2). Configure Cosmos DB multi-region writes. Implement Azure Front Door for geographic routing and failover.",
                ),
                (
                    "French/Spanish Localization QA",
                    "Widget supports 8 languages but admin SPAs are English-only. Extend localization to admin interfaces. Professional translation review for fr-CA and es-MX.",
                ),
                (
                    "Mobile-Responsive Admin SPA",
                    "Optimize admin dashboards for tablet and mobile viewports. Priority: standalone admin SPA used by non-Shopify merchants.",
                ),
            ],
        ),
        (
            ...
            [
                (
                    ...
                    "Publish a partner SDK enabling CRM systems (like OrbaTech) to integrate bidirectionally. Include webhooks, OAuth, and event schemas.",
                ),
                (
                    "Self-Serve Onboarding",
                    "Automate tenant provisioning with self-serve signup, Stripe checkout, and guided setup wizard. Reduce time-to-value for new merchants.",
                ),
                (
                    "Marketplace / App Store",
                    "Enable third-party plugin agents via the existing plugin architecture. Publish agent marketplace for domain-specific AI capabilities.",
                ),
            ],
        ),
        (
            ...
            [
                (
                    "OrbaTech CRM Integration",
                    "Implement bidirectional data flow: CRM contact data enriches AI context, AI conversation insights flow back to CRM records. See Annex B for detailed architecture.",
                ),
            ],
        ),
    ]
    ...
    doc.paragraphs[-1].alignment = WD_ALIGN_PARAGRAPH.CENTER
    add_figure_caption(
        doc, 9, "Side-by-side comparison of Agent Red current and OrbaTech recommended Azure deployments."
    )
    doc.add_paragraph()
    ...
    diffs = [
        (
            "Compute Model",
            "Agent Red uses Azure Container Apps with 9 independently scaled microservices, enabling fine-grained resource allocation per agent. OrbaTech's recommended App Service PaaS model is simpler and more cost-effective for a monolithic .NET application.",
        ),
        (
            "Database Strategy",
            "Agent Red's Cosmos DB provides schema-flexible document storage with native partition-key tenant isolation, optimized for real-time AI conversations. OrbaTech's recommended Azure SQL with Elastic Pool is appropriate for structured CRM data with complex queries and reporting.",
        ),
        (
            "Identity Approach",
            "Agent Red implements custom identity with multiple authentication paths (JWT, API keys, widget keys) to support diverse integration scenarios (Shopify, standalone, embedded widget). OrbaTech's recommended Entra ID B2C provides managed consumer identity suitable for direct user authentication.",
        ),
        (
            "CI/CD Maturity",
            "Agent Red has a mature 8-workflow pipeline with 5-shard parallel testing, vulnerability scanning, and automated deployment. OrbaTech needs to build CI/CD from the ground up, but could leverage Agent Red's pipeline patterns as a template.",
        ),
        (
            "Security Depth",
            "Agent Red implements per-tenant envelope encryption with HSM-backed KEK, managed identities, and pre-auth rate limiting. OrbaTech's recommended configuration includes Row-Level Security and Key Vault, but has less encryption depth.",
        ),
    ]
    ...
    ar_to_crm = [
        (
            ...
            "AI-generated summaries of customer interactions are written to CRM contact records as activities, creating a complete interaction timeline accessible to sales teams.",
        ),
        (
            "Sentiment Analysis",
            "Real-time customer sentiment scores from AI conversations are attached to opportunity records, providing sales teams with early warning signals on at-risk deals.",
        ),
        (
            "Quality Metrics",
            "Per-interaction quality scores (from the critic agent) help sales managers identify training opportunities and customer satisfaction trends.",
        ),
        (
            "Lead Qualification",
            "AI-assessed purchase intent signals from customer conversations automatically create or update opportunity records in the CRM pipeline.",
        ),
    ]
    ...
    crm_to_ar = [
        (
            "Contact Context",
            "Customer profiles, purchase history, and communication preferences from OrbaTech enrich Agent Red's AI context, enabling personalized responses.",
        ),
        (
            "Opportunity Status",
            "Active deals and pipeline stages inform Agent Red's conversation routing, ensuring high-value prospects receive priority handling and appropriate agent assignment.",
        ),
        (
            "Activity History",
            "Recent emails, calls, and meetings from CRM provide conversation context, reducing customer frustration from repeating information.",
        ),
        (
            "Custom Fields",
            "Tenant-specific custom fields from OrbaTech enable Agent Red to adapt its responses to each business's unique CRM taxonomy and domain vocabulary.",
        ),
    ]
