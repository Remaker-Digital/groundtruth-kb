...
d.insert_spec(
    ...
    assertions=[{
        "type": "grep",
        "file": "admin/shared/ConfigurationPage.tsx",
        "pattern": "conflict|warning|mismatch"
    }],
)
...
tests = [
    ...
    ("TEST-9650", "SPEC-1713", "System prompt contains config authority instruction",
     "grep", "system_prompt_builder.py contains explicit instruction that config fields override KB for policy values",
     "test_config_kb_authority.py", "TestConfigAuthority", "test_prompt_contains_authority_instruction"),
    ("TEST-9651", "SPEC-1713", "Config values appear before KB section in prompt",
     "structural", "MERCHANT CONFIGURATION section appears before VERIFIED KNOWLEDGE BASE section in assembled prompt",
     "test_config_kb_authority.py", "TestConfigAuthority", "test_config_before_kb_in_prompt_ordering"),
    ("TEST-9652", "SPEC-1713", "Authority rule covers all injected_in_prompt fields",
     "structural", "Authority instruction references return_policy, shipping_info, brand_voice, support_hours, warranty_info",
     "test_config_kb_authority.py", "TestConfigAuthority", "test_authority_covers_all_injected_fields"),
    ("TEST-9653", "SPEC-1713", "fields.yaml injected_in_prompt fields match authority list",
     "structural", "All fields with injected_in_prompt: true in fields.yaml are covered by the authority rule",
     "test_config_kb_authority.py", "TestConfigAuthority", "test_fields_yaml_injected_match_authority_list"),

    # SPEC-1714 tests (scanner extension)
    ("TEST-9654", "SPEC-1714", "Scanner detects numerical duration contradiction between config and KB",
     "functional", "Scanner returns HIGH severity conflict when config return_policy says 14 days and KB article says 30 days",
     "test_config_kb_conflict_scanner.py", "TestConfigKBConflictScanner", "test_detects_numerical_duration_contradiction"),
    ("TEST-9655", "SPEC-1714", "Scanner detects price contradiction between config and KB",
     "functional", "Scanner returns HIGH severity when config says free shipping over $50 and KB says $75",
     "test_config_kb_conflict_scanner.py", "TestConfigKBConflictScanner", "test_detects_price_contradiction"),
    ("TEST-9656", "SPEC-1714", "Conflict results include config_vs_kb source_type",
     "structural", "Conflict results from config-vs-KB checks include source_type config_vs_kb",
     "test_config_kb_conflict_scanner.py", "TestConfigKBConflictScanner", "test_conflict_source_type_is_config_vs_kb"),
    ("TEST-9657", "SPEC-1714", "Scanner checks all policy config fields",
     "functional", "Scanner compares return_policy, shipping_info, warranty_info, support_hours, custom_policies against KB",
     "test_config_kb_conflict_scanner.py", "TestConfigKBConflictScanner", "test_checks_all_policy_fields"),
    ("TEST-9658", "SPEC-1714", "Scan endpoint includes config-vs-KB results",
     "functional", "POST /api/admin/knowledge/scan response contains config_vs_kb conflicts alongside article conflicts",
     "test_config_kb_conflict_scanner.py", "TestConfigKBConflictScanner", "test_scan_endpoint_includes_config_conflicts"),
    ("TEST-9659", "SPEC-1714", "No false positive when config and KB agree",
     "functional", "Scanner returns no config_vs_kb conflicts when config and KB articles state the same values",
     "test_config_kb_conflict_scanner.py", "TestConfigKBConflictScanner", "test_no_false_positive_when_aligned"),
    ("TEST-9660", "SPEC-1714", "Empty config fields skip conflict check",
     "functional", "Scanner skips comparison for config fields that are null or empty string",
     "test_config_kb_conflict_scanner.py", "TestConfigKBConflictScanner", "test_empty_config_fields_skip_check"),

    # SPEC-1715 tests (admin UI warnings)
    ("TEST-9661", "SPEC-1715", "Config save triggers conflict check against KB",
     "functional", "After PUT /api/admin/config, UI runs conflict check and displays warning if contradictions found",
     "test_config_kb_warning_ui.py", "TestConfigKBWarningUI", "test_config_save_triggers_conflict_check"),
    ("TEST-9662", "SPEC-1715", "Warning displays specific field and conflicting article",
     "functional", "Warning Alert shows field name, config value, KB article title, and conflicting KB excerpt",
     "test_config_kb_warning_ui.py", "TestConfigKBWarningUI", "test_warning_shows_field_and_article_details"),
    ("TEST-9663", "SPEC-1715", "KB article upload triggers conflict check against config",
     "functional", "After saving a KB article, UI checks content against current config field values",
     "test_config_kb_warning_ui.py", "TestConfigKBWarningUI", "test_kb_upload_triggers_conflict_check"),
    ("TEST-9664", "SPEC-1715", "Warning is dismissible without action",
     "functional", "Merchant can close the conflict warning without making changes -- advisory only",
     "test_config_kb_warning_ui.py", "TestConfigKBWarningUI", "test_warning_is_dismissible"),
    ("TEST-9665", "SPEC-1715", "No warning shown when no conflicts exist",
     "functional", "Config save with no KB contradictions shows no warning Alert",
     "test_config_kb_warning_ui.py", "TestConfigKBWarningUI", "test_no_warning_when_no_conflicts"),
]
