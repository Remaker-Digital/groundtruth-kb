#!/usr/bin/env python3
"""Generate compact Ollama skill adapters from canonical GT-KB skills."""
...
OLLAMA_SKILLS_RELATIVE_PATH = Path(".ollama") / "skills"
MANIFEST_NAME = "MANIFEST.json"
GENERATED_MARKER = "<!-- GTKB-OLLAMA-SKILL-ADAPTER"
GENERATED_END_MARKER = "GTKB-OLLAMA-SKILL-ADAPTER -->"
FRONTMATTER_KEY_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_-]*$")


class OllamaSkillAdapterError(ValueError):
    """Raised when canonical skill metadata cannot produce an Ollama adapter."""


@dataclass(frozen=True)
class OllamaSkillAdapter:
    capability_id: str
    canonical_name: str
    description: str
...
def _adapter_relative_path(source_relative_path: str) -> str:
    ...
    return (OLLAMA_SKILLS_RELATIVE_PATH / source_path.parent.name / "SKILL.md").as_posix()


def validate_skill_frontmatter(text: str, path: str) -> dict[str, str]:
    lines = text.splitlines()
    if not lines or lines[0].lstrip("\ufeff").strip() != "---":
        raise OllamaSkillAdapterError(f"{path}: missing opening YAML frontmatter delimiter")
    ...
    if closing_index is None:
        raise OllamaSkillAdapterError(f"{path}: missing closing YAML frontmatter delimiter")
    ...
    for offset, line in enumerate(lines[1:closing_index], start=2):
        ...
        if not stripped or stripped.startswith(("#", "- ")):
            continue
        if ":" not in stripped:
            raise OllamaSkillAdapterError(f"{path}:{offset}: malformed frontmatter line")
        key, value = stripped.split(":", 1)
        key = key.strip()
        if not FRONTMATTER_KEY_RE.match(key):
            raise OllamaSkillAdapterError(f"{path}:{offset}: invalid frontmatter key {key!r}")
        fields[key] = value.strip().strip("\"'")

    for required in ("name", "description"):
        if not fields.get(required):
            raise OllamaSkillAdapterError(f"{path}: missing non-empty {required!r} frontmatter field")
    return fields
...
def build_adapters(project_root: Path) -> list[OllamaSkillAdapter]:
    registry = _load_registry(project_root)
    adapters: list[OllamaSkillAdapter] = []
...
def _generated_block(adapter: OllamaSkillAdapter, generated_at: str) -> str:
    return "\n".join(
        [
            GENERATED_MARKER,
            "Generated: true",
...
def render_adapter(adapter: OllamaSkillAdapter, *, generated_at: str) -> str:
    lines = [
        _generated_block(adapter, generated_at),
        f"# Ollama Skill Adapter: {adapter.canonical_name}",
        "",
...
def _manifest_content(adapters: list[OllamaSkillAdapter]) -> str:
    payload = {
        "schema_version": 1,
...
def _remove_orphan_adapters(project_root: Path, adapters: list[OllamaSkillAdapter], *, check: bool) -> list[str]:
    skills_root = project_root / OLLAMA_SKILLS_RELATIVE_PATH
...
def generate(project_root: Path, *, check: bool = False) -> tuple[list[str], list[str]]:
    ...
    manifest_path = project_root / OLLAMA_SKILLS_RELATIVE_PATH / MANIFEST_NAME
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (OllamaSkillAdapterError, OSError, tomllib.TOMLDecodeError) as exc:
        print(f"Ollama skill adapters: FAIL ({exc})", file=sys.stderr)
        return 1

    if changed:
        action = "would update" if args.check else "updated"
        print(f"Ollama skill adapters: {action} {len(changed)} file(s)")
        for path in changed:
            print(f"- {path}")
        return 1 if args.check else 0

    print(f"Ollama skill adapters: PASS ({len(adapter_paths)} adapters current)")
    return 0
