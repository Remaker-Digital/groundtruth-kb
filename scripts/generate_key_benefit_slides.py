...
def add_textbox(slide, left, top, width, height, text, font_size, color=WHITE,
                bold=False, alignment=PP_ALIGN.LEFT, font_name="Montserrat"):
...
def add_bullet_list(slide, left, top, width, height, bullets, font_size=20,
...
def create_benefit_slide(prs, slide_data, slide_num):
    ...
    add_textbox(
        slide, margin_left, indicator_y, Inches(3), Inches(0.4),
        f"KEY BENEFIT {slide_num} OF 3",
        font_size=12, color=LIGHT_GRAY, bold=True,
        font_name="Montserrat"
    )

    # ── Brand accent line ──
    add_accent_line(
        slide, margin_left, Inches(1.1),
        Inches(2.5), color=BRAND_RED, thickness=Pt(4)
    )

    # ── Main title ──
    add_textbox(
        slide, margin_left, Inches(1.4), content_width, Inches(1.2),
        slide_data["title"],
    ...
    # ── Subtitle ──
    add_textbox(
        slide, margin_left, Inches(2.6), content_width, Inches(0.6),
        slide_data["subtitle"],
        font_size=24, color=BRAND_RED, bold=True,
        font_name="Montserrat"
    )
    ...
    add_bullet_list(
        slide, margin_left, bullet_top, content_width, bullet_height,
        slide_data["points"],
        font_size=22, color=WHITE, bullet_color=BRAND_RED
    )
    ...
    rect = add_rounded_rect(
        slide, callout_left, callout_top,
    ...
    # Callout text
    add_textbox(
        slide, callout_left + Inches(0.4), callout_top + Inches(0.15),
        callout_width - Inches(0.8), callout_height - Inches(0.3),
        slide_data["callout"],
        font_size=20, color=BRAND_RED, bold=True,
...
def main():
    ...
    print(f"New dimensions: {prs.slide_width} x {prs.slide_height} EMU ({prs.slide_width/914400:.2f}\" x {prs.slide_height/914400:.2f}\")")
