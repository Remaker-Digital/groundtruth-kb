...
OUTPUT_PATH = os.path.join(
    BASE, "applications", "Agent_Red", "pdf-tooling", "OrbaTech-Technical-Evaluation-Report.docx"
)
...
def chart_maturity_radar():
    ...
    ax.set_title("OrbaTech Engineering Maturity Assessment", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=30)
...
def chart_competitive_comparison():
    ...
    ax.set_title("Feature Capability Comparison", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15)
...
def chart_azure_costs():
    ...
    colors = ["#1565C0", "#2196F3", "#64B5F6", "#90CAF9", "#4CAF50", "#81C784", "#FFC107", "#FF7043"]
    ...
    for i, total in enumerate(totals):
        ax.text(
            i, total + 30, f"${total:,}/mo", ha="center", va="bottom", fontsize=12, fontweight="bold", color=DARK_GRAY
        )
    ...
    ax.set_title("Estimated Azure Monthly Costs by Scale Tier", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def chart_architecture_current():
    ...
    ax.set_ylim(0, 8)
    ax.axis("off")
    ax.set_title("OrbaTech Current Architecture", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15)
    ...
    def draw_box(x, y, w, h, label, color, sublabel=None, fontsize=10):
        ...
        ax.text(
            ...
            fontsize=fontsize,
            fontweight="bold",
            color="white" if color not in ["#FFF9C4", "#E3F2FD", "#F3E5F5", "#E8F5E9"] else DARK_GRAY,
        )
        if sublabel:
            ax.text(
                x + w / 2,
    ...
    draw_box(
        ...
        "Presentation Layer",
        "#1565C0",
        "Blazor Server + WebAssembly (Hybrid) | DevExpress UI Components",
        fontsize=12,
    )

    # API layer
    draw_box(0.5, 5.0, 5, 1.0, "REST API Controllers", "#2196F3", "7 Controllers | Swagger/OpenAPI")
    draw_box(6, 5.0, 5.5, 1.0, "ASP.NET Core Identity", "#4CAF50", "Cookie Auth | 2FA | 4 Roles")

    # Business logic
    draw_box(0.5, 3.5, 3.5, 1.0, "Service Layer", "#7B1FA2", "Per-entity Services")
    draw_box(4.5, 3.5, 3.5, 1.0, "Repository Layer", "#7B1FA2", "EF Core + Dapper")
    draw_box(8.5, 3.5, 3, 1.0, "Email Service", "#FF6F00", "Windows Service")

    # Data layer
    draw_box(0.5, 2.0, 5, 1.0, "SQL Server", "#1A237E", "EF Core 8.0 | 40+ Stored Procedures | 5 Triggers")
    draw_box(6, 2.0, 5.5, 1.0, "File Storage", "#455A64", "Local Filesystem (C:\\Uploads)")
    ...
    draw_box(
        ...
        11,
        1.0,
        "Multi-Tenant Isolation: Application-Level TenantId Filtering (No DB-Level RLS)",
        "#FFF9C4",
        fontsize=11,
    )
...
def chart_azure_recommended():
    ...
    ax.axis("off")
    ax.set_title(
        "Recommended Azure Architecture for OrbaTech CRM", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15
    )
    ...
    def draw_box(x, y, w, h, label, color, sublabel=None, fontsize=10):
        ...
        ax.text(
            ...
            fontsize=fontsize,
            fontweight="bold",
            color="white" if color not in ["#FFF9C4", "#E3F2FD", "#F3E5F5", "#E8F5E9", "#FBE9E7"] else DARK_GRAY,
        )
        if sublabel:
            ax.text(
                x + w / 2,
    ...
    draw_box(
        ...
        "#0D47A1",
        "SSL Termination | WAF | Geographic Routing (CA/US)",
        fontsize=12,
    )

    # App tier
    draw_box(0.5, 5.8, 6, 1.2, "Azure App Service (Production)", "#1565C0", "P1v3 | Auto-Scale 2-6 | Deployment Slots")
    draw_box(7, 5.8, 6.5, 1.2, "Azure App Service (Staging)", "#42A5F5", "S1 | Deployment Slot Swap")

    # Services
    draw_box(0.5, 4.2, 3, 1.1, "Azure SQL Database", "#1A237E", "S2 (50 DTU) → Elastic Pool")
    draw_box(4, 4.2, 3, 1.1, "Azure Blob Storage", "#455A64", "File Attachments + Backups")
    draw_box(7.5, 4.2, 3, 1.1, "Azure Key Vault", "#2E7D32", "Secrets | API Keys | Certs")
    draw_box(11, 4.2, 2.5, 1.1, "Entra ID B2C", "#6A1B9A", "SSO | MFA")

    # Supporting services
    draw_box(0.5, 2.7, 3, 1.0, "Azure Monitor", "#E65100", "App Insights | Log Analytics")
    draw_box(4, 2.7, 3, 1.0, "Azure Comm Services", "#00695C", "Email Delivery")
    draw_box(7.5, 2.7, 3, 1.0, "Azure Redis Cache", "#B71C1C", "Session | Tenant Config")
    draw_box(11, 2.7, 2.5, 1.0, "Azure Backup", "#37474F", "SQL + Blob PITR")
    ...
    draw_box(
        ...
        13,
        1.0,
        "Security: Row-Level Security (RLS) | Managed Identities | Private Endpoints | RBAC",
        "#E8F5E9",
        fontsize=11,
    )
...
def chart_priority_roadmap():
    ...
    ax.set_title("Recommended Technical Priority Roadmap", fontsize=14, fontweight="bold", color=DARK_GRAY, pad=15)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.set_xlim(0, 13)
    ax.grid(axis="x", alpha=0.3)
...
def chart_integration_architecture():
    ...
    ax.axis("off")
    ax.set_title(
        "Bidirectional Integration: OrbaTech CRM ↔ Agent Red AI",
        fontsize=14,
        fontweight="bold",
        color=DARK_GRAY,
        pad=15,
    ...
    def draw_box(x, y, w, h, label, color, sublabel=None, fontsize=10):
        ...
        if sublabel:
            ax.text(x + w / 2, cy - 0.3, sublabel, ha="center", va="center", fontsize=8, color="#DDDDDD")

    # OrbaTech side
    draw_box(0.3, 5.0, 5, 1.3, "OrbaTech CRM", "#1565C0", ".NET 8 | Blazor | SQL Server", fontsize=13)
    ...
    # Agent Red side
    draw_box(7.7, 5.0, 5, 1.3, "Agent Red AI", REMAKER_RED, "Python | Azure | Cosmos DB", fontsize=13)
    ...
    # Flow labels
    ax.text(6.5, 6.5, "CRM Data → AI Context", fontsize=10, ha="center", color="#1565C0", fontweight="bold")
    ax.text(6.5, 0.7, "AI Insights → CRM Records", fontsize=10, ha="center", color=REMAKER_RED, fontweight="bold")
...
def chart_annex_comparison():
    ...
    ax.axis("off")
    ax.set_title(
        "Annex A: OrbaTech Recommended vs Agent Red Azure Deployment",
        fontsize=14,
        fontweight="bold",
        color=DARK_GRAY,
        pad=15,
...
def build_report():
    ...
    run.font.size = Pt(9)
    run = p.add_run(
        "Current OrbaTech application architecture showing the N-tier layout with "
    ...
    maturity_rows = [
        ...
        (
            "Documentation",
            "3/10",
            "Developer-oriented README. No architecture docs, no API guide, no deployment instructions.",
        ),
    ...
    run.font.size = Pt(9)
    run = p.add_run(
        "Security posture assessment across ten domains. Red and orange areas require immediate remediation."
    )
    run.font.size = Pt(9)
    run.font.color.rgb = RGBColor(0x66, 0x66, 0x66)
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    findings = [
        (
            "CRITICAL",
            "Hardcoded connection strings in appsettings.json and appsettings.Production.json expose database server names. An API access token is committed in EmailService/appsettings.json.",
        ),
        (
            "CRITICAL",
            'Default administrator account seeded with the password "Password123!" — while gated to development environment, the seed logic and credentials are visible in the repository.',
        ),
        (
            "HIGH",
            "Database backup files (.bak) committed to the repository may contain production data and should be removed from version control history.",
        ),
        (
            "HIGH",
            "File attachment paths are hardcoded to Windows-specific local directories with no visible path traversal protection.",
        ),
        (
            "MEDIUM",
            "No API rate limiting observed. API key validation exists but keys appear stored in plaintext in the database.",
        ),
    ...
    mt_gaps = [
        (
            ...
            "Add HasQueryFilter on every entity to automatically apply TenantId filtering, preventing accidental data leakage from queries that omit the filter.",
        ),
        (
            "No Database-Level RLS",
            "Implement SQL Server Row-Level Security policies as a defense-in-depth mechanism that prevents cross-tenant access even if application code is bypassed.",
        ),
        (
            "Integer-Based TenantId",
            "Consider migrating to GUID-based tenant identifiers to prevent enumeration attacks and simplify multi-region database merges.",
        ),
        (
            "No Tenant Provisioning Automation",
            "Implement automated tenant onboarding with configuration validation, license enforcement, and audit logging.",
        ),
    ]
    ...
    run.font.size = Pt(9)
    run = p.add_run(
        "Feature capability comparison across twelve CRM dimensions. "
    ...
    competitors = [
        (
            "Salesforce",
            "$25-$300/user/mo",
            "Market leader with the most comprehensive feature set. Often perceived as complex and expensive for SMBs. Strong enterprise adoption.",
        ),
        (
            "HubSpot CRM",
            "Free-$1,200/mo",
            "Leading SMB CRM with a generous free tier. Strong marketing automation and content management integration. Growing enterprise presence.",
        ),
        (
            "Zoho CRM",
            "$14-$52/user/mo",
            "Cost-effective alternative with broad feature coverage. Multi-language support (28 languages). Strong in price-sensitive SMB segments.",
        ),
        (
            "Pipedrive",
            "$14-$99/user/mo",
            "Sales-focused CRM known for intuitive pipeline management. Popular with small sales teams. Limited customization compared to larger platforms.",
        ),
    ]
    ...
    reliability_items = [
        (
            ...
            "Without automated test execution, regressions can be introduced with every commit and may not be detected until user-facing failures occur.",
        ),
        (
            "No health check endpoints",
            "No visible health or readiness probe endpoints for load balancer or orchestrator integration.",
        ),
        (
            "Single-server deployment model",
            "No horizontal scaling, load balancing, or failover configuration observed.",
        ),
        (
            "Windows service dependency",
            "The EmailService runs as a Windows service tied to a specific machine, creating a single point of failure.",
        ),
        (
            "No error tracking",
            "No integration with error tracking services (e.g., Application Insights, Sentry) was observed.",
        ),
    ]
    ...
    service_items = [
        (
            "Deployment process",
            "No documented deployment process. No deployment automation. Updates likely require manual intervention on the server.",
        ),
        (
            "Logging",
            "Standard ASP.NET Core logging configured. No structured logging or centralized log aggregation observed.",
        ),
        ("Diagnostics", "No diagnostic endpoints, no performance counters, no distributed tracing."),
        (
            "Database management",
            "EF Core migrations present but no automated migration strategy for zero-downtime deployments.",
        ),
        (
            "Configuration management",
            "Environment-specific appsettings files exist but contain hardcoded values that should be externalized.",
        ),
    ]
    ...
    perf_items = [
        (
            "Query Performance",
            "The dual ORM approach (EF Core for CRUD, Dapper for stored procedures) provides good flexibility for performance optimization. Stored procedures can be tuned independently of application code.",
        ),
        (
            "Connection Pooling",
            "Standard ADO.NET connection pooling is used. At scale (100+ tenants), connection pool exhaustion may occur without pool partitioning or connection limits per tenant.",
        ),
        (
            "Caching",
            "No caching layer observed. Adding Redis or in-memory caching for frequently accessed tenant configurations and reference data would significantly improve response times.",
        ),
        (
            "Blazor Server SignalR",
            "Blazor Server maintains a persistent SignalR connection per user session. At 1,000+ concurrent users, this creates significant memory and connection pressure on the server.",
        ),
        (
            "Blazor WebAssembly",
            "The hybrid rendering model allows offloading interactive components to the client. This is a positive architectural choice for reducing server load.",
        ),
    ]
    ...
    services = [
        (
            ...
            "Primary compute for the Blazor application. P1v3 provides 2 vCPUs, 8 GB RAM, and supports deployment slots for zero-downtime updates. Auto-scale from 2 to 6 instances based on CPU/memory thresholds.",
        ),
        (
            "Azure SQL Database",
            "Start with Standard S2 (50 DTU) tier. Migrate to Elastic Pool as tenant count grows beyond 100 to share resources across tenant-specific workloads efficiently.",
        ),
        (
            "Azure Front Door",
            "Global load balancing with SSL termination, Web Application Firewall (WAF), and geographic routing between Canada Central and East US regions.",
        ),
        (
            "Azure Blob Storage",
            "Replace local filesystem for file attachments. Standard LRS tier with lifecycle management policies for cost optimization.",
        ),
        (
            "Azure Key Vault",
            "Centralized secrets management with managed identity authentication. Stores connection strings, API keys, and encryption keys.",
        ),
        (
            "Azure Entra ID B2C",
            "Cloud-native identity management replacing ASP.NET Core Identity. Supports SSO, MFA, and social login providers. Scales automatically with user growth.",
        ),
        (
            "Azure Communication Services",
            "Replace the Windows-based EmailService with a managed email delivery service. Supports email, SMS, and push notifications.",
        ),
        (
            "Azure Monitor + Application Insights",
            "Full-stack monitoring with distributed tracing, live metrics, and AI-powered anomaly detection. Log Analytics workspace for centralized log aggregation.",
        ),
        (
            "Azure Redis Cache",
            "Caching layer for tenant configuration, session state, and frequently accessed reference data. Basic C0 tier at startup, Standard C1 at growth.",
        ),
    ]
    ...
    run.font.size = Pt(9)
    run = p.add_run(
        "Estimated monthly Azure costs across three scaling tiers. "
    ...
    run.font.size = Pt(9)
    run = p.add_run(
        "Recommended technical priority roadmap showing parallel and sequential work streams over 12 months."
    )
    run.font.size = Pt(9)
    run.font.color.rgb = RGBColor(0x66, 0x66, 0x66)
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        ...
        (
            ...
            [
                (
                    ...
                    "Remove all committed credentials from repository history (use git filter-repo). Rotate all exposed secrets. Implement Azure Key Vault for secrets management. Enable HTTPS redirection.",
                ),
                (
                    "CI/CD Pipeline",
                    "Implement GitHub Actions workflows for build, test, and deployment. Include vulnerability scanning (Dependabot). Add branch protection rules requiring PR reviews.",
                ),
                (
                    "Automated Testing",
                    "Establish unit test coverage for repository and service layers. Add integration tests for multi-tenant data isolation. Target minimum 60% code coverage.",
                ),
            ],
        ),
        (
            ...
            [
                (
                    "Multi-Tenant RLS",
                    "Implement SQL Server Row-Level Security policies. Add EF Core global query filters. Conduct cross-tenant penetration testing.",
                ),
                (
                    "Containerization",
                    "Create Dockerfile for the main application and EmailService. Implement docker-compose for local development. Prepare for Azure Container Apps or App Service deployment.",
                ),
                (
                    "Azure Deployment",
                    "Implement Infrastructure as Code (Bicep or Terraform). Set up staging and production environments. Configure deployment slots for zero-downtime releases.",
                ),
                (
                    "API Authentication",
                    "Implement OAuth 2.0 / OpenID Connect for API endpoints. Add rate limiting. Implement API versioning strategy.",
                ),
            ],
        ),
        (
            ...
            [
                (
                    "Localization (EN/FR)",
                    "Implement resource-based localization for all user-facing strings. Add French Canadian (fr-CA) translations. Plan for Spanish (es) as next language.",
                ),
                (
                    "Performance Optimization",
                    "Add Redis caching layer. Implement connection pool management per tenant. Optimize stored procedures for high-volume queries. Add response compression.",
                ),
            ],
        ),
        (
            ...
            [
                (
                    "Mobile Responsive",
                    "Optimize Blazor components for mobile viewports. Consider Progressive Web App (PWA) capabilities.",
                ),
                (
                    "AI/Automation",
                    "Add predictive lead scoring, email templates, and workflow automation. Consider integration with AI services for natural language processing.",
                ),
                (
                    "Agent Red Integration",
                    "Implement bidirectional data flow with Agent Red AI for AI-powered customer service enrichment of CRM data (see Annex B).",
                ),
            ],
        ),
    ]
    ...
    diff_items = [
        (
            "Compute Model",
            "OrbaTech is recommended for Azure App Service (PaaS) for simplicity and managed infrastructure. Agent Red uses Azure Container Apps for microservice flexibility and independent scaling of 8+ service containers.",
        ),
        (
            "Database Strategy",
            "OrbaTech uses relational SQL Server, appropriate for structured CRM data with complex queries and reporting. Agent Red uses Cosmos DB for globally distributed, schema-flexible document storage optimized for real-time AI conversations.",
        ),
        (
            "Identity Approach",
            "OrbaTech should adopt Entra ID B2C for managed consumer identity. Agent Red implements custom identity with widget keys and API keys for embedded SaaS scenarios.",
        ),
        (
            "Tenant Isolation",
            "Both platforms use application-level tenant isolation. Agent Red additionally uses Cosmos DB partition keys for physical data separation. OrbaTech is recommended to add SQL RLS.",
        ),
        (
            "CI/CD Maturity",
            "Agent Red has a mature 13-phase pipeline with automated testing, security scanning, and staged deployment. OrbaTech needs to build this capability from the ground up.",
        ),
    ]
    ...
    ltr_items = [
        (
            "Contact Context",
            "Customer profiles, purchase history, and communication preferences flow from OrbaTech to Agent Red, enabling personalized AI responses.",
        ),
        (
            "Opportunity Status",
            "Active deals and pipeline stages inform Agent Red's conversation routing, ensuring high-value prospects receive priority handling.",
        ),
        (
            "Activity History",
            "Recent emails, calls, and meetings provide Agent Red with conversation context, reducing customer frustration from repeating information.",
        ),
        (
            "Custom Fields",
            "Tenant-specific custom fields (e.g., industry, preferred language) enable Agent Red to adapt its responses to each business's unique CRM taxonomy.",
        ),
    ]
    ...
    rtl_items = [
        (
            ...
            "AI-generated summaries of customer interactions are written back to CRM contact records as activities, creating a complete interaction timeline.",
        ),
        (
            "Sentiment Analysis",
            "Customer sentiment scores from AI conversations are attached to opportunity records, providing sales teams with early warning on at-risk deals.",
        ),
        (
            "Quality Metrics",
            "Per-interaction quality scores help sales managers identify training opportunities and customer satisfaction trends.",
        ),
        (
            "Lead Qualification",
            "AI-assessed purchase intent signals from customer conversations create or update opportunity records in the CRM pipeline.",
        ),
    ]
