...
TESTS = [
    (
        "SPEC-0484",
        "TestSpec0484TrialActivationTime",
        [
        ...
        "Trial activation time recorded",
        "TenantDocument has trial_expires_at, created_at, trial_conversation_limit",
    ),
    (
        "SPEC-0485",
        "TestSpec0485UpgradeTimeRecorded",
        [
            "test_tenant_document_has_updated_at",
            "test_tenant_status_has_active",
        ],
        "Upgrade time and tier recorded",
        "TenantDocument has updated_at, TenantStatus has ACTIVE",
    ),
    (
        "SPEC-0486",
        "TestSpec0486CancelledContinues",
        [
            "test_tenant_status_has_grace_period",
            "test_tenant_document_has_grace_period_ends_at",
            "test_tenant_document_has_deactivated_at",
        ],
        "Cancelled tenancy continues during grace",
        "TenantStatus has GRACE_PERIOD, TenantDocument has grace_period_ends_at",
    ),
    (
        "SPEC-0487",
        "TestSpec0487ExpiredAccessible",
        [
            "test_tenant_status_has_trial_expired",
            "test_middleware_checks_trial_expiry",
            "test_middleware_checks_access_expiry",
        ],
        "Expired tenancy accessible but read-only",
        "TenantStatus has TRIAL_EXPIRED, middleware checks expiry",
    ),
    (
        "SPEC-0311",
        "TestSpec0311TeamMembersCollection",
        [
            "test_team_members_collection_defined",
            "test_team_members_in_all_collections",
        ],
        "Team members dedicated collection",
        "COLLECTION_TEAM_MEMBERS = 'team_members' in ALL_COLLECTIONS",
    ),
    (
        "SPEC-0475",
        "TestSpec0475HardDeleteTeamMember",
        [
            "test_delete_endpoint_exists",
            "test_delete_uses_hard_delete",
        ],
        "Team member hard delete (no soft delete)",
        "delete_team_member is async, calls repo.delete not deactivate",
    ),
    (
        "SPEC-0763",
        "TestSpec0763EscalationAgentNoAdmin",
        [
            "test_escalation_agent_role_exists",
            "test_admin_only_prefixes_restrict_config",
            "test_is_admin_only_path_function_exists",
        ],
        "Escalation agent has no admin access",
        "TeamMemberRole.ESCALATION_AGENT exists, /api/config is admin-only",
    ),
    (
        "SPEC-0283",
        "TestSpec0283TenantScopedRepository",
        [
        ...
        "TenantScopedRepository mandatory for data access",
        "Validates tenant_id, detects mismatch, has proper exception hierarchy",
    ),
    (
        "SPEC-0582",
        "TestSpec0582ConversationTimestamps",
        [
        ...
        "Conversation transcripts show timestamps",
        "ConversationDocument has started_at, messages, agents_invoked",
    ),
    (
        "SPEC-0692",
        "TestSpec0692AiResponsesInTranscripts",
        [
            "test_messages_field_description_mentions_roles",
            "test_conversation_has_message_count",
            "test_conversation_has_turn_count",
        ],
        "AI responses in conversation transcripts",
        "Messages include role/content, message_count and turn_count tracked",
    ),
    (
        "SPEC-0715",
        "TestSpec0715EscalatedConversationCategory",
        [
            "test_conversation_has_escalation_category",
            "test_conversation_has_assigned_to",
        ],
        "Escalated conversations show category",
        "ConversationDocument has escalation_category and assigned_to",
    ),
    (
        "SPEC-0613",
        "TestSpec0613EscalationCategories",
        [
        ...
        "Escalation categories with separate config",
        "ESCALATION_CATEGORIES >= 4, includes service/support/sales",
    ),
    (
        "SPEC-0366",
        "TestSpec0366WhoamiEndpoint",
        [
            "test_whoami_function_exists",
            "test_whoami_response_model_exists",
        ],
        "Whoami endpoint returns identity",
        "whoami() is async, WhoamiResponse model exists",
    ),
    (
        "SPEC-0265",
        "TestSpec0265StripeWebhookSecurity",
        [
        ...
        "Stripe webhook security (IP + signature + dedup)",
        "10+ IPs, _check_stripe_ip, 3 event handlers, _is_duplicate",
    ),
    (
        "SPEC-0750",
        "TestSpec0750ConversationMeter",
        [
            "test_conversation_has_is_billable",
            "test_conversation_billable_default_false",
        ],
        "ConversationMeter billing classification",
        "is_billable field defaults to False",
    ),
    (
        "SPEC-0598",
        "TestSpec0598ProvisioningMigratedToCosmos",
        [
            "test_tenant_document_is_pydantic_model",
            "test_tenant_repository_module_exists",
        ],
        "Provisioning migrated to Cosmos DB",
        "TenantDocument is Pydantic BaseModel, TenantScopedRepository exists",
    ),
    (
        "SPEC-0319",
        "TestSpec0319ActivateOnlyWithChanges",
        [
            "test_activation_service_has_draft_state_check",
            "test_activation_requires_draft_state",
        ],
        "Activate only when unapplied changes",
        "ActivationService has get_draft_state, activate references draft",
    ),
    (
        "SPEC-0402",
        "TestSpec0402SavePersistsOnly",
        [
            "test_save_draft_exists",
            "test_save_draft_returns_result",
        ],
        "Save persists without making live",
        "ActivationService.save_draft is async",
    ),
    (
        "SPEC-1622",
        "TestSpec1622SmtpNonBlocking",
        [
            "test_magic_link_email_uses_executor",
        ],
        "SMTP sends non-blocking",
        "Magic link email uses run_in_executor",
    ),
    (
        "SPEC-1641",
        "TestSpec1641ShopifyDomainMapping",
        [
            "test_tenant_document_has_shopify_domain",
            "test_shopify_domain_unique_per_tenant",
        ],
        "Shopify domain maps to one tenant",
        "TenantDocument.shopify_shop_domain references myshopify.com",
    ),
    (
        "SPEC-1621",
        "TestSpec1621PreAuthRateLimiter",
        [
            "test_middleware_has_rate_limiting",
            "test_middleware_has_auth_class",
        ],
        "PreAuth rate limiter records failures",
        "RateLimitMiddleware and TenantAuthMiddleware exist",
    ),
    (
        "SPEC-0815",
        "TestSpec0815ApiKeysNotRegeneratedOnDeploy",
        [
            "test_tenant_document_stores_key_hash",
            "test_tenant_document_stores_widget_key_hash",
        ],
        "API keys persistent across deploys",
        "TenantDocument stores api_key_hash and widget_key_hash",
    ),
    (
        "SPEC-0869",
        "TestSpec0869CamelCaseApi",
        [
            "test_team_member_response_uses_camel_case",
        ],
        "API uses camelCase convention",
        "TeamMemberResponse model has camelCase-capable fields",
    ),
    (
        "SPEC-1627",
        "TestSpec1627RepositoryNamingConvention",
        [
            "test_majority_collection_names_are_plural",
        ],
        "Repository naming convention",
        "70%+ of collections use plural names",
    ),
]
...
SLA_TESTS = [
    (
        ...
        "SLA_TARGETS for all tiers, enterprise 99.95% uptime, RTO 4/8/24hr",
    ),
]
