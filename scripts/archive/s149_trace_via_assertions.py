#!/usr/bin/env python3
"""S149: Trace test artifacts via their linked spec assertion results.

For test artifacts that have no test_file (conceptual verification items),
check if their linked spec_id has a passing assertion. If so, mark the
test as 'pass' with last_executed_at = now.

This is valid because these tests DESCRIBE the same verification that
the assertion system performs mechanically.
...
def main():
    ...
    # Get untraced tests with NULL test_file
    untraced = c.execute('''
        SELECT t.id, t.version, t.spec_id
        FROM tests t
    ...
    print(f"Untraced tests with NULL test_file: {len(untraced)}")

    # Match and update
    now = datetime.now(timezone.utc).isoformat()
    traced = 0
    for test_id, version, spec_id in untraced:
        if spec_id and spec_id in passing_specs:
            # Insert new version with last_result
            current = c.execute(
            ...
            c.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), ?)
            ''', (
                test_id, new_version, cd['title'], cd['spec_id'], cd['test_type'],
                cd['test_file'], cd['test_class'], cd['test_function'],
                cd['description'], cd['expected_outcome'],
                'pass', now, 'claude',
                'S149: Traced via spec assertion pass'
            ))
            traced += 1
    ...
    print(f"\nTraced {traced} tests via assertion linkage")
