...
TESTS = [
    (
        "SPEC-0306",
        "TestSpec0306TrialExpiryMiddleware",
        [
        ...
        "Trial expiry enforced at middleware level",
        "Expired trials raise AuthenticationError, non-trial and valid-trial pass",
    ),
    (
        "SPEC-0362",
        "TestSpec0362AdminFullAccess",
        [
            "test_admin_role_exists_in_enum",
            "test_admin_role_different_from_viewer",
            "test_admin_role_in_protected_check",
        ],
        "Admin role grants full access to configuration features",
        "ADMIN exists in TeamMemberRole enum and is in _ADMIN_ROLES set",
    ),
    (
        "SPEC-0364",
        "TestSpec0364PerUserApiKeys",
        [
            "test_api_key_resolves_to_team_member",
            "test_tenant_context_has_team_member_fields",
        ],
        "Per-user API keys resolve to team member identity",
        "TenantContext carries team_member_id, team_member_email, team_member_role",
    ),
    (
        "SPEC-0423",
        "TestSpec0423TwoFactorMethods",
        [
        ...
        "Multiple 2FA methods (TOTP, SMS, backup codes)",
        "MFA router has endpoints for TOTP, SMS, and backup code verification",
    ),
    (
        "SPEC-0428",
        "TestSpec0428SuperadminMfaOptOut",
        [
        ...
        "Superadmin MFA opt-out",
        "requires_2fa returns False when mfa_opt_out flag is set",
    ),
    (
        "SPEC-0429",
        "TestSpec0429MagicLinkThenTwoFa",
        [
            "test_admin_with_mfa_gets_pending_2fa_token",
            "test_non_admin_gets_direct_session",
        ],
        "Magic link verify triggers 2FA for admin with MFA",
        "Admin+MFA gets pending_2fa_token; non-admin gets direct session",
    ),
    (
        "SPEC-0607",
        "TestSpec0607ApiKeyRegeneration",
        [
            "test_rotation_router_has_rotate_endpoint",
            "test_apikey_api_has_rotate_endpoint",
            "test_reset_via_email_endpoint_exists",
        ],
        "API key rotation and regeneration endpoints",
        "rotation_router and admin_apikey_api have rotate/reset endpoints",
    ),
    (
        "SPEC-0758",
        "TestSpec0758SuperadminProtection",
        [
            "test_superadmin_role_exists",
            "test_superadmin_in_protected_roles",
            "test_protected_roles_is_a_set",
        ],
        "Superadmin protection from deletion/demotion",
        "SUPERADMIN in TeamMemberRole; 'superadmin' in PROTECTED_ROLES set",
    ),
    (
        "SPEC-0868",
        "TestSpec0868TwoLayerAuth",
        [
        ...
        "Two-layer authentication (tenant + user)",
        "TenantContext supports api_key, user_api_key, shopify_session, widget_key auth methods",
    ),
    (
        "SPEC-1618",
        "TestSpec1618SingleTenantEmail",
        [
            "test_single_member_gets_one_email",
            "test_multiple_members_get_separate_emails",
        ],
        "Single-tenant magic link emails (no combined multi-tenant)",
        "Each tenant_id+member gets separate email; no multi-tenant batching",
    ),
    (
        "SPEC-1633",
        "TestSpec1633TenantScopedRequest",
        [
            "test_request_model_requires_tenant",
            "test_request_model_accepts_tenant",
            "test_lookup_uses_tenant_scoped_query",
        ],
        "Magic link request is tenant-scoped",
        "MagicLinkRequest requires tenant field; lookup calls find_by_email with tenant_id",
    ),
    (
        "SPEC-1634",
        "TestSpec1634NoCrossTenantLookups",
        [
            "test_magic_link_does_not_call_find_all_by_email",
        ],
        "No cross-tenant email lookups",
        "find_all_by_email is never called in magic link flow",
    ),
    (
        "SPEC-1635",
        "TestSpec1635NoOtherTenancyReferences",
        [
            "test_email_template_has_single_sign_in_link",
            "test_email_template_uses_single_magic_link_url",
        ],
        "Email contains no other-tenancy references",
        "Email template has exactly 1 link and 1 magic_link_url placeholder",
    ),
    (
        "SPEC-1642",
        "TestSpec1642OneAdminUrl",
        [
            "test_url_includes_tenant_param",
            "test_url_is_deterministic",
        ],
        "One admin URL with tenant parameter",
        "Magic link URL includes ?tenant= and is deterministic",
    ),
    (
        "SPEC-1644",
        "TestSpec1644UrlIdentifiesTenant",
        [
            "test_magic_link_request_requires_tenant_from_url",
            "test_magic_link_request_tenant_description_references_spec",
        ],
        "URL identifies tenant (SPEC-1644)",
        "MagicLinkRequest tenant field description references SPEC-1644",
    ),
]
...
MFA_BRUTE_FORCE = (
    ...
    "MFA brute force protection (5 attempt limit)",
    "_MAX_FAILED_ATTEMPTS=5, brute-force check/record/clear functions work correctly",
)
