...
"""Dispatcher daemon runtime helpers.

This module is the harness-agnostic detection and dispatch helper surface used
by the dispatcher daemon. It re-reads live bridge state every daemon tick,
computes a per-recipient actionable signature, compares against durable
dispatch state, and dispatches the recipient harness only when the signature
changes.

Design contract:

- TAFE/dispatcher bridge state plus the versioned bridge file chain are the
  live authority after the 2026-06-15 cutover. This module reads numbered
...
- Signature normalization: ``[{document_name, top_status, top_file}]`` per
  recipient, JSON sorted, SHA-256 hex. This preserves the existing dispatch
  dedupe contract without reintroducing a retired automation path.
- Loop prevention: durable per-recipient signature state in
  ``<state-dir>/dispatch-state.json``. When a subsequent daemon tick sees the
  same selected work, the signature matches the just-written value and the
  ...
  Manual owner assignment is the fallback when daemon dispatch is paused; no
  hook-triggered fallback is propagated to child harness environments.
- Fire-and-forget: daemon ticks must not stall regular work. Dispatch failures
  are appended to
  ``<state-dir>/dispatch-failures.jsonl`` for diagnosis.
- Path: defaults to ``<project_root>/.gtkb-state/bridge-poller/``. Path is
  parameterized via ``--state-dir`` for tests and diagnostics.

This module is not a hook, scheduler, or fallback entrypoint. The daemon is the
only automated owner of these helpers; fallback work assignment is manual.
"""
...
# The function-level ``import harness_projection_reader`` in
# _resolve_dispatch_target resolves a sibling module under scripts/. Direct
# importlib loading in tests does not put scripts/ on sys.path, so ensure this
# module's directory is available before that import runs.
...
# sibling-scripts bootstrap above.
...
# signature in ``.gtkb-state/bridge-poller/dispatch-state.json``:
...
RUNTIME_INFLIGHT_LOCK_FILENAME = "dispatcher-runtime-inflight.lock"
...
RUNTIME_INFLIGHT_LOCK_STALE_SECONDS = 600.0
...
# WI-3265 (bridge/gtkb-bridge-poller-dispatch-state-lag-003.md, Codex
    ...
    if project_root is not None:
        try:
            ...
            for h_id, h_info in harnesses.items():
                if (
                    isinstance(h_info, dict)
                    and h_info.get("status") == "active"
                    and _record_can_receive_dispatch(h_info)
                ):
                    raw = h_info.get("role")
...
def _rename_with_retry(
    ...
    *,
    ...
    Per ``bridge/gtkb-bridge-poller-windows-rename-race-001-003.md``
    GO at ``-004`` (REVISED-1):
...
def _runtime_inflight_lock_path(state_dir: Path) -> Path:
    return state_dir / RUNTIME_INFLIGHT_LOCK_FILENAME


def _runtime_inflight_lock_stale(payload: dict[str, Any], *, now_epoch: float | None = None) -> bool:
    try:
        acquired_epoch = float(payload.get("acquired_at_epoch"))
    except (TypeError, ValueError):
        return True
    now = time.time() if now_epoch is None else now_epoch
    return (now - acquired_epoch) > RUNTIME_INFLIGHT_LOCK_STALE_SECONDS


def _read_runtime_inflight_lock(state_dir: Path) -> dict[str, Any] | None:
    lock_path = _runtime_inflight_lock_path(state_dir)
    try:
...
def runtime_inflight_active(state_dir: Path, *, now_epoch: float | None = None) -> bool:
    """Return True when another trigger invocation holds a fresh in-flight lock."""
    payload = _read_runtime_inflight_lock(state_dir)
    if payload is None:
        return False
    if _runtime_inflight_lock_stale(payload, now_epoch=now_epoch):
        _safe_unlink(_runtime_inflight_lock_path(state_dir))
        return False
    return True


def _try_acquire_runtime_inflight_lock(state_dir: Path) -> str | None:
    """Acquire the per-state-dir trigger in-flight lock, replacing stale locks."""
    state_dir.mkdir(parents=True, exist_ok=True)
    lock_path = _runtime_inflight_lock_path(state_dir)
    ...
    try:
        ...
        if runtime_inflight_active(state_dir):
            return None
        try:
            with lock_path.open("x", encoding="utf-8") as handle:
                json.dump(payload, handle, sort_keys=True)
...
def _release_runtime_inflight_lock(state_dir: Path, token: str) -> None:
    payload = _read_runtime_inflight_lock(state_dir)
...
def _direct_pid_create_time_matches(pid: int, expected_epoch: Any) -> bool:
    """Validate PID provenance without using monkeypatchable module helpers."""
    try:
        expected = float(expected_epoch)
    except (TypeError, ValueError):
        return False
...
def _force_kill_verified_pid_tree(pid: int, expected_create_time: Any) -> None:
    """Last-resort kill for a PID whose OS create-time still matches evidence."""
    if not _direct_pid_create_time_matches(pid, expected_create_time):
        return
    try:
        import psutil  # noqa: PLC0415
...
def _record_dispatch_failure(state_dir: Path, payload: dict[str, Any]) -> None:
    ...
    in dispatcher-runtime callers is covered with no call-site changes. Routing is by
...
def _runtime_retention_policy(project_root: Path) -> dict[str, int]:
    ...
    runtime = data.get("dispatcher_daemon", {})
...
def reap_inflight_dispatched_workers(runs_dir: Path) -> int:
    ...
    for pid_file in sorted(runs_dir.glob("*.pid")):
        ...
        if not _pid_alive(pid):
            continue
        expected_create_time = _read_pid_create_time_sidecar(runs_dir, dispatch_id)
        if expected_create_time is None or not _pid_create_time_matches(pid, expected_create_time):
            continue
        ...
        if not _wait_for_pid_exit(pid, timeout_seconds=2.0):
            _force_kill_verified_pid_tree(pid, expected_create_time)
        try:
...
def _classify_invocation_outcome(last_result: str) -> str:
    ...
    Vocabulary per bridge/gtkb-bridge-poller-dispatch-state-lag-003.md
...
def _command_without_prompt_payload(
    ...
    *,
    ...
    cleaned: list[str] = []
    index = 0
    prompt_value_flags = {"-p", "--prompt", "--print"}
...
def _read_role_assignments(project_root: Path) -> dict[str, Any]:
    ...
    (including ``_resolve_dispatch_target``) keeps working unchanged.
...
def _read_harness_identities(project_root: Path) -> dict[str, Any]:
    ...
    ``_invert_identities`` keeps working unchanged. Raises ValueError on a
    missing or unreadable projection.
    """
    projection = load_harness_projection(project_root)
    harnesses = {
...
def _spawn_harness(
    *,
    ...
    # dispatcher engine must set it itself or the dispatch chain breaks.
...
def _is_dispatcher_daemon_active_substrate(project_root: Path) -> bool:
    """Return True when the dispatcher engine should dispatch.

    Reads harness-state/bridge-substrate.json when present.
    The configured ``dispatcher_daemon`` substrate is active.
    ``none`` is mismatch-inert and returns
    ...
    if path.is_file():
        try:
            ...
            if isinstance(data, dict):
                ...
                if substrate and substrate != "dispatcher_daemon":
                    return False
        except Exception:
            pass
    return True
...
def _record_substrate_mismatch_skip(state_dir: Path, active_substrate: str) -> None:
    ...
    ts = _now_iso()
    error_message = (
        f"Dispatcher engine inert because the active bridge substrate is configured as {active_substrate!r} "
...
def run_dispatch_cycle(
    *,
    ...
    inflight_token = _try_acquire_runtime_inflight_lock(state_dir)
    ...
    try:
        ...
        # bridge/gtkb-bridge-poller-dispatch-state-lag-003.md (Codex GO -004).
        ...
        pending_by_target: list[
            tuple[DispatchTarget | None, list[Any], str, str | None, list[dict[str, Any]] | None]
        ] = []
        ...
        for _legacy_recipient, needed_role_label, items in (
            ...
            for _target_index, target in enumerate(targets):
                processed_target_keys.add(target.dispatch_state_key)
                h_info = harnesses.get(target.harness_id) or {}
                harness_type = str(h_info.get("harness_type") or "unknown").strip().lower()
                if not _is_dispatch_ready(target.harness_id, h_info, project_root, state_dir, needed_role_label):
                    reason = f"{harness_type}_dispatch_not_ready"
                ...
                if not any(getattr(item, "dispatchable", True) for item in remaining_items):
                    break
            if selected_any_target and not any(getattr(item, "dispatchable", True) for item in remaining_items):
                for target in targets:
                    if target.dispatch_state_key in processed_target_keys:
                        continue
                    pending_by_target.append((target, [], target.dispatch_state_key, None, None))
        ...
        for target, items, recipient, failure_reason, fallback_skipped_candidates in pending_by_target:
            if target is None or failure_reason is not None:
                ...
                lo_failover_exhausted = _is_lo_failover_exhausted(
                    failure_reason, recipient, fallback_skipped_candidates
                )
                if lo_failover_exhausted:
                    reason = "lo_failover_exhausted"
                else:
                    reason = (
                        _lo_provider_backoff_hold_reason(failure_reason, recipient, fallback_skipped_candidates)
                        or reason
                    )
                recipient_state["last_result"] = reason
                    ...
                    if target.needed_role_label == "prime-builder" and not dispatched_selected:
                        ...
                        if prior_dispatched == dispatched_signature and previous_launch_failure is None:
                            ...
                            if is_retry_pending:
                                ...
                                prior_launched_at = (
                                    prior_last_launch.get("launched_at")
                                    if isinstance(prior_last_launch, dict)
                                    else None
                                )
                                if prior_launched_at:
                                    try:
                                        launched_time = dt.datetime.fromisoformat(
                                            prior_launched_at.replace("Z", "+00:00")
                                        )
        ...
        # recipient processed this invocation. The dispatcher engine handles
...
def _emit_diagnose_summary(state_dir: Path, *, include_rotated_failures: bool = False) -> str:
    ...
    Per ``bridge/gtkb-bridge-poller-windows-rename-race-001`` GO
    at ``-004``.
    """
    lines: list[str] = []
    lines.append(f"Dispatcher engine diagnose — {_now_iso()}")
    lines.append("")
    ...
    for name in all_keys:
        ...
        if not rec:
            ...
            lines.append(
                ...
                f"dispatcher engine disabled).{annotation}"
            )
...
def _build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Dispatcher daemon engine — event-driven replacement for the "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help=("Dispatch-state directory. Default: <project_root>/.gtkb-state/bridge-poller/."),
    )
    parser.add_argument(
        "--max-items",
        type=int,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "bridge/gtkb-bridge-poller-windows-rename-race-001 GO at -004."
        ),
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.diagnose:
            ...
            return 0
        print(
            "dispatcher_runtime.py is a daemon-owned helper library, not a dispatch entrypoint. "
            "Use `gt bridge dispatch daemon start|tick|status` for automated dispatch; "
            "manual fallback is owner-assigned work.",
            file=sys.stderr,
        )
        return 2
    except SystemExit:
        ...
        try:
            ...
            pass
        print(f"dispatcher engine error (suppressed): {exc}", file=sys.stderr)
    return 0
