...
def _as_list(value: Any, *, field: str) -> list[str]:
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        raise OpenRouterHarnessError(f"{field} must be a list of strings")
    return value


def _as_non_empty_string(value: Any, *, field: str) -> str:
    if not isinstance(value, str) or not value:
        raise OpenRouterHarnessError(f"{field} must be a non-empty string")
    return value
...
def _parse_skill_routes(routing: Mapping[str, Any], models: Mapping[str, ModelRoute]) -> dict[str, str]:
    ...
    if not isinstance(skills_raw, dict):
        raise OpenRouterHarnessError("routing.skills must be a table when present")
    skill_routes: dict[str, str] = {}
    for skill_name, route_spec in skills_raw.items():
        if not isinstance(skill_name, str) or not skill_name:
            raise OpenRouterHarnessError("routing.skills entries must use non-empty skill names")
        if isinstance(route_spec, str):
            route_key = route_spec
        elif isinstance(route_spec, dict):
            route_key = route_spec.get("model")
        else:
            raise OpenRouterHarnessError(f"routing.skills.{skill_name} must name a configured model")
...
def load_routing_config(project_root: Path) -> RoutingConfig:
    ...
    try:
        ...
        raise OpenRouterHarnessError(f"routing config is missing: {config_path}") from exc
    except tomllib.TOMLDecodeError as exc:
        raise OpenRouterHarnessError(f"routing config is invalid TOML: {exc}") from exc

    if raw.get("schema_version") != 1:
        raise OpenRouterHarnessError("routing config schema_version must be 1")
    models_raw = raw.get("models")
    if not isinstance(models_raw, dict) or not models_raw:
        raise OpenRouterHarnessError("routing config must define [models.<key>] rows")
    ...
    for key, row in models_raw.items():
        ...
        if row.get("tool_calling_supported") is not True:
            raise OpenRouterHarnessError(f"models.{key}.tool_calling_supported must be true")
    ...
    if not isinstance(routing, dict):
        raise OpenRouterHarnessError("routing config must define [routing.openrouter]")
    default_model = routing.get("default_model")
    if not isinstance(default_model, str) or default_model not in models:
        raise OpenRouterHarnessError("routing.openrouter.default_model must name a configured OpenRouter model")
...
def resolve_model(config: RoutingConfig, requested_model: str | None, skill: str | None = None) -> ModelRoute:
    if skill is not None and not skill:
        raise OpenRouterHarnessError("skill route key must be a non-empty string")
    ...
    try:
        ...
        raise OpenRouterHarnessError(f"unknown model route: {route_key}") from exc
...
def build_tool_schemas(allowed_tools: Iterable[str]) -> list[dict[str, Any]]:
    ...
    if unknown:
        raise OpenRouterHarnessError(f"unknown allowed tools: {unknown}")
    ...
    for attempt in range(1, CHAT_MAX_ATTEMPTS + 1):
        try:
            with urllib.request.urlopen(
                request,
                timeout=_remaining_timeout(deadline, "OpenRouter completions request timed out"),
            ...
            if attempt < CHAT_MAX_ATTEMPTS:
                ...
                continue
            raise OpenRouterHarnessError(
                f"OpenRouter completions request failed after {attempt} attempt(s): {exc}"
            ) from exc
        try:
            ...
            raise OpenRouterHarnessError(
                f"OpenRouter completions response was not JSON after {attempt} attempt(s) "
                f"(body snippet: {snippet!r}): {exc}"
            ) from exc
        if not isinstance(parsed, dict):
            raise OpenRouterHarnessError("OpenRouter completions response must be a JSON object")
        return parsed
    raise OpenRouterHarnessError(
        f"OpenRouter completions request failed after {CHAT_MAX_ATTEMPTS} attempt(s): {last_error}"
    )


def _resolve_tool_path(project_root: Path, path_text: str, *, allow_missing: bool) -> Path:
    if not path_text or not path_text.strip():
        raise OpenRouterHarnessError("tool path must be a non-empty string")
    raw = Path(path_text)
...
def _ensure_under_root(project_root: Path, resolved: Path, original: str) -> None:
    ...
    if resolved != root and root not in resolved.parents:
        raise OpenRouterHarnessError(f"tool path escapes project root: {original}")
...
def _guard_tool_input(tool_name: str, arguments: Mapping[str, Any], project_root: Path) -> dict[str, Any]:
    ...
    raise OpenRouterHarnessError(f"guard adapter does not support tool: {tool_name}")
...
def _guard_paths_for(tool_name: str, tool_input: Mapping[str, Any], project_root: Path) -> tuple[Path, ...]:
    ...
    raise OpenRouterHarnessError(f"unsupported guarded tool: {tool_name}")
    ...
    *,
    ...
    for relative_guard_path in paths:
        ...
        if not guard_path.is_file():
            raise OpenRouterHarnessError(f"guard script is missing: {relative_guard_path.as_posix()}")
        result = runner(guard_path, payload, env, timeout)
        if result.timed_out:
            raise OpenRouterHarnessError(f"guard timed out: {_relative_path(project_root, guard_path)}")
        ...
        if not stdout:
            raise OpenRouterHarnessError(f"guard emitted empty output: {_relative_path(project_root, guard_path)}")
        try:
...
def _require_string(arguments: Mapping[str, Any], *names: str) -> str:
    for name in names:
        ...
        if isinstance(value, str) and value:
            return value
    raise OpenRouterHarnessError(f"missing required argument: {'/'.join(names)}")
...
def _positive_int_argument(arguments: Mapping[str, Any], name: str, default: int) -> int:
    ...
    if parsed is None or parsed <= 0:
        raise OpenRouterHarnessError(f"{name} must be a positive integer")
    return parsed
...
def _dispatch_bash(
    ...
    try:
        ...
        raise OpenRouterHarnessError(f"Bash command timed out: {command}") from exc
...
def dispatch_tool_call(
    ...
    *,
    ...
    if tool_name not in CANONICAL_TOOLS:
        raise OpenRouterHarnessError(f"unsupported tool: {tool_name}")
    if tool_name == "Read":
...
def _tool_call_parts(call: Any, index: int) -> tuple[str, dict[str, Any], str]:
    ...
    if isinstance(raw_arguments, str):
        try:
            ...
            raise OpenRouterHarnessError("tool_call arguments string must be JSON") from exc
        raw_arguments = parsed
    if not isinstance(raw_arguments, dict):
        raise OpenRouterHarnessError("tool_call arguments must be an object")
...
def run_tool_loop(
    ...
    *,
    ...
    if max_turns < 1:
        raise OpenRouterHarnessError("max_turns must be at least 1")
    if session_timeout <= 0:
        raise OpenRouterHarnessError("session_timeout must be positive")
    ...
    for _turn in range(max_turns):
        ...
        operation_timeout = min(
            timeout,
            _remaining_timeout(session_deadline, "session timeout exceeded before OpenRouter chat turn"),
        )
        ...
        for index, call in enumerate(tool_calls):
            ...
            except OpenRouterHarnessError as tool_err:
    ...
    raise OpenRouterHarnessError("max-turn exhaustion before final assistant text")
