...
def extract_2a() -> list[dict]:
    ...
    for tsx_file in sorted(pages_dir.glob("*.tsx")):
        ...
        for m in re.finditer(
            r'<(?:TextInput|Textarea|NumberInput|Select)\s[^>]*label=["\{](?:["\'])?([^"\'}\)]+)', content
        ):
        ...
        for m in re.finditer(
            r'<NumberInput[^>]*min=\{?(\d+)\}?[^>]*max=\{?(\d+)\}?[^>]*label=["\']([^"\']+)["\']', content
        ):
            specs.append(
                {
...
def extract_2d() -> list[dict]:
    ...
    for fname in ["auth.py", "security_middleware.py"]:
        ...
        # Security headers
        for m in re.finditer(
            r'["\']([A-Z][a-z]+-[A-Z][a-z]+-[A-Za-z-]+|X-[A-Za-z-]+|Content-Security-Policy|Strict-Transport-Security)["\']',
            content,
        ):
            specs.append(
                {
