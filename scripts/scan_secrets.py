...
PATTERNS = [
    ...
    ("AWS Secret Key", re.compile(r'(?:aws_secret_access_key|secret_access_key)\s*[=:]\s*["\']?[A-Za-z0-9/+=]{40}["\']?', re.IGNORECASE), "high"),
    ("Azure Connection String", re.compile(r'(DefaultEndpointsProtocol|AccountName|AccountKey|SharedAccessSignature)=', re.IGNORECASE), "medium"),
    ...
    ("Password Assignment", re.compile(r'(?:password|passwd|pwd)\s*[=:]\s*["\']?[^\s"\'$]{8,}', re.IGNORECASE), "medium"),
    ("Secret Key Assignment", re.compile(r'(?:secret|api[_-]?key|access[_-]?key|auth[_-]?token)\s*[=:]\s*["\']?[A-Za-z0-9+/=_-]{16,}', re.IGNORECASE), "medium"),
    ...
    ("Connection String w/ Password", re.compile(r'(?:postgresql|mysql|mongodb|redis)://[^\s]*:[^\s@]*@[^\s]*', re.IGNORECASE), "high"),
    ...
    ("Hardcoded env secret", re.compile(r'(?:os\.environ|getenv)\s*\(\s*["\'](?:SECRET|PASSWORD|TOKEN|KEY|CREDENTIAL)', re.IGNORECASE), "low"),
]

# File extensions to scan (text files only)
TEXT_EXTENSIONS = {
    '.py', '.js', '.ts', '.tsx', '.jsx', '.json', '.toml', '.yaml', '.yml',
    '.md', '.txt', '.cfg', '.ini', '.env', '.sh', '.bash', '.ps1', '.bat',
    '.cmd', '.html', '.htm', '.css', '.scss', '.xml', '.sql', '.graphql',
    '.dockerfile', '.tf', '.hcl', '.rs', '.go', '.java', '.kt', '.rb',
    '.php', '.cs', '.swift', '.m', '.h', '.hpp', '.c', '.cpp', '.vue',
    '.svelte', '.astro', '.toml', '.lock',
}
...
def is_text_file(path: str) -> bool:
    ...
    if name_lower in ('dockerfile', 'makefile', 'procfile', '.env', '.gitignore',
                       '.dockerignore', '.gitattributes', 'gemfile', 'rakefile'):
        return True
    return False
        ...
        for line_num, line in enumerate(lines, 1):
            if pattern.search(line):
                ...
                if any(marker in stripped.lower() for marker in [
                    ...
                    'your-', 'xxx', 'redacted', 'sample', 'template',
                    '<your', '{{your', '${your',
                ]):
                    continue
