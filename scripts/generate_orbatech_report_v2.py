...
add_table(
    ...
    [
        ...
        [
            "ORM",
            "Entity Framework Core 8.0 + Dapper 2.1",
        ...
        [
            "Testing",
            "xUnit + bUnit + Testcontainers + FsCheck",
...
add_chart(
    doc,
    "fig3",
    "Figure 3: Security posture assessment across ten domains. Red and orange areas require immediate remediation.",
)
...
add_table(
    ...
    [
        ...
        [
            "Zoho CRM",
            "$14\u2013$52/user/mo",
            "Cost-effective with broad feature coverage. 28 languages. Strong in price-sensitive SMB segments.",
        ],
        [
            "Pipedrive",
            "$14\u2013$99/user/mo",
            "Sales-focused with intuitive pipeline management. Popular with small sales teams. Limited customization.",
        ],
        [
...
add_table(
    doc,
    ["Platform", "Self-Service Signup", "Admin Provisioning", "Admin/Config APIs", "Trial Management", "Assessment"],
    [
        [
            "OrbaTech",
...
add_table(
    doc,
    ["Platform", "Billing Integration", "Usage Metering", "Entitlement Enforcement", "Subscription Mgmt", "Assessment"],
    [
        [
            "OrbaTech",
...
add_chart(
    doc, "fig5", "Figure 5: Recommended Azure architecture for OrbaTech CRM production deployment.", width=Inches(6.5)
)
...
services = [
    ...
    ("Azure SQL Database:", "Start with Standard S2 (50 DTU). Migrate to Elastic Pool at 100+ tenants."),
    ("Azure Front Door:", "Global load balancing with SSL, WAF, and geographic routing (Canada Central + East US)."),
    ("Azure Blob Storage:", "Replace local filesystem for attachments. Standard LRS with lifecycle management."),
    ("Azure Key Vault:", "Centralized secrets management with managed identity authentication."),
    ("Azure Entra ID B2C:", "Cloud-native identity replacing ASP.NET Core Identity. SSO, MFA, social login."),
    ("Azure Communication Services:", "Replace Windows-based EmailService. Email, SMS, push notifications."),
    (
        "Azure Monitor + Application Insights:",
        "Full-stack monitoring with distributed tracing, live metrics, AI anomaly detection.",
    ),
    ("Azure Redis Cache:", "Caching for tenant config, session state, reference data. Basic C0 at startup."),
]
...
add_chart(doc, "fig6", "Figure 6: Estimated monthly Azure costs across three scaling tiers.", width=Inches(6.5))
...
add_chart(doc, "fig7", "Figure 7: Recommended technical priority roadmap over 12 months.", width=Inches(6.5))
...
p2_items = [
    "Localization (EN/FR): Resource-based localization. French Canadian (fr-CA) translations. Plan for Spanish (es).",
    ...
    "Integration Expansion: Calendar sync (Google/Outlook). VoIP integration. Social media connectors.",
]
...
p3_items = [
    ...
    "Agent Red Integration: Bidirectional data flow for AI-powered customer service enrichment (see Annex B).",
...
add_chart(doc, "fig8", "Figure 8: Side-by-side OrbaTech and Agent Red Azure deployments.", width=Inches(6.5))
...
arch_diffs = [
    ...
    "CI/CD Maturity: Agent Red has a mature 13-phase pipeline. OrbaTech needs to build this from the ground up.",
]
...
add_chart(doc, "fig9", "Figure 9: Bidirectional integration architecture.", width=Inches(6.5))
...
l2r = [
    ...
    "Custom Fields: Tenant-specific custom fields enable Agent Red to adapt to each business's CRM taxonomy.",
]
for item in l2r:
    doc.add_paragraph(item, style="List Bullet")
...
doc.add_paragraph(
    "This annex documents the sources used in this evaluation to provide clear provenance for all findings and claims."
)
