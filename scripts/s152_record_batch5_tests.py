...
TESTS = [
    ("SPEC-0620", "TestSpec0620SystemPromptBuilder", [
        "test_system_prompt_builder_class_exists",
        "test_agent_role_enum_has_seven_roles",
        "test_build_method_accepts_agent_role",
    ], "SystemPromptBuilder per-agent prompts", "SystemPromptBuilder has build(), AgentRole has 7 agents"),

    ("SPEC-0291", "TestSpec0291GdprPiiScrubbing", [
        "test_pii_scrubber_class_exists",
        "test_scrub_returns_new_dict",
        "test_scrub_text_method_exists",
    ], "GDPR PII scrubbing at logging layer", "PiiScrubber.scrub() returns new dict, scrub_text() exists"),

    ("SPEC-0292", "TestSpec0292GdprGracePeriod", [
    ...
    ], "GDPR grace period (Shopify 48hr, Stripe 30d)", "SHOPIFY_GRACE_PERIOD_HOURS=48, STRIPE_GRACE_PERIOD_DAYS=30"),

    ("SPEC-0293", "TestSpec0293GdprDataExport", [
        "test_data_export_service_class_exists",
        "test_export_tenant_is_async",
        "test_export_result_dataclass_fields",
    ], "GDPR data export service", "DataExportService.export_tenant is async, ExportResult fields verified"),

    ("SPEC-0294", "TestSpec0294GdprDataDeletion", [
        "test_data_deletion_service_class_exists",
        "test_delete_tenant_is_async",
        "test_deletion_result_dataclass_fields",
    ], "GDPR data deletion service", "DataDeletionService.delete_tenant is async, DeletionResult fields verified"),

    ("SPEC-0295", "TestSpec0295GdprConsentManagement", [
    ...
    ], "GDPR consent management", "ConsentManager.CONSENT_REQUIRED_LAYERS={2,3,4}, layer 1 always allowed"),

    ("SPEC-0640", "TestSpec0640CriticPolicyFailClosed", [
        "test_critic_policy_class_exists",
        "test_safe_fallback_message_exists",
        "test_critic_verdict_enum",
        "test_validate_response_is_async",
    ], "CriticPolicy fail-closed safety gate", "CriticPolicy, SAFE_FALLBACK_MESSAGE, CriticVerdict enum"),

    ("SPEC-0700", "TestSpec0700ChatPipelineOrchestrator", [
        "test_chat_pipeline_class_exists",
        "test_chat_pipeline_inherits_mixins",
        "test_execute_method_exists",
    ], "ChatPipeline orchestrator", "ChatPipeline inherits AgentDispatchMixin+CriticEscalationMixin+AnalyticsMixin"),

    ("SPEC-0710", "TestSpec0710PipelineTimeoutBudget", [
    ...
    ], "Pipeline timeout budget (8s deadline)", "PipelineTimeoutBudget, PipelineTimeoutError(stage, budget_ms, elapsed_ms)"),

    ("SPEC-0740", "TestSpec0740ConversationMeterBilling", [
    ...
    ], "ConversationMeter billing rules", "IDLE_TIMEOUT=1800, MAX_TURNS=50, NON_BILLABLE_PREFIXES, STRIPE_METER_EVENT"),

    ("SPEC-0237", "TestSpec0237AlertDeliveryService", [
    ...
    ], "AlertDeliveryService routes alerts", "AlertDeliveryService, AlertType(12), AlertSeverity, log channel always present"),

    ("SPEC-0833", "TestSpec0833ConfigSuggestionEngine", [
    ...
    ], "ConfigSuggestionEngine brand inference", "ConfigSuggestionEngine.generate_suggestions is async, 5 tones, 20+ escalation patterns"),

    ("SPEC-0720", "TestSpec0720ResponseExplainability", [
        "test_decision_trace_builder_exists",
        "test_knowledge_source_dataclass",
        "test_builder_has_build_method",
    ], "Response explainability", "DecisionTraceBuilder(conversation_id, tenant_id), KnowledgeSource dataclass"),

    ("SPEC-0827", "TestSpec0827AcsEmailDelivery", [
        "test_send_acs_email_function_exists",
        "test_sender_address_defined",
        "test_sync_email_helper_exists",
    ], "ACS email delivery with rate-limit handling", "send_acs_email is async, SENDER_ADDRESS includes azurecomm.net"),

    ("SPEC-0320", "TestSpec0320ConfigRestorePrevious", [
        "test_restore_previous_is_async",
        "test_restore_previous_method_exists",
    ], "Config restore_previous atomicity", "ActivationService.restore_previous is async"),

    ("SPEC-0321", "TestSpec0321ConfigActivateAtomicity", [
        "test_activate_is_async",
        "test_activation_result_exists",
    ], "Config activate atomicity", "ActivationService.activate is async, ActivationResult exists"),

    ("SPEC-0322", "TestSpec0322ConfigValidationBeforeActivation", [
        "test_validation_result_exists",
        "test_draft_save_result_exists",
    ], "Config validation before activation", "ValidationResult and DraftSaveResult exist"),

    ("SPEC-0480", "TestSpec0480ConfigInitialStateDraft", [
        "test_config_state_has_draft",
        "test_draft_is_distinct_from_active",
    ], "Config initial state DRAFT", "ConfigState.DRAFT != ConfigState.ACTIVE"),

    ("SPEC-1557", "TestSpec1557CoPilotAgent", [
        "test_co_pilot_agent_role_exists",
        "test_admin_assistance_intent_constant",
    ], "Co-Pilot agent in pipeline", "AgentRole.CO_PILOT='co-pilot', ADMIN_ASSISTANCE_INTENT defined"),

    ("SPEC-1558", "TestSpec1558IntentDetectionDispatch", [
        "test_escalation_intent_constant",
        "test_agent_dispatch_mixin_exists",
    ], "Intent detection dispatches to correct agent", "ESCALATION_INTENT defined, AgentDispatchMixin exists"),

    ("SPEC-0766", "TestSpec0766AutomatedProvisioningLifecycle", [
        "test_provisioning_function_is_async",
        "test_provisioning_module_has_result_type",
    ], "Automated provisioning lifecycle", "spa_provision_tenant is async, SpaProvisionResult has credentials"),
]

# Additional alert severity mapping tests
ALERT_TESTS = [
    ("SPEC-0237", "TestSpec0237bAlertSeverityMapping", [
        "test_default_severity_mapping_exists",
        "test_critical_alert_types",
    ], "Alert severity mapping", "_DEFAULT_SEVERITY maps all types, SLA/outage/100% are CRITICAL"),

    ("SPEC-0741", "TestSpec0741ConversationBilling3Tier", [
        "test_usage_alert_type_enum",
        "test_volume_spike_multiplier",
        "test_reconciliation_threshold",
        "test_pack_low_threshold",
    ], "Conversation billing 3-tier consumption", "UsageAlertType(4), VOLUME_SPIKE_MULTIPLIER=2.0, RECONCILIATION=0.05"),
]
