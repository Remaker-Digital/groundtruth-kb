...
export function useIngestionStatus(apiFetch) {
    return useApi(apiFetch, '/api/admin/knowledge/ingest/status');
}
/** List available category templates. */
export function useTemplates(apiFetch) {
    return useApi(apiFetch, '/api/admin/knowledge/templates');
}
/** Start an ingestion job. */
export function useStartIngestion(apiFetch) {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const start = useCallback(async (sourceType, options) => {
        setLoading(true);
        setError(null);
        try {
...
export function useCancelIngestion(apiFetch) {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
...
export function useApplyTemplate(apiFetch) {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const apply = useCallback(async (categoryId) => {
        setLoading(true);
        setError(null);
        try {
...
//# sourceMappingURL=useIngestion.js.map