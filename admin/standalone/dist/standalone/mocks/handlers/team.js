...
export function registerTeamHandlers() {
    ...
    PUT("/api/admin/team/:id/role", (req) => {
        const member = s().members.find((m) => m.id === req.params.id);
        if (!member)
            return { status: 404, body: { detail: "Member not found" } };
        const body = req.body;
    ...
    PUT("/api/admin/team/:id", (req) => {
        ...
        if (body.is_active !== undefined)
            member.isActive = body.is_active;
        if (body.escalation_categories !== undefined) {
            member.escalationCategories = body.escalation_categories;
        }
        member.updatedAt = new Date().toISOString();
        return { status: 200, body: member };
    });
    DELETE("/api/admin/team/:id", (req) => {
        const store = s();
        const idx = store.members.findIndex((m) => m.id === req.params.id);
        if (idx === -1)
            return { status: 404, body: { detail: "Member not found" } };
