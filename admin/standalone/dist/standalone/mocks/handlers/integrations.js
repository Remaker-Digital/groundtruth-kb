...
export function registerIntegrationHandlers() {
    ...
    POST('/api/admin/integrations/:type/deactivate', (req) => {
        const item = integrationState.find((i) => i.type === req.params?.type);
        if (!item)
            return { status: 404, body: { detail: 'Integration not found' } };
        item.enabled = false;
    ...
    DELETE('/api/admin/integrations/:type', (req) => {
        const item = integrationState.find((i) => i.type === req.params?.type);
        if (!item)
            return { status: 404, body: { detail: 'Integration not found' } };
        item.enabled = false;
    ...
    GET('/api/admin/integrations/stripe/credentials', (_req) => {
        return { status: 200, body: { hasCredentials: false, publishableKey: null } };
    });
    POST('/api/admin/integrations/stripe/credentials', (_req) => {
        return { status: 200, body: { success: true, message: 'Stripe credentials saved' } };
    });
    POST('/api/admin/integrations/stripe/test', (_req) => {
