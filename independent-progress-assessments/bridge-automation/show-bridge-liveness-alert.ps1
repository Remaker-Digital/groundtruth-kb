...
function Write-AlertLog {
    ...
    Add-Content -LiteralPath $AlertLogPath -Value "$stamp $Message"
}
...
if (-not (Test-Path -LiteralPath $WatcherPath)) {
    throw "Poller liveness watcher not found: $WatcherPath"
}
...
if (-not (Test-Path -LiteralPath $LivenessPath)) {
    $message = "liveness output missing: $LivenessPath"
    Write-AlertLog "ERROR: $message"
    Show-BridgeToast -Title "Agent Red bridge liveness" -Message $message
    exit 1
}

$liveness = Get-Content -Raw -LiteralPath $LivenessPath | ConvertFrom-Json -ErrorAction Stop
