...
1. **Role resolution** — resolve session role strictly from explicit interactive
   owner input (`::init gtkb pb|lo`) or the bridge item header (`::init gtkb <pb|lo>`)
   in dispatched contexts per `GOV-SESSION-ROLE-AUTHORITY-001` and
   `DCL-SESSION-ROLE-RESOLUTION-001`. There is never a registry-fallback resolution for role.
