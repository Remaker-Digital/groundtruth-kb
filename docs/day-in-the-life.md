...
Review MemBase through the web UI:
...
Deliberation Archive (DA). You can see at a glance how much of your project is
...
These components implement ADR-0001: Three-Tier Memory Architecture — Knowledge Database (MemBase), MEMORY.md (operational notepad), and the Deliberation Archive (DA).

This gives you:

| Component | What it does | CLI command |
|-----------|-------------|-------------|
| MemBase (knowledge database) | SQLite store for specs, tests, work items, deliberations | `gt init`, `gt assert`, `gt serve` |
...
| Deliberation Archive (DA) | Decision history with semantic search | `gt deliberations search "..."` |
