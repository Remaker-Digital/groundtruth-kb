...
| 2026-02-28 | Claude (session 120) | — | Phase 2: **COMPLETE**, Phase 5: **COMPLETE**, Phase 6: **COMPLETE** | 0 | Phase 4: SKIP | Phase 1+2+3+5+6 | **Erroneous artifacts corrected (SPEC-1534).** Removed false opt-out language from docs. `mcp_client.py` refactored to use `AgntcyFactory.create_client("MCP", ...)`. `USE_AGENT_CONTAINERS` default `true`. Phase 2: transport-first routing, 6 container build/provision scripts. Phase 5: OTel per-agent tracing + cost attribution. Phase 6: reversible PII tokenization (pii_tokenizer.py, Cosmos container #17). 13 specs (SPEC-1534..1546), 117/117 assertions PASS. Deployed v1.62.0 to staging, 70/70 upgrade verification PASS. |
