...
def get_scaling_targets(environment: str) -> list[str]:
    """Return the ordered set of deploy-managed Container Apps."""
    return [
...
def _lib_enforce_all_scaling(targets, scaling_config, resource_group, runner, log):
    ...
    for app_name in targets:
        ...
        if cfg is None:
            ...
            continue
        results[app_name] = _lib_enforce_one(
            app_name,
            cfg["min_replicas"],
            cfg["max_replicas"],
            resource_group,
...
def _init_log() -> None:
    ...
    _log_file = open(log_dir / f"deploy-{ts}.log", "w", encoding="utf-8")  # noqa: SIM115
...
def main() -> int:
    ...
    # 3d. Enforce scaling on every deploy-managed container (WI-3171).
    # Runs AFTER all deploys so it lands on the freshly-updated revisions.
    # Extends WI-3156 (gateway only) to all 8 deploy-managed container apps.
    log("")
