...
try:
    ...
    from deploy_config import ENVIRONMENTS  # noqa: E402
...
def _install_app_scaling_compat_modules() -> None:
    """Expose app-local scaling taxonomy through the legacy lib import path."""
    import deploy as app_deploy

    lib_pkg = sys.modules.setdefault("lib", types.ModuleType("lib"))
    scaling_targets = types.ModuleType("lib.scaling_targets")
    scaling_targets.RESOURCE_GROUP = app_deploy.RESOURCE_GROUP
    scaling_targets.CONTAINER_APPS = app_deploy.CONTAINER_APPS
    scaling_targets.AGENT_CONTAINER_APPS = app_deploy.AGENT_CONTAINER_APPS
    scaling_targets.INFRA_CONTAINER_APPS = app_deploy.INFRA_CONTAINER_APPS
    scaling_targets.SCALING_CONFIG = app_deploy.SCALING_CONFIG
    scaling_targets.get_scaling_targets = app_deploy.get_scaling_targets
    sys.modules["lib.scaling_targets"] = scaling_targets
    lib_pkg.scaling_targets = scaling_targets


_install_app_scaling_compat_modules()
...
try:
    ...
    def _stream(cmd, cwd=None, timeout=600, prefix=""):  # noqa: ANN001, ANN202
        try:
        ...
        for line in stdout.splitlines():
            log("INFO", f"{prefix}{line}" if prefix else line)
        return result
...
def phase_0_validate_environment(args: argparse.Namespace) -> PhaseResult:
    ...
    if args.env not in ENVIRONMENTS:
        detail = f"Unknown environment '{args.env}' (valid: {list(ENVIRONMENTS.keys())})"
        log("FAIL", f"  Environment validation failed: {detail}")
        return PhaseResult(0, "Validate Environment", "FAIL", time.time() - t0, detail)

    # Production approval gate (GOV-16, SPEC-1882, Codex WP1) must fail
    # before any build, ACR, or live-environment validation work.
    ...
    if args.dry_run:
        log("INFO", "  [DRY RUN] Skipping external environment checks")
        log("INFO", f"  Python: {sys.version.split()[0]}")
        return PhaseResult(0, "Validate Environment", "PASS", time.time() - t0, "dry-run")
...
def phase_15_enforce_scaling(args: argparse.Namespace) -> PhaseResult:
    ...
    # Import lazily so unit tests can mock the pipeline runner without importing
    # deploy.py at module load time. The app-local deploy module owns the same
    # scaling taxonomy as the smoke deploy path for Agent Red.
    from deploy import RESOURCE_GROUP, SCALING_CONFIG, get_scaling_targets
    ...
    def _enforce_one_scaling(app_name: str, min_r: int, max_r: int) -> bool:
        cmd = (
        ...
        log("INFO", f"  Enforcing scaling: min={min_r} max={max_r} on {app_name}...")
        code, output = _shell_runner(cmd, 120)
        if code != 0:
            log("WARN", f"  WARNING: Scaling enforcement failed for {app_name}: {output}")
            return False
        log("INFO", f"  Scaling enforced on {app_name}.")
        return True

    log("INFO", f"  Enforcing scaling baselines on {args.env} apps (WI-3171)...")
    results = {}
    for app_name in get_scaling_targets(args.env):
        cfg = SCALING_CONFIG.get(app_name)
        if cfg is None:
            log("INFO", f"  SKIP scaling: no SCALING_CONFIG entry for {app_name}")
            results[app_name] = True
            continue
        results[app_name] = _enforce_one_scaling(
            app_name,
            cfg["min_replicas"],
            cfg["max_replicas"],
        )
