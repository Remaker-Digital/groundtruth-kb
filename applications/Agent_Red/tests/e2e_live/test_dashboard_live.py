...
class TestPageStructure:
    ...
    def test_page_title_exists(self, shared_dashboard_page: Page):
        """[EL-001/A1] 'Dashboard' title is present."""
        expect(shared_dashboard_page.locator("text=Dashboard").first).to_be_visible()

    # B1: Page title correct value
    def test_page_title_value(self, shared_dashboard_page: Page):
        """[EL-001/B1] Title text is exactly 'Dashboard'."""
        heading = shared_dashboard_page.locator("h1, h2, h3, [class*='title' i]").first
        text = heading.text_content() or ""
        assert "Dashboard" in text, f"Title missing 'Dashboard': {text[:80]}"

    # A1: Subtitle exists
    def test_subtitle_exists(self, shared_dashboard_page: Page):
    ...
    # B1: Subtitle correct
    def test_subtitle_value(self, shared_dashboard_page: Page):
    ...
    def test_store_name_or_absent(self, shared_dashboard_page: Page):
...
class TestPeriodFilter:
    ...
    # A1: Filter exists
    def test_period_filter_exists(self, shared_dashboard_page: Page):
        """[EL-004/A1] Period filter control is visible."""
        el = self._find_period_label(shared_dashboard_page, "30d")
        assert el is not None, "No period filter '30d' control found"

    # B1: Default value — 30d is the active selection
    def test_period_filter_default_30d(self, shared_dashboard_page: Page):
    ...
    def test_period_filter_all_options(self, shared_dashboard_page: Page):
        """[EL-004/A3] All 4 period options are present: 7d, 14d, 30d, 90d."""
        for label in ["7d", "14d", "30d", "90d"]:
            el = self._find_period_label(shared_dashboard_page, label)
            assert el is not None, f"Period option '{label}' not found"

    # E1: Clicking 7d changes chart
    def test_period_filter_7d_action(self, shared_dashboard_page: Page):
        """[EL-005/E1] Clicking '7d' updates the chart period label."""
        el = self._find_period_label(shared_dashboard_page, "7d")
    ...
    def test_period_filter_90d_action(self, shared_dashboard_page: Page):
        """[EL-007/E1] Clicking '90d' updates the chart period label."""
        el = self._find_period_label(shared_dashboard_page, "90d")
    ...
    def test_period_filter_14d_action(self, shared_dashboard_page: Page):
        """[EL-006/E1] Clicking '14d' updates the chart period label."""
        el = self._find_period_label(shared_dashboard_page, "14d")
    ...
    def test_period_filter_30d_action(self, shared_dashboard_page: Page):
    ...
    def test_period_filter_cycle_all(self, shared_dashboard_page: Page):
        """[EL-004..007/E1] Clicking each period option in sequence updates chart."""
        for label in ["7d", "14d", "30d", "90d"]:
            el = self._find_period_label(shared_dashboard_page, label)
            assert el is not None, f"Period option '{label}' not found"
            el.click()
            shared_dashboard_page.wait_for_timeout(800)
            num = label.replace("d", "")
            text = _main_text(shared_dashboard_page)
            assert num in text, (
...
class TestSetupChecklist:
    ...
    def test_checklist_conditional_display(self, shared_dashboard_page: Page):
    ...
    def test_checklist_items_text(self, shared_dashboard_page: Page):
...
class TestTestModeAlert:
    ...
    def test_test_mode_alert_not_present(self, shared_dashboard_page: Page):
...
class TestStatCardLabels:
    ...
    def test_stat_card_label_exists(self, shared_dashboard_page: Page, el_id: str, label: str):
        """[{el_id}/A1] Stat card label is visible."""
        text = _main_text(shared_dashboard_page)
        assert label.lower() in text.lower(), f"Stat card label '{label}' not found"

    @pytest.mark.parametrize("el_id,label", CARDS, ids=[c[0] for c in CARDS])
    def test_stat_card_label_value(self, shared_dashboard_page: Page, el_id: str, label: str):
        """[{el_id}/B1] Stat card label text matches expected value."""
        locator = shared_dashboard_page.locator(f"text={label}").first
...
class TestStatCardValues:
    ...
    def test_total_conversations_is_numeric(self, shared_dashboard_page: Page):
    ...
    def test_avg_response_time_format(self, shared_dashboard_page: Page):
    ...
    def test_resolution_rate_format(self, shared_dashboard_page: Page):
    ...
    def test_customer_satisfaction_format(self, shared_dashboard_page: Page):
    ...
    def test_escalation_rate_format(self, shared_dashboard_page: Page):
    ...
    def test_resolution_detail_text(self, shared_dashboard_page: Page):
    ...
    def test_escalation_detail_text(self, shared_dashboard_page: Page):
...
class TestStatCardGrid:
    ...
    def test_stat_cards_grid_exists(self, shared_dashboard_page: Page):
    ...
    def test_stat_card_count(self, shared_dashboard_page: Page):
...
class TestStatCardTooltips:
    ...
    def test_stat_card_tooltip_appears(self, shared_dashboard_page: Page, el_id: str, label: str):
        """[{el_id}/E4] Help tooltip appears when hovering the '?' icon near stat card."""
        card_label = shared_dashboard_page.locator(f"text={label}").first
        ...
        shared_dashboard_page.wait_for_timeout(600)
    ...
    @pytest.mark.parametrize("el_id,label", TOOLTIP_CARDS, ids=[c[0] for c in TOOLTIP_CARDS])
    def test_stat_card_tooltip_has_doc_link(self, shared_dashboard_page: Page, el_id: str, label: str):
...
class TestConversationChart:
    ...
    def test_chart_header_exists(self, shared_dashboard_page: Page):
    ...
    def test_chart_header_value(self, shared_dashboard_page: Page):
        """[EL-034/B1] Chart header text is 'Daily usage' (S160 SPEC-1685 rename)."""
        header = shared_dashboard_page.locator("text=Daily usage").first
        if header.count() > 0:
            expect(header).to_be_visible()

    # A1: Chart period label
    def test_chart_period_label_exists(self, shared_dashboard_page: Page):
    ...
    # A1: Chart container
    def test_chart_container_exists(self, shared_dashboard_page: Page):
        """[EL-036/A1] Chart canvas/SVG container is present."""
        chart = shared_dashboard_page.locator(
            "[class*='recharts'], svg[class*='chart' i], "
            "[class*='Chart'], canvas"
        )
        has_chart = chart.count() > 0
        if not has_chart:
            svgs = shared_dashboard_page.locator("svg")
    ...
    # B1: Chart legend
    def test_chart_legend_exists(self, shared_dashboard_page: Page):
        """[EL-041/A1] Chart legend with 'Conversations' label is present."""
        text = _main_text(shared_dashboard_page)
        assert "conversations" in text.lower()

    # K1: Empty state when no data
    def test_chart_empty_or_data(self, shared_dashboard_page: Page):
...
class TestRecentConversations:
    ...
    # A1: Section header
    def test_section_header_exists(self, shared_dashboard_page: Page):
        """[EL-044/A1] 'Recent conversations' header is visible."""
        text = _main_text(shared_dashboard_page)
        assert "recent conversations" in text.lower()

    # B1: Section header value
    def test_section_header_value(self, shared_dashboard_page: Page):
        """[EL-044/B1] Header text is exactly 'Recent conversations'."""
        header = shared_dashboard_page.locator("text=Recent conversations").first
        if header.count() > 0:
            expect(header).to_be_visible()

    # A1,B6: Conversation items or empty state
    def test_conversations_or_empty(self, shared_dashboard_page: Page):
    ...
    def test_status_badges_valid(self, shared_dashboard_page: Page):
        """[EL-047/B1] Status badges show only valid status values."""
        VALID = {"active", "idle", "ended", "escalated"}
        badges = shared_dashboard_page.locator(
    ...
    def test_message_count_format(self, shared_dashboard_page: Page):
    ...
    def test_empty_state_text(self, shared_dashboard_page: Page):
...
class TestTopTopics:
    ...
    def test_topic_counts_numeric(self, shared_dashboard_page: Page):
...
class TestAnalyticsDivider:
    ...
    def test_divider_exists(self, shared_dashboard_page: Page):
        """[EL-059/A1] 'Detailed analytics' divider is visible."""
        text = _main_text(shared_dashboard_page)
        assert "detailed analytics" in text.lower()

    def test_divider_label(self, shared_dashboard_page: Page):
        """[EL-059/B1] Divider label text is 'Detailed analytics'."""
        divider = shared_dashboard_page.locator("text=Detailed analytics").first
        if divider.count() > 0:
...
class TestTopicBreakdown:
    ...
    def test_table_exists(self, shared_dashboard_page: Page):
    ...
    def test_table_columns(self, shared_dashboard_page: Page):
...
class TestKnowledgeGaps:
    ...
    def test_description_text(self, shared_dashboard_page: Page):
    ...
    def test_count_badge_or_empty(self, shared_dashboard_page: Page):
...
class TestDashboardIntegrity:
    ...
    def test_all_sections_present(self, shared_dashboard_page: Page):
    ...
    def test_no_duplicate_sections(self, shared_dashboard_page: Page):
        """[CROSS/A3] No dashboard section header appears more than once."""
        text = _main_text(shared_dashboard_page).lower()
        sections = [
    ...
    def test_data_populated(self, shared_dashboard_page: Page):
    ...
    def test_no_lingering_skeletons(self, shared_dashboard_page: Page):
        """[CROSS/I1] No loading skeletons visible after data load."""
        skeletons = shared_dashboard_page.locator(
    ...
    def test_no_error_text(self, shared_dashboard_page: Page):
    ...
    def test_stat_cards_all_visible(self, shared_dashboard_page: Page):
    ...
    def test_all_values_non_negative(self, shared_dashboard_page: Page):
    ...
    def test_percentages_valid_range(self, shared_dashboard_page: Page):
...
class TestSegmentedControl:
    ...
    def test_segmented_control_present(self, shared_dashboard_page: Page):
        """SegmentedControl root element exists on the page."""
        sc = shared_dashboard_page.locator("[class*='SegmentedControl'], [role='radiogroup']")
        # Mantine SegmentedControl renders a root div; also accept button fallback
        has_sc = sc.count() > 0
        has_buttons = shared_dashboard_page.get_by_role("button", name="30 days").count() > 0
        assert has_sc or has_buttons, "No period filter control found"

    def test_all_period_labels_present(self, shared_dashboard_page: Page):
    ...
    def test_period_selection_changes_chart_header(self, shared_dashboard_page: Page):
        """Selecting '7d' updates the chart subtitle to 'Last 7 days'."""
        # Click 7d segment
        seg = shared_dashboard_page.locator(
...
class TestTestModeAlertDetails:
    ...
    def test_test_mode_list_items(self, shared_dashboard_page: Page):
    ...
    def test_test_mode_switch_note(self, shared_dashboard_page: Page):
...
class TestSetupChecklistDetails:
    ...
    def test_checklist_progress_counter(self, shared_dashboard_page: Page):
...
class TestRecentConversationCards:
    ...
    def test_customer_name_displayed(self, shared_dashboard_page: Page):
    ...
    def test_assignment_or_unassigned(self, shared_dashboard_page: Page):
    ...
    def test_message_count_in_cards(self, shared_dashboard_page: Page):
    ...
    def test_status_badge_colors_valid(self, shared_dashboard_page: Page):
...
class TestTopTopicsDetails:
    ...
    def test_top_topics_help_tooltip(self, shared_dashboard_page: Page):
        """Top topics header has a HelpTooltip '?' icon."""
        # Find the "Top topics" text, then look for sibling span[role='button']
        header = shared_dashboard_page.locator("text=Top topics").first
        if header.count() == 0:
            return
    ...
    def test_topic_items_have_counts(self, shared_dashboard_page: Page):
...
class TestChartDetails:
    ...
    def test_chart_help_tooltip(self, shared_dashboard_page: Page):
        """Conversation volume header has a HelpTooltip."""
        header = shared_dashboard_page.locator("text=Conversation volume").first
        if header.count() == 0:
            return
        parent = header.locator("xpath=ancestor::div[1]")
        help_icon = parent.locator("span[role='button']")
    ...
    def test_chart_svg_gradient_def(self, shared_dashboard_page: Page):
        ...
        gradient = shared_dashboard_page.locator("svg linearGradient")
        if gradient.count() == 0:
    ...
    def test_chart_axes_present(self, shared_dashboard_page: Page):
        """Chart has X and Y axis elements."""
        text = _main_text(shared_dashboard_page)
        if "no volume data" in text.lower():
            return
        # Recharts renders .recharts-xAxis and .recharts-yAxis groups
        x_axis = shared_dashboard_page.locator("[class*='recharts-xAxis'], .recharts-xAxis")
        y_axis = shared_dashboard_page.locator("[class*='recharts-yAxis'], .recharts-yAxis")
        has_axes = x_axis.count() > 0 and y_axis.count() > 0
        if not has_axes:
            # Fallback: check for any SVG ticks
            ticks = shared_dashboard_page.locator("svg g text")
            has_axes = ticks.count() >= 2
        assert has_axes, "Chart missing X/Y axis elements"

    def test_chart_legend_color_swatch(self, shared_dashboard_page: Page):
        """Chart legend shows a colored swatch box next to 'Conversations'."""
        text = _main_text(shared_dashboard_page)
        if "no volume data" in text.lower():
            return
        # Legend renders a 10x10 box + "Conversations" text
        legend_text = shared_dashboard_page.locator("text=Conversations")
...
class TestTopicBreakdownDetails:
    ...
    def test_topic_breakdown_help_tooltip(self, shared_dashboard_page: Page):
        """Topic breakdown header has a HelpTooltip."""
        header = shared_dashboard_page.locator("text=Topic breakdown").first
        if header.count() == 0:
            return
        parent = header.locator("xpath=ancestor::div[1]")
        help_icon = parent.locator("span[role='button']")
...
class TestKnowledgeGapsDetails:
    ...
    def test_knowledge_gaps_help_tooltip(self, shared_dashboard_page: Page):
        """Knowledge gaps header has a HelpTooltip."""
        header = shared_dashboard_page.locator("text=Knowledge gaps").first
        if header.count() == 0:
            return
        parent = header.locator("xpath=ancestor::div[1]")
        help_icon = parent.locator("span[role='button']")
    ...
    def test_gap_count_badge(self, shared_dashboard_page: Page):
    ...
    def test_gap_table_5_columns(self, shared_dashboard_page: Page):
    ...
    def test_gap_status_badges(self, shared_dashboard_page: Page):
    ...
    def test_gap_turn_and_message_counts(self, shared_dashboard_page: Page):
    ...
    def test_gap_started_date_format(self, shared_dashboard_page: Page):
    ...
    def test_description_mentions_resolve(self, shared_dashboard_page: Page):
...
class TestSectionHelpTooltips:
    ...
    def test_section_has_help_tooltip(self, shared_dashboard_page: Page, section: str):
        """Section header '{section}' contains a HelpTooltip '?' icon."""
        header = shared_dashboard_page.locator(f"text={section}").first
        if header.count() == 0:
...
class TestResponsiveLayout:
    ...
    def test_stat_cards_in_paper_containers(self, shared_dashboard_page: Page):
        ...
        papers = shared_dashboard_page.locator("[class*='Paper-root' i]")
        if papers.count() == 0:
            papers = shared_dashboard_page.locator("[class*='paper' i]")
    ...
    def test_two_column_layout_exists(self, shared_dashboard_page: Page):
    ...
    def test_loading_skeletons_absent(self, shared_dashboard_page: Page):
        """After data load, no Skeleton placeholders remain visible."""
        # Mantine Skeleton has class *Skeleton*
        skeletons = shared_dashboard_page.locator("[class*='Skeleton' i]")
        visible = sum(
