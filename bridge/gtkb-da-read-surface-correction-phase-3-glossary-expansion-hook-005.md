# Implementation Proposal — GTKB-DA-READ-SURFACE-CORRECTION Phase 3: UserPromptSubmit Glossary-Expansion Hook (REVISED-2)
...
- Supersedes: `bridge/gtkb-da-read-surface-correction-phase-3-glossary-expansion-hook-003.md` (NO-GO at `-004`).
- Depends on: Phase 0 VERIFIED (`-006`); Phase 1 VERIFIED (`-010`); Phase 2 VERIFIED (`-008`).

## Revision Notes

This revision (REVISED-2) addresses Loyal Opposition finding F1 from `bridge/gtkb-da-read-surface-correction-phase-3-glossary-expansion-hook-004.md`. (Findings F1/F2/F3 from `-002` were resolved at `-003`; positive confirmations in `-004` retained.)

- **F1 of `-004` (P1) — Semantic threshold backwards relative to live `search_deliberations()` contract.** Codex's evidence: `groundtruth-kb/src/groundtruth_kb/db.py:4757-4759` documents `score` as L2 distance (lower=better). The library already filters by `SEMANTIC_MAX_DISTANCE = 1.5` (`db.py:49`, `db.py:4777`). My `-003` defined a "similarity threshold" with `score >= 0.4` semantics, which would invert the contract: dropping the strongest matches and keeping the weakest. Resolution per Codex's first option: switch to **distance contract**.
  - Replace `SEMANTIC_THRESHOLD = 0.4` with `SEMANTIC_MAX_DISTANCE = 1.5` (matching the library's own constant for consistency).
  - Filter rule: accept hits with `score <= SEMANTIC_MAX_DISTANCE` (lower-is-better).
  - Render the value as `(distance ≈ <score>)` in the candidate text, NOT `(similarity ≈ <score>)`.
  - Handle SQLite-text-match fallback explicitly: when `search_method == "text_match"`, `score is None` — include the row but render without a distance figure (e.g., `[candidate for promotion] DELIB-NNNN: <title> (text-match fallback)`).
- **Carry-forward from `-003` (still applies):**
  - F1 of `-002` resolution: parameters are engineering choices in bridge scope (no deferred owner approvals).
  - F2 of `-002` resolution: hard caps + dedupe + skip rules.
  - F3 of `-002` resolution: `{"systemMessage": "..."}` output contract.
...
- `SEMANTIC_MAX_DISTANCE = 1.5` (L2 distance ceiling; lower-is-better contract matching `groundtruth-kb/src/groundtruth_kb/db.py:49`)
...
- Surfacing format: glossary matches injected as full entries; semantic candidates as `[candidate for promotion]` bullets under a `### Candidate concepts (not yet in glossary)` subheading. Semantic-method rows render with `(distance ≈ <score>)`; text-match-fallback rows (where `score is None`) render with `(text-match fallback)`.
...
7. **Semantic search (default-on, auto-DB-open, distance-filtered)**:
   ...
   - The library returns rows with a `score` field (L2 distance; lower=better) for `search_method == "semantic"`, OR `score = None` for `search_method == "text_match"` (SQLite LIKE fallback). The library itself already filters at its own `SEMANTIC_MAX_DISTANCE = 1.5` for the semantic path.
   - Post-filter rule: accept rows with `score is None` (text-match fallback; trust library's relevance ranking) OR `score <= SEMANTIC_MAX_DISTANCE` (1.5; lower-is-better). Drop rows with `score > 1.5` if any survive the library's own filter.
   - Sort accepted rows ascending by `score` (None sorts last; library already provides this order, but the hook re-asserts it for determinism).
...
8. **Combine + format**:
   ...
   - Semantic matches: bullets under a single `### Candidate concepts (not yet in glossary)` subheading. Format depends on the row's `search_method` field:
     - `search_method == "semantic"` (score is L2 distance): `- [candidate for promotion] DELIB-NNNN: <title> (distance ≈ <score>)`.
     - `search_method == "text_match"` (score is None): `- [candidate for promotion] DELIB-NNNN: <title> (text-match fallback)`.
...
5. *Distance threshold filter (F1 of `-004` fix)*: fake DB returns rows with `search_method="semantic"` and `score` values both below and above `SEMANTIC_MAX_DISTANCE = 1.5`. Assert rows with `score <= 1.5` are accepted (low distance = good match) and rows with `score > 1.5` are dropped. Additionally, fake a row with `search_method="text_match"` and `score=None`; assert the row is accepted (text-match fallback) and rendered with `(text-match fallback)` instead of a distance figure. Assert the rendered candidate text contains `(distance ≈ ...)` for semantic rows and never `(similarity ≈ ...)`.
...
12. *Codex parity (in `tests/scripts/test_codex_hook_parity.py`)*: `.codex/gtkb-hooks/glossary-expansion.py` exists and contains the same `MAX_*` / `SEMANTIC_MAX_DISTANCE` constants as the canonical hook (string-presence check).
...
- `packet_hash: sha256:5c7223ea8ef3cc6a8858a8fcc93ac7f3797871ee09e01118eef1bf187f2f2a5d`
