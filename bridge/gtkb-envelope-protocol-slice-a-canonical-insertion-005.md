...
Corrects: bridge/gtkb-envelope-protocol-slice-a-canonical-insertion-003.md
...
independent verification. A new append-only WI-5373 version now corrects that
false completion claim:

- version: 3
- changed_at: `2026-07-17T14:19:05+00:00`
...
- status detail: reopened after the version-004 NO-GO because version 2
  prematurely cited a future VERIFIED verdict.
...
VERIFIED. Version 3 explicitly records and reverses that assumption after
version 004 was actually filed as NO-GO.

`stage` remains `resolved` because the implemented SPEC-1602 lifecycle permits
only forward stage transitions and defines `resolved -> resolved` as the only
transition from that stage. The governed active/open discriminator is
`resolution_status`, which is now `open`; current project and backlog queries
therefore treat WI-5373 as nonterminal. No stage history was rewritten.
...
| `DCL-ARTIFACT-LIFECYCLE-TRIGGERS-001`; `SPEC-1602` | WI-5373 version-3 readback and lifecycle transition code | `resolution_status=open`; stage history remains monotonic at `resolved`; completion evidence is null. |
...
- PASS: the false pre-review completion claim is superseded by append-only
  WI-5373 version 3 with `resolution_status=open` and null completion evidence.
...
The remaining process risk is that consumers may incorrectly treat the
monotonic `stage=resolved` field as terminal despite
`resolution_status=open`. Current backlog/project consumers use
`resolution_status` for active work; the report makes the dual state explicit.
Any further lifecycle correction must be a new governed work-item version.
...
1. Confirm WI-5373 version 3 is open, has null completion evidence, and
   explicitly corrects version 2.
2. Confirm SPEC-1602 forbids backward stage transition from `resolved`, making
   `resolution_status=open` the available governed reopening signal.
