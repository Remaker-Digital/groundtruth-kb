...
target_paths: ["config/dispatcher-next/requirements-spike.txt", "platform_tests/groundtruth_kb/test_dispatcher_next_spike_manifest.py"]
...
configuration  config/dispatcher-next/requirements-spike.txt
...
`config/dispatcher-next/requirements-spike.txt`, which classifies `configuration` - a class
...
Create `config/dispatcher-next/requirements-spike.txt` declaring exactly the two
...
1. `config/dispatcher-next/requirements-spike.txt` exists, completing the sixth target of the cohort accepted by the prior
