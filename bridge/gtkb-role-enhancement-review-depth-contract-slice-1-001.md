...
author_harness_id: A
author_session_context_id: keep-working-20260607T0515Z
...
target_paths: [".claude/rules/report-depth-prime-builder-context.md", "groundtruth-kb/templates/rules/report-depth.md", "platform_tests/scripts/test_report_depth_review_methodology.py"]

## Claim

This child proposal implements Slice 1 from the GO-approved parent
`bridge/gtkb-role-enhancement-004.md`: review-depth methodology.

The implementation will add an explicit Loyal Opposition proposal-review depth
contract for proposals that make output-layout, artifact-inventory,
target-path, or surface-coverage claims. The contract requires review to compare
those claims against the proposal's implementation/output list at proposal
review time, not only during post-implementation verification.
...
`scripts/implementation_authorization.py begin --bridge-id gtkb-role-enhancement-review-depth-contract-slice-1`
...
- Update `.claude/rules/report-depth-prime-builder-context.md` with a concise
  review-depth methodology clause for proposal review.
- Update `groundtruth-kb/templates/rules/report-depth.md` so scaffolded or
  upgraded rule surfaces carry the same review-depth doctrine.
- Add focused tests in `platform_tests/scripts/test_report_depth_review_methodology.py`
  proving both target rule surfaces contain the required methodology anchors.

Out of scope:

- Mutating `.claude/rules/loyal-opposition.md`, `.claude/rules/file-bridge-protocol.md`,
  `.claude/rules/expedited-paths.md`, or any formal GOV, ADR, DCL, PB, SPEC,
  or requirement artifact.
...
filing for review-depth methodology. `DELIB-S310-ROLE-DEFINITION-ASSESSMENT`
identifies review-depth methodology as an underdefined role-contract gap, and
`DELIB-S312-ROLE-CONTRACT-EFFECTIVENESS-UPDATE` records the specific heuristic:
Loyal Opposition should walk proposal output-layout diagrams and related output
claims against the proposal's described output files during proposal review.
No new owner decision is required for this child proposal.
...
- `GOV-ARTIFACT-APPROVAL-001` - implementation touches protected narrative
  ...
  requires it.
- `ADR-ARTIFACT-ORIENTED-DEVELOPMENT-001` - the review-depth contract is
  advanced through an explicit lifecycle artifact rather than ad hoc rule edits.
...
- `ADR-CODEX-HOOK-PARITY-FALLBACK-001` - generated or scaffolded rule surfaces
  ...
  alongside the live Prime Builder context rule.

## Prior Deliberations

- `DELIB-S310-ROLE-DEFINITION-ASSESSMENT` - originating nine-gap role-definition
  assessment; identifies review-depth methodology as a role-contract gap.
- `DELIB-S312-ROLE-CONTRACT-EFFECTIVENESS-UPDATE` - empirical update preserving
  the post-isolation sequencing constraint and the review-depth heuristic.
...
- `DELIB-2322`, `DELIB-2323`, and `DELIB-2741` - prior review-depth-methodology
  bridge history and review context.
...
### IP-1 - Live Prime Builder Context Rule

Add a focused review-depth methodology clause to
`.claude/rules/report-depth-prime-builder-context.md`.

Required anchors:

- Output-layout claims must be compared against the proposal's implementation
  or output list.
- Artifact-inventory claims must be checked against proposed target artifacts.
- Target-path and surface-coverage claims must be checked against concrete
  target paths and described generated outputs.
- This comparison must happen during proposal review, before implementation is
  authorized.

### IP-2 - Scaffold Template Rule

Update `groundtruth-kb/templates/rules/report-depth.md` with matching doctrine
so managed/scaffolded rule output does not drift from the live Prime Builder
context rule.

### IP-3 - Focused Tests

Add `platform_tests/scripts/test_report_depth_review_methodology.py` with tests
...
- surface coverage
- proposal review
- implementation/output list comparison
...
| CQ-TESTS-001 | Yes | Add a focused pytest module covering both target rule surfaces. | `python -m pytest platform_tests/scripts/test_report_depth_review_methodology.py -q --tb=short`. | |
...
- `GOV-FILE-BRIDGE-AUTHORITY-001`: run
  `python .claude/skills/bridge/helpers/show_thread_bridge.py gtkb-role-enhancement-review-depth-contract-slice-1 --format json --preview-lines 20`.
...
- `DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001`: run
  `python -m pytest platform_tests/scripts/test_report_depth_review_methodology.py -q --tb=short`.
...
- Python quality for the new test: run
  `python -m ruff check platform_tests/scripts/test_report_depth_review_methodology.py`
  and `python -m ruff format --check platform_tests/scripts/test_report_depth_review_methodology.py`.
...
- Loyal Opposition proposal review has an explicit depth contract for
  output-layout, artifact-inventory, target-path, and surface-coverage claims.
- The live Prime Builder context rule and scaffold template carry matching
  review-depth doctrine.
- Focused tests fail if either target surface loses the core methodology
  anchors.
...
- Risk: the rule text becomes too prescriptive for small proposals. Mitigation:
  implementation should keep the clause concise and trigger it only when the
  proposal makes output-layout, artifact-inventory, target-path, or
  surface-coverage claims.
- Risk: updating the template without updating the live context rule creates
  drift. Mitigation: focused tests assert both surfaces.
...
Candidate-content checks before live filing:

```powershell
python scripts/bridge_applicability_preflight.py --bridge-id gtkb-role-enhancement-review-depth-contract-slice-1 --content-file .gtkb-state\bridge-propose-drafts\gtkb-role-enhancement-review-depth-contract-slice-1-001.md
python scripts/adr_dcl_clause_preflight.py --bridge-id gtkb-role-enhancement-review-depth-contract-slice-1 --content-file .gtkb-state\bridge-propose-drafts\gtkb-role-enhancement-review-depth-contract-slice-1-001.md
```

Expected filing condition: applicability preflight passes with no missing
required specs, and ADR/DCL clause preflight exits 0 with zero blocking gaps.
