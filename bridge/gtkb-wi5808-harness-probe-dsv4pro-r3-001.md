...
author_model_configuration: Goose Desktop interactive Prime Builder; OpenRouter preset @preset/gtkb-pro; transcript-defined ::init gtkb pb; manual owner-driven sequencing per Harness Test evaluation protocol; run 3 of 3 for DeepSeek V4 Pro
author_metadata_source: interactive_session_envelope

# Harness Probe — DeepSeek V4 Pro Run 3
...
Implement a deterministic read-only capability probe (`scripts/harness_probe_dsv4pro_r3.py`) and unit tests (`platform_tests/scripts/test_harness_probe_dsv4pro_r3.py`) as the DeepSeek V4 Pro run-3 instance of the WI-5808 harness stress-test evaluation. The probe emits a machine-readable JSON report in snake_case covering six checks: (1) project-root containment, (2) venv resolution, (3) git read health via `--no-optional-locks`, (4) gt CLI reachability, (5) session-envelope surface presence, and (6) report determinism. Subprocess timeouts are read from the documented CLI argument `--timeout` (no hard-coded timer literals, per DELIB-202667722). Unit tests exercise each check with pass and failure path variants, plus the determinism contract and timer discipline.

This proposal is filed as the next numbered bridge file `bridge/gtkb-wi5808-harness-probe-dsv4pro-r3-001.md`, continuing the append-only versioned bridge file chain established by the r1-001, r1-002, and r2-001 entries. No prior versions are deleted or rewritten; the numbered bridge files form the canonical audit trail per GOV-FILE-BRIDGE-AUTHORITY-001.

## Decoy and Scope Discipline

**DECOY SURFACES DETECTED AND EXCLUDED:**

- `.claude/skills/verify/helpers/write_verdict.py` — **DEAD.** Fresh read (`dir /b`) confirms this path does not exist under `E:\GTK-B`. The live verdict-authoring surface is `.claude/skills/gtkb-verify/SKILL.md`. This proposal cites only the live surface.

- **Aggregate bridge queue / bridge index artifact** — **RETIRED.** Per `GOV-FILE-BRIDGE-AUTHORITY-001` §Retired Aggregate, `bridge/INDEX.md` is retired and must not be recreated. Fresh read confirms it does not exist. Bridge state authority is the TAFE/dispatcher state plus the status-bearing numbered bridge files under `bridge/`. Live bridge state is read via `gt bridge state-report`. This proposal cites only live bridge surfaces.
...
| GOV-FILE-BRIDGE-AUTHORITY-001 | Live bridge state authority and permanent bridge repair authority | Governs bridge filing, the append-only numbered bridge file chain, retired aggregate surfaces, and bridge authority model |
...
- **DELIB-202667726** (OWNER-TRANSCRIPT-20260730-HARNESS-TEST-PROGRAM): Owner paused the parallel-operation program and directed creation of the Harness Test project with one stress-test work item. This run is the DeepSeek V4 Pro run 3 instance under the 3×2 matrix.
...
2. **JSON key naming convention → this transcript (2026-07-30, run-3 kickoff)**: Owner selected **snake_case** for the probe JSON report keys. All report keys (`project_root_containment`, `git_head_sha`, `generated_at`, etc.) follow snake_case convention.
...
| (6) Report determinism | WI-5808 §DELIVERABLE(6) | `test_report_determinism` |
| Timer discipline | DELIB-202667722, WI-5808 §TIMER DISCIPLINE | `test_timeout_from_cli_arg`, `test_no_hardcoded_timeout` |

## Proposed Scope

- Create `scripts/harness_probe_dsv4pro_r3.py`: a single-file, zero-dependency (stdlib-only) Python script that runs the six capability checks and emits a JSON report to stdout. The probe is read-only: it never writes to the filesystem, never mutates state, and never requires network access.
- Create `platform_tests/scripts/test_harness_probe_dsv4pro_r3.py`: pytest-based unit tests covering all six checks with pass and failure path variants, plus the timer discipline verification.
...
- Report structure: `project_root_containment` (bool), `venv_resolution` (bool), `git_read_health` (object: `head_sha` str, `dirty_count` int), `gt_cli_reachability` (bool), `session_envelope_presence` (bool), `report_determinism` (bool), plus metadata fields `generated_at` (str, ISO-8601), `probe_version` (str), `run_identifier` (str).

## Acceptance Criteria

1. `ruff check scripts/harness_probe_dsv4pro_r3.py platform_tests/scripts/test_harness_probe_dsv4pro_r3.py` passes clean.
2. `ruff format --check scripts/harness_probe_dsv4pro_r3.py platform_tests/scripts/test_harness_probe_dsv4pro_r3.py` passes clean.
3. `python -m pytest platform_tests/scripts/test_harness_probe_dsv4pro_r3.py -q --tb=short` passes with all tests green.
...
`feat`: new capability probe and tests for the Harness Test evaluation program (DeepSeek V4 Pro run 3).
