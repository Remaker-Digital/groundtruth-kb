...
Version: 026
Responds to: bridge/gtkb-dispatcher-black-box-spec-foundation-025.md
Supersedes for implementation authority: bridge/gtkb-dispatcher-black-box-spec-foundation-024.md, bridge/gtkb-dispatcher-black-box-spec-foundation-025.md
...
Version 026 preserves version 024's canonical-only evidence, exact five native
bodies, record metadata, and executable evaluation contracts. It withdraws the
version-025 GO before implementation because the approved target set was not
closed over the actual governed `gt spec update` behavior.

The governed update command requires an in-root `--content-file` and writes one
formal-approval packet before each MemBase update. Version 024 authorized only
`groundtruth.db`, prohibited generated scratch input, and said generated
approval files were not outputs. Consuming that GO would therefore mutate
undeclared paths and contradict the approved procedure. Version 026 declares
the ten unavoidable operational file targets, requires deterministic
same-session extraction from canonical version 024, and keeps every transient
or generated file outside canonical evidence.
...
run record, or other ephemeral artifact is an evidentiary citation or
verification dependency of this proposal.

The requested durable target is the canonical MemBase carrier. Five exact
same-session content carriers are declared only because the governed CLI
requires `--content-file`; they must be extracted from canonical version 024,
hash-checked, consumed in this Prime Builder session, and deleted before the
implementation report. Five formal-approval packet paths are declared because
the governed CLI creates them as operation-time governance outputs. Those ten
files are operational targets, not canonical authorities, and the report and
verdict must not cite them as evidence. No source, test, hook, skill,
configuration, dispatcher, TAFE, harness runtime, Git index, commit, push,
deployment, credential, or external-system mutation is requested.

## Current Canonical State

Fresh governed CLI reads after version 023 show that all five specification
identifiers now exist at version 1 and `specified`. Their exact native bodies
match the owner-approved hashes embedded below. WI-5268 remains at version 8,
`stage=resolved`, and `resolution_status=resolved`.

The current version-1 specification records are not terminally sufficient:

- every record has `assertions=null`, which is `UNASSESSED` under
  `DCL-CHANGE-CONTROLLED-ARTIFACT-EVALUABILITY-001`;
- their canonical metadata retains ephemeral and generated-output references
  that may not serve as cross-session evidence under
  `DELIB-20260717-CANONICAL-ARTIFACT-REFERENCE-BOUNDARY`; and
- WI-5268 still claims closure against the superseded version-015 bridge state.

This proposal therefore does not create the five identifiers. It appends
version 2 to each existing specification through `gt spec update`, preserving
the exact native body while replacing the invalid metadata with the
canonical-only fields and executable contracts below. The first authorized
MemBase action remains an append-only WI-5268 correction to open/backlogged.
The source of the intervening row reappearance is not needed for this repair
and is not inferred from ephemeral state.
...
All five records receive append-only version 2 with these common fields:
...
```json
{
  ...
  "change_reason": "WI-5268 canonical-reference and evaluability repair of the dispatcher black-box governance/spec foundation",
  "auq_id": "CHAT-WI5268-FOUNDATION-PACKET-V2-20260715",
  "auq_answer": "APPROVE WI5268 FOUNDATION PACKET V2",
...
```json
{
  ...
  "constraints": {
    ...
    "evaluation_contract": {
      ...
      "currentness": "invalid when canonical version 024 is absent or superseded by changed native content",
  ...
  "assertions": [
    {
      ...
      "assertions": [
        {
          "type": "file_exists",
          "file": "bridge/gtkb-dispatcher-black-box-spec-foundation-024.md",
          "description": "Canonical version 024 exists."
        },
        {
          "type": "grep",
          "file": "bridge/gtkb-dispatcher-black-box-spec-foundation-024.md",
...
After a fresh independent GO responding to version 026:

1. Acquire a fresh `go_implementation` claim and create a fresh schema-v3
   implementation-start packet bound to version 026, the GO, WI-5268, the
   active project authorization, this session, and all eleven declared
   operational targets.
2. Re-run applicability and mandatory-clause preflights against the exact
   authorized proposal and fail closed on any gap.
3. Read WI-5268 and all five specification identifiers through a new governed
   CLI process. Require the canonical precondition documented above: WI-5268
   version 8/resolved and five exact version-1 native bodies with
   `assertions=null`. Any body conflict or unexpected newer version stops
   implementation and returns to review.
...
5. In this Prime Builder session, deterministically extract the five native
   Markdown bodies from the canonical version-024 numbered bridge artifact
   into the five declared `work_area/wi5268-*.md` content carriers.
   Verify each exact SHA-256 from the packet above before use. Do not inspect,
   reuse, or cite any pre-existing scratch or generated file.
6. Append version 2 for all five `specified` formal records through
   `gt spec update`, using only those verified same-session content carriers
   and the metadata above. Permit the command to create only the five exact
   declared formal-approval packet outputs. Delete the five content carriers
   immediately after all updates and do not cite either file class as
   canonical evidence. Do not use raw SQL, another database, binary
   replacement, or whole-file restore.
7. Close the writer process. In a separate process, read WI-5268 and all five
   specification records and compare every declared field and each exact
   native-body hash.
8. After a delay spanning at least two normal dispatcher cycles, repeat the
   six separate-process reads without inspecting or mutating dispatcher
   configuration or runtime internals.
9. File an implementation report only if both readback rounds pass.
10. Independent verification must use read-only canonical CLI and numbered
...
No existing generated approval file is an implementation input. The five new
approval packets are declared operation-time outputs but are not canonical
bridge evidence. Formal approval authority is the exact owner phrase and the
canonical Deliberation Archive records embedded above.

## Specification-Derived Verification Plan

| Specification or requirement | Canonical verification evidence | Required result |
| --- | --- | --- |
| `GOV-PROJECT-IMPLEMENTATION-AUTHORIZATION-001`; `PB-PROJECT-AUTHORIZATION-NO-BRIDGE-BYPASS-001` | Active project-authorization read plus fresh claim/start readback | Exact active PAUTH, independent GO, current claim, schema-v3 start, and eleven exact operational targets |
| Five proposed ADR/DCL artifact updates; `DCL-CHANGE-CONTROLLED-ARTIFACT-EVALUABILITY-001` | Five separate `gt spec show` results compared to the embedded bodies and metadata | Exact IDs, types, titles, contents, hashes, fields, tags, constraints, canonical provenance, version 2, five executable outer assertions, and `specified` |
...
| Canonical reference boundary; WI-5491 | Review of version 024 citations and self-contained packet | No scratch, retired, generated-output, harness-local, or runtime-log evidence dependency |
| Governed `gt spec update` execution closure | Exact target comparison plus post-use path checks | Five hash-checked same-session content carriers are removed; exactly five declared approval packets are generated and never cited as evidence |
...
The filing helper must run final-byte compliance checks over version 026 and
fail closed on any intervening change. Applicability must select the same
active PAUTH for all eleven operational targets, and the mandatory ADR/DCL
clause preflight must report zero blocking gaps. Canonical-boundary review must
distinguish declared operational paths from evidentiary citations and must
confirm that no transient or generated path is proposed as evidence.

## Acceptance Criteria

- [x] Version 024 and its version-025 GO are withdrawn before implementation.
...
- [x] The durable scope is the canonical MemBase carrier; all ten unavoidable
  CLI file effects are exactly declared and excluded from evidence.
...
- [ ] Fresh independent GO approves version 026 and all eleven operational
  targets.
- [ ] Fresh claim and schema-v3 start authorize the eleven exact targets.
- [ ] WI-5268 is truthfully restored to open/backlogged.
- [ ] All five exact version-2 formal records persist through immediate and
  delayed separate-process readback.
...
A secondary risk is treating CLI-required transient or generated files as
canonical evidence. The containment is deterministic extraction from version
024 in this session, pre-use body hashes, exact path authorization, immediate
content-carrier deletion, and report/verdict evidence limited to MemBase,
Deliberation Archive records, and numbered bridge artifacts.
