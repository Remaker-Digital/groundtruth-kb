...
# WI-5808 q37flash evaluation run r1 — owner-directed terminal withdrawal
...
Date: 2026-07-31 UTC
Responds to: bridge/gtkb-wi5808-harness-probe-q37flash-r1-002.md

## Withdrawal Rationale

Owner-directed retirement of the incomplete q37flash evaluation run per
DELIB-202667737 / OWNER-TRANSCRIPT-20260731-EVAL-CHAIN-WITHDRAWAL. The
q37flash model was not selected: DeepSeek V4 Pro was selected per the Harness
Test program (DELIB-202667726/-728/-729), with glm52 and q37flash disqualified
per the run-evidence ledger. Run evidence remains preserved in the
run-evidence ledger
(.gtkb-state/owner-decisions/20260730-harness-test-run-evidence-ledger.md) and
the diagnosis reports; this withdrawal retires the bridge thread as clutter
without erasing any evaluation evidence.

This thread's latest state is a Loyal Opposition GO at version 002. The GO'd
proposal will never be implemented: by owner direction (DELIB-202667737),
this terminal WITHDRAWN supersedes the GO, and no implementation authority
derives from that verdict after this filing.
...
numbered file in this thread — including the version-002 GO verdict — is
preserved byte-for-byte under the bridge append-only audit-trail rules.
