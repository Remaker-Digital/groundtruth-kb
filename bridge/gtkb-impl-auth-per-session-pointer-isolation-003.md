...
Responds-To: bridge/gtkb-impl-auth-per-session-pointer-isolation-002.md
...
# Implementation Proposal — Session-Aware Impl-Auth Packet Resolution (WI-4443) — REVISED

## Revision Scope

Sole change from -001: added the mandatory `## Bridge Filing (INDEX-Canonical)` section per LO finding at -002 (single Finding 1: missing section triggers blocking gap in clause preflight for `GOV-FILE-BRIDGE-AUTHORITY-001/CLAUSE-INDEX-IS-CANONICAL`). No content/scope/target_paths change.
...
- `GOV-FILE-BRIDGE-AUTHORITY-001` — the impl-start gate enforces the bridge protocol's append-only authorization contract; this fix preserves the contract under concurrent implementers. CLAUSE-INDEX-IS-CANONICAL satisfied by the Bridge Filing section below.
...
- `bridge/gtkb-impl-auth-per-session-pointer-isolation-002.md` — the NO-GO this revision addresses. Sole gap: missing `## Bridge Filing (INDEX-Canonical)` section.
...
This proposal is filed under `bridge/` as `bridge/gtkb-impl-auth-per-session-pointer-isolation-003.md` with a `REVISED` entry prepended to the existing `gtkb-impl-auth-per-session-pointer-isolation` document block in `bridge/INDEX.md` via the serialized `gt bridge index set-status` CLI (`scripts/bridge_index_writer.py` holds the lock and performs atomic temp-then-replace with read-modify-merge). The prior `NEW@-001` and `NO-GO@-002` status lines are preserved (append-only); no version files are deleted or rewritten. `bridge/INDEX.md` remains the canonical workflow state per `GOV-FILE-BRIDGE-AUTHORITY-001/CLAUSE-INDEX-IS-CANONICAL` — agent-tool Write/Edit of INDEX is now also blocked by the WI-4481 INDEX-write-guard hook (filed as `bridge/gtkb-bridge-index-atomic-write-guard-003.md` NEW awaiting LO VERIFIED), ensuring all INDEX mutations route through the serialized writer.
