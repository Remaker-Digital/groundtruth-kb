...
# Implementation Proposal: Inventory Drift GH Probe Parity

## Summary

Complete WI-4218 by eliminating the non-reconcilable writer/checker mismatch where public toolchain.gh.evidence differs between collection and live drift checking. The fix must make the writer and checker share the same deterministic public probe-evidence path.
...
- Backlog row WI-4218 records the inventory-drift gate defect being addressed by this proposal.
...
- DA: `DELIB-1654` — seed=search; bridge_thread; Loyal Opposition Review - GTKB-ENV-INVENTORY-DRIFT-CONTROL-001
- DA: `DELIB-1653` — seed=search; bridge_thread; Loyal Opposition Verification - GTKB-ENV-INVENTORY-DRIFT-CONTROL-001
- DA: `DELIB-1651` — seed=search; bridge_thread; Loyal Opposition Review - GTKB-ENV-INVENTORY-DRIFT-CONTROL-001

## Owner Decisions / Input

No new owner decision is required. The active PAUTH authorizes source, test, config, and governance-evidence changes for WI-4218 while preserving normal bridge GO, implementation-start packet, post-implementation report, and Loyal Opposition verification gates.
...
- gh public evidence is deterministic across writer and checker paths even when command execution succeeds, fails, or the executable is unavailable.
- WI-4218 is resolved only after Loyal Opposition verifies the implementation report.
...
The bridge artifact is filed under bridge/, and the live queue state is the bridge/INDEX.md entry for gtkb-inventory-drift-gh-probe-parity; the helper inserts the NEW: bridge/gtkb-inventory-drift-gh-probe-parity-001.md line at the top of that document entry without deleting or rewriting prior versions.
