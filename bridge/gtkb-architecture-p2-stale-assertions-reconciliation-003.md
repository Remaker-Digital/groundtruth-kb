...
revision_reason: Self-revision before Loyal Opposition review. Version 002 fixed the mandatory clause evidence. Version 003 corrects the `show_thread_bridge.py` verification command by removing the obsolete `--repo-root` argument, and changes the proposed related bridge thread citation from `-001` to the latest revised proposal file. Scope is unchanged.
