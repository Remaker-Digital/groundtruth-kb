...
# Implementation Proposal — GTKB-ISOLATION-018 Sub-slice 18.B: PDF Cluster Move
...
**Cluster:** PDF tooling + Agent Red identity reports (10 files)
...
- No Python/JS imports reference any cluster file (verified live: `grep -rn "Generate-PDF-Report\|generate-pdf-report\|PDF-Generation\|prechat-form-phone-screenshot\|AgentRed-Technical-Evaluation\|OrbaTech-Technical-Evaluation\|PRODUCTION-READINESS-ASSESSMENT\|PRODUCTION-READINESS-SUMMARY" --include="*.py" --include="*.js" --include="*.json" --include="*.toml" --include="*.yml" --include="*.yaml"` returns empty after excluding `bridge/`/`memory/`/`independent-progress-assessments/`).
...
- The cluster is the smallest active cluster by file count (10 files).
...
Move the 10 PDF-cluster files from `E:/GT-KB/` root to `E:/GT-KB/applications/Agent_Red/pdf-tooling/`, update `.gitignore` and the isolation registry to reflect the new locations, and verify all 5 binding rules from `GOV-AGENT-RED-NESTED-IN-APPLICATIONS-001` are satisfied for the cluster.
...
Out of cluster:
- `package-pdf.json` (npm package config for the PDF generation) — moves separately in **18.H** per the umbrella's manifest cluster. Although topically related, splitting it from `package.json` now would be premature; the umbrella's clustering keeps both manifests together.
...
Remove the 7 root-anchored exact-name `.gitignore` patterns for the moved files. Add a single replacement pattern under `applications/Agent_Red/pdf-tooling/` that matches their new paths (or leave the new locations untracked-but-not-ignored — owner-confirmable; default below).
...
| **T-rule-1** | `GOV-AGENT-RED-NESTED-IN-APPLICATIONS-001` RULE 3 (Agent Red files at applications/Agent_Red/) | `ls applications/Agent_Red/pdf-tooling/` | Returns all 10 cluster files |
| **T-rule-2** | `GOV-AGENT-RED-NESTED-IN-APPLICATIONS-001` RULE 1 (no Agent Red files at GT-KB root for this cluster) | `for f in AgentRed-Technical-Evaluation-Report.docx OrbaTech-Technical-Evaluation-Report.docx prechat-form-phone-screenshot.png Generate-PDF-Report.ps1 generate-pdf-report.js generate-pdf-report.py generate-pdf.bat PDF-Generation-Instructions.md PRODUCTION-READINESS-ASSESSMENT.md PRODUCTION-READINESS-SUMMARY.txt; do test ! -f "$f" || echo "STILL AT ROOT: $f"; done` | Empty (all moved out of root) |
| **T-inv-1** | umbrella inventory match | `find applications/Agent_Red/pdf-tooling -type f \| wc -l` | 10 files |
...
| **T-import-1** | no import breakage (sanity) | `grep -rn "Generate-PDF-Report\|generate-pdf-report\|PDF-Generation\|prechat-form-phone-screenshot\|AgentRed-Technical-Evaluation\|OrbaTech-Technical-Evaluation\|PRODUCTION-READINESS-ASSESSMENT\|PRODUCTION-READINESS-SUMMARY" --include="*.py" --include="*.js" --include="*.json" --include="*.toml" --include="*.yml" --include="*.yaml" \| grep -v "^bridge/\|^memory/\|^independent-progress-assessments/\|^applications/Agent_Red/pdf-tooling/"` | Empty (no other files reference moved cluster) |
...
- [ ] All 11 tests T-bridge-1 through T-platform-1 PASS with command output captured in post-impl REPORT
- [ ] Codex VERIFIED on the post-impl REPORT
- [ ] No regression in GT-KB platform tests (T-platform-1)
- [ ] `applications/Agent_Red/pdf-tooling/` exists with all 10 files; root no longer contains any cluster file (T-rule-2 + T-rule-1)
...
| **OQ-C** | Should the 2 evaluation-report `.docx` files (Agent Red + OrbaTech) move to a separate `evaluation-reports/` subdirectory under `pdf-tooling/`, or stay flat? | Default: flat (10 files in one directory; subdivision deferred to a future cleanup if needed) |

## Out of Scope

- `package-pdf.json` (handled in 18.H manifest cluster).
- Any other Agent Red files outside the 10-file PDF cluster (handled in their respective sub-slices).
