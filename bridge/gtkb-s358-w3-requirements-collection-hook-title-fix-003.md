...
Version 003 (REVISED) supersedes the `-002` NO-GO. The `-002` review confirmed the W3 title-only correction is coherent, that both mandatory preflights pass, and that the owner-decision evidence and specification linkage are sound. It raised one finding:

- FINDING-F1 (P1): the `-001` target_paths listed only the formal-artifact-approval-packet glob, but IP-1 inserts a v4 GOV-spec record into MemBase (groundtruth.db). The implementation-start gate classifies `insert_spec` / `update_spec` as protected KB-mutation work and would block the v4 insert because `groundtruth.db` was outside the declared scope.

This revision fixes F1: `target_paths` now includes `groundtruth.db` (covering the GOV v4 insert) in addition to the approval-packet glob (covering the formal-artifact-approval packet). The title-only IP-1 scope, the Specification Links, the Prior Deliberations, the Owner Decisions / Input section, and the field-level verification plan are otherwise unchanged from `-001`.
...
A Deliberation Archive search was performed for the requirements-collection hook and the LLM-classifier-abandonment decision.
...
- DELIB-S332-NO-LLM-API-PARALLEL-USE-DIRECTIVE - the S332 owner directive that abandoned the LLM-classifier design on cost grounds and motivated the v1-to-v2 supersession of this spec. `.claude/hooks/spec-classifier.py` cites this deliberation in its header. W3 removes the last title-level remnant of the design this deliberation abandoned.

No prior deliberation rejected the W3 title correction; W3 completes the S332-motivated correction by aligning the spec's title with the already-corrected v3 body and the already-correct implementation.
...
A REVISED entry for gtkb-s358-w3-requirements-collection-hook-title-fix pointing at this -003 file is inserted at the top of that document's version list in the bridge index, above the NO-GO: -002 and NEW: -001 lines. No prior bridge file and no prior index entry is deleted or rewritten; -001 and -002 remain on disk and in the index as the append-only audit trail.
