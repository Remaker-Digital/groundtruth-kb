...
Version 005 (REVISED) supersedes the `-004` GO. This is a Prime-initiated self-correction, not a response to a Codex NO-GO. During pre-implementation verification of the already-implemented IP-1/IP-2/IP-3 working-tree changes against the `-003` proposal, Prime Builder self-detected that IP-5's test scope and `target_paths` were incomplete.

- FINDING-F3 (P1, self-detected): IP-2 reconciles `scripts/project_verified_completion_scanner.py` from the authorization envelope's `included_work_item_ids` to v2's project-to-work-item membership-link gating set. That change breaks the scanner's dedicated test file `platform_tests/scripts/test_project_verified_completion_scanner.py` - 2 of its 3 tests fail (`test_scanner_marks_all_verified_authorization_completion_ready` and `test_scanner_skips_authorization_with_one_non_verified_wi`), because their seed populates `included_work_item_ids` without an active project-to-work-item membership link. That test file was neither in the `-003` `target_paths` nor named in IP-5. The counterpart review gate forbids implementing a fix to a file outside the GO'd `target_paths`, so the gap is closed by this REVISED before implementation continues.

This revision closes F3. `target_paths` adds `platform_tests/scripts/test_project_verified_completion_scanner.py`, and IP-5 is extended to re-signature that file's seed helper and tests to the v2 membership-link gating model. A repo-wide sweep of every reference to the IP-1/IP-2/IP-3 changed surfaces confirmed F3 names the only broken-and-unauthorized file: `scripts/retroactive_harvest_bridge_threads.py` keeps an independent private copy of the old gating logic and is not functionally affected (only a cross-reference comment is now stale - cosmetic, out of W1 scope), and `platform_tests/scripts/test_bridge_index_archival.py` exercises only the `verified_work_items()` helper, which IP-2 leaves unchanged. The owner directed filing this REVISED via AskUserQuestion (2026-05-18, S358). IP-1 through IP-8 scope, the Specification Links, the Prior Deliberations, and the verification-plan strengths are otherwise unchanged from `-003`, whose own Revision Note closed the `-002` NO-GO F1/F2 findings. Versions `-001` through `-004` remain on disk and in the index as the append-only audit trail.
...
All target paths are in-root under the GT-KB project root: `groundtruth.db` at the root, source under groundtruth-kb/src/, scripts/, .claude/hooks/, .codex/gtkb-hooks/, .claude/settings.json, .codex/hooks.json, tests under groundtruth-kb/tests/, platform_tests/hooks/, and platform_tests/scripts/, and the two formal-artifact-approval-packet globs under .groundtruth/formal-artifact-approvals/. This bridge proposal file resides under the bridge directory. No target path is outside the GT-KB project root, and no application path under applications/ is touched.
...
- DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001 - this proposal carries a complete, relevance-closed Specification Links section and, per the -002 NO-GO and the -005 FINDING-F3 self-correction, declares every protected mutation surface in target_paths.
...
- GOV-ARTIFACT-APPROVAL-001 - the v3 of GOV-PROJECT-VERIFIED-COMPLETION-RETIREMENT-001 and the provenance deliberation are formal artifacts; each MemBase mutation is gated by a formal-artifact-approval packet presented to and approved by the owner before insertion. The two packet globs are declared in target_paths.
...
- 2026-05-18, S358: the owner directed filing this `-005` REVISED (rather than an in-place owner override of the `target_paths` boundary) to bring `platform_tests/scripts/test_project_verified_completion_scanner.py` into W1 scope after Prime Builder self-detected FINDING-F3. Collected via AskUserQuestion.
...
A REVISED entry for gtkb-s358-w1-retirement-machinery-correction pointing at this `-005` file is inserted at the top of that document's version list in the bridge index, above the `GO: -004`, `REVISED: -003`, `NO-GO: -002`, and `NEW: -001` lines. No prior bridge file and no prior index entry is deleted or rewritten; `-001` through `-004` remain on disk and in the index as the append-only audit trail.
...
In groundtruth-kb/tests/test_project_artifacts.py, delete the tests that assert the owner-confirmation gate behavior (the v1 model) and re-signature the completion/retirement tests to the stripped function signature and the membership-link gating set. In platform_tests/hooks/test_project_completion_surface.py, update the hook tests to the repurposed automatic-transition-plus-notification behavior. In platform_tests/scripts/test_project_verified_completion_scanner.py (added to scope per FINDING-F3), re-signature the scanner test seed helper and tests from the authorization-envelope `included_work_item_ids` model to v2's active project-to-work-item membership-link gating set, so the scanner's dedicated tests exercise the IP-2 reconciliation. New tests cover the automatic-transition path, the membership-link gating set, and the CLI subcommand; the CLI subcommand test is placed in groundtruth-kb/tests/test_project_artifacts.py (the authorized test target), not a separate CLI test module. The exact count of deleted vs. re-signatured tests is settled in implementation; the per-IP spec-derived test mapping is in the verification plan below.

### IP-6: Retire PROJECT-GTKB-LO-OPPORTUNITY-RADAR

Retire PROJECT-GTKB-LO-OPPORTUNITY-RADAR in MemBase (groundtruth.db) via the project-lifecycle retirement path. This is an owner-directed retirement (DELIB-S358 decision 4), outside the automatic VERIFIED-gated path, and explicitly supersedes the DELIB-S353 Option-B keep-open choice. The retirement carries a change_reason citing DELIB-S358 and this bridge thread.

### IP-7: Issue v3 of GOV-PROJECT-VERIFIED-COMPLETION-RETIREMENT-001

Insert a version 3 of GOV-PROJECT-VERIFIED-COMPLETION-RETIREMENT-001 into MemBase (groundtruth.db) that re-frames v1 accurately as a Prime Builder error in its supersession narrative. The behavioral rule is unchanged from v2 (automatic completion and retirement, no owner-AUQ); v3 corrects only the historical-record framing of v1. v1 and v2 remain on the append-only record. The v3 insertion requires a formal-artifact-approval packet at .groundtruth/formal-artifact-approvals/<date>-gov-project-verified-completion-retirement-001.json (declared in target_paths).

### IP-8: Archive a provenance deliberation for the S350 manufactured-variant error

Insert a Deliberation Archive record into MemBase (groundtruth.db) capturing the provenance of the S350 manufactured-variant error - the chain by which a manufactured AskUserQuestion variant produced the incorrect v1 of GOV-PROJECT-VERIFIED-COMPLETION-RETIREMENT-001. The deliberation's intended id is DELIB-S358-S350-MANUFACTURED-VARIANT-PROVENANCE; its insertion requires a formal-artifact-approval packet at .groundtruth/formal-artifact-approvals/<date>-delib-s358-s350-manufactured-variant-provenance.json (declared in target_paths).

### Out of scope

W1 does not change the v2 behavioral rule. It does not touch the owner-directed `gt projects retire` path beyond leaving it intact. It does not perform backlog-data remediation of project-to-work-item link completeness (v2 Consequence 4 names that as separate work). It does not touch the four W4 mechanical-gate surfaces, the W2 Agent-Red GOV specs, or the W3 title fix. It does not modify scripts/retroactive_harvest_bridge_threads.py (which keeps an independent private copy of the old gating logic; its now-stale cross-reference comment is cosmetic comment-rot outside W1's machinery-correction scope). IP-6, IP-7, and IP-8 are MemBase mutations to groundtruth.db through the project-lifecycle and formal-artifact-approval workflows; groundtruth.db and the two approval-packet globs are declared in target_paths.
...
| GOV-PROJECT-VERIFIED-COMPLETION-RETIREMENT-001 | The scanner's dedicated test file exercises the v2 membership-link gating set (FINDING-F3) | re-signatured tests in platform_tests/scripts/test_project_verified_completion_scanner.py |
...
| GOV-PROJECT-VERIFIED-COMPLETION-RETIREMENT-001 | The gt projects complete-authorization CLI subcommand invokes the stripped completion path | CLI subcommand test in groundtruth-kb/tests/test_project_artifacts.py |
...
Execution: the post-implementation report records the exact pytest commands run over groundtruth-kb/tests/test_project_artifacts.py, platform_tests/hooks/test_project_completion_surface.py, and platform_tests/scripts/test_project_verified_completion_scanner.py, a ruff check over the changed Python files, the MemBase evidence for IP-6/IP-7/IP-8, and both bridge preflights.
...
- The gt projects complete-authorization CLI subcommand exists, invokes the stripped completion path, and is covered by a test in test_project_artifacts.py.
- The covering tests pass; the v1 owner-confirmation-gate tests are deleted; ruff is clean over the changed files.
- platform_tests/scripts/test_project_verified_completion_scanner.py is re-signatured to the v2 membership-link gating model and passes (FINDING-F3).
...
- Risk: the membership-link reconciliation changes which work items gate a given authorization. Mitigation: v2's "Explicitly linked" definition is the membership link; the scanner and lifecycle service are changed together so they agree; IP-5 tests (including the scanner's dedicated test file per FINDING-F3) cover the membership-link gating set.
