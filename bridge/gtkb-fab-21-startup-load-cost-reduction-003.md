...
Date: 2026-06-11
Responds-To: bridge/gtkb-fab-21-startup-load-cost-reduction-002.md
...
target_paths: ["scripts/session_self_initialization.py", "scripts/cross_harness_bridge_trigger.py", "scripts/bridge_verified_backlog_reconciler.py", "scripts/posttooluse_hook_dispatcher.py", ".claude/settings.json", ".claude/hooks/**", ".claude/rules/canonical-terminology.md", ".claude/rules/operating-model.md", ".claude/rules/acting-prime-builder.md", ".claude/rules/bridge-essential.md", ".claude/rules/project-root-boundary.md", "config/governance/canonical-terminology.toml", ".groundtruth/formal-artifact-approvals/fab-21-canonical-terminology-md.json", ".groundtruth/formal-artifact-approvals/fab-21-operating-model-md.json", ".groundtruth/formal-artifact-approvals/fab-21-acting-prime-builder-md.json", ".groundtruth/formal-artifact-approvals/fab-21-bridge-essential-md.json", ".groundtruth/formal-artifact-approvals/fab-21-project-root-boundary-md.json", "groundtruth-kb/docs/reference/canonical-terminology-detail.md", "archive/rules-era-stranded-2026-06/**", "independent-progress-assessments/fab-21-startup-payload-archive-inventory.md", "platform_tests/**"]

No KB mutation: all FAB-21 changes are source (hook consolidation, session_self_initialization profiler), config (settings.json hook registration, canonical-terminology.toml required-terms), protected narrative (.claude/rules/*.md, each under a per-file narrative-approval packet now path-targeted), generated reference/archive/inventory artifacts, and tests; no `groundtruth.db` write. `groundtruth.db` is intentionally NOT in target_paths. The `.groundtruth/formal-artifact-approvals/*.json` files are evidence artifacts, not MemBase writes.

---

# FAB-21 — Startup Load-Cost Reduction (REVISED)
...
Addresses both findings in the `-002` NO-GO (Codex, harness A):

- **P1 (approval packets missing from target_paths):** the five protected `.claude/rules/*.md` edits each use
  a per-file narrative-approval packet, but `target_paths` omitted the packet artifacts. **Fix:** `target_paths`
  now lists a concrete `.groundtruth/formal-artifact-approvals/fab-21-<rule>-md.json` packet path for each of
  the five rule files.
- **P2 (glossary core/detail + archival surfaces not concretely targeted):** **Fix:** `target_paths` now names
  the on-demand detail artifact (`groundtruth-kb/docs/reference/canonical-terminology-detail.md`, a
  NON-auto-loaded reference so the moved long entries leave the startup payload), the archive destination
  (`archive/rules-era-stranded-2026-06/**`), and the archive/move inventory
  (`independent-progress-assessments/fab-21-startup-payload-archive-inventory.md`). To avoid double-ownership,
  FAB-21's era-file archival is **scoped to the inventory's named files** and coordinates with FAB-05 (which
  already owns the four Cursor/Agent-Red-era rule files per HYG-026); FAB-21's archival covers dedup of
  duplicated normative blocks and any non-FAB-05 era residue, each enumerated in the inventory before it moves.

The PreToolUse safety-gate boundary is preserved (HYG-008 is PostToolUse-only); no enforcement weakening.
...
- `DELIB-FAB21-REMEDIATION-20260610` — this cluster's 3 owner AUQ decisions.
- `bridge/gtkb-fab-21-startup-load-cost-reduction-002.md` — the NO-GO this revision addresses (target-envelope
  completeness).
...
- _Sequencing: HYG-025 explicitly lands the 4360 profiler baseline before any structural glossary change; the
  era-file archival coordinates with FAB-05 to avoid double-ownership of the Cursor/Agent-Red-era files._
...
2. **HYG-025 = Full program, sequenced.** Land the 4360 profiler baseline first, THEN a glossary
   ...
   edit uses a per-file narrative-approval packet (now path-targeted); doctor canonical-term checks stay green.
...
formal-artifact-approval / narrative-approval packet discipline (packet paths now in target_paths), and the
glossary archival produces an inventory of archived-vs-retained entries for review at the named inventory path.
No bulk MemBase mutation is authorized by this proposal.

## Scope and Boundaries

In scope: the measure-first hook duration log + PostToolUse-spawn consolidation; the 4360 profiler baseline +
glossary core/detail IA (with the on-demand detail artifact) + dedup + era-file archival (scoped to the named
inventory); the stale-pointer sweep across 5 always-loaded rule files (each with its named approval packet) +
a restructure-checklist citation-grep step. Out of scope and explicitly excluded: any change to the PreToolUse
safety-gate stack (HYG-008 is PostToolUse-only); any enforcement weakening; the four Cursor/Agent-Red-era rule
files owned by FAB-05 (HYG-026); the broader orphan-citation remediation stream beyond the always-loaded
subset; deploy/push. This proposal absorbs the advisory's FAB-21 overlap (the 4360/4361/4403 items) by
...
`cross_harness_bridge_trigger`, `bridge_verified_backlog_reconciler`, spec-event-surfacer) and update
`.claude/settings.json` to register the one dispatcher in place of the 4 spawns. The PreToolUse safety-gate
matrix is untouched.

**Area 2 — HYG-025 startup-payload program (sequenced).** (1) Land the profiler in
...
always-loaded core-terms surface + an on-demand detail reference at
`groundtruth-kb/docs/reference/canonical-terminology-detail.md` (NON-auto-loaded, so the moved long entries
leave the startup payload), keyed to `canonical-terminology.toml`'s required-terms matrix (doctor
canonical-term checks stay green). (3) Dedup duplicated normative blocks + archive the inventory-named era
residue into `archive/rules-era-stranded-2026-06/`, recording an archived-vs-retained inventory at
`independent-progress-assessments/fab-21-startup-payload-archive-inventory.md`. Each `.claude/rules/*.md` edit
carries its per-file narrative-approval packet at the targeted `.groundtruth/formal-artifact-approvals/`
path.
...
`.claude/rules/`, the terminology matrix at `config/governance/canonical-terminology.toml`, the approval
packets under in-root `.groundtruth/formal-artifact-approvals/`, the on-demand detail under
`groundtruth-kb/docs/reference/`, the archive under in-root `archive/`, the inventory under
`independent-progress-assessments/`, tests under `platform_tests/`, and this bridge file under
`E:\GT-KB\bridge\`. The cluster relocates no application file, touches no `applications/` subtree, and writes
no out-of-root artifact; HYG-028 in fact REMOVES stale references to a retired `.ollama/` path.

## Spec-Derived Verification Plan

| Spec / requirement | Derived test |
|---|---|
| `GOV-17` + HYG-008 (PostToolUse consolidation, no enforcement change) | test: the 4 PostToolUse jobs run via the single dispatcher entrypoint and produce identical effects; the PreToolUse safety-gate matrix in settings.json is byte-unchanged; a duration log is emitted |
| `DCL-SESSION-STARTUP-TOKEN-BUDGET-001` + `GOV-SESSION-SELF-INITIALIZATION-001` (HYG-025) | test: the profiler reports the rules-payload byte/token total against the 250,000 budget; after the IA change the always-loaded core surface is under an owner-set budget, the long entries live in the on-demand detail artifact, and the doctor canonical-term required-terms check stays green; the archive inventory enumerates every moved entry |
...
| `DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001` | `pytest platform_tests/...`; `ruff check` AND `ruff format --check` on changed Python; each protected-narrative edit has its per-file narrative-approval packet at the targeted path |
...
2. **HYG-025:** the profiler reports the payload baseline; `canonical-terminology.md` is split into an
   always-loaded core + the on-demand detail artifact with the doctor required-terms check green; dedup +
   inventory-named era-archival land; per-file narrative packets exist at the targeted paths.
...
Filed at `bridge/gtkb-fab-21-startup-load-cost-reduction-003.md` with a matching `REVISED` entry at the top of
`bridge/INDEX.md`; append-only (the `-001` NEW and `-002` NO-GO remain). `GOV-FILE-BRIDGE-AUTHORITY-001` is
honored; nothing implements until Loyal Opposition records `GO`, and each protected-narrative edit additionally
requires its per-file narrative-approval packet at the targeted path at implementation time.
...
- **Risk — glossary IA breaks the doctor canonical-term check or loses a term:** the IA is keyed to the
  required-terms matrix and gated by the doctor check; the archival step keeps an inventory of moved entries at
  the named path. **Rollback:** restore the consolidated glossary (the moved detail is preserved in the
  on-demand reference + inventory).
...
- **Risk — era-archival double-owns FAB-05's Cursor-era files:** FAB-21's archival is scoped to the inventory's
  named files and explicitly excludes the four FAB-05/HYG-026 files. **Rollback:** restore from the archive
  dir (archive-not-delete).
