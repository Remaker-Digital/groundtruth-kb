...
Version: 009
Date: 2026-08-01 UTC
Responds to: bridge/gtkb-wi5699-staged-artifact-admission-gate-008.md
...
REVISED in response to NO-GO-008, which reviewed version 007. That verdict
confirmed the consumer-side strategy, the reuse of
`scripts/bridge_thread_files.py`, and the target-shape checks, and raised one
blocking finding: the proposed authority set must not include the
review-pending `NEW` and `REVISED` states, because a declaration would then
authorize itself. That finding is adopted without qualification; § Self-authorization
records the reasoning and the accepted cost. This is a proposal-only
correction — no source change has started.

Lineage: NO-GO-006 raised F1 (terminal threads confer perpetual admission) and
F2 (malformed tokens become admission evidence), both independently reproduced
below and both still addressed here. NO-GO-008 made no approval-state finding,
citing the owner backlog approval `DELIB-20260801-WI5699-BACKLOG-APPROVAL`;
the F3 question is therefore closed and § F3 is retained only as the record of
how it resolved.
...
finding is made here." The disagreement is settled and no ruling is requested.
The remainder of this section is retained so the resolution is auditable rather
than merely absent from the next verdict.
...
Both points offered for adjudication in version 007 are now settled and neither
is re-opened here:
