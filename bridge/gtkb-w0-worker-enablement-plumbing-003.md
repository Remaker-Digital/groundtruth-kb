...
Version: 003
Date: 2026-08-06 UTC
Responds to: bridge/gtkb-w0-worker-enablement-plumbing-002.md
...
Two adjacent hygiene defects observed during the same investigation — the
untracked/unignored `.tmp-lo-verdict-drafts/` scratch directory and the
untracked `.driveignore` missing its `.pytest-tmp/` line — are OUT OF SCOPE
for this thread. PAUTH operation-time classification denies their mutation
classes (repository_metadata / unclassified) under the cited authorization;
they are routed to the W0.2 custodial lane under owner decision evidence
(per the -002 LO verdict F1 remedy, option 1).
...
4. `scripts/gtkb_bridge_writer.py:1232` — the single production mint call
   ...
   publication.
5. (context only; remediation routed to W0.2) `.gitignore` — no
   `.tmp-lo-verdict-drafts/` entry (grep verified); the directory exists
   untracked in the worktree today.
6. (context only; remediation routed to W0.2) `.driveignore` — untracked, so
   its Drive-sync protections are not durable in git. Drift note against the
   earlier register: the live untracked file ALREADY contains `.gtkb-state/`
   (line 57); only `.pytest-tmp/` is missing.
...
4. Tests: create `platform_tests/scripts/test_bridge_publication_capability_ttl_sizing.py`
...
1. Only the seven declared target_paths change; `timer_config.py`,
...
3. Goose envelope-open binds a pre-set GOOSE_SESSION_ID; goose resolves to
   ...
   unchanged.
4. No ignore-file is modified by this thread: `.gitignore` and
   `.driveignore` are absent from target_paths and from the implementation
   diff (their hygiene work is W0.2 custodial-lane scope).
...
```json
{
  ...
  "before_behavior": "goose envelope-open generates a fresh session id while the claim CLI resolves GOOSE_SESSION_ID from env -> id mismatch -> go_implementation claims denied 'not prime-eligible' (reproduced); mint defaults ttl_seconds=120 ignoring the configured 800 (config raise decorative at runtime)",
  "after_behavior": "goose envelope-open binds GOOSE_SESSION_ID when set (parity with codex/cursor mapping); minted capabilities carry the resolver TTL under the existing 800 ceiling; no ignore-file change occurs in this thread",
...
## Revision Notes (v003 vs v001, responding to -002 F1)

- **F1 (P1, scope contradiction) — resolved via the verdict's option (1).** All
  ignore-file obligations are removed coherently: Proposed Change items (d)/(e)
  deleted; Acceptance Criterion 4 replaced with an explicit no-ignore-file-change
  assertion; the two verification probes removed; the disposition JSON
  `before_behavior`/`after_behavior` strings and the `driveignore` baseline row
  purged. The deferral disclosure in `primary_route` stands, now uncontradicted:
  ignore hygiene is W0.2 custodial-lane scope under owner decision evidence.
- **Residual note (writer-file overlap) — sequencing declared.** This thread's
  only `scripts/gtkb_bridge_writer.py` change is the resolver-derived TTL at the
  single mint call (line 1232 region). The sibling GO'd thread
  `gtkb-w0-executable-go-pre-verdict-validation` touches the status/NO-ACTION
  validation region of the same file; the hunks are disjoint. Declared order:
  THIS thread implements first; the sibling's implementer re-verifies line
  anchors at implementation start (its filed coordination note already says so).
- No other section changed; anchors, spec links, PAUTH linkage, tests, and the
  three plumbing fixes are as approved-for-review in v001.
