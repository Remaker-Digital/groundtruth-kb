...
Version: 005
Date: 2026-07-29 UTC
Responds to: bridge/gtkb-wi5714-registry-write-linearizability-004.md
...
generation-CAS mechanism while retaining every correction accepted in v004.
It adds the omitted `GOV-SOURCE-OF-TRUTH-FRESHNESS-001` linkage and requires
fresh canonical reads at implementation-report and terminal-verification time
for every numeric, governance-state, bridge-state, authorization, claim,
packet, and registry-parity conclusion. It does not treat the dated proposal
measurements as current closure evidence and does not widen owner authorization.
...
- `GOV-SOURCE-OF-TRUTH-FRESHNESS-001` - implementation reports and terminal verification must freshly reread every mutable or numeric authority surface instead of reusing this proposal's dated measurements.
...
### V004 freshness-linkage finding

`GOV-SOURCE-OF-TRUTH-FRESHNESS-001` is now a concrete specification link and
has a discriminating verification row below. The implementation report and the
independent terminal verdict must rerun the decisive reads from live canonical
sources and record commands, timestamps or generation anchors, collected/pass
counts, and results. Neither may reuse the dated reproduction, `29 passed`
baseline, PAUTH summary, target-cleanliness observation, bridge state, claim,
implementation-start packet, or registry parity recorded in this proposal as
...
| `GOV-SOURCE-OF-TRUTH-FRESHNESS-001` | At implementation-report time and again at independent terminal-verification time, freshly run the promoted deterministic lost-update regression, the complete focused pytest module, exact WI-5714/project/PAUTH queries, exact two-target cleanliness checks, canonical TAFE plus status-bearing numbered-file bridge reads, live claim and implementation-start packet checks, and canonical/package/projection/journal parity checks after success and retry exhaustion. Record the commands, timestamps or generation anchors, actual collected/pass counts, and results; copied proposal measurements do not qualify. |
...
9. The implementation report and independent terminal verdict each perform
   fresh canonical reads for the reproducer/regression test, focused pytest
   count, WI/project/PAUTH state, exact target cleanliness, TAFE and numbered
   bridge state, claim/start-packet liveness, and registry generation parity;
   each records commands, timestamps or generation anchors, and results.

## Pre-Filing Preflight Subsection

The version-005 candidate is evaluated with both commands immediately before
filing:

```powershell
groundtruth-kb\.venv\Scripts\python.exe scripts\bridge_applicability_preflight.py --bridge-id gtkb-wi5714-registry-write-linearizability --content-file .gtkb-state\bridge-revisions\drafts\gtkb-wi5714-registry-write-linearizability-005.md --json
groundtruth-kb\.venv\Scripts\python.exe scripts\adr_dcl_clause_preflight.py --bridge-id gtkb-wi5714-registry-write-linearizability --content-file .gtkb-state\bridge-revisions\drafts\gtkb-wi5714-registry-write-linearizability-005.md
```

Fresh candidate results on 2026-07-29 UTC: applicability preflight exit `0`,
`preflight_passed=true`, no missing required or advisory specifications, and no blocking errors; clause
preflight exit `0`, five clauses evaluated, four `must_apply`, one `may_apply`,
zero evidence gaps in must-apply clauses, and zero blocking gaps. The governed
revision helper repeats the compliance audit on its final metadata-enriched
candidate and refuses publication on any nonzero result.
