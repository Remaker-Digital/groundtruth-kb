...
# Implementation Proposal - Make LO VERIFIED verdict recording atomic with final commit

## Summary

This proposal fixes WI-4680, a P0 bridge lifecycle defect reported by the owner on 2026-06-19.

Today, Loyal Opposition can record a terminal `VERIFIED` verdict while the implementation report, verdict artifact, and verified source changes are still only dirty or staged worktree state. That makes the git commit a post-verification best-effort action. When the commit is blocked or skipped, the bridge thread is terminal but the repository does not contain the verified change as durable git history. The owner directive is to invert that relationship: the commit must be the final step of verification, not a separate follow-up.
...
4. A stale implementation-start packet, dirty overlap, shell-composition issue, or hook failure can block the later commit.
...
The owner directive adds a lifecycle invariant but does not require a new formal GOV/DCL before implementation. The invariant is implementable under the existing bridge authority, mandatory verification, project authorization, generated-adapter parity, and commit-time gate requirements cited above. If implementation discovers that `VERIFIED` must carry a new formal schema field beyond bridge-file prose and commit evidence, Prime Builder will file that as a separate formal artifact change rather than smuggling it into this repair.
...
This candidate was checked before filing:

```text
python scripts\bridge_applicability_preflight.py --bridge-id gtkb-lo-verified-commit-atomicity --content-file .gtkb-tmp\gtkb-lo-verified-commit-atomicity-001-body.md --json
python scripts\adr_dcl_clause_preflight.py --bridge-id gtkb-lo-verified-commit-atomicity --content-file .gtkb-tmp\gtkb-lo-verified-commit-atomicity-001-body.md
```

Expected filing evidence: applicability preflight passes with no missing required specs; clause preflight passes with zero blocking gaps.
