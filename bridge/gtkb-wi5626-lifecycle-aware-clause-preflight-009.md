...
Version: 009 (REVISED; restores mandatory proposal sections after GO v008)
Responds to: bridge/gtkb-wi5626-lifecycle-aware-clause-preflight-008.md
...
# REVISED Implementation Proposal - WI-5626 lifecycle-aware clause preflight (restored sections)

## Revision Claim

Version 007 received GO at version 008 ("REVISED closes F4 by consuming landed
WI-5629 resolve_bridge_lifecycle with bounded two-file scope; targets clean.
Approved to implement under claim/start."). During the implementation-start
gate, the canonical authorization checker
(`scripts/implementation_authorization.py begin`) reported that the approved
proposal is missing two mandatory sections: `## Requirement Sufficiency` and a
spec-derived verification plan. Both sections existed in version 005 but were
omitted from the version 007 rewrite.

This revision restores those two mandatory sections **without changing any
substantive design or scope** that version 007 (and GO v008) approved. The
design, the operation-neutral WI-5629 resolver contract consumption, the
two-file scope, and all out-of-scope boundaries are unchanged from version 007.
This revision re-presents the identical proposal plus the mandatory sections so
the implementation-start gate can pass.

## Restored: Requirement Sufficiency
...
## Restored: Specification-Derived Verification
...
## Unchanged From Version 007 (GO-approved design)

The substantive design approved by GO v008 is retained unchanged:
...
- `bridge/gtkb-wi5626-lifecycle-aware-clause-preflight-007.md` - the prior
  REVISED proposal approved by GO v008.
- `bridge/gtkb-wi5626-lifecycle-aware-clause-preflight-008.md` - Loyal
  Opposition GO.
...
proposal. This revision changes no substantive design from the GO-approved
version 007; it only restores the two mandatory proposal sections
(`## Requirement Sufficiency` and `## Specification-Derived Verification`) that
the canonical implementation-start gate requires. If GO, Prime Builder will
pass schema-v3 implementation-start for the two declared targets, implement the
consumer alignment, and file an implementation report requesting VERIFIED.
