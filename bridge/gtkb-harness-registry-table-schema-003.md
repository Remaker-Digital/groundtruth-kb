...
- **003 (REVISED)** — Authorization-metadata correction only; no substantive
  change to scope, plan, tests, or specification linkage. The GO'd `-001`
  cited `Project Authorization: PAUTH-PROJECT-ANTIGRAVITY-INTEGRATION-…`
  (the parent project's authorization) while its `Project:` line named the
  `PROJECT-HARNESS-REGISTRY-REFACTOR` sub-project — an internal inconsistency
  that `scripts/implementation_authorization.py` correctly rejects (the
  authorization's project must match the proposal's cited project). This
  revision cites the sub-project's own authorization
  (`PAUTH-PROJECT-HARNESS-REGISTRY-REFACTOR-…`, created under the same owner
  decision `DELIB-2079`) so the metadata is consistent. The `-001` proposal
  body was not edited in place (it carries the `-002` GO); this is a new
  version per the append-only bridge protocol.
- **001 (NEW)** — Initial proposal; Codex GO at `-002`.
...
- `GOV-FILE-BRIDGE-AUTHORITY-001` — the file bridge is the canonical workflow
  state; this proposal is filed as a versioned bridge entry.
...
- `PAUTH-PROJECT-HARNESS-REGISTRY-REFACTOR-HARNESS-REGISTRY-REFACTOR-IMPLEMENTATION-AUTHORIZATION`
  — active project authorization for the `PROJECT-HARNESS-REGISTRY-REFACTOR`
  sub-project (owner-decision `DELIB-2079`, includes `REQ-HARNESS-REGISTRY-001`).
  The parent project `PROJECT-ANTIGRAVITY-INTEGRATION` is separately authorized
...
- Owner AskUserQuestion "persist and kick off" (2026-05-16) authorized creating
  ...
  approved the Phase 3c work-item breakdown ("Go") and directed implementing
  WI-3337.
...
This proposal is filed as `bridge/gtkb-harness-registry-table-schema-003.md`
(a REVISED version) with a corresponding `bridge/INDEX.md` entry per the
file-bridge protocol. No prior bridge version is deleted or rewritten — the
`-001` proposal and its `-002` GO verdict are preserved; this is a new
appended version. `bridge/INDEX.md` remains the canonical workflow state per
