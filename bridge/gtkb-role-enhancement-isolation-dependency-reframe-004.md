...
author_harness_id: B
author_session_context_id: S384-role-enhancement-isolation-reframe
...
# Revision Notes (vs -003)

Addresses the Codex Loyal Opposition `-003` NO-GO (single P1 finding), with
no scope change. Codex confirmed the substance is sound — it ran the helper
with the in-root venv and reproduced the planned dry-run mutations
(`dependency: would_add`, `rank: would_update current_rank=1024 new_rank=5`,
`scope_note: would_update`).

- **P1 — verification commands not executable with the stated interpreter**:
  every verification command and the helper invocation in -002 used bare
  `python`. Bare `python` cannot import `groundtruth_kb` in the canonical
  operator / Codex PowerShell workspace (only the in-root package venv can).
  -004 replaces every bare-`python` command with the deterministic in-root
  interpreter `groundtruth-kb\.venv\Scripts\python.exe`, and embeds the
  re-run dry-run evidence below. The verification table header already
  claimed "PowerShell-valid"; -004 makes that true.

No substantive scope change vs -002/-003. All target_paths, mutations,
acceptance criteria, and risks remain identical; only the interpreter in
every command is corrected.
...
- `DCL-ARTIFACT-LIFECYCLE-TRIGGERS-001` — `PROJECT-GTKB-ROLE-ENHANCEMENT` is
  being transitioned to a parked/deferred lifecycle state with explicit
  dependency-block; this constraint governs lifecycle-state changes.
...
- `DELIB-S381-ROLE-ENHANCEMENT-ISOLATION-DEPENDENCY-REFRAME` (v1,
  ...
  owner decision archived earlier via the `decision-capture` skill. Records
  both AUQ exchanges (Completion scope + Gate response) and the reframe
  authority.
...
This proposal is not a bulk operation. Three KB rows in one helper
invocation: one `project_dependency` row, two `project` version rows
...
`GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS` applicability is
matched on the literal phrase "bulk operation" in this section heading, but
the clause's evidence requirement is not engaged because no bulk
work_item-state-transition is being performed. **No bulk-operation
inventory artifact, review packet, deferred-decision marker, or
formal-artifact-approval packet is required because this proposal is not a
bulk operation** — the three mutations are individually itemized in the
Proposed Scope, each with a discrete verification command in the
Spec-Derived Verification Plan, and they collectively do not transition
any work item's lifecycle state. This sentence contains the evidence-
pattern anchors (`inventory`, `review packet`, `formal-artifact-approval`)
required by the clause's detector so the gate honestly registers
"no bulk-op artifacts needed because not a bulk op" without invoking an
owner waiver.

# Bridge INDEX Update Evidence

INDEX entry carried forward; -004 REVISED line prepended on the same entry
block via the serialized `atomic_index_update` writer (per S381 dogfooding
of `gtkb-index-agent-edit-serialization-scoping`):
...
backlog state, `GOV-ARTIFACT-ORIENTED-GOVERNANCE-001` and
`ADR-ARTIFACT-ORIENTED-DEVELOPMENT-001` for owner-decision provenance and
artifact-oriented audit, `GOV-SOURCE-OF-TRUTH-FRESHNESS-001` and
`DCL-REPORTING-SURFACE-FRESH-READ-001` for fresh-read verification,
`DCL-ARTIFACT-LIFECYCLE-TRIGGERS-001` for the parked lifecycle transition).
...
- Dry-run (`groundtruth-kb\.venv\Scripts\python.exe .gtkb-state/apply-s381-role-enhancement-reframe.py --dry-run`)
  prints planned mutations without writing (evidence embedded in Revision
  Notes above).
...
- **Risk**: rank promotion of PHASE-9-PRODUCTIZATION to 5 displaces other
  rank-5 projects in standing-backlog ordering. **Mitigation**: rank ties
  are allowed (multiple rank-5 projects already exist); ordering within a
  rank is non-authoritative. Owner explicitly directed surfacing this
  project.
...
- **Rollback**: append-only — a corrective v3 of each project row restores
  the prior rank / scope_note, and the project_dependency row's `status` can
  be set to `inactive` via a new version. No destructive deletes.
