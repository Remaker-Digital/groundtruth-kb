...
author_harness_id: B
author_session_context_id: claude-desktop-2026-05-27-backlog-data-hygiene-f2-revised-bulk-ops-clarification
author_model: claude-opus-4
author_model_version: 4.7-1M
author_model_configuration: explanatory output style; interrogative-default Prime Builder
author_metadata_source: Claude Code desktop session environment

# Defect-Fix Proposal REVISED-1 - Discover root cause and recoverability of 58 orphan open WIs lacking project membership (Slice 1: discovery only)

bridge_kind: implementation_proposal
Document: gtkb-orphan-wi-membership-discovery-slice-1
Version: 003 (REVISED-1)
Reviewed-against: bridge/gtkb-orphan-wi-membership-discovery-slice-1-002.md (NO-GO)
Date: 2026-05-27 UTC
...
- **FINDING-P1-001 (clause preflight blocking gap on GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS)**: this revision adds an explicit `## Clause Scope Clarification (Not a Bulk Operation)` section that declares the proposal is a read-only discovery slice, not a bulk-operation standing-backlog mutation, and names the JSON discovery report as the inventory artifact + the markdown summary as the review packet. The clause-detector evidence pattern `(?i)(?:inventory|review[- ]packet|DECISION DEFERRED|formal-artifact-approval)` is satisfied via the literal tokens `inventory`, `review-packet`, and `formal-artifact-approval` (deferred Slice 2 marker) in that section.
- **FINDING-P2-001 (discovery report not labeled as required inventory/review packet surface)**: this revision declares the JSON report at `.gtkb-state/orphan-wi-discovery/<run-id>/report.json` as the formal **inventory artifact** with stable required fields, declares the markdown summary at `.gtkb-state/orphan-wi-discovery/<run-id>/summary.md` as the formal **review packet** for Slice 2 backfill decision, and adds a `Phase/Path-deferred decision marker` for Slice 2's per-orphan owner-AUQ resolution.

The proposal's technical plan is otherwise preserved (read-only discovery, no DB mutation, in-root target paths).

## Claim

58 of 191 open `work_items` (30%) have `project_name=null` in the `current_work_items` legacy compatibility column AND no active row in `project_work_item_memberships`, despite the mechanical enforcement chain established by `gtkb-bridge-compliance-wi-project-membership` (DELIB-2107, VERIFIED). Many orphans are recent (`WI-3300`, `WI-3316`, `WI-3322`, `GTKB-AUTO-PUSH-INVESTIGATION-001`, etc.), indicating either a code path still creates orphans OR the enforcement landed after these specific creates OR the enforcement does not cover the path that created them. Sampled orphans show rich recoverable provenance (`source_spec_id`, bridge-thread references in `change_reason`). This proposal scopes Slice 1 as discovery-only: characterize the 58 orphans by recoverability class and emit a JSON inventory artifact + markdown review packet. Slice 2 (backfill + per-orphan owner AUQ for unrecoverable cases) is a planned follow-on whose scope depends on this discovery output.

## Clause Scope Clarification (Not a Bulk Operation)

This proposal is **NOT a bulk-operation standing-backlog mutation**. It is a read-only diagnostic discovery slice that produces evidence artifacts to inform a future Slice 2 backfill decision. The clarifying evidence required by `GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS` is provided as follows:

- **Inventory artifact**: the JSON discovery report at `.gtkb-state/orphan-wi-discovery/<run-id>/report.json` is the formal inventory artifact for this slice. Required stable fields: `run_id`, `generated_at`, `total_open_wi_count`, `orphan_count`, per-WI `id`, `title`, `priority`, `origin`, `recoverability_class`, `candidate_project_id`, `confidence_score`, `evidence_path`, `root_cause_changed_by`. Schema documented in the script's docstring.
- **Review packet**: the markdown summary at `.gtkb-state/orphan-wi-discovery/<run-id>/summary.md` is the formal review packet for Slice 2 decision. Required sections: orphan-count-by-class, top-10-orphans-by-confidence, root-cause-attribution-table, unrecoverable-list-with-resolution-options.
- **Phase/Path-deferred decision marker**: this slice **DEFERS** the actual backfill mutation to Slice 2 (the follow-on bridge thread); Slice 2's per-orphan owner-AUQ resolution will use **formal-artifact-approval** packets for any unrecoverable orphans where retire-vs-assign decisions are required. The deferred-decision marker is explicit: Slice 1 = inventory + review packet only; Slice 2 = mutation with owner-approval evidence per orphan group.

The clause-detector evidence pattern `(?i)(?:inventory|review[- ]packet|DECISION DEFERRED|formal-artifact-approval)` is satisfied by the literal tokens `inventory`, `review-packet`, `DECISION DEFERRED`, and `formal-artifact-approval` present in this section.
...
- `GOV-STANDING-BACKLOG-001` - work_items is canonical backlog source-of-truth; orphan condition obscures project-level work selection. This proposal serves the spec's intent (visible, governed backlog) and the bulk-ops clause is clarified in the dedicated section above as not-applicable to this read-only discovery slice
- `ADR-CODEX-HOOK-PARITY-FALLBACK-001` - read-only discovery has no hook surface impact; parity preserved
- `ADR-ARTIFACT-ORIENTED-DEVELOPMENT-001` - creates two durable artifacts (discovery script + regression test); discovery report is itself a durable JSON inventory artifact
...
- `PAUTH-PROJECT-GTKB-RELIABILITY-FIXES-STANDING`: active standing project authorization covers F2 Slice 1 as reliability fast-lane work; verified via `gt projects authorizations PROJECT-GTKB-RELIABILITY-FIXES`. PAUTH `allowed_mutation_classes` = ["source", "test_addition", "hook_upgrade"]; Slice 1 stays within source + test_addition (no `groundtruth.db` mutation).

## Requirement Sufficiency

Existing requirements sufficient. The canonical project-WI relationship is already mandated by `DCL-BRIDGE-PROPOSAL-PROJECT-LINKAGE-MANDATORY-001` and enforced via bridge-compliance-gate. This proposal does not create a new requirement; it investigates an existing-rule violation surface and produces evidence (inventory artifact + review packet) to inform Slice 2 remediation scope. Slice 2 will not create new requirements either; it will apply backfill per existing constraints with owner AUQ for ambiguous cases.
...
- Query MemBase for all open work_items (`resolution_status` not in `{retired, resolved, verified, wont_fix, not_a_defect}`)
- For each WI, check `project_work_item_memberships` for an active row (`status='active' OR status IS NULL`); legacy `current_work_items.project_name` is informational, not authoritative
...
- Emit JSON **inventory artifact** to `.gtkb-state/orphan-wi-discovery/<run-id>/report.json` with required stable fields per the Clause Scope Clarification section
- Emit markdown **review packet** to `.gtkb-state/orphan-wi-discovery/<run-id>/summary.md` with required stable sections per the Clause Scope Clarification section
- Discovery is purely read-only; no DB writes; stays strictly within PAUTH `allowed_mutation_classes = ["source", "test_addition", "hook_upgrade"]`
...
4. `test_inventory_artifact_schema_compliance` - verifies the JSON report has all required stable fields per the Clause Scope Clarification section
5. `test_review_packet_schema_compliance` - verifies the markdown summary has all required stable sections

## Specification-Derived Verification Plan

| Spec citation | Verification artifact | Command | Expected outcome |
|---|---|---|---|
| GOV-STANDING-BACKLOG-001 (orphan rate visible) | Discovery script JSON inventory artifact | python scripts/discover_orphan_wi_memberships.py --run-id manual-test | report.json includes orphan_count, orphan_count_by_class |
...
| GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS (inventory artifact schema) | test_inventory_artifact_schema_compliance | pytest tests/scripts/test_discover_orphan_wi_memberships.py::test_inventory_artifact_schema_compliance -v | PASS |
| GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS (review packet schema) | test_review_packet_schema_compliance | pytest tests/scripts/test_discover_orphan_wi_memberships.py::test_review_packet_schema_compliance -v | PASS |

## Acceptance Criteria

1. Discovery script runs to completion against live MemBase and emits JSON inventory artifact + markdown review packet.
2. All 58 known orphan WIs from S363 are classified into one of the five recoverability classes.
3. All 5 regression tests PASS via `pytest tests/scripts/test_discover_orphan_wi_memberships.py -v`.
...
5. JSON inventory artifact has all required stable fields per Clause Scope Clarification section.
6. Markdown review packet has all required stable sections per Clause Scope Clarification section.
7. Clause preflight `python scripts/adr_dcl_clause_preflight.py --bridge-id gtkb-orphan-wi-membership-discovery-slice-1` exits 0 (no blocking gaps).
8. WI-3397 transitions to `resolved` upon VERIFIED for Slice 1 (Slice 2 will be a separate WI + bridge thread).
...
- Risk: discovery surfaces a large unrecoverable class that requires significant owner AUQ time in Slice 2. Mitigation: discovery output informs Slice 2 scoping; Slice 2 can batch AUQs or apply default-retire policy for sufficiently-old unrecoverable orphans with owner pre-approval via formal-artifact-approval packet.
