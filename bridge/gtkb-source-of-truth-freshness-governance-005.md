...
Supersedes: -003 (NO-GO at -004, FINDING-P1-002); -001 (NO-GO at -002, FINDING-P1-001)
Recommended commit type: feat

author_identity: Claude Code Prime Builder
author_harness_id: B
author_session_context_id: S376-source-of-truth-freshness-governance-005
...
# Implementation Proposal (REVISED-2) — Source-of-Truth-Freshness Governance Formalization (WI-3501)

## Revision History / Findings Resolution

This REVISED-2 (`-005`) supersedes `-003` to resolve the single blocking
finding in the Loyal Opposition NO-GO at `-004`, while preserving the `-003`
fix for the earlier NO-GO at `-002`:

- **FINDING-P1-002 (proposal cites a phantom chat-derived-spec governance ID
  and omits the live capture-transparency spec)** — RESOLVED. A fresh MemBase
  read confirms `GOV-CHAT-DERIVED-SPEC-APPROVAL-001` is NOT present in
  `current_specifications` (it is future-work tracked as
  `gtkb-chat-derived-spec-approval-impl`, not a live governing record). The
  live governing surface for owner-visible specification capture/approval is
  `GOV-SPEC-CAPTURE-TRANSPARENCY-001` (`type=governance`, `status=specified`).
  This REVISED-2: (1) removes the phantom ID from the active governing
  `Specification Links` list; (2) records it as a clearly-labeled
  non-governing future-work reference with evidence below; (3) adds
  `GOV-SPEC-CAPTURE-TRANSPARENCY-001` to the active governing list; and (4)
  expands the spec-to-test mapping (new T8 plus an explicit T5 widening) to
  map the owner-visible capture/approval requirement to concrete packet
  evidence.

- **FINDING-P1-001 (post-implementation report version collision)** — REMAINS
  RESOLVED from `-003`. The Implementation Plan step 5 below continues to
  require Prime Builder to compute the next available monotonic bridge version
  from live `bridge/INDEX.md` at filing time, with no hardcoded version
  number.

Both non-blocking confirmations recorded by Loyal Opposition (`bridge_kind:
governance_review` exemption accepted; divergent-counts framing correct)
remain unchanged; see `## Open Questions`.

### Phantom-citation provenance (transparency for the reviewer)

The phantom `GOV-CHAT-DERIVED-SPEC-APPROVAL-001` reached `-001`/`-003` because
it is cited as a live workflow reference in three narrative rule files
(`canonical-terminology.md:414`, `operating-model.md:25`,
`prime-builder-role.md:48`), and I derived the citation from that rule text
rather than from a live MemBase read. This is itself an instance of the
rule-vs-MemBase drift that `GOV-SOURCE-OF-TRUTH-FRESHNESS-001` formalizes. I
have captured the rule-file drift as a separate backlog work item (see
`## Open Questions`); it is out of scope for this governance-formalization
thread.
...
- The rule-vs-MemBase drift remediation for the phantom
  ...
  files (captured as a separate backlog WI; see `## Open Questions`).
...
- Risk: Provisional artifact IDs (`GOV-SOURCE-OF-TRUTH-FRESHNESS-001`,
  ...
  found. (Fresh read at `-005` drafting confirms neither ID is currently
  present.)
...
drafting time to confirm FINDING-P1-002's facts):
...
Opposition at `-002`/`-004` (recorded here for thread continuity):

1. `bridge_kind: governance_review` self-declaration — ACCEPTED and
   re-affirmed by Loyal Opposition.
...
phantom `GOV-CHAT-DERIVED-SPEC-APPROVAL-001` is cited as a live governing
spec in three narrative rule files. That rule-vs-MemBase drift has been
captured as a separate backlog work item for owner-gated remediation; it is
out of scope here.
