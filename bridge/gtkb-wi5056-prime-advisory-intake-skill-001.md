...
Work Item: WI-5056
target_paths: [".claude/skills/advisory-intake/SKILL.md", ".codex/skills/advisory-intake/SKILL.md", ".codex/skills/MANIFEST.json", "config/agent-control/harness-capability-registry.toml", "platform_tests/skills/test_advisory_intake_skill.py", "bridge/gtkb-wi5056-prime-advisory-intake-skill-*.md", "groundtruth.db"]
implementation_scope: Add the Prime Builder intake workflow that consumes live ADVISORY entries only after owner-grilling answers, explicit project/work-item approval, and child proposal filing are present.
requires_review: true
requires_verification: true

# Implementation Proposal - Prime Builder Advisory Intake Skill

## Summary
Add the Prime Builder intake workflow that consumes live ADVISORY entries only after owner-grilling answers, explicit project/work-item approval, and child proposal filing are present.
...
The parent proposal and child work items already encode the operative requirement: ADVISORY material can seed scoped proposals only after owner-grilling and explicit project/work-item approval, and advisory capture itself must not become implementation approval. The linked manual test TEST-11294 provides the acceptance anchor for this child item.

## Proposed Scope
- Create the advisory-intake skill and Codex adapter for Prime Builder selection of live ADVISORY entries, source summarization, owner-grilling answer capture, explicit project/WI approval, and child proposal filing.
- Register the skill in the skill manifest and harness capability registry without making ADVISORY entries implementation-dispatchable.
- Add focused tests for the intake instructions and approval boundaries.

## Out Of Scope
- No deliberation-side advisory authoring workflow is included; that is WI-5055.
- No source implementation is authorized from an ADVISORY entry without the separate child bridge proposal GO path.
...
- `python -m pytest platform_tests/skills/test_advisory_intake_skill.py -q --tb=short` passes and maps to TEST-11294.
- Manual skill review confirms live ADVISORY selection, source summary, owner-grilling answers, explicit project/WI approval, and separate child bridge proposals before implementation.
- `python scripts/bridge_applicability_preflight.py --bridge-id gtkb-wi5056-prime-advisory-intake-skill --json` returns no blocking applicability findings for the final bridge thread.

## Acceptance Criteria
- Prime Builder can follow a documented advisory intake path without treating advisory capture as implementation approval.
- TEST-11294 is satisfied by test output and implementation-report evidence.
