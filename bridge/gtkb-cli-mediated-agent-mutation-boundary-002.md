...
This REVISED version preserves the `-001` scope and adds explicit
specification-derived verification language so the mandatory clause preflight
can evaluate the proposal without relying on informal wording.
...
This may not belong solely in the bridge-index-retirement umbrella, but it
materially affects the cleanup design. The no-index work should not merely
replace `bridge/INDEX.md` reads with better prose; it should move mutating
bridge, dispatcher, project, spec, skill, and application-operation workflows
toward deterministic CLI surfaces that skills call rather than asking agents
to hand-edit GT-KB artifacts.
...
The owner statement is already captured as requirement candidate
`INTAKE-f8bc08a3`, but it is still `outcome="deferred"` and has no confirmed
`spec_id`. This proposal records the design constraint for the active no-index
cleanup and asks Loyal Opposition to review the scope and sequencing. Source
or configuration implementation should wait until either:
...
3. **Direct source/file edits only for platform implementation work:** agents may still edit GT-KB code through reviewed bridge proposals when the task is to build or repair the platform itself, but ordinary use of GT-KB to build or operate adopter applications should not require modifying GT-KB code.
4. **Artifact mutation boundary:** agents should be barred from mutating GT-KB artifacts except through governed skills or direct CLI access. Raw file or database edits should be treated as implementation work needing a specific bridge proposal, not as normal operation.
...
The implementation report for any GO version of this proposal must include a
spec-to-test mapping and observed command results. The expected mapping is:

| Specification / requirement | Required implementation evidence | Required command evidence |
|---|---|---|
| `INTAKE-f8bc08a3` or its confirmed successor spec | Skills and prompts route mutating GT-KB operation through `gt ...` commands or through skills that call those commands. | `rg -n "direct edit|hand-edit|mutate.*artifact|CLI|gt " .claude\skills .codex\skills .agent\skills .api-harness\skills .claude\rules groundtruth-kb\templates config\agent-control groundtruth-kb\src\groundtruth_kb` |
| `SPEC-CENTRALIZED-DISPATCH-SERVICE-001` | Dispatcher configuration, status, and health remain exposed through the bridge dispatch CLI. | `gt bridge dispatch config`; `gt bridge dispatch status --json`; `gt bridge dispatch health --json` |
| `SPEC-DISPATCH-ENVELOPE-ELEMENT-001` | Dispatch routing remains rule based rather than relying on prompt-only conventions or direct artifact edits. | `python -m pytest platform_tests\scripts\test_bridge_claim_cli.py platform_tests\scripts\test_implementation_authorization.py -q --tb=short` plus any new dispatcher/CLI tests added by the implementation |
| `DCL-CROSS-HARNESS-ENFORCEMENT-001` | Claude, Codex, Antigravity, OpenRouter, and Ollama skill/prompt surfaces are checked for conflicting raw-mutation instructions. | `python -m pytest platform_tests -q --tb=short` for the touched enforcement surfaces, narrowed if the implementation is narrower |
| `DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001` | The post-implementation bridge report carries this table forward, lists every linked specification, and records observed results for each verification command. | `python -m ruff check scripts groundtruth-kb\src platform_tests`; `python -m ruff format --check scripts groundtruth-kb\src platform_tests` |

For this proposal review, Loyal Opposition should additionally run:
...
rg -n "direct edit|hand-edit|mutate.*artifact|CLI|gt " .claude\skills .codex\skills .agent\skills .api-harness\skills .claude\rules groundtruth-kb\templates config\agent-control groundtruth-kb\src\groundtruth_kb
```
...
- Active no-index cleanup proposals are either accepted as compatible with CLI-first design or revised to narrow or remove raw artifact mutation assumptions.
...
This proposal is non-implementing unless and until it receives GO and an
implementation-start packet. Rolling it back means superseding it with a
revised proposal or rejecting it by NO-GO; do not delete the bridge file.
