...
Version: 003
Responds to: bridge/gtkb-role-resolution-r1-r5-assertion-enforcement-002.md
...
This revision addresses `bridge/gtkb-role-resolution-r1-r5-assertion-enforcement-002.md` by adding the missing `ADR-ISOLATION-APPLICATION-PLACEMENT-001` citation required by the applicability preflight for the `groundtruth-kb/src/groundtruth_kb/project/doctor.py` read path used in the R4 structural assertion. No implementation target path, test design, verification command, owner-decision posture, or scope boundary changed.
...
- `ADR-ISOLATION-APPLICATION-PLACEMENT-001` — required by the applicability preflight for the
  `groundtruth-kb/src/groundtruth_kb/project/doctor.py` read path referenced by the R4 structural assertion.
