...
Version: 003
Responds to: bridge/gtkb-wi5942-bridge-helper-publication-capability-002.md
...
# REVISED Implementation Proposal - Systemic fix: bridge-filing helper must mint/consume publication capabilities

## Response To Version 002 NO-GO

This REVISED responds to the version 002 NO-GO.

### Finding 1 (P1) - placeholder cleanup - RESOLVED
The version 001 proposal carried an unresolved helper pre-population placeholder
(`_No prior deliberations: <fill in reason before filing>._`) alongside concrete
citations. This revision removes the placeholder and retains only concrete
citations (see Prior Deliberations below). No other content change.

### Finding 2 (P3) - target path confirmation - CONFIRMED
The declared target path `.goose/skills/gtkb-bridge-propose/helpers/write_bridge.py`
is confirmed as the intended Goose helper surface under the active
`PAUTH-PROJECT-GTKB-PLATFORM-MODERNIZATION-ASSURANCE-20260715-PROJECT-SCOPE`.
The mint/consume integration plan is retained unchanged.
...
1. Before writing the bridge file, call
   ...
   bytes, author `session_id`, and a compliance digest, recording a
...
2. After a verified byte-exact write, call
   ...
   revision linkage.
3. On any write failure, compensate the pending publication and fail closed.
4. Add focused tests asserting a helper-written bridge file gains a `consumed`
   publication capability with matching content digest, and that a failed write
   compensates without stranding.

This prevents future WI-5825-class stranding. It does NOT retroactively recover
already-stranded files (WI-5314 -015, WI-5368 -023/-027/-028); those require a
separate owner-authorized exceptional recovery.
...
- Retroactive recovery of already-stranded files.
...
- `bridge/gtkb-wi5314-nonspawn-session-envelope-suppression-016.md` - NO-GO surfacing the WI-5314 stranding.
- `bridge/gtkb-wi5368-codex-git-window-command-family-030.md` - NO-GO surfacing the WI-5368 stranding.
