...
**Addresses:** NO-GO at `bridge/gtkb-adoption-gap-closure-010.md`

---

## NO-GO Response Summary (from -010)

Both findings addressed:

| Finding | Severity | Resolution in this revision |
|---------|----------|-----------------------------|
| F1 — Default generated CI fails because `gt assert` runs against seed-example assertions targeting `src/tasks.py` which doesn't exist | P1 | Generate a `src/tasks.py` stub for **all profiles** when `seed_example=True` (default). The stub satisfies the seeded `SPEC-001` grep assertions (`def create_task`, `status.*=.*'open'`) and `SPEC-002` assertion (`def list_tasks`). A fresh default scaffold now exits 0 on `gt assert`. `--no-seed-example` suppresses the stub. Docs updated: "expected failure" framing replaced with "stubs provided — evolve them." |
| F2 — `full` tier runs `pytest tests/` but `dual-agent-webapp` generates no `tests/` tree | P1 | Generate `tests/__init__.py` and `tests/test_smoke.py` for `dual-agent-webapp` profile. Smoke test has one passing assertion (`assert True`). A fresh `dual-agent-webapp` scaffold now exits 0 on `pytest tests/ -q --tb=short`. Acceptance tests added for both generated CI commands. |

Prior resolved findings from -008 (CI default contradiction, Docker/pytest misalignment) are preserved unchanged from -009.
...
- `bridge/gtkb-adoption-gap-closure-008.md`, `-010.md`: Prior NO-GOs. `-008` rejected tristate design and misaligned CI tiers. `-010` rejected CI default that fails on fresh default scaffold (Finding 1) and `full` tier pytest without a `tests/` tree (Finding 2).
...
### Phase G1: Adopter Documentation (unchanged from -009)
...
| `docs/start-here.md` | Verify no Agent Red-specific references remain; confirm scaffold output references are accurate; **update "expected failure" framing (L137-154) to reflect that stubs are now provided** (see G3.2d below) |
...
### Phase G2: Cross-Platform Bridge (unchanged from -009)
...
### Phase G3: GitHub CI Template Profile-Tiering (revised from -010)

**Goal:** Generated CI workflows from `gt project init` are tiered to the selected profile (minimal / standard / full), contain correct package-name and Python-version substitutions, are aligned with the files each profile actually generates, **and are green on first push for a fresh default scaffold without any adopter code changes**. `--no-include-ci` always suppresses all CI regardless of profile.

#### G3.1 — CI Default Rule (unchanged from -009)
...
#### G3.2 — Profile-Aligned CI Tier Contents (unchanged from -009)

**What each profile generates (empirically verified by Codex review -008, updated for -011 stubs):**

| Profile | Dockerfile | `src/` | `src/tasks.py` | `tests/` | `pyproject.toml` | `requirements.txt` |
|---------|-----------|--------|----------------|----------|------------------|--------------------|
| `local-only` | No | Yes* | Yes* | No | No | No |
| `dual-agent` | No | Yes* | Yes* | No | No | No |
| `dual-agent-webapp` | Yes | Yes | Yes* | Yes** | Yes | Yes |

> \* When `seed_example=True` (default). Suppressed by `--no-seed-example`.
> \*\* `tests/__init__.py` + `tests/test_smoke.py` generated for `dual-agent-webapp` always (not tied to `seed_example`). See G3.2e.
...
- **CI is green on fresh default scaffold** because `src/tasks.py` stub satisfies seeded assertions
...
- `{{PYTHON_VERSION}}` placeholder in setup-python step
- **CI is green on fresh default scaffold** because `src/tasks.py` stub + `tests/test_smoke.py` are both generated

#### G3.2d — `dual-agent-webapp` scaffold stub generation (revised to add `src/tasks.py`, `tests/`)

`scaffold_project()` generates the following for `dual-agent-webapp`:

| File | Generated when | Content |
|------|----------------|---------|
| `src/__init__.py` | Always | Empty stub (`# {{PROJECT_NAME}} application package`) |
| `pyproject.toml` | Always | Minimal PEP 517 stub: `[project]` name = `{{PACKAGE_NAME}}`, version = `"0.1.0"`, `requires-python = ">=3.11"` |
| `requirements.txt` | Always | Comment-only stub (`# Add your runtime dependencies here`) |
| `src/tasks.py` | `seed_example=True` (default) | See G3.2e stub spec |
| `tests/__init__.py` | Always | Empty |
| `tests/test_smoke.py` | Always | See G3.2e stub spec |

#### G3.2e — Seed-example stub generation for all profiles (new in -011)

**Purpose:** Make `gt assert` exit 0 on a fresh default scaffold. Without this, the seeded `SPEC-001` and `SPEC-002` grep assertions check for `src/tasks.py` which doesn't exist yet, causing CI to fail before the adopter writes any code.

**Trigger:** `options.seed_example=True` (the default). Suppressed when `--no-seed-example` is passed.

**Files generated (all profiles, when `seed_example=True`):**

| File | Content |
|------|---------|
| `src/tasks.py` | Minimal stub satisfying `SPEC-001` and `SPEC-002` grep assertions (see below) |

No `src/__init__.py` is generated for `local-only` / `dual-agent` profiles by this rule — only `src/tasks.py`. The `src/` directory is created as needed. For `dual-agent-webapp`, the `src/__init__.py` stub from G3.2d takes precedence (already generated).

**`src/tasks.py` stub content:**
...
This content satisfies the three seeded grep assertions:
- `SPEC-001` assertion 1: `grep "def create_task"` in `src/tasks.py` → matches line 16
- `SPEC-001` assertion 2: `grep "status.*=.*['\"]open['\"]"` in `src/tasks.py` → matches the `"status": "open"` return value
- `SPEC-002` assertion 1: `grep "def list_tasks"` in `src/tasks.py` → matches line 32

**`tests/test_smoke.py` stub content** (for `dual-agent-webapp`; generated always):
...
**Doc update required in `docs/start-here.md:137-154`:** Remove the "expected failure" framing (`gt assert` returning `File not found: src/tasks.py`). Replace with: "On a fresh scaffold, `gt assert` exits 0 because `src/tasks.py` is pre-generated as a stub. The tutorial teaches you to evolve it."

**Behavior contract:**

| Command | Fresh default scaffold (`seed_example=True`) | Fresh `--no-seed-example` scaffold |
|---------|----------------------------------------------|-------------------------------------|
| `gt assert` | Exit 0, FAILED: 0 | Exit 0, FAILED: 0 (no seeded assertions) |
| `pytest tests/` (`dual-agent-webapp`) | Exit 0, 1 passed | Exit 0, 1 passed (smoke test always generated) |
| `ls src/tasks.py` | Exists | Does not exist |

#### G3.3 — Placeholder Sources (unchanged from -009)
...
#### G3.4 — Integration Config Templates (unchanged from -009)
...
| G3.2 | **`dual-agent-webapp` stub generation** | Add generation of `src/__init__.py`, `pyproject.toml` stub, `requirements.txt` stub, `tests/__init__.py`, and `tests/test_smoke.py` to `scaffold.py` for `dual-agent-webapp` profile. |
...
| **G3.7** | **Seed-example stub generation** | Add `src/tasks.py` stub generation to `scaffold.py` for all profiles when `seed_example=True`. Add suppression when `--no-seed-example`. Update `docs/start-here.md:137-154`. |
...
| **n** | **Fresh default `local-only` scaffold → `python -m groundtruth_kb --config groundtruth.toml assert` exits 0** |
| **o** | **Fresh default `dual-agent` scaffold → `python -m groundtruth_kb --config groundtruth.toml assert` exits 0** |
| **p** | **Fresh default `dual-agent-webapp` scaffold → `python -m pytest tests/ -q --tb=short` exits 0** |
| **q** | **Fresh `--no-seed-example` scaffold (any profile) → no `src/tasks.py` generated** |
| **r** | **`src/tasks.py` stub (when generated) contains `def create_task`, `"status": "open"`, and `def list_tasks`** |
| **s** | **`dual-agent-webapp` scaffold generates `tests/__init__.py` and `tests/test_smoke.py`** |
| **t** | **Fresh `--no-seed-example` `dual-agent-webapp` scaffold → no `src/tasks.py`; `tests/test_smoke.py` still generated** |

**Note on tests n and o:** These are integration/acceptance tests that invoke the actual `gt assert` CLI in a `tmp_path` scaffold. They require `groundtruth_kb` importable from the source under test (set `PYTHONPATH=src`). They must be added to the existing test suite alongside the scaffold tests (`tests/test_scaffold_project.py` or a new `tests/test_scaffold_ci_acceptance.py`).
...
- **Fresh default `local-only` scaffold: `gt assert` exits 0 (FAILED: 0). Verified by acceptance test n.**
- **Fresh default `dual-agent` scaffold: `gt assert` exits 0 (FAILED: 0). Verified by acceptance test o.**
- **Fresh default `dual-agent-webapp` scaffold: `pytest tests/ -q --tb=short` exits 0 (1 passed). Verified by acceptance test p.**
- **`--no-seed-example` scaffold (any profile): `src/tasks.py` absent. Verified by acceptance test q.**
- CI template tests (a)–(t) all pass.
- `pip install groundtruth-kb` base install does not require Jinja2.

---

### Phase G5: Second Customer Field Trial (unchanged from -009)
...
|-------|-----|-----------|-------|
| G1 doc delta | 3 new docs + 2 file fixes | Small | Mostly writing; fix to `bootstrap.md:12`; update `start-here.md:137-154` framing |
...
| G3 CI templates | Tier dirs + profiles + `--no-include-ci` fix + `webapp` stubs + seed stubs + smoke test + placeholders + integrations flag + tests | Medium | Stdlib substitution only; no new dep; 7 WIs (G3.7 added) |
...
3. **CI out of the box — green on first push:** `gt project init` generates profile-tiered GitHub CI workflows with correct tier contents; `--no-include-ci` always suppresses all CI; no Agent Red references; **and a fresh default scaffold's generated CI commands pass without any adopter code changes.**
...
## Risk Assessment (updated for -011)
...
| `dual-agent-webapp` stub generation changes scaffold output; existing tests may break | Low | Scope additions to `dual-agent-webapp` only; existing `local-only`/`dual-agent` scaffold tests unaffected for existing output. Add specific assertions for the new stubs. |
| `src/tasks.py` stub satisfies grep assertions but `gt assert` evaluates them differently | Low | Codex ran empirical verification: assertions use `type: grep` against the file. The stub content directly matches the three grep patterns. |
| `src/` directory appearance in `local-only` / `dual-agent` profiles surprises adopters | Low | Stub file header comment explains origin; `--no-seed-example` suppresses entirely. |
| `src/tasks.py` stub triggers Python import errors in any generated CI step | None | `minimal` and `standard` CI tiers have no pytest or import steps. The stub is Python-valid (ruff-clean). |
...
| `--no-include-ci` precedence fix changes behavior for `dual-agent-webapp` adopters who relied on profile-forced CI | Low | The prior behavior was arguably a bug. Docs update makes intent explicit. |
| Acceptance tests n/o/p require spawning subprocesses in CI | Low | Tests are scoped to `tmp_path` sandboxes; no network; subprocess invocation pattern already used in `tests/test_scaffold_smoke.py`. |
...
## Open Questions (unchanged from -009)
