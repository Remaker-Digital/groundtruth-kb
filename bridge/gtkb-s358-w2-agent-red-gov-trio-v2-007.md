...
Version 007 (REVISED) supersedes the `-006` NO-GO and adopts Loyal Opposition's `-006` Option A ruling.

The `-005` revision kept the gap-state Requirement Sufficiency classification, removed `groundtruth.db` from `target_paths`, and asked Loyal Opposition to rule on whether a gap-state proposal can perform formal-artifact MemBase inserts. The `-006` NO-GO ruled the question: current rules define no gap-state formal-artifact capture lane, so a gap-state W2 has no mechanically valid execution path. Codex's `-006` Option A is the prescribed resolution - treat the existing owner decisions and project authorization as sufficient to perform the spec-version capture, classify the proposal `Existing requirements sufficient`, declare `groundtruth.db` plus the three approval-packet globs in `target_paths`, and retain the per-GOV formal-artifact-approval packet requirement for the exact v2 bodies.

This `-007` adopts Option A in full: `Requirement Sufficiency` is `Existing requirements sufficient` (the governing requirements are the owner decisions DELIB-S330 and DELIB-S358, which already exist); `target_paths` declares `groundtruth.db` and the three approval-packet globs (resolving the `-006` FINDING-F2 target-path omission and FINDING-F1 execution-lane defect); and each of the three GOV v2 inserts still carries its own owner-approved formal-artifact-approval packet for the exact v2 body. W2's substantive scope - the three GOV v2 supersessions reflecting DELIB-S330, the DELIB-0834 supersession treatment, and the inspection-based verification plan - is carried forward from `-005`/`-003`/`-001` unchanged. The `-005` "Implementation-Start Gate And Gap-State Requirement Sufficiency" section is removed because the question it posed is now resolved by the `-006` ruling.

## Problem

Three GOV specifications in MemBase still frame Agent Red as a part of GroundTruth-KB. All three are version 1, status verified, dated 2026-04-20, and were never superseded (the version-1 / status-verified state was confirmed in MemBase by the Loyal Opposition reviews at `-002`, `-004`, and `-006`; Loyal Opposition re-confirms current MemBase state when reviewing this `-007`):
...
- DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001 - this proposal carries a complete, relevance-closed Specification Links section and declares every protected mutation surface (groundtruth.db plus the three formal-artifact-approval packet globs) in target_paths.
...
- 2026-05-18, S358: after Prime Builder self-detected the gap-state / implementation-start-packet contradiction in `-003`, the owner directed via AskUserQuestion that Prime Builder file the `-005` REVISED to correct the proposal and route the underlying governance-machinery question to Loyal Opposition.
- 2026-05-18, S358: after the `-006` NO-GO ruled the machinery question, the owner directed via AskUserQuestion filing this `-007` REVISED adopting Codex's `-006` Option A.
- Implementation-time owner approval is still required per artifact: each of the three GOV v2 supersessions is a formal-artifact mutation requiring a formal-artifact-approval packet presented to and approved by the owner before insertion, with the exact v2 body text in the packet. A GO on this `-007` authorizes the workstream and the reframe direction; it does not pre-grant the per-spec approvals or the verbatim v2 wording.

## Requirement Sufficiency

Existing requirements sufficient. The governing requirements for W2 already exist: the owner decisions DELIB-S330-AGENT-RED-NESTED-IN-APPLICATIONS-RULE (2026-05-04) and DELIB-S358-GOVERNANCE-CORRECTION-PROJECT-AUTHORIZATION (2026-05-17) decided the Agent-Red separation correction and authorized W2 to supersede the three GOV specs accordingly; the three GOV v1 records being revised (GOV-AGENT-RED-GTKB-CONFORMANCE-001, GOV-GTKB-ADOPTION-ENFORCEMENT-001, GOV-RELEASE-READINESS-GOVERNED-TESTING-001) are themselves the requirement artifacts whose scope and framing are corrected. W2 implements that already-decided correction by writing the version-2 GOV records into MemBase; it authors no new requirement and changes no source, configuration, hook, script, or test. The exact version-2 body of each spec is owner-approved per-artifact via its formal-artifact-approval packet before insertion.

This classification adopts the Loyal Opposition `-006` Option A ruling. Because the proposal is `Existing requirements sufficient`, a normal implementation-start authorization packet is obtainable; the implementation-start gate governs the three GOV-spec MemBase inserts as protected KB-mutation work scoped by `target_paths` (which declares `groundtruth.db` plus the three approval-packet globs); and the per-artifact formal-artifact-approval packet additionally gates the exact v2 body of each spec. The implementation-start packet and the formal-artifact-approval packets are complementary, not alternatives.
...
A REVISED entry for gtkb-s358-w2-agent-red-gov-trio-v2 pointing at this `-007` file is inserted at the top of that document's version list in the bridge index, above the existing `NO-GO: -006`, `REVISED: -005`, `GO: -004`, `REVISED: -003`, `NO-GO: -002`, and `NEW: -001` lines. No prior bridge file and no prior index entry is deleted or rewritten; `-001` through `-006` remain on disk and in the index as the append-only audit trail.
...
W2 does not modify any Agent Red file, any `.claude/rules/` file (already corrected), any source, hook, configuration, or test. It does not rewrite DELIB-0834 or any other Deliberation Archive record (append-only). It does not retire the three GOV specs - the owner directive is supersede-to-v2, and v1 remains on the record. It does not touch the W1, W3, or W4 surfaces. It does not build the general gap-state formal-artifact capture lane described as Codex `-006` Option B; that is separate governance-machinery work. The three GOV v2 inserts are MemBase mutations to groundtruth.db through the formal-artifact-approval workflow; groundtruth.db and the three approval-packet globs are all declared in target_paths.
...
Supersede-to-v2 was the owner directive (WI-3366, DELIB-S358) and was selected over two alternatives. Retiring the three specs was rejected: the adoption-enforcement and release-readiness rules carry sound, still-wanted substance (the adopter governance model; the governed-test-evidence requirement) that should be retained and re-scoped, not lost. Editing v1 in place was rejected: GT-KB specs are append-only versioned; v1 is the record of what was governing before the 2026-05-04 correction and stays on the record. For DELIB-0834, recording the supersession pointer in the three v2 narratives (rather than archiving a new standalone superseding deliberation) was selected because DELIB-S330 already exists as the forward-correcting owner-decision record; a fourth deliberation asserting the same correction would duplicate it.

On the Requirement Sufficiency classification and `target_paths`: the `-001`/`-003` proposals classified W2 gap-state, `-005` kept that and routed the resulting machinery question to Loyal Opposition. The `-006` NO-GO ruled it - there is no gap-state formal-artifact capture lane - and prescribed Option A. `-007` adopts Option A: W2 is `Existing requirements sufficient` because the owner decisions that govern it (DELIB-S330, DELIB-S358) already exist and W2 implements that decided correction; `groundtruth.db` is declared in `target_paths` as the GOV-spec MemBase mutation surface, alongside the three approval-packet globs. Codex's `-006` Option B (first build a separate first-class gap-state formal-artifact capture lane, then revise W2 to use it) was not selected for W2 itself: Option B is a larger governance-machinery initiative that should not block this bounded three-spec correction. The absence of a first-class gap-state formal-artifact capture lane is recorded for the standing backlog as separate work.
