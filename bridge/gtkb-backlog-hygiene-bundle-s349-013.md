# Bridge Proposal: gtkb-backlog-hygiene-bundle-s349 (REVISED @ 013)
...
**Revision history:** REVISED from -001 NEW after Codex NO-GO at -002 (F1: machine-readable target_paths metadata missing). REVISED again from -003 GO at -004 after implementation-start gate revealed has_spec_derived_verification() heading-allowlist mismatch. REVISED from -005 NO-GO at -006 to remove stale bridge-version references in the Implementation Plan and replace residual 'Test Plan' prose with 'Verification Plan'. REVISED again from -007 NO-GO at -008 to remove the last stale '-004.md' reference in the Files Expected To Change paragraph. Proposal received GO at -010; implementation completed; post-implementation report filed at -011 received NO-GO at -012 (F1: per-row change_reason cites bundle-level evidence but does not name the corresponding S349 finding number per the approved verification plan). REVISED from -011 implementation cycle to explicitly authorize Phase 4 (appending corrected versions of WI-3282..WI-3293 with per-finding change_reason text); no scope change beyond the additional row versions.
...
| ADR-ARTIFACT-ORIENTED-DEVELOPMENT-001 (traceability) | Each work_item's latest-version `change_reason` field shall cite this bridge document path AND the corresponding S349 finding number in the explicit form `"S349 backlog hygiene bundle Finding N (see bridge/gtkb-backlog-hygiene-bundle-s349-013.md Owner Decisions section)."` where N is the finding number per the Phase 4 mapping in the Implementation Plan. Verification query: `python -m groundtruth_kb backlog list --json` filtered to WI-3282..WI-3293, projected to `(id, change_reason)`, must show each row's latest change_reason containing the substring `Finding N` with N matching the WI-to-Finding mapping. |
...
2. **Phase 1 (creation, if not already done):** Create the 2 new project records via `python -m groundtruth_kb projects create <NAME> --id <PROJECT-ID> --purpose <text> --scope-note <text> --changed-by prime-builder/s349-bundle --change-reason <citation>`. One invocation per project. If projects already exist from a prior implementation cycle on this thread, skip.
3. **Phase 2 (creation, if not already done):** Create the 12 new work_items via the Python API `groundtruth_kb.db.KnowledgeDB.insert_work_item(...)` directly. Each invocation supplies `id` (WI-3282..WI-3293), `title`, `origin`, `component`, `resolution_status="open"`, `stage="backlogged"`, `priority`, `description`, `project_name`, `changed_by`, `change_reason`, and `related_bridge_threads="gtkb-backlog-hygiene-bundle-s349"`. If rows already exist from a prior implementation cycle on this thread, skip.
4. **Phase 3 (membership, if not already done):** Link each new work_item to its parent project via `python -m groundtruth_kb projects add-item <PROJECT-ID> <WORK-ITEM-ID> --source "S349 bundle (gtkb-backlog-hygiene-bundle-s349)" --changed-by prime-builder/s349-bundle --change-reason <citation>`. One invocation per work_item. If memberships already exist, skip.
5. **Phase 4 (per-finding change_reason correction):** For each of WI-3282..WI-3293, call `KnowledgeDB.insert_work_item(...)` again with the same `id`, `title`, `origin`, `component`, `resolution_status`, `priority`, `stage`, `description`, `project_name`, and `related_bridge_threads`, but with a corrected `change_reason` that explicitly names the corresponding S349 finding number. The required form is: `"S349 backlog hygiene bundle Finding N (see bridge/gtkb-backlog-hygiene-bundle-s349-013.md Owner Decisions section)."` Mapping of WI to Finding: WI-3282->Finding 1, WI-3283->Finding 2, WI-3284->Finding 3, WI-3285->Finding 4, WI-3286->Finding 5, WI-3287->Finding 6, WI-3288->Finding 7, WI-3289->Finding 8, WI-3290->Finding 9, WI-3291->Finding 10, WI-3292->Finding 11, WI-3293->Finding 12. The append-only versioning produces a new version of each row; the prior version with the generic change_reason is preserved in the audit trail.
6. Run the post-impl verification commands listed in the Verification Plan section; paste exact outputs into the implementation report. The per-row change_reason verification query must show each WI's latest change_reason citing its specific Finding N.
7. File the post-implementation report as the next unused bridge version on this thread (do NOT reuse any existing version number; consult the live `bridge/INDEX.md` chain to pick the next free `-NNN.md` number) and add a `NEW` line at the top of the bridge thread's INDEX entry.
8. Recommended commit type: `chore` - this is governed metadata capture, not new functionality.
...
REVISED @ 013. Addresses Codex report-NO-GO at -012 F1: the implementation under the -010 GO inserted the 12 work_items with a generic bundle-level change_reason that did not cite the per-finding number required by the approved ADR-ARTIFACT-ORIENTED-DEVELOPMENT-001 verification step. This revision adds an explicit Phase 4 implementation step authorizing corrective row versions with per-finding change_reason text, and tightens the corresponding verification row to specify the exact format. The 12 work_items and 2 projects from the -010 cycle remain in place; Phase 4 appends a new version per row to fix only the change_reason field. Awaiting Codex re-review.
