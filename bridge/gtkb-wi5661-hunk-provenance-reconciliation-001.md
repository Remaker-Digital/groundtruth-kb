...
target_paths: ["bridge/gtkb-wi5661-skill-rename-live-breaks-003.md", "bridge/gtkb-wi5661-skill-rename-live-breaks-004.md", ".claude/hooks/bridge-axis-2-surface.py", "config/hooks/gtkb-bridge-axis-2-surface.py", "scripts/gtkb_bridge_writer.py", "scripts/harness_parity_phase2.py", "scripts/per_thread_finalization_repair.py", "platform_tests/scripts/test_gtkb_bridge_writer.py"]
...
Establish the named foreign-hunk provenance prerequisite for clean WI-5661 recovery.
...
All target paths are inside `E:\GT-KB`: `bridge/gtkb-wi5661-skill-rename-live-breaks-003.md`, `bridge/gtkb-wi5661-skill-rename-live-breaks-004.md`, `.claude/hooks/bridge-axis-2-surface.py`, `config/hooks/gtkb-bridge-axis-2-surface.py`, `scripts/gtkb_bridge_writer.py`, `scripts/harness_parity_phase2.py`, `scripts/per_thread_finalization_repair.py`, `platform_tests/scripts/test_gtkb_bridge_writer.py`.
...
- Record each dirty path and its observed hunk count without modifying or attributing it.
- Quarantine the untracked predecessor -003/-004 chain and expired predecessor claim.
- Produce the required clean-baseline or owner-attribution predicate before WI-5661 source implementation.

## Cross-Harness Disposition

- **Claude**: Bridge-only provenance audit; no Claude hook behavior changes.
- **Codex**: Bridge-only provenance audit; no Codex adapter or writer behavior changes.
...
```json
{
  ...
  "after_behavior": "Establish the named foreign-hunk provenance prerequisite for clean WI-5661 recovery.",
  ...
  "expected_result": {
    "summary": "Establish the named foreign-hunk provenance prerequisite for clean WI-5661 recovery.",
    "scope": [
      "Record each dirty path and its observed hunk count without modifying or attributing it.",
      "Quarantine the untracked predecessor -003/-004 chain and expired predecessor claim.",
      "Produce the required clean-baseline or owner-attribution predicate before WI-5661 source implementation."
    ...
    "acceptance_criteria": [
      "Every listed path has a durable observed state, claim status, owner disposition, and no-source-mutation result.",
      "The artifact identifies the exact prerequisite completion required before the WI-5661 source proposal can request a new GO."
    ]
  },
  "rollback": {
...
| Spec | Verification |
| --- | --- |
| `GOV-FILE-BRIDGE-AUTHORITY-001` | Run bridge and claim status checks; verify no target path changes. |
| `GOV-ARTIFACT-ORIENTED-GOVERNANCE-001` | Run candidate and live bridge applicability preflights; implementation report must add targeted tests. |
| `DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001` | Run candidate and live bridge applicability preflights; implementation report must add targeted tests. |
| `DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001` | Run git status, numstat, and diff-check across every listed target path. |
