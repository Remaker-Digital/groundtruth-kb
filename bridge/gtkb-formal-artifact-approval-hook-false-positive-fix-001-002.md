...
Revision 1 adds the advisory specifications reported by the post-filing applicability preflight on `-001`: `ADR-ARTIFACT-ORIENTED-DEVELOPMENT-001`, `DCL-ARTIFACT-LIFECYCLE-TRIGGERS-001`, and `GOV-ARTIFACT-ORIENTED-GOVERNANCE-001`.
...
The initial `-001` proposal was filed to break the current false-positive scanner loop, then immediately checked. It passed required specs but reported missing advisory specs:
...
This `REVISED` file cites those advisory specs and should be the operative proposal for review.
