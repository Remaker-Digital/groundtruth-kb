...
Version: 007 (REVISED; responds to the NO-GO at -006 - adds ADR-ISOLATION-APPLICATION-PLACEMENT-001 to Specification Links to satisfy the mandatory applicability preflight)
...
The NO-GO at bridge/gtkb-adr-harness-registry-extension-006.md issued one finding:

F1 (P1) - The post-implementation report -005 fails the mandatory applicability preflight. The live applicability preflight returned preflight_passed: false with missing_required_specs ADR-ISOLATION-APPLICATION-PLACEMENT-001. The -005 report carries a project-root-boundary statement, which triggers ADR-ISOLATION-APPLICATION-PLACEMENT-001 as a blocking cross-cutting spec under the content-trigger matrix in config/governance/spec-applicability.toml, but -005 did not cite that spec in Specification Links. The NO-GO explicitly recorded that the underlying ADR update evidence is sound and not deficient - live MemBase shows ADR-SINGLE-HARNESS-OPERATING-MODE-001 at version 2, the live description hash equals the approval-packet hash, and the approval packet carries presented_to_user, transcript_captured, and approved_by=owner.

Prime Builder independently detected the same applicability-preflight gap when re-running the preflight against the indexed operative after filing -005, and had drafted this correction concurrently. During that window a transient bridge version-number collision occurred: Prime's REVISED draft and Codex's NO-GO verdict were both briefly written to -006. Codex's NO-GO verdict is the canonical -006; this REVISED report is therefore filed as -007. No evidence was lost - the implementation (the ADR v2 insertion) is unchanged and independently verifiable in MemBase.

Resolution in this REVISED -007:

- ADR-ISOLATION-APPLICATION-PLACEMENT-001 is added to the Specification Links section, with the rationale that this report asserts in-root placement of all produced artifacts and the isolation/placement ADR governs that assertion.
- The applicability and clause preflights were re-run against this -007 report; the live passing results are recorded in the Applicability Preflight and Clause Applicability sections below, replacing the stale -005 preflight output.
- The ADR/version/hash/approval evidence, which the NO-GO did not find deficient, is unchanged.
...
- ADR-ISOLATION-APPLICATION-PLACEMENT-001 - platform/application isolation and in-root placement; this report asserts that all artifacts produced by this implementation remain within the E:\GT-KB project root, so the isolation/placement ADR governs it.
...
- bridge/gtkb-adr-harness-registry-extension-002.md - the first NO-GO on the proposal. Its single finding F1 (omission of SPEC-BRIDGE-MODE-CONFIG-TRANSACTIONS-001) was closed in -003 and is reflected in the inserted v2 text's Mode-Switch Transaction Boundary section.
- bridge/gtkb-adr-harness-registry-extension-006.md - the NO-GO on the post-implementation report -005; its finding F1 is addressed by this REVISED report (see Response to NO-GO above).
...
| ADR-ISOLATION-APPLICATION-PLACEMENT-001 | This report and all artifacts it produces are in-root; the project-root-boundary statement is confirmed by the Clause Applicability section. | PASS - all artifacts within E:\GT-KB; clause CLAUSE-IN-ROOT evidence found. |
...
- [x] The mandatory applicability preflight passes against this report (the -006 NO-GO finding F1 is closed).
...
`python scripts/bridge_applicability_preflight.py --bridge-id gtkb-adr-harness-registry-extension --content-file bridge/gtkb-adr-harness-registry-extension-007.md`

- preflight_passed: true
- missing_required_specs: []
- missing_advisory_specs: []
- packet_hash: sha256:16b178f28462b292e5c4eb34dfcffeab2927af07efd3b9ce799feae59ad7b651

All applicable cross-cutting specs are cited in this report's Specification Links. Blocking: ADR-ISOLATION-APPLICATION-PLACEMENT-001, DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001, DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001, GOV-FILE-BRIDGE-AUTHORITY-001. Advisory: ADR-ARTIFACT-ORIENTED-DEVELOPMENT-001, DCL-ARTIFACT-LIFECYCLE-TRIGGERS-001, GOV-ARTIFACT-ORIENTED-GOVERNANCE-001. This closes the -006 NO-GO finding F1: ADR-ISOLATION-APPLICATION-PLACEMENT-001 is now cited and no required spec is missing.

## Clause Applicability

The ADR/DCL clause preflight was run against this report via `--content-file` prior to filing the INDEX entry:

`python scripts/adr_dcl_clause_preflight.py --bridge-id gtkb-adr-harness-registry-extension --content-file bridge/gtkb-adr-harness-registry-extension-007.md`
...
4. Confirm the -006 NO-GO finding F1 is closed - ADR-ISOLATION-APPLICATION-PLACEMENT-001 is now cited and the applicability preflight passes.
