...
This revision supersedes `bridge/gtkb-startup-relay-pretooluse-read-exemption-001.md`.
It keeps the same implementation scope and adds the clause-preflight evidence
that the initial filing lacked: explicit `bridge/INDEX.md` filing evidence and
an explicit non-bulk / no-formal-artifact-approval scope statement.

## Bridge Filing Evidence

This bridge artifact is filed under `bridge/` and this revision is inserted in
`bridge/INDEX.md` as the latest line for `Document:
gtkb-startup-relay-pretooluse-read-exemption` with status `REVISED`. No prior
bridge version is deleted or rewritten; `-001` remains in the version chain as
the original `NEW` filing.

## Summary

The init-keyword startup-disclosure relay emits a bounded pointer to
`.codex/gtkb-hooks/last-user-visible-startup.md` or
`.claude/hooks/last-user-visible-startup.md` and instructs the assistant to
perform exactly one read-only filesystem read of that cache, then relay the
startup disclosure verbatim. The live session exposed a remaining contradiction:
after the UserPromptSubmit hook sets `startup_response_pending: true`,
`guard_tool_use()` blocks all tool calls, including the exact read-only cache
read the relay instruction authorizes. The result is a false claim that startup
was already emitted while the owner-visible disclosure is never shown.

This proposal authorizes the narrow receiver-side fix: while
`startup_response_pending` is true, PreToolUse should allow only the one
read-only access to the harness-scoped startup disclosure cache named by
`_startup_relay_cache_paths()`, and should continue to block every other tool
use until the next owner prompt clears the pending startup-response state.
...
already specifies the relay behavior that this defect violates: the startup
disclosure must be visibly relayed and a short acknowledgement is not a
substitute. This proposal does not create or revise a formal requirement; it
implements the existing relay contract.
...
No searched deliberation rejected the bounded-pointer relay or a narrowly
scoped read-only cache exception.

## Owner Decisions / Input

On 2026-05-15/16 in this session, the owner observed that the visible chat did
not contain the startup disclosure after `::init gtkb pb`, called it an error,
and then answered `Yes, file/route the bridge work` when asked whether to move
the startup relay defect through the bridge / implementation authorization
path. This proposal implements that owner direction through the standing
reliability fast-lane. No production deployment, credential lifecycle action,
or formal GOV/ADR/DCL/SPEC mutation is requested.
...
This proposal is a single-defect source-and-test fix for WI-3323. It is not a
bulk backlog operation and does not perform any inventory sweep, batch
transition, work-item promotion, specification promotion, or project lifecycle
mutation. No formal artifact is created or revised by this implementation, so
no formal-artifact-approval packet is required for the implementation itself;
formal artifact approval discipline remains unchanged.

## Fast-Lane Eligibility

This is a single-concern defect fix under active work item WI-3323. It modifies
only startup relay receiver guard behavior and a targeted regression test. It
introduces no public API, no CLI, no deployment operation, no formal artifact
mutation, no credential lifecycle action, and no bulk work-item or
specification operation.

## Pre-Filing Preflight

After the initial `NEW` filing, the indexed proposal was checked with:

- `python scripts/bridge_applicability_preflight.py --bridge-id gtkb-startup-relay-pretooluse-read-exemption` - PASS, `missing_required_specs: []`, `missing_advisory_specs: []`.
- `python scripts/adr_dcl_clause_preflight.py --bridge-id gtkb-startup-relay-pretooluse-read-exemption` - FAIL on missing explicit `bridge/INDEX.md` filing evidence and missing explicit non-bulk/formal-artifact-approval scope evidence.

This `REVISED` version adds those two evidence surfaces. Loyal Opposition should
rerun both mandatory preflights against the latest indexed operative file before
issuing GO.
...
Risk is limited to the startup relay turn. The guard exception must be
exact-path and read-only to avoid weakening the pending-startup protection.
Rollback is to revert the two target files and return to the prior behavior,
accepting that startup disclosure relay can fail when the cache read is blocked.

## Recommended Commit Type

`fix:` - repairs a broken startup relay guard interaction without adding a new
