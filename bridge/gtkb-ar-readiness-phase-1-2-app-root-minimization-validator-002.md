...
Revision note: this version narrows work-item harvesting language from -001 while preserving the same implementation scope. This proposal is intentionally limited to the validator slice. It does not activate the later work-subject write guard, does not perform later partition-in-place data migration, and does not create or revise formal GOV/ADR/DCL/SPEC rows. It may update the registry's `validator_contract.implementation_status` from pending to implemented after the validator and release-gate wiring are in place.
...
- No work-subject write-guard activation. That belongs to a later Phase 1 slice.
- No data partition-in-place or bulk MemBase path rewrite. That belongs to a later Phase 1 slice.
...
This proposal is filed as the next status-bearing numbered bridge file for `gtkb-ar-readiness-phase-1-2-app-root-minimization-validator`; no prior version is deleted or rewritten (append-only). Dispatcher/TAFE state plus the numbered file chain are the live workflow state per `GOV-FILE-BRIDGE-AUTHORITY-001`.
