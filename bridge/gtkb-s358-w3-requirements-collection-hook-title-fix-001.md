...
target_paths: [".groundtruth/formal-artifact-approvals/*-gov-requirements-collection-hook-001.json"]
...
W3's only write is one MemBase GOV-spec version-4 record and its one formal-artifact-approval packet under .groundtruth/formal-artifact-approvals/. Both MemBase (groundtruth.db) and the approval-packet directory are in-root under the GT-KB project root. This bridge proposal file resides under the bridge directory. No application path under applications/ is touched.
...
- ADR-ISOLATION-APPLICATION-PLACEMENT-001 - the only write target is in-root; no application path is touched.
...
A NEW entry for gtkb-s358-w3-requirements-collection-hook-title-fix is inserted at the top of the bridge index, pointing at this -001 file. No prior bridge file and no prior index entry is deleted or rewritten.

## Proposed Scope

### IP-1: Issue v4 of GOV-REQUIREMENTS-COLLECTION-HOOK-001 with the corrected title

Insert a version 4 of GOV-REQUIREMENTS-COLLECTION-HOOK-001 whose title is the v3 title with the trailing " (LLM classification + retrieval-augmented options)" removed. The v4 title is exactly:
