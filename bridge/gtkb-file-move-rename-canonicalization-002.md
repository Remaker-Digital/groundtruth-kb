...
Version: 002
Responds to: bridge/gtkb-file-move-rename-canonicalization-001.md
Date: 2026-07-22 UTC
...
target_paths: [".claude/hooks", ".claude/rules", ".claude/settings.json", ".codex/hooks.json", ".codex/skills", ".codex/gtkb-hooks", "config/hooks", "config/agent-control", "groundtruth-kb/src", "groundtruth-kb/tests", "groundtruth-kb/docs", "groundtruth-kb/templates", "platform_tests", "tests", "scripts", "dashboard", "docs", "bridge", "memory", ".github", "pyproject.toml", "groundtruth.toml"]
...
Revision note: Version 002 corrects the author_identity metadata from the underspecified `codex` value in version 001 to `prime-builder/codex`; implementation scope, target paths, acceptance criteria, and verification plan are unchanged.

CSV-driven canonical relocation and gtkb-prefix rename of legacy .claude hook, rule, and agent-control artifacts, with repository-wide load-bearing reference repair and verification.
...
- Move and rename each manifest source to its canonical destination, preserving file contents, executable behavior, and bridge/session startup semantics; create config/hooks as needed.
- Update load-bearing references across the authorized repository surfaces so hooks, rules, startup envelopes, templates, tests, generated registry copies, skills, dashboard/docs references, and CI/runtime commands resolve to the new canonical paths.
...
```json
{
  ...
  "after_behavior": "CSV-driven canonical relocation and gtkb-prefix rename of legacy .claude hook, rule, and agent-control artifacts, with repository-wide load-bearing reference repair and verification.",
  ...
  "expected_result": {
    "summary": "CSV-driven canonical relocation and gtkb-prefix rename of legacy .claude hook, rule, and agent-control artifacts, with repository-wide load-bearing reference repair and verification.",
    "scope": [
      "Parse E:\\GT-KB\\gtkb-file-move-and-rename-list.csv into a normalized manifest before mutation, applying the owner-approved inferred corrections for hidden dot-log/json filenames, spec-before-code.py, and skill-scenarios.toml.",
      "Move and rename each manifest source to its canonical destination, preserving file contents, executable behavior, and bridge/session startup semantics; create config/hooks as needed.",
      "Update load-bearing references across the authorized repository surfaces so hooks, rules, startup envelopes, templates, tests, generated registry copies, skills, dashboard/docs references, and CI/runtime commands resolve to the new canonical paths.",
    ...
    "acceptance_criteria": [
      ...
      "Repository-wide text and structured scans find no remaining live references to the retired .claude/hooks, .claude/rules, or pre-prefix config/agent-control names except documented historical/provenance references.",
      "Session startup, bridge proposal/review, hook command resolution, and harness parity tests continue to pass after relocation.",
...
| `DCL-CROSS-HARNESS-ENFORCEMENT-001` | Run repo-native tests that exercise hook resolution, startup-control loading, bridge proposal filing, and implementation authorization after relocation. |
| `DCL-CROSS-HARNESS-PARITY-ENFORCEMENT-001` | Verify Claude/Codex-facing control surfaces agree on the canonical config/hooks and config/agent-control locations after the move. |
