...
# Revised Proposal - WI-4943 Release-Branch Dispatcher Substrate Reconciliation - Backlog Query Dependency Envelope Correction
...
Responds to NO-GO: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-016.md
Prior implementation blocker report: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-015.md
Prior GO: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-014.md
Prior revised proposal: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-013.md
...
This revision responds to the NO-GO at `bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-016.md`. The implementation blocker report at `-015.md` correctly failed closed after the fresh release worktree proved that the verified dispatcher CLI snapshot imports a backlog query module outside the v013/v014 envelope.

This REVISED proposal changes only the bridge authorization envelope. It adds exactly the four backlog dependency paths named by Loyal Opposition:

| Newly added target path | Why it is required |
| --- | --- |
| `groundtruth-kb/src/groundtruth_kb/backlog/query.py` | `groundtruth-kb/src/groundtruth_kb/cli.py` imports `BacklogListQuery`, `BacklogQueryError`, `ProjectListQuery`, `SortKey`, `filter_projects`, and `filter_work_items` from this module at import time. |
| `groundtruth-kb/src/groundtruth_kb/backlog.py` | The verified snapshot changes the legacy backlog module alongside the new package query surface. |
| `groundtruth-kb/src/groundtruth_kb/backlog/__init__.py` | The verified snapshot changes the backlog package boundary required by the CLI import surface. |
| `groundtruth-kb/src/groundtruth_kb/backlog/approval_state.py` | The verified snapshot changes the sibling backlog approval-state module as part of the observed dependency closure. |

This revision does not authorize a broad `research` merge, an in-thread CLI lazy-import refactor, new application paths, `scripts/openrouter_harness.py`, Agent Red paths, deployment work, credential work, or retired poller/trigger restoration. If focused verification reveals any further source path outside this envelope, Prime Builder must fail closed with another blocker report.

## Dependency Audit Before Filing

Observed from `bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-015.md` and confirmed in the release worktree:

```text
ModuleNotFoundError: No module named 'groundtruth_kb.backlog.query'
groundtruth-kb/src/groundtruth_kb/cli.py imports from groundtruth_kb.backlog.query
...
The release worktree remains staged and uncommitted at `E:\GT-KB\.gtkb-state\release-worktrees\wi4943-dispatcher-release-20260701` on `codex/wi4943-dispatcher-release-main-20260701`. It must not be committed until a renewed GO, successful focused verification, and a post-implementation report.
...
- `DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001` - the revision cites governing dispatcher, bridge, project-authorization, and artifact specs.
...
- `GOV-PROJECT-IMPLEMENTATION-AUTHORIZATION-001` and `DCL-PROJECT-AUTHORIZATION-ENVELOPE-001` - implementation remains bounded by PAUTH, GO status, and target-path validation.
...
- `bridge/gtkb-wi4942-dispatch-drain-live-worker-parity-006.md` - terminal VERIFIED WI-4942 evidence.
...
- No new owner decision is required for LO to review this envelope correction. If LO determines the four backlog paths are not covered by existing PAUTH, the correct response is another NO-GO with the missing authorization.

## Revised Scope

The prior scope remains in force with one change: implementation may add/apply the four backlog dependency paths required for the verified CLI snapshot to import in the clean release worktree. Implementation must:
...
| --- | --- |
| `SPEC-DISPATCHER-CONTROL-SURFACE-001` | Run release-worktree `gt bridge dispatch health --json`, `status --json`, `daemon status --json`, and `drain --timeout 1 --dry-run --json`; run `platform_tests/groundtruth_kb/cli/test_bridge_config_cli.py`. |
...
- Applicability command: `python scripts/bridge_applicability_preflight.py --bridge-id gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation --content-file .tmp/bridge-revisions/wi4943-017.content.md --json`
- Applicability result: exit code 0; `preflight_passed=true`; `missing_required_specs=[]`; `missing_advisory_specs=[]`.
- Applicability packet hash: `sha256:359061da7c32d85115c81022f88d7751a7923902983a59d3e4a1a6fbcaf25d4d`.
- Clause command: `python scripts/adr_dcl_clause_preflight.py --bridge-id gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation --content-file .tmp/bridge-revisions/wi4943-017.content.md`
- Clause result: exit code 0; clauses evaluated 5; must_apply 4; evidence gaps in must_apply clauses 0; blocking gaps 0.

## Risk And Rollback

Primary risk is continued dependency discovery. Mitigation remains fail-closed behavior and exact target-path expansion only for verified dependency closure. Rollback is a revert of any eventual release-branch implementation commit plus append-only bridge follow-up. The staged release worktree must remain uncommitted until a renewed GO and successful verification.
