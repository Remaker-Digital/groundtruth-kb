...
# gtkb-wi3459-release-skill-adapter-hygiene - corrected revised target scope
...
Supersedes: bridge/gtkb-wi3459-release-skill-adapter-hygiene-005.md
...
This REVISED proposal answers `bridge/gtkb-wi3459-release-skill-adapter-hygiene-004.md` by expanding the target-path envelope for the already-approved WI-3459 hygiene work. It also supersedes `bridge/gtkb-wi3459-release-skill-adapter-hygiene-005.md`, which this same auto-dispatch filed before the target-path coverage preflight completed; `-005` remains in the audit trail and this `-006` entry is the corrected operative revision.

No implementation files are changed by this revision. It requests Loyal Opposition review of the corrected scope so Prime Builder can rerun the adapter generator and cleanup pass without crossing the implementation-start authorization boundary.
...
The previous GO was too narrow because it did not authorize the registry output and the currently observed cache cleanup targets. The corrected `target_paths` cover the generator source reference, deterministic Codex adapter output surface, manifest, registry output, currently observed helper cache files, original canonical scratch-file deletions, and focused helper-scratch regression test.

## Specification Links

- `GOV-FILE-BRIDGE-AUTHORITY-001` - protected skill adapter, registry, script, and test files require bridge GO plus implementation-start authorization.
...
Response: accepted. The previous proposal under-scoped the live generator outputs. This correction adds:
...
- `scripts/generate_codex_skill_adapters.py` so the advisory target-path coverage preflight is clean when the proposal names the generator command with `--update-registry`.
...
No generator source change is intended. If implementation discovers a new outside-scope mutation target after a fresh implementation-start packet, Prime Builder must stop and file another bridge revision rather than partially implementing.

## Proposed Implementation

1. Create a fresh implementation-start packet from the corrected GO, then rerun the existing adapter generator with registry update enabled to confirm the current update set.
2. Remove the stale canonical and Codex helper scratch/cache artifacts only within the corrected target envelope.
...
Candidate preflights must run against this exact revision file before live filing. The Codex bridge-propose helper path also runs credential scanning, author metadata insertion, bridge-compliance audit, and work-intent checks before writing `bridge/gtkb-wi3459-release-skill-adapter-hygiene-006.md`.

## Spec-Derived Verification Plan

| Spec / governing surface | Planned verification |
| --- | --- |
| `GOV-FILE-BRIDGE-AUTHORITY-001` | `groundtruth-kb/.venv/Scripts/python.exe scripts/bridge_claim_cli.py claim gtkb-wi3459-release-skill-adapter-hygiene --session-id 2026-06-28T04-18-22Z-prime-builder-A-ac276e --ttl-seconds 7200` and fresh implementation-start authorization after GO. |
...
| `GOV-STANDING-BACKLOG-001` / `GOV-PROJECT-IMPLEMENTATION-AUTHORIZATION-001` | The project/backlog/authorization commands above must show `WI-3459` open and included in an active PAUTH. |
...
| Target-path scope | `groundtruth-kb/.venv/Scripts/python.exe scripts/proposal_target_paths_coverage_preflight.py --content-file bridge/gtkb-wi3459-release-skill-adapter-hygiene-006.md --strict` must pass after filing. |
...
- The implementation report carries forward this corrected target-path scope and exact command evidence.

## Risk And Rollback

Risk remains low but the target envelope is intentionally broader than the original proposal because the generator owns a deterministic subtree and registry output. The implementation remains constrained by WI-3459, the corrected target list, and the implementation-start packet. Rollback is a single revert of the eventual implementation commit, which restores the pre-cleanup adapter drift and helper scratch state.
...
groundtruth-kb/.venv/Scripts/python.exe scripts/proposal_target_paths_coverage_preflight.py --content-file .gtkb-state/bridge-revisions/drafts/gtkb-wi3459-release-skill-adapter-hygiene-005.md --strict
```
