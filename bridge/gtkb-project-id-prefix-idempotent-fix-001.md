...
Reliability-fast-lane fix per the standing `PROJECT-GTKB-RELIABILITY-FIXES` convention: one function made idempotent, one focused regression test. The existing phantom projects + mis-filed memberships are a separate bulk-reconciliation concern, explicitly deferred (see Out of Scope).
...
The existing phantom `PROJECT-PROJECT-*` projects and their mis-filed memberships are a bulk data-reconciliation concern (retire the phantoms; ensure each affected work item is an active member of its real project). That is a bulk mutation touching multiple projects and membership rows; it is NOT performed by this proposal and is NOT covered by the reliability fast-lane. It remains captured in WI-3355's "Proposed follow-up" and will be a separate proposal. This code fix stops new drift; the cleanup reconciles existing drift.
...
This proposal changes ONE derivation function and adds ONE regression test. It performs no backlog mutation, no work_items insert/update/retire, no project create/retire, no membership row insert/delete, no authorization change, no bulk reconciliation. The phantom-cleanup bulk operation is explicitly deferred. Evidence tokens: "single function", "one regression test", "read-only w.r.t. existing rows", "code fix", "deferred follow-on".
...
No other path is authorized. No CLI change (the CLI does not prefix). No schema migration. No bulk membership mutation. No bridge/INDEX.md mutation by the implementation itself.
...
- WI-3355 - the root-cause diagnosis work item (orphan; documents the db.py defect site and the three fix options, with normalization as primary). Its phantom-cleanup follow-up remains the deferred bulk-reconciliation item.
- WI-3411 - the CLI-symptom work item (member of PROJECT-GTKB-RELIABILITY-FIXES); records the `gt projects add-item` workaround and 2026-05-27 instances (WI-3394, WI-3410).
...
- S356 (2026-05-16): WI-3353 reproduced the defect and produced Codex NO-GO F1 on `gtkb-governance-hook-worktree-root-resolution-002.md`; the S356 repair used the `link_project_work_item` workaround.
...
- Risk: scope creep into phantom cleanup. Mitigation: target_paths is two files; the impl-start packet fails closed outside it; cleanup is explicitly deferred.
...
3. Confirm deferring the phantom-cleanup bulk reconciliation to a separate proposal is the correct scope boundary.
