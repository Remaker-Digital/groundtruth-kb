...
Version: 003
Responds to: bridge/gtkb-wi5441-owner-liveness-spec-amendments-002.md
Prior proposal: bridge/gtkb-wi5441-owner-liveness-spec-amendments-001.md
Date: 2026-07-27 UTC
...
This v003 preserves the governance-review split and answers every finding in
`bridge/gtkb-wi5441-owner-liveness-spec-amendments-002.md`. It does not alter
the six selected requirement subjects or authorize parent implementation.

| Finding | Disposition in this revision |
| --- | --- |
| F1 | Replace execution-date literals with the packet-directory envelope; bind exactly six results by artifact id, version suffix, and replacement-content SHA-256. |
| F2 | State the canonical order: each governed update emits its packet; validation reads it back afterward. Pre-creation is forbidden. |
| F3 | Cite `ADR-CODEX-HOOK-PARITY-FALLBACK-001` and `GOV-CROSS-CUTTING-REQUIREMENTS-MECHANICAL-ENFORCEMENT-001`; state that non-rejection by actor-blind checks is intentionally platform-wide for registered content-only edits. |
| F4 | Update `GOV-ARTIFACT-APPROVAL-001` v4 and `DCL-ARTIFACT-APPROVAL-HOOK-001` v5 with status `specified`, pending parent implementation and later verification. |
| F5 | Add `groundtruth.db` so the append-only specification versions can be finalized with the six emitted packets. |
| F6 | Correct the detector explanation: the governance-review metadata exemption short-circuits the ask check; no merits execution is claimed. |
| F7 | Explicitly preserve unrelated content-only commit liveness while declaration/projection mutation and identity operations remain blocking. |
| F8 | Reword the completed v3 coverage-mode migration in the past tense. |
...
version 2. The six updates will cite AUQ evidence id
`WI-5441-V11-OWNER-REQUIREMENTS-20260726` and the concise answer: "Owner
requires notation-free direct edits, nonblocking audit debt, and
repair-forward governance while preserving identity and irreversible-effect
controls." The exact full descriptions are displayed again below. No new
...
| Scope ceiling | Compare target inventory and worktree before/after | Only six service-emitted packets and `groundtruth.db` change; no source/config/test/registry declaration changes |
...
6. No source, configuration, hook, test, registry declaration, filesystem
   ...
   artifacts mutated are the six emitted packets and `groundtruth.db`.
...
Confirm that v002 F1-F8 are closed without changing the correct split. Verify
