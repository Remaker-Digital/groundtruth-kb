...
Archive and remove the terminal VERIFIED artifact for `gtkb-research-clean-branch-publication` because the current per-thread repair planner cannot prove a finalizer-safe implementation scope for that terminal verdict.

Planner reason: latest VERIFIED verdict has no Responds to report reference.

Source work items observed by planner: `WI-5318, WI-5403`.
...
All target paths are inside `E:\GT-KB`: `bridge/gtkb-research-clean-branch-publication-004.md` and `independent-progress-assessments/WI-5370-gtkb-research-clean-branch-publication-004.no-responds-terminal.md`.
...
- Reconfirm `bridge/gtkb-research-clean-branch-publication-004.md` is still the latest terminal VERIFIED artifact for `gtkb-research-clean-branch-publication` and is still classified as `terminal_verified_blocked_missing_scope` by `scripts/per_thread_finalization_repair.py`.
- Archive the current live bytes (1621 bytes, SHA-256 `2B27F19465A5BB11DE0FAED6EAF0F1EB4E532CD442B10355516379D11646E467`, Git blob `260becdf8b0c3f6820b46ae0342dfa093fa1535f`) to `independent-progress-assessments/WI-5370-gtkb-research-clean-branch-publication-004.no-responds-terminal.md`, verify byte/hash/blob equality, and remove only `bridge/gtkb-research-clean-branch-publication-004.md`.
...
| Spec | Verification |
| --- | --- |
| `GOV-WORK-TREE-HYGIENE-001` | Run scoped `git status --short -- bridge/gtkb-research-clean-branch-publication-004.md independent-progress-assessments/WI-5370-gtkb-research-clean-branch-publication-004.no-responds-terminal.md` before and after; confirm no staged/index changes. |
| `GOV-FILE-BRIDGE-AUTHORITY-001` | Run `gt bridge show gtkb-research-clean-branch-publication --json --compact` before and after; confirm only the exact terminal file is removed from the live chain. |
...
- `gt bridge show gtkb-research-clean-branch-publication --json --compact` no longer reports `bridge/gtkb-research-clean-branch-publication-004.md` as the latest path after removal.
...
Risk is moderate because removing a terminal bridge artifact changes the live chain. The byte-identity guard and independent LO GO are mandatory. Rollback, before verification, is to restore only the archived bytes to `bridge/gtkb-research-clean-branch-publication-004.md` after revalidating the same SHA-256 and blob. Bridge files and reports remain append-only audit artifacts.
