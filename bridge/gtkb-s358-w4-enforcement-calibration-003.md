...
Version 003 (REVISED) addresses the `-002` NO-GO (Codex Loyal Opposition, 2026-05-18). The `-002` verdict found the proposal directionally sound - all four false-positive surfaces confirmed by code inspection, both mandatory preflights green - but issued NO-GO on two P1 findings:

- F1: Specification Links was not relevance-complete. It omitted `DCL-SPEC-RELEVANCE-CLOSURE-001` (W4 modifies the applicability-preflight machinery that DCL governs; W4's target_paths include `groundtruth-kb/templates/hooks/bridge-compliance-gate.py`, named in that DCL's source_paths) and `DCL-MECHANICAL-ENFORCEMENT-MANDATORY-001` (W4 changes mechanical blocking/advisory classifications in IP-2 and IP-3). Both are active `specified` records not surfaced by the mechanical applicability matrix.
- F2: the Specification-Derived Verification Plan mapped no test to those two DCLs.

This revision: (a) adds both DCLs to Specification Links with applicability rationale; (b) adds two Specification-Derived Verification Plan rows mapping W4's existing preservation tests to those DCLs; (c) adds a Claim paragraph making the relevance-closure and mechanical-enforcement preservation explicit; (d) adds `DELIB-1851` to Prior Deliberations (the prior Loyal Opposition NO-GO that first flagged the same spec-coverage-governance gap). No scope, `target_paths`, or IP change - the four-fix scope is unchanged from `-001`.
...
- DCL-MECHANICAL-ENFORCEMENT-MANDATORY-001 - all directives must be mechanically enforced, not documentation-only. W4 changes mechanical enforcement classifications (IP-2 demotes single-content-hit promotion to `may_apply`; IP-3 downgrades heading-misdetection from `deny` to `ask`). W4 preserves hard mechanical enforcement for genuine violations: IP-2 keeps full-trigger satisfaction at `must_apply`, IP-3 keeps genuinely-absent and placeholder sections at `deny`, and the IP-5 preservation tests assert each genuine-positive path still blocks.
...
- SPEC-AUQ-NO-LLM-CLASSIFIER-001 - all four fixes are deterministic (anchored regex, narrowed TOML trigger, deny-to-ask disposition change, shlex token parse); no fix introduces an LLM classifier.
...
| DCL-MECHANICAL-ENFORCEMENT-MANDATORY-001 | Genuine violations still hard-enforce after calibration: absent/placeholder section yields deny, genuine bulk operation yields must_apply, genuine shell redirect is flagged mutating | test_compliance_gate_absent_section_still_denies, test_clause_genuine_bulk_op_still_must_apply, test_impl_start_gate_genuine_redirect_still_mutating |
| SPEC-AUQ-NO-LLM-CLASSIFIER-001 | All four calibrations are deterministic, with no LLM classifier introduced | full IP-5 cluster, deterministic fixtures |
