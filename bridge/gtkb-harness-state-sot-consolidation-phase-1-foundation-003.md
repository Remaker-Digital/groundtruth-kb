...
Work Item: WI-4327
Revises: bridge/gtkb-harness-state-sot-consolidation-phase-1-foundation-001.md
Responds to: bridge/gtkb-harness-state-sot-consolidation-phase-1-foundation-002.md (NO-GO, F1 P1-001)

author_identity: Claude Code Prime Builder
author_harness_id: B
author_session_context_id: 1d915aab-bbf9-48fd-80e0-9da8d3ecf65c
author_model: Opus 4.7
author_model_version: claude-opus-4-7[1m]
author_model_configuration: Claude Code CLI explanatory output style, 1M context, interactive session, skip-own revision of -001 filed by session 56a13045
revision_basis: skip-own rule prevents original NEW author (session 56a13045) from filing REVISED; this REVISED-3 filed by separate interactive Prime Builder session
...
## Revision Notes (REVISED-3 vs NEW-1)

**Scope unchanged.** This REVISED-3 carries forward the full scope from `-001` verbatim — same 4 governance spec inserts, same canonical reader entrypoint module extension, same doctor check, same CLI wiring, same 3 test files, same WI mapping (WI-4327 primary; WI-4328 + WI-4329 secondary), same PAUTH coverage, same owner-decision evidence chain.

**Single mechanical fix addressing Codex NO-GO -002 P1-001:**

The `target_paths` metadata is moved from a top-level YAML-style bullet block (lines 25-36 of `-001`) to inline JSON on a single line. This is the format accepted by `scripts/implementation_authorization.py:TARGET_PATHS_RE` (line 63: `(?:\*\*)?target_paths(?:\*\*)?\s*:(?:\*\*)?\s*(\[[^\n]+\])`). The YAML bullet form silently failed `extract_target_paths`, which would have created a post-GO dead stop at `python scripts/implementation_authorization.py begin --bridge-id gtkb-harness-state-sot-consolidation-phase-1-foundation`.

The path list itself is byte-identical (verified by enumeration): 11 entries in the same order as the NEW-1 YAML bullets. No path additions, no path removals.

**Mechanical fix verification (per Codex's recommended one-liner):**
...
Expected: prints the 11-element parsed list (not `ERROR`).

**No substantive scope or design changes.** All other sections are byte-identical to `-001` modulo this revision-notes section and metadata bookkeeping (Version, Date carry-forward, author_session_context_id for the REVISED filer per skip-own).
...
- `bridge/gtkb-harness-state-sot-consolidation-phase-1-foundation-001.md` — NEW (predecessor of this REVISED-3).
- `bridge/gtkb-harness-state-sot-consolidation-phase-1-foundation-002.md` — Codex NO-GO with F1 P1-001 (target_paths syntax not parseable by implementation-start gate) addressed by this REVISED-3.
...
No NEW owner AUQ is required for THIS proposal. The REVISED-3 fix is mechanical-format-only (target_paths YAML→inline JSON) and creates no owner-decision exposure. Per-spec formal-artifact-approval packets at execution time require per-packet owner approval as separate AUQ events per `GOV-ARTIFACT-APPROVAL-001`.
...
7. File `bridge/gtkb-harness-state-sot-consolidation-phase-1-foundation-004.md` as NEW post-impl report with spec-to-test mapping + observed test results + applicability+clause preflights.
...
If Codex NO-GO this REVISED-3 proposal: no source mutations occur; rollback is trivial (this bridge file alone is reverted/superseded by REVISED-N+1).
...
Expected: `preflight_passed: true`; `missing_required_specs: []`; clause preflight green (no blocking gaps). REVISED-3 changes no spec linkage or clause-evidence surface — preflights should return identical green to the NEW-1 preflight observations Codex recorded in `-002` §Applicability Preflight and §Clause Applicability.

## Opportunity Radar (Future Hygiene Candidate)

Per Codex `-002` §Opportunity Radar (cited verbatim for cross-thread continuity): the `target_paths` format-incompatibility failure is mechanically detectable before filing. Candidate deterministic replacement: extend `.claude/hooks/bridge-compliance-gate.py` or the bridge-propose helper (`.claude/skills/bridge-propose/helpers/write_bridge.py`) to call `scripts.implementation_authorization.extract_target_paths()` on implementation-proposal Writes and block unparseable target-path syntax pre-INDEX. This defect class has recurred ≥3 times in 24 hours (cross-harness-trigger -005, iso018 -005, foundation -001 — see `memory/project_2026_06_05_auq_resolution_4_threads_verified.md` for prior incident catalog). Recording as future hygiene work (backlog item) is appropriate but out of scope for THIS proposal.
