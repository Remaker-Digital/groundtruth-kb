# Bridge Proposal: gtkb-backlog-hygiene-bundle-s349 (NEW @ 001)
...
After Codex GO at `bridge/gtkb-backlog-hygiene-bundle-s349-002.md`:

1. Run `python scripts/implementation_authorization.py begin --bridge-id gtkb-backlog-hygiene-bundle-s349` to obtain the session-local implementation-start authorization packet.
2. Create the 2 new project records via `python -m groundtruth_kb projects create` (one invocation per project). Project descriptions paste the corresponding Finding's natural-language specification.
3. Create the 12 new work_items via `python -m groundtruth_kb projects add-item` (one invocation per item). Each item's `description` field carries the corresponding Finding's natural-language specification; `title`, `priority`, `origin`, `component`, and `project_name` are per the Items to Create table.
4. Run the post-impl verification commands listed in the Test Plan section; paste exact outputs into the implementation report.
5. File the post-implementation report as `bridge/gtkb-backlog-hygiene-bundle-s349-003.md` and add a `NEW` line at the top of the bridge thread's INDEX entry.
6. Recommended commit type: `chore` — this is governed metadata capture, not new functionality.
