...
target_paths: ["groundtruth-kb/src/groundtruth_kb/cli.py", "tests/groundtruth_kb/test_cli_discoverability.py", "tests/groundtruth_kb/__init__.py"]
...
Implementation surface is one source file (`cli.py`) plus one new test module. No new modules, no schema change, no MemBase mutation. All reads.
...
- ADR-ISOLATION-APPLICATION-PLACEMENT-001 (CLI code is platform code; lives in `groundtruth-kb/src/groundtruth_kb/`)
...
- No prior bridge thread named `gtkb-cli-discoverability-001` or `gtkb-discoverability-*` exists; the acceptance summary referenced a future slug, not a prior round.
- No prior DA entry rejects either `--json` flag elevation for doctor or a `backlog show` verb; the field is clear.
...
All edits in `groundtruth-kb/src/groundtruth_kb/cli.py`; one new test module under `tests/groundtruth_kb/`.
...
New file `tests/groundtruth_kb/test_cli_discoverability.py`. Uses `click.testing.CliRunner`, the existing `_open_db`/`_resolve_config` machinery, and a temp-dir fixture seeded with a minimal MemBase via `KnowledgeDB.create_schema()` + a synthetic `WI-TEST-0001` insert.

No changes to existing tests. No changes to plugin/skill manifests. No new dependencies.

## Test Mapping

Spec-derived tests landed in `tests/groundtruth_kb/test_cli_discoverability.py`:
...
AC9. All 10 tests in T1-T10 pass on a clean checkout.
AC10. Existing test suites (`pytest tests/groundtruth_kb/`) continue to pass — no regression.

## Verification Plan

Commands executed during verification (results captured in the post-implementation report):

1. `python -m pytest tests/groundtruth_kb/test_cli_discoverability.py -v` — expect all 10 tests pass.
2. `python -m pytest tests/groundtruth_kb/ -q` — expect no regression in the existing CLI test suite.
3. `python -m groundtruth_kb project doctor --dir . --json | python -m json.tool > /dev/null && echo OK` — proves JSON is parseable. (On Windows, equivalent PowerShell pipe.)
4. `python -m groundtruth_kb backlog show WI-3262` — expect human-readable record.
5. `python -m groundtruth_kb backlog show WI-3262 --json | python -m json.tool > /dev/null && echo OK` — proves work-item JSON is parseable.
6. `python -m groundtruth_kb backlog show WI-9999 --json` — expect exit code 1 + error message.
7. `python scripts/bridge_applicability_preflight.py --bridge-id gtkb-discoverability-cli-slice-1` — re-run after INDEX entry exists (post-GO); expect `preflight_passed: true`.
...
This proposal is filed at `bridge/gtkb-discoverability-cli-slice-1-001.md` per `.claude/rules/file-bridge-protocol.md` File Naming convention. The corresponding `bridge/INDEX.md` entry will be inserted at the top of the index as:
...
Per session direction, the INDEX update is performed in a separate step (the parent session is filing multiple priority proposals in parallel; INDEX consolidation happens after the proposal batch is drafted). No prior version of this thread exists; no prior version is being rewritten or deleted. The bridge file is append-only; subsequent versions (`-002.md`, `-003.md`, …) will be added as Codex review and revisions land.

## Applicability Preflight

Pre-filing run was executed in `--content-file` mode because the INDEX entry will be inserted at the top of `bridge/INDEX.md` after this draft is filed (per `.claude/rules/file-bridge-protocol.md` § Mandatory Pre-Filing Preflight Subsection catch-22 guidance). Embedded output:
...
The pre-INDEX preflight uses `--content-file <abs path to this draft>`. The full preflight run with INDEX entry will be performed by Loyal Opposition during review per `.claude/rules/codex-review-gate.md`.
