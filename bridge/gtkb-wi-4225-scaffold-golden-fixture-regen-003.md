...
Date: 2026-06-03 UTC
Responds to: bridge/gtkb-wi-4225-scaffold-golden-fixture-regen-002.md (NO-GO)
Supersedes: bridge/gtkb-wi-4225-scaffold-golden-fixture-regen-001.md (NEW)
...
# WI-4225 — Regenerate scaffold golden fixtures to absorb VERIFIED template drift (REVISED-1)

## Revision Summary (vs -001; addresses NO-GO -002 F1)

- **F1 (P1) corrected.** The -001 statement "WI-4225 has no project/PAUTH" was a
  factual error: it inferred "no project" from a null `project` field on the
  `list_work_items()` row, but membership/PAUTH live in separate tables. A fresh
  live read confirms (matching Codex's evidence):
  - active membership `PWM-PROJECT-GTKB-RELIABILITY-FIXES-WI-4225` (WI-4225 in
    `PROJECT-GTKB-RELIABILITY-FIXES`, attached S388);
  - active authorization `PAUTH-WI-4225-REGISTRY-SCAFFOLD-FIXTURE-DRIFT-001`,
    `included_work_item_ids=["WI-4225"]`, `allowed_mutation_classes=["source",
    "test_modification", "test_fixture_update"]`, no expiry, scope "registry/scaffold
    drift repair … scaffold golden fixture trees".
  The `test_fixture_update` mutation class exactly bounds this golden-fixture regen.
  The three project-linkage metadata lines are now present above; the false statement
  is removed.
- **Sequencing gate cleared.** WI-4279 is now latest `VERIFIED` at
  `bridge/gtkb-wi-4279-scaffold-phantom-spec-citation-repoint-004.md` and committed
  (`c4e7dfd3 docs(bridge): verify WI-4279 scaffold citation repoint`). The phantom is
  absent from the template + both goldens in the committed tree.
- Spec-derived verification plan, drift inventory, and Owner Decisions / Input are
  preserved unchanged.
...
(`test_fixture_update`). Three committed byte-equality golden-master tests are RED on
`develop` because the scaffold *templates* moved forward through VERIFIED bridge threads
while the committed golden fixtures were never recaptured:
...
| 2 | `.claude/rules/canonical-terminology.md` | forward | `79df6c13` / same | `bridge` entry statuses `…VERIFIED, ADVISORY.` → `…VERIFIED, ADVISORY, DEFERRED, WITHDRAWN.` (phantom line already corrected by VERIFIED WI-4279) |
...
surface, no runtime behavior change, no KB mutation. The single mutation class
exercised is `test_fixture_update`, which is **within** the active
`PAUTH-WI-4225-REGISTRY-SCAFFOLD-FIXTURE-DRIFT-001` envelope cited in the metadata
block above. `governance_review` is metadata-exempt under `bridge-compliance-gate`
`BRIDGE_KIND_METADATA_EXEMPT`, but the live project-linkage metadata is supplied
anyway for accurate traceability and so the implementation-start packet binds to the
correct project-authorization envelope.
...
The bridge-compliance-gate emits a non-blocking collision warning for these; it is
...
- **AUQ Q2 — Sequencing vs WI-4279 → Option 1 ("Sequence after WI-4279 lands").**
  ...
  `_capture_scaffold_golden.py` run on WI-4279 being committed + VERIFIED. **As of this
  revision that gate is CLEARED** (see Implementation Sequencing below).
...
**Live status (2026-06-03):** WI-4279 is latest **`VERIFIED` at
`bridge/gtkb-wi-4279-scaffold-phantom-spec-citation-repoint-004.md`** and committed
(`c4e7dfd3`). `rg -uu --hidden "GOV-CHAT-DERIVED-SPEC-APPROVAL-001"
groundtruth-kb/templates/rules/canonical-terminology.md` returns **no hits**; both
golden `canonical-terminology.md` files likewise. The precondition is therefore
**satisfied** — the regen will reproduce the corrected token automatically, and
WI-4279's golden edits are a strict subset of this regen's output.

## Exact Implementation (post-GO)

1. Confirm precondition still holds: `rg -uu "GOV-CHAT-DERIVED-SPEC-APPROVAL-001" groundtruth-kb/templates/rules/canonical-terminology.md` returns no hits.
2. `python scripts/implementation_authorization.py begin --bridge-id gtkb-wi-4225-scaffold-golden-fixture-regen` (impl-start packet bound to `PAUTH-WI-4225-REGISTRY-SCAFFOLD-FIXTURE-DRIFT-001`, scoped to the golden dirs).
...
- `GOV-PROJECT-IMPLEMENTATION-AUTHORIZATION-001` / `DCL-PROJECT-AUTHORIZATION-ENVELOPE-001` — the project-authorization envelope governing `PAUTH-WI-4225-REGISTRY-SCAFFOLD-FIXTURE-DRIFT-001`; the `test_fixture_update` mutation class authorizes this regen.
...
- `bridge/gtkb-wi-4225-scaffold-golden-fixture-regen-002.md` (NO-GO) — the verdict this revision answers: F1 required adding the live project/PAUTH metadata and updating the WI-4279 state. Both done.
- `bridge/gtkb-wi-4279-scaffold-phantom-spec-citation-repoint-004.md` (**VERIFIED**) — predecessor scaffold-citation fix; clears this thread's sequencing precondition.
- `gtkb-deferred-authority-protocol-alignment` (chain -001…-011, VERIFIED) — produced drift files #1–#7; `79df6c13`.
- `gtkb-session-id-shared-resolver-unification` (VERIFIED, WI-4270) — produced drift files #8–#9; `f8d74257`.
...
| Mutation stays within PAUTH envelope | `test_fixture_update` only; no source/scaffold.py change | `git diff --name-only` shows only `scaffold_golden/**` paths | within `PAUTH-WI-4225-REGISTRY-SCAFFOLD-FIXTURE-DRIFT-001` |

## Risk / Rollback

Low. Test-fixture data regeneration only (`test_fixture_update`, within the active
PAUTH); no source/`scaffold.py` change (Option A); no runtime behavior change; no KB
mutation. Every absorbed byte traces to a VERIFIED/landed source change. The single
removal (3 golden INDEX rows) is owner-AUQ approved. Rollback is a single-commit revert
of the `scaffold_golden/` tree. The WI-4279 precondition (now VERIFIED+committed)
prevents re-blessing the phantom. The capture sandbox is in-root and auto-removed.
...
Filed under `bridge/` as `REVISED` at the top of the existing
`gtkb-wi-4225-scaffold-golden-fixture-regen` document version list in `bridge/INDEX.md`;
append-only — no prior entry deleted or rewritten (NEW -001 and NO-GO -002 preserved).
`bridge/INDEX.md` remains canonical per `GOV-FILE-BRIDGE-AUTHORITY-001`.
