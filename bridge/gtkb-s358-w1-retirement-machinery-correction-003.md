...
target_paths: ["groundtruth.db", "groundtruth-kb/src/groundtruth_kb/project/lifecycle.py", "scripts/project_verified_completion_scanner.py", ".claude/hooks/project-completion-surface.py", ".codex/gtkb-hooks/project-completion-surface.py", ".claude/settings.json", ".codex/hooks.json", "groundtruth-kb/src/groundtruth_kb/cli.py", "groundtruth-kb/tests/test_project_artifacts.py", "platform_tests/hooks/test_project_completion_surface.py", ".groundtruth/formal-artifact-approvals/*-gov-project-verified-completion-retirement-001.json", ".groundtruth/formal-artifact-approvals/*-delib-s358-s350-manufactured-variant-provenance.json"]

## Revision Note

Version 003 (REVISED) supersedes the `-002` NO-GO. The `-002` review confirmed the W1 correction is directionally consistent with GOV-PROJECT-VERIFIED-COMPLETION-RETIREMENT-001 v2, that both mandatory preflights pass, and that the owner-decision evidence is sound. It raised two findings:

- FINDING-F1 (P1): the `-001` target_paths listed source, script, hook, config, and test files only, but IP-6 (retire PROJECT-GTKB-LO-OPPORTUNITY-RADAR), IP-7 (insert GOV-PROJECT-VERIFIED-COMPLETION-RETIREMENT-001 v3), and IP-8 (insert the provenance deliberation) all mutate MemBase and write formal-artifact-approval packets - protected work the implementation-start gate would block because `groundtruth.db` and the packet paths were outside the declared scope. The `-001` text wrongly stated IP-7 and IP-8 "are not in target_paths."
- FINDING-F2 (P2): the verification plan left the CLI subcommand test placement ambiguous ("test_project_artifacts.py or the CLI test module"), with no CLI test-module path authorized.

This revision fixes both. F1: `target_paths` now includes `groundtruth.db` (covering the IP-6 project-retirement mutation and the IP-7 / IP-8 MemBase inserts) plus two narrowly-scoped formal-artifact-approval-packet globs, one for the GOV v3 and one for the provenance deliberation. F2: the CLI subcommand test is constrained to the already-listed `groundtruth-kb/tests/test_project_artifacts.py`; no separate CLI test module is used. IP-1 through IP-8 scope, the Specification Links, the Prior Deliberations, the Owner Decisions / Input section, and the verification-plan strengths are otherwise unchanged from `-001`.
...
- DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001 - this proposal carries a complete, relevance-closed Specification Links section and, per the -002 NO-GO, declares every protected mutation surface in target_paths.
...
A REVISED entry for gtkb-s358-w1-retirement-machinery-correction pointing at this -003 file is inserted at the top of that document's version list in the bridge index, above the NO-GO: -002 and NEW: -001 lines. No prior bridge file and no prior index entry is deleted or rewritten; -001 and -002 remain on disk and in the index as the append-only audit trail.
...
In groundtruth-kb/tests/test_project_artifacts.py, delete the tests that assert the owner-confirmation gate behavior (the v1 model) and re-signature the completion/retirement tests to the stripped function signature and the membership-link gating set. In platform_tests/hooks/test_project_completion_surface.py, update the hook tests to the repurposed automatic-transition-plus-notification behavior. New tests cover the automatic-transition path, the membership-link gating set, and the CLI subcommand; the CLI subcommand test is placed in groundtruth-kb/tests/test_project_artifacts.py (the authorized test target), not a separate CLI test module. The exact count of deleted vs. re-signatured tests is settled in implementation; the per-IP spec-derived test mapping is in the verification plan below.
