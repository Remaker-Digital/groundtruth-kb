...
# Bridge-compliance-gate test hygiene + `_pending_proposal_ask_reason` whole-bridge-dir scan hang fix (REVISED-1)
...
The `-002` NO-GO requested no source or test change. The reviewer's Positive Confirmations validated the design ("hook performance improvements are decision-preserving and do not alter the gate's core logic"; "unit-level coverage replacement is clean and correct"). The NO-GO was solely a clause-preflight gate: two `must_apply` clauses lacked positive evidence —

1. `ADR-ISOLATION-APPLICATION-PLACEMENT-001/CLAUSE-IN-ROOT` — the proposal discussed the bridge directory using the forward-slash form, which did not match the clause's in-root evidence pattern.
2. `GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS` — triggered by an incidental mention of the work backlog; this fix is a single-work-item defect, not a bulk backlog operation.

This REVISED-1 adds the `## Clause Applicability Evidence` section below and rewords the incidental backlog phrase. No change to the proposed source, tests, design, or scope.

## Clause Applicability Evidence

- **`ADR-ISOLATION-APPLICATION-PLACEMENT-001/CLAUSE-IN-ROOT`**: All four target paths are in-root, under `E:\GT-KB`: `platform_tests/hooks/test_bridge_compliance_gate_index_exemption.py`, `platform_tests/hooks/test_bridge_compliance_gate_w4_calibration.py`, `.claude/hooks/bridge-compliance-gate.py`, and `groundtruth-kb/templates/hooks/bridge-compliance-gate.py`. The bridge thread files reside under `E:\GT-KB\bridge\`. The only generated runtime artifact (the optional pending-proposal cache from change B.3) is written in-root under `.gtkb-state/bridge-compliance/`. No artifact is created, read as a live dependency, updated, or required outside `E:\GT-KB`.
- **`GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS`**: This is a single-work-item (WI-4746) defect fix, not a bulk backlog operation; it performs no bulk work-item transition and touches no backlog records. No bulk-operation inventory or review-packet is required. Visibility is provided by the owner-decision deliberation (DELIB-20260621-BRIDGE-COMPLIANCE-GATE-TEST-HYGIENE-HANG-FIX-SCOPE) and this proposal's own review packet.
...
- DCL-ARTIFACT-LIFECYCLE-TRIGGERS-001 (advisory) — artifact lifecycle triggers; the defect is tracked as WI-4746 in MemBase work_items with verification artifacts.
...
- `bridge/gtkb-bridge-compliance-gate-test-hygiene-pending-scan-hang-fix-002.md` (NO-GO) — the clause-preflight NO-GO this REVISED-1 responds to.
