...
# WI-5200..5202 — Narrow replacement: generous cloud-harness recovery, runtime envelopes, and truthful H parity
...
This is the narrow replacement for the broad proposal and GO at `bridge/gtkb-wi5200-5202-generous-harness-repair-001.md` / `-002.md`. The mandatory implementation-start gate quarantined that GO because its `groundtruth.db` target overlapped the still-live WI-5199 H-proof report. Prime Builder filed `NO-ACTION` at `-003.md` before any protected implementation edit. This replacement removes both `groundtruth.db` and `harness-state/harness-registry.json`: the repair can determine durable dispatch capability from existing headless invocation metadata, while later H eligibility mutations remain in the already-authorized WI-5199 proof lifecycle.
...
4. Add H to the shared API capability surfaces and make Phase 2 provider/runtime evidence data-driven enough to recognize its underscore-named adapter, shared bridge skills, native-full hook/guard path, universal dispatcher no-window wrapper, and the existing headless invocation as durable receive capability distinct from current eligibility.
5. Preserve unrelated pre-existing hunks in `.api-harness/routing.toml` and `config/agent-control/harness-capability-registry.toml`. This repair performs no MemBase or generated registry mutation.
...
| `SPEC-DISPATCHER-CONTROL-SURFACE-001` | Read-only `gt bridge dispatch status/health --json` for the repair; later WI-5199 proof uses governed config transactions | The repair performs no registry mutation; the proof restores final B eligible/H ineligible state through the existing authorized thread. |
...
Longer envelopes increase the time a genuinely non-progressing worker can remain alive. The implementation preserves semantic no-progress detection, per-operation transport limits, guard failures, dispatcher concurrency caps, telemetry, and circuit-breaker evidence; it removes only impatience-based false failure. Blank responses can consume the generous overall envelope, but cannot bypass the session deadline or turn cap. A focused single commit permits rollback of source/test/config changes. No registry, credential, or deployment state changes.

## Bridge Filing

This proposal is filed under `bridge/` as the first status-bearing bridge file for `gtkb-wi5200-5202-generous-harness-repair-narrow`; the quarantined broad thread remains append-only and latest `NO-ACTION`. Dispatcher/TAFE state plus the numbered file chains are the live workflow state per `GOV-FILE-BRIDGE-AUTHORITY-001`.
