...
WI-4395 (P1, `command-surface`, origin=defect): last-24h Codex transcripts repeatedly showed `uv` failing before GT-KB commands could run — `Failed to initialize cache at <user-profile uv cache>; Cannot create a file when that file already exists` (the default cache resolves under `%LOCALAPPDATA%\uv\cache`). The default user-profile uv cache lives outside the repo, is subject to host/ACL/Drive-sync races, and is denied to the harness for direct inspection.

## Revision Note (v002)

v002 is a proactive self-correction filed before any Loyal Opposition review of v001. The v001 Summary quoted the original error string with its literal user-profile path, which the `ADR-ISOLATION-APPLICATION-PLACEMENT-001/CLAUSE-IN-ROOT` clause-preflight `failure_pattern` flagged as an out-of-root path reference (a false positive — it was an error-quote, not an output path). v002 paraphrases that diagnostic path to the `%LOCALAPPDATA%` env-var form so the clause preflight reports zero blocking gaps. No scope, design, target-path, or verification change otherwise.
