...
Version: 003
Responds to: bridge/gtkb-wi5441-registry-control-plane-reverse-coverage-002.md
Date: 2026-07-25 UTC
...
- `bridge/gtkb-wi5279-strict-lifecycle-fixture-recovery-v2-005.md` and the
  WI-5178 operation-time authority chain - governed ownership of the five
  residual work-intent enforcement failures excluded from this proposal.
...
- Add a dedicated `gt backlog update --reopen-terminal --owner-approved` path.
  It applies only when the current stage is `resolved`, the new
  `resolution_status` is nonterminal, a nonterminal target stage is explicit,
  and an owner-approved reason plus canonical bridge linkage are provided.
- The operation appends a new work-item version and event; it never rewrites or
  deletes historical rows. Ordinary backward stage transitions remain invalid.
- Apply it to WI-5441, then advance through the normal existing transition to
  `stage=implementing`. The implementation report must show open/implementing
  state, all canonical thread links including v003 and its verdict, and preserved
  historical versions.
...
| Work-item reopen | `groundtruth-kb/tests/test_backlog_update_cli.py` terminal reopen tests | Owner-approved exact reopen appends; missing approval/reason/linkage and ordinary backward transitions deny |
...
8. WI-5441 ends implementation at open/in-progress and `stage=implementing`
   with canonical bridge linkage; no historical row or bridge artifact is
   rewritten.
9. Strict bridge fixture metadata is corrected without relaxing the resolver.
   The five WI-5178 residual failures are disclosed and excluded; all new
   registry-specific selectors pass.
...
Review this v003 replacement scope finding-by-finding, especially the universal
...
and governed WI lifecycle repair. File GO or NO-GO as v004 from the independent
interactive review session. Do not grant implementation authority outside the
