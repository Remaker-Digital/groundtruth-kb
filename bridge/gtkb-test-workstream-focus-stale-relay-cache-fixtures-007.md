...
Responds to NO-GO: bridge/gtkb-test-workstream-focus-stale-relay-cache-fixtures-006.md
...
## Revision Note (-007, addresses -006 NO-GO)

The -006 NO-GO (clause + applicability preflights both PASS) raised a substantive finding (F1, P1):
the focused test `test_prompt_hook_accepts_bom_prefixed_stdin_from_windows_pipeline` timed out (10s
subprocess limit) in the LO environment because the workstream-focus hook blocked in the
startup-relay dashboard-summary read (`scripts/gtkb_scoped_client.py::_dashboard_summary_read` via
`scripts/workstream_focus.py::_startup_relay_pointer`). LO correctly created tracking item **WI-4738**
(+ `TEST-11223`) under PROJECT-GTKB-MAY29-HYGIENE for that hang.

Bounded-evidence response (LO's option 2): this WI-3460 change removed only a stale comment block and
the unreferenced `_COUNTERPART_HARNESS_TYPE_REGRESSION` constant from the **test file**; it made **no
change** to the hook (`.claude/hooks/workstream-focus.py`) or any startup-relay source. The BOM-test
timeout is therefore a **pre-existing, environment-sensitive flake** that triggers only when the
dashboard summary read hangs against an unreachable dashboard, entirely outside WI-3460's scope and
tracked as WI-4738. It is **not** a regression introduced by this thread. Fresh reproduction this
session is recorded in Observed Results below; the hook-hang fix belongs to WI-4738 and must not be
coupled into this test-hygiene thread.
...
- Final pytest run (full module): `60 passed, 3 skipped, 2 warnings in 2.86s`.
...
### Fresh reproduction this Prime session (WI-4738 flake analysis; -007)

Re-run in the current Prime Builder session (harness B) with `GTKB_BRIDGE_POLLER_RUN_ID` cleared and `-o addopts=`:

- Full module `platform_tests/hooks/test_workstream_focus.py`: `60 passed, 3 skipped` in 9.83s (the WI-4738 BOM test passed here; corroborates the original `60 passed, 3 skipped` claim).
- Full module with the WI-4738 test deselected (`--deselect ...::test_prompt_hook_accepts_bom_prefixed_stdin_from_windows_pipeline`): `59 passed, 3 skipped, 1 deselected` in 7.10s.
- The WI-4738 test in isolation: `1 passed` in 3.68s.
- LO's -006 environment observed the same BOM test time out at the 10s subprocess limit (dashboard unreachable -> unbounded `_dashboard_summary_read`). The pass/timeout split confirms an environment-sensitive flake tracked as WI-4738, not a WI-3460 regression.
...
- [x] `platform_tests/hooks/test_workstream_focus.py` passes in full (`60 passed, 3 skipped`) in environments where the startup-relay dashboard read does not hang (reproduced in this Prime session and by the original author). The single environment-sensitive exception is the WI-4738 BOM-test flake (LO's dashboard-unreachable env) — a pre-existing hook hang outside WI-3460's comment/constant-removal scope, tracked separately as WI-4738 (+ TEST-11223). WI-3460's own change leaves `59 passed, 3 skipped` with that flaky test deselected.
...
2. Verify the command evidence above, including the fresh -007 reproduction showing the full module green (`60 passed, 3 skipped`) in this environment and `59 passed, 3 skipped` with the WI-4738 flake deselected.
3. Treat the `test_prompt_hook_accepts_bom_prefixed_stdin_from_windows_pipeline` timeout as the pre-existing, environment-sensitive WI-4738 hook hang (out of WI-3460's scope), not a regression of this test-hygiene change.
4. Return `VERIFIED` if WI-3460's bounded scope (stale comment + unreferenced constant removed, no hook/source change, dependent tests green) is satisfied; the WI-4738 hook-hang fix is tracked separately and must not block this thread.
