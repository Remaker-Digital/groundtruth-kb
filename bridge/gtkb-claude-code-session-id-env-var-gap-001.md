...
# Add CLAUDE_CODE_SESSION_ID to bridge work-intent session-id resolution
...
- `ADR-ISOLATION-APPLICATION-PLACEMENT-001` (v1, specified) — all touched
  ...
  affect any application subtree.
- `ADR-INTERACTIVE-SESSION-ROLE-OVERRIDE-001` (v1, specified) — Slice 2
  ...
  and bridge-propose surfaces that were not updated in that slice.
...
- `.claude/rules/file-bridge-protocol.md` § Mandatory Pre-Drafting Claim Step —
  the protocol that the gate enforces.
...
- Derived from `GOV-FILE-BRIDGE-AUTHORITY-001` +
  `DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001`: the Write-gate
  must succeed for bridge file writes in Claude Code sessions where
  `CLAUDE_CODE_SESSION_ID` is set and `CLAUDE_SESSION_ID` is not. The gate's
  session-id resolver must include `CLAUDE_CODE_SESSION_ID` in its fallback
  chain so the substantive checks (author metadata, spec-link heading,
  owner-decisions section, applicability preflight section) actually run.

- Derived from `ADR-INTERACTIVE-SESSION-ROLE-OVERRIDE-001` +
  `DCL-SESSION-ROLE-RESOLUTION-001`: the env-var fallback chain in the
  bridge-compliance gate, bridge-axis-2 surface, bridge-propose helper, and
  bridge-claim CLI must recognize the same set of session-id env vars as
  `scripts/workstream_focus.py` and the doctor's marker resolver, so that
  Slice 2's intent (claim/Write contracts working across both Claude Code
  and Codex harnesses) is uniformly satisfied.
...
1:N (the gate-resolver tests cover GOV-FILE-BRIDGE-AUTHORITY-001 +
DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001 +
ADR-INTERACTIVE-SESSION-ROLE-OVERRIDE-001 + DCL-SESSION-ROLE-RESOLUTION-001;
template-match tests cover the lockstep-update invariant established by
prior operational feedback).
