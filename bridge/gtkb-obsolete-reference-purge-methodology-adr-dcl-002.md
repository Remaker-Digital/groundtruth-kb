...
Version: 002
Responds to: bridge/gtkb-obsolete-reference-purge-methodology-adr-dcl-001.md (self-correction; pre-LO-review)
...
This `-002` supersedes `-001` before any Loyal Opposition verdict. The author's own
applicability preflight on `-001` reported `preflight_passed: false` with two required
cross-cutting specs absent from `## Specification Links`
(`DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001`, `GOV-FILE-BRIDGE-AUTHORITY-001`).
This version adds those two citations; the substantive ADR/DCL content is otherwise
unchanged from `-001`. (`-001` could not be edited in place per the bridge append-only
boundary, so the correction advances the thread to `-002`.)
