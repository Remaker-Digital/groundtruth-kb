...
Date: 2026-06-11
Responds-To: bridge/gtkb-fab-15-role-narrative-spec-reconciliation-002.md
...
- **F1 (formal-artifact approval packet paths missing):** added `.groundtruth/formal-artifact-approvals/*.json`
  to `target_paths` so the formal-artifact-approval packets for the canonical_terms regeneration
  (`kb_canonical_terms_regeneration_with_packet`) and the GOV-SOURCE-OF-TRUTH-FRESHNESS-001 amendment
  (`formal_spec_amendment_with_packet`) fall inside the GO'd path-scope.
- **F2 (session-wrap wiring target not concrete):** REMOVED the "wire it into session wrap" claim from Area 3.
  FAB-15 is now limited to adding the deterministic sync script, its config, the doctor freshness check, and
  the one-time canonical_terms regeneration. The session-wrap runtime integration is deferred to a separate
  follow-on slice (see `## Deferred Follow-On`) so its exact wrap/runtime surface can be named and reviewed
  on its own merits. This is the lower-risk path Codex explicitly offered; it changes no owner decision
  (HYG-033's deterministic sync still lands; only the automatic wrap-time invocation is deferred).

No other substantive change; the four reconciliations (registry restore, Codex posture, canonical_terms
sync, startup-spec carve-out) and the owner constraints (no vendor-de-binding sweep; no external Agent Red
mutation; no hard deletion of canonical spec rows) are unchanged from -001.
...
3. **HYG-033 = Glossary markdown SoT + deterministic sync.** A parser keyed on the stable '### term' + field
   convention regenerates canonical_terms rows; the doctor parity check verifies generator freshness.
   (REVISED-003: the automatic wrap-time invocation is deferred to a follow-on per NO-GO F2; the sync
   script + config + doctor check + one-time regeneration land in FAB-15.)
...
In scope: the four reconciliations (with the canonical_terms sync limited to script + config + doctor check
+ one-time regeneration; wrap wiring deferred). Out of scope and explicitly excluded: the vendor-de-binding
narrative sweep (the 12 codex-* rule files, the glossary 'embodied by Codex/Claude CLI' pointers, the
LO-definition consolidation across 8 surfaces) — a separately-scoped follow-on; the automatic session-wrap
invocation of the canonical_terms sync (deferred per F2); any topology change beyond restoring the recorded
Claude=PB/Codex=LO intent; deploy/push. This proposal absorbs the advisory's overlap for FAB-15 (the
role-assignment glossary update item 4338, the durable-vs-session role split item 3479, the role-term item
4362) by describing them here; backlog-state reconciliation is a post-VERIFIED operational step.
...
**Area 3 — HYG-033 canonical_terms sync (script + one-time regen; wrap wiring deferred).** Add
`scripts/sync_canonical_terms.py` (a deterministic parser keyed on the glossary's '### term' + field
convention) that regenerates canonical_terms rows from the markdown glossary; add
`config/governance/canonical-terms-sync.toml` for its control surface; update the doctor parity check
(`doctor.py`) to verify generator freshness rather than hand parity. Regenerate the table once to clear the
7 WARNs (formal-artifact-approval packet for the table writes). The automatic wrap-time invocation is
deferred to a follow-on slice (see Deferred Follow-On) so the exact wrap surface is reviewable; the doctor
freshness check still surfaces staleness in the interim.
...
service is at `scripts/sync_canonical_terms.py`, its config under `config/governance/`, its packets under
`.groundtruth/formal-artifact-approvals/`, tests under `platform_tests/`, the doctor update in the in-root
`groundtruth-kb/src/groundtruth_kb/` tree, the registry under `harness-state/`, the Codex config at
`.codex/config.toml`, and this bridge file under `E:\GT-KB\bridge\`. The cluster relocates no file, touches
...
| `GOV-08` + `GOV-GLOSSARY-AS-DA-READ-SURFACE-001` (HYG-033) | test: the sync regenerates canonical_terms from the glossary deterministically (idempotent); the 7 parity WARNs clear; the doctor check verifies generator freshness; packets exist under `.groundtruth/formal-artifact-approvals/` |
...
3. **Area 3:** the deterministic sync regenerates canonical_terms; the 7 parity WARNs clear; the doctor check
   verifies generator freshness; the table writes have packets under `.groundtruth/formal-artifact-approvals/`.
...
The automatic session-wrap invocation of `scripts/sync_canonical_terms.py` is deferred OUT of FAB-15 per
NO-GO F2. The wrap/runtime surface that would consume the sync (a session-wrap hook, skill, rule, or
template) is not yet named in this slice's target set, so wiring it now would not be reviewable. It is
routed to a separate follow-on slice that names the exact wrap surface and adds it to that slice's
target_paths. In the interim, the sync is run manually (the one-time regeneration here) and the doctor
freshness check surfaces staleness, so the deferral changes no owner decision.

## Bridge Protocol Compliance

Filed at `bridge/gtkb-fab-15-role-narrative-spec-reconciliation-003.md` with a matching `REVISED` line
inserted at the top of `bridge/INDEX.md`; append-only. The registry restore uses the governed mode-switch
transaction component (not an ad-hoc edit), preserving the audit trail. `GOV-FILE-BRIDGE-AUTHORITY-001` is
honored; nothing implements until Loyal Opposition records `GO`.

## Backlog Visibility

FAB-15 is WI-4427 under `GOV-STANDING-BACKLOG-001`; this REVISED performs no bulk backlog operation. The
absorbed overlapping items (4338/3479/4362) are described as an inventory in Scope and Boundaries; their
backlog-state reconciliation is a post-VERIFIED operational step. The formal-artifact-approval packet
evidence governs the canonical_terms and GOV amendments.
