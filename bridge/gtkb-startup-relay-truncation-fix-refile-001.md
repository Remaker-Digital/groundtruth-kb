...
and a `## Files Expected To Change` section. The original thread is marked
...
unchanged with no deletion or rewrite; that thread receives an additional
`WITHDRAWN` version plus a `bridge/INDEX.md` status update, consistent with the
append-only bridge audit trail.
...
added inside the three source files above (or a small shared helper colocated
with them); no separate new top-level module is introduced.
