...
Version: 005 (REVISED; prime-builder proposal revised under Antigravity-C identity)
Responds-To: bridge/gtkb-fab-13-retention-policy-umbrella-004.md
...
author_model_configuration: interactive owner session

target_paths: ["groundtruth.db", "groundtruth.db-*", "groundtruth (*).db-*", "groundtruth (*).db", ".claude/hooks/owner-decision-tracker.py", ".claude/hooks/*.json", "memory/pending-owner-decisions.md", "memory/archive/**", "scripts/cross_harness_bridge_trigger.py", "groundtruth-kb/src/groundtruth_kb/session/envelope.py", "config/governance/runtime-evidence-retention.toml", ".gtkb-state/**", ".codex/gtkb-hooks/**", ".driveignore", ".gitignore", "platform_tests/scripts/**"]
...
REVISED-005 responds to the single NO-GO finding in
`bridge/gtkb-fab-13-retention-policy-umbrella-004.md`:

- **F1 (root SQLite duplicates outside target_paths):** added `"groundtruth.db-*"`, `"groundtruth (*).db-*"`, and `"groundtruth (*).db"` to the `target_paths` array. The implementation-start preflight now authorizes the deletion of root duplicate files such as `groundtruth (N).db-*` and `groundtruth.db-wal` / `groundtruth.db-shm` without violating path boundaries.
- Separated the destructive-cleanup perimeter verification from the code/config changes in the verification plan.
...
3. **HYG-056 = Purge dups + extend .driveignore.** One-time purge of the 62 ' (N)' duplicate files (gitignore
   already covers the families, no history risk) + extend .driveignore to .gtkb-state/, .claude/hooks/*.json +
...
- repo-root SQLite sidecar ' (N)' duplicates (siblings of `groundtruth.db`, specifically authorized in `target_paths` via `"groundtruth.db-*"`, `"groundtruth (*).db-*"`, and `"groundtruth (*).db"`).
...
3. **Area 3:** the 62 duplicates are purged within the enumerated deletion perimeter; .driveignore covers
   .gtkb-state/hook-state/locks; the .gitignore ' (N)' family is generalized; the .driveignore patterns are added; the full-unsync recommendation
...
Filed at `bridge/gtkb-fab-13-retention-policy-umbrella-005.md` with a matching `REVISED` line inserted at
