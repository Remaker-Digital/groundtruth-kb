# Implementation Proposal — Harness Role Portability and Single-Prime-Builder Invariant (WI-3341 Slice A, FR9) — REVISED-2
...
This is REVISED-2 of WI-3341 Slice A, responding to the Loyal Opposition NO-GO
at `bridge/gtkb-harness-role-portability-fr9-004.md`. It implements the
...
The active-harness eligibility gate became implementable when
`gtkb-harness-registry-seed` (WI-3342 Slice A) reached VERIFIED at
`bridge/gtkb-harness-registry-seed-004.md`: the `harnesses` table is now seeded
with harnesses `A` and `B` at `status = active`, so `set-role` can gate
eligibility against an authoritative registry rather than against legacy
role-map membership.
...
The `-002` NO-GO raised two P1 blocking findings (F1, F2); the `-004` NO-GO on
REVISED-1 raised one more (F1). All three are resolved here.

- **`-002` F1 — the proposal omitted FR9's active-harness eligibility gate.**
  Resolved by the NO-GO's *preferred* path. `gt harness set-role` now reads the
  ...
  `active`. This became implementable when `gtkb-harness-registry-seed`
  (WI-3342 Slice A) reached VERIFIED.
- **`-002` F2 — the test plan did not cover the three-harness / demote-all
  case.** Resolved by adding
  `test_harness_set_role_three_harness_demotes_all_non_targets` and
  `test_harness_set_role_rejects_non_active_harness`. FR6 ("no logic hard-codes
  a two-harness assumption") was added to the Specification Links.
- **`-004` F1 — REVISED-1 omitted the verified role-portability governance spec
  `GOV-HARNESS-ROLE-PORTABILITY-001`.** Resolved by the smallest correction the
  NO-GO named: `GOV-HARNESS-ROLE-PORTABILITY-001` is added to `Specification
  Links`; its owner-decision source `DELIB-0831` is added to `Prior
  Deliberations` and `Owner Decisions / Input`; and the Spec-to-Test Mapping
  maps the role-portability governance contract to the existing
  `test_harness_set_role_reassigns_prime_builder` and
  `test_harness_set_role_three_harness_demotes_all_non_targets` tests. Those
  tests are strengthened to register harnesses with distinct `harness_type`
  values and to assert that role assignment moves by durable harness id without
  any code path branching on type. No design change and no new target path.

Target paths are unchanged across all versions. The eligibility gate is
implemented inline in the `gt harness set-role` command body in `cli.py`,
mirroring how the sibling `gt harness show` command reads
`KnowledgeDB.get_harness`; no new module or target path is required.
...
3. Spec-derived tests: a new
   ...
   three-harness demote-all test and the non-active-harness rejection test.
...
- `GOV-HARNESS-ROLE-PORTABILITY-001` — the verified governance specification
  ...
  fixed model or vendor identities. `gt harness set-role` is the operational
  command that reassigns those roles between durable harness ids; this proposal
  preserves that contract — the command resolves and assigns by harness id, and
  no code path branches on `harness_type`.
...
- `DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001` — the verification plan
  derives tests from FR9's eligibility gate and invariant, from FR6's
  arbitrary-harness-count requirement, and from the
...
- `DELIB-0831` — "Owner decision: Prime Builder and Loyal Opposition are
  ...
  `GOV-HARNESS-ROLE-PORTABILITY-001`; `gt harness set-role` is the command that
  exercises that role portability, so the deliberation directly governs this
  work. Cited per the `-004` NO-GO finding F1.
- `bridge/gtkb-harness-role-portability-fr9-004.md` — the Loyal Opposition
  NO-GO on REVISED-1 this REVISED-2 responds to (finding F1: missing
  `GOV-HARNESS-ROLE-PORTABILITY-001` linkage).
- `bridge/gtkb-harness-role-portability-fr9-002.md` — the earlier Loyal
  Opposition NO-GO (findings F1 and F2, both addressed by REVISED-1 and carried
  forward here).
- `bridge/gtkb-harness-registry-seed-004.md` — the VERIFIED WI-3342 Slice A
  ...
  eligibility gate implementable. The seed thread's `-001` proposal and `-003`
  report both identify this WI-3341 Slice A revision as the consumer they
  unblock.
...
- `DELIB-0831` (`outcome=owner_decision`) — owner decision that Prime Builder
  ...
  decision behind `GOV-HARNESS-ROLE-PORTABILITY-001`, which `gt harness
  set-role` operationalizes.
...
- Owner AskUserQuestion of 2026-05-16 ("Seed the harnesses table first") — the
  ...
  enforce active-harness eligibility against the authoritative table.
...
single-prime-builder invariant; `GOV-HARNESS-ROLE-PORTABILITY-001` is the
verified governance contract that those roles are portable harness assignments.
Two interpretation notes are recorded transparently for the reviewer.
...
unchanged and out of scope. This note is carried unchanged from `-001`; no
NO-GO contested it.

Interpretation note 2 — eligibility substrate (revised at REVISED-1). FR9 says
...
eligibility gate and treated role-map membership as eligibility; the `-002`
NO-GO's finding F1 rejected that. With WI-3342 Slice A VERIFIED, the
`harnesses` table is seeded and authoritative for lifecycle status, so this
proposal implements the eligibility gate directly against the table. The only
element still deferred is the broader substrate unification (making the table
the role-write authority and retiring the legacy JSON), which is the WI-3342
Slice B reader migration and is genuinely separate from eligibility.
...
`GOV-HARNESS-ROLE-PORTABILITY-001`'s portable-role contract. The body:
...
eligibility, atomic demotion, the single-prime-builder invariant), FR6
(arbitrary harness count), and the `GOV-HARNESS-ROLE-PORTABILITY-001`
portable-role governance contract are verified as follows.
...
- `test_harness_set_role_reassigns_prime_builder` — the two harnesses are
  ...
  contract that the operating roles are portable harness assignments — not
  bound to a model or vendor identity — and that no `set-role` code path
  branches on `harness_type` (finding `-004` F1).
- `test_harness_set_role_three_harness_demotes_all_non_targets` — registers and
  ...
  seeds a three-harness role map; `gt harness set-role --harness C` makes `C`
  the sole `prime-builder` and demotes BOTH `A` and `B` to `loyal-opposition`.
  This proves the plural "every other harness" demotion, the FR6
  no-two-harness-assumption requirement (finding `-002` F2), and — because the
  three harnesses carry distinct types — that demote-all behavior is
  harness-type-independent (`GOV-HARNESS-ROLE-PORTABILITY-001`).
- `test_harness_set_role_rejects_non_active_harness` — a harness in the
  ...
  (findings `-002` F1 and F2). The gate's `status != "active"` predicate
  equally rejects a `retired` harness.
...
- **`set-role` promote-to-prime-builder interpretation (note 1).**
  *Mitigation:* documented transparently; it is the FR9-faithful reading; no
  NO-GO contested it; if the owner later wants a `--role`-style form it is a
...
- Role assignment attaches to the durable harness id; no `gt harness set-role`
  ...
  `GOV-HARNESS-ROLE-PORTABILITY-001`'s portable-role contract.
...
This proposal is filed as `bridge/gtkb-harness-role-portability-fr9-005.md`, a
REVISED version responding to the NO-GO at `-004`, with a `REVISED` line
...
REVISED, `-004` NO-GO, `-005` REVISED) is preserved; no prior bridge version is
