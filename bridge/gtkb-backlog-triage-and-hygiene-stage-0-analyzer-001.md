...
# Stage 0 — Backlog Triage Analyzer + Criteria Baseline (read-only)
...
4. Emits a `BenchmarkResult` snapshot (JSON) plus a markdown summary to `.gtkb-state/benchmarks/<run_id>/backlog_triage.json|md` via the shared `write_run_outputs` helper. Each open item carries its computed signal vector so later stages can filter deterministically.
...
|---|---|
| `SPEC-1662` (GOV-18) + `GOV-STANDING-BACKLOG-001` (deterministic, meaningful classification) | test: on a synthetic fixture of known items, each signal classifier (router-generated, content-hash duplicate, bridge/spec/owner-linked, approval_state, age, articulation length, project_name-vs-membership) returns the asserted label; running twice on identical input yields byte-identical output (determinism) |
...
| `GOV-08` (read-only; no canonical mutation) | test: AST scan of `backlog_triage.py` asserts no `insert_*`/`update_*`/`resolve_*`/`commit`/`UPDATE`/`DELETE` write path; the DB connection is opened read-only; the run leaves `work_items`/`projects` row counts unchanged |
...
2. Running the benchmark emits `.gtkb-state/benchmarks/<run_id>/backlog_triage.json` + `.md` with a per-item signal vector and platform/AR partitions.
