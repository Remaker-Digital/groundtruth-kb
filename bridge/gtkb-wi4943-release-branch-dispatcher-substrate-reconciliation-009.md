...
# Revised Proposal - WI-4943 Release-Branch Dispatcher Substrate Reconciliation - Dependency Envelope Correction
...
author_harness_id: A
author_session_context_id: 2026-07-01T11-11-50Z-prime-builder-A-1c2144
...
Responds to NO-GO: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-008.md
Prior implementation blocker report: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-007.md
Prior revised proposal: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-005.md
Prior GO: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-006.md
First blocker report: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-003.md
First NO-GO: bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-004.md

Project Authorization: PAUTH-PROJECT-GTKB-AD-HOC-RELEASE-20260701-WI-4943-DISPATCHER-SUBSTRATE
Project: PROJECT-GTKB-AD-HOC-RELEASE-20260701
Work Item: WI-4943

target_paths: ["scripts/gtkb_dispatcher_daemon.py", "scripts/dispatcher_runtime.py", "scripts/bridge_work_intent_registry.py", "scripts/ensure_dispatcher_daemon.py", "scripts/install_dispatcher_daemon_task.ps1", "scripts/cursor_harness.py", "groundtruth-kb/src/groundtruth_kb/cli.py", "groundtruth-kb/src/groundtruth_kb/bridge_dispatch_config.py", "groundtruth-kb/src/groundtruth_kb/bridge_dispatch_reset.py", "groundtruth-kb/src/groundtruth_kb/dispatcher_supervisor.py", "groundtruth-kb/src/groundtruth_kb/project/doctor.py", "groundtruth-kb/src/groundtruth_kb/bridge/role_state.py", "scripts/ops/harness_storm_watchdog_launcher.py", "scripts/ops/harness_storm_watchdog.ps1", "config/dispatcher/rules.toml", "harness-state/harness-registry.json", "platform_tests/scripts/test_gtkb_dispatcher_daemon.py", "platform_tests/scripts/test_dispatcher_daemon_supervision.py", "platform_tests/scripts/test_dispatcher_runtime.py", "platform_tests/scripts/test_cursor_harness.py", "platform_tests/groundtruth_kb/cli/test_bridge_config_cli.py", "platform_tests/groundtruth_kb/cli/test_bridge_dispatch_daemon_supervisor.py", "groundtruth-kb/docs/method/12-file-bridge-automation.md", "docs/gtkb-dashboard/grafana/README.md", "groundtruth-kb/docs/wiki/release-health.md", "README.md", "bridge/gtkb-wi4933-*.md", "bridge/gtkb-wi4937-dispatcher-supervisor-governance-*.md", "bridge/gtkb-wi4942-dispatch-drain-live-worker-parity-*.md", "bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-*.md"]

---

## Revision Claim

This revision responds to the NO-GO at `bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-008.md`. The implementation report at `-007.md` correctly failed closed after focused release-worktree testing exposed two additional dispatcher-substrate dependency paths outside the approved v005/v006 target envelope:

| Newly added target path | Why it is required |
| --- | --- |
| `scripts/bridge_work_intent_registry.py` | The staged `scripts/dispatcher_runtime.py` imports `MalformedBridgeStatusError` and related dispatch-batch/work-intent surfaces that are absent from the older release-branch copy. |
| `scripts/ops/harness_storm_watchdog.ps1` | The staged `scripts/ops/harness_storm_watchdog_launcher.py` resolves this PowerShell script at runtime, and the focused supervisor test fails when it is absent. |

This REVISED proposal changes only the bridge authorization envelope. It does not implement release-branch reconciliation, modify the two added paths, narrow acceptance criteria, amend credentials, deploy anything, restore retired pollers or hook-driven automation, rewrite history, or bind GT-KB release health to Azure or any other deployment provider.

The corrected envelope now contains the prior v005 target set plus exactly these two paths. If post-GO implementation finds another required path outside this revised envelope, Prime Builder must fail closed with another blocker report rather than expanding scope directly.
...
git -c safe.directory=E:/GT-KB/.gtkb-state/release-main-20260630 -C .gtkb-state/release-main-20260630 diff --cached --name-only
Test-Path .gtkb-state\release-main-20260630\scripts\bridge_work_intent_registry.py
Test-Path .gtkb-state\release-main-20260630\scripts\ops\harness_storm_watchdog.ps1
rg -n "bridge_work_intent_registry|harness_storm_watchdog|MalformedBridgeStatusError|harness_storm_watchdog\.ps1" .gtkb-state/release-main-20260630/scripts .gtkb-state/release-main-20260630/platform_tests .gtkb-state/release-main-20260630/groundtruth-kb/src
```

Observed audit result:

- The release worktree still contains the v007 staged dispatcher-substrate set and no new implementation changes from this dispatch.
- `scripts/bridge_work_intent_registry.py` exists in the release worktree but is older than the staged dispatcher runtime; the root checkout copy contains `MalformedBridgeStatusError`.
- `scripts/ops/harness_storm_watchdog.ps1` is absent from the release worktree and present in the root checkout.
- The focused literal-reference search confirmed the two concrete blockers from v008: `dispatcher_runtime.py` imports the newer work-intent registry surface, and `harness_storm_watchdog_launcher.py` hard-resolves the missing PowerShell script.
- No third concrete out-of-envelope dependency path was proven by this read-only audit. The implementation report after any GO must still rerun the focused tests and fail closed if a later dependency layer appears.

## Specification Links

- `SPEC-DISPATCHER-CONTROL-SURFACE-001` - dispatcher health/status/drain/config commands must agree and expose release-operable dispatcher state from the release branch; the work-intent registry dependency is required for dispatcher runtime batching and status parsing to function under focused tests.
- `SPEC-CENTRALIZED-DISPATCH-SERVICE-001` - bridge dispatch must remain daemon-owned, bounded, and operational without restoring retired trigger or poller paths; the registry dependency supports bounded dispatch-batch classification, and the watchdog script supports daemon/supervisor lifecycle control.
...
- `DCL-SINGLE-HARNESS-DISPATCHER-DESKTOP-TASK-001` - Windows dispatcher persistence must remain headless/no-window safe; the PowerShell watchdog script is included only as the dependency invoked by the headless launcher.
...
- `DCL-PROJECT-AUTHORIZATION-ENVELOPE-001` - the revision directly corrects the second envelope mismatch found by implementation authorization validation and focused tests.
...
- `bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-003.md` - Prime Builder blocker report proving the first target-path mismatch.
- `bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-004.md` - Loyal Opposition NO-GO confirming the first mismatch.
- `bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-007.md` - Prime Builder blocker report proving the second target-path mismatch.
- `bridge/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-008.md` - Loyal Opposition NO-GO confirming the two added dependency paths.
...
- This revision uses the v008 NO-GO's first remediation path: file a revised bridge proposal with the two newly discovered dependency paths. It does not choose the alternate option of narrowing WI-4943 acceptance criteria.
- No new owner decision is required before Loyal Opposition reviews this corrected envelope. If Loyal Opposition determines the current PAUTH text is insufficient for the two added paths, the correct verdict is NO-GO identifying the exact missing owner authorization.
...
The proposed implementation scope from v001 and v005 remains in force with one change: the implementation envelope now includes the two paths identified in the v008 NO-GO. The implementation must:
...
4. Include `scripts/bridge_work_intent_registry.py` only to reconcile the staged dispatcher runtime's work-intent and malformed-status dependency surface.
5. Include `scripts/ops/harness_storm_watchdog.ps1` only as the script invoked by `scripts/ops/harness_storm_watchdog_launcher.py` for headless supervisor/watchdog operation.
6. Preserve the v005-added topology/support paths: `config/dispatcher/rules.toml`, `harness-state/harness-registry.json`, `groundtruth-kb/src/groundtruth_kb/bridge/role_state.py`, and `scripts/ops/harness_storm_watchdog_launcher.py`.
7. Run a brief dependency check before implementation changes: verify every staged dispatcher file's root-relative script/config references either already exist in the release worktree or are listed in this target envelope.
8. Preserve retired automation retirement: do not restore the retired OS poller, smart poller, hook-triggered automation path, or retired cross-harness trigger as a fallback.
9. Preserve provider-neutral dashboard state: no default Azure warning, no Azure credential or CLI requirement, and mock application deployment data remains acceptable for dashboard release-health surfaces.
10. Avoid broad staging, broad commits, broad pushes, whole-branch `research` merges, unrelated dirty files, local DB churn, scratch artifacts, or history rewrite.
...
| `GOV-PROJECT-IMPLEMENTATION-AUTHORIZATION-001` and `DCL-PROJECT-AUTHORIZATION-ENVELOPE-001` | After GO, run `implementation_authorization.py begin --bridge-id gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation`; validate the two added paths as authorized before modifying them. |
...
- Applicability command: `groundtruth-kb/.venv/Scripts/python.exe scripts/bridge_applicability_preflight.py --bridge-id gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation --content-file .gtkb-state/bridge-revisions/drafts/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-009.md --json`
- Applicability result: `preflight_passed=true`; `missing_required_specs=[]`; `missing_advisory_specs=[]`.
- Applicability packet hash: `sha256:06030cc80fbef7c86a3c42b3f95bb2816710a962f12047d319a369c51569b898`.
- Clause command: `groundtruth-kb/.venv/Scripts/python.exe scripts/adr_dcl_clause_preflight.py --bridge-id gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation --content-file .gtkb-state/bridge-revisions/drafts/gtkb-wi4943-release-branch-dispatcher-substrate-reconciliation-009.md`
- Clause result: exit code 0; clauses evaluated 5; must_apply 4; evidence gaps in must_apply clauses 0; blocking gaps 0.

## Risk And Rollback

Primary risk is that the corrected envelope becomes a broad release-branch merge. Mitigation: Loyal Opposition should review the two added paths as narrow support for the already stated acceptance criteria, not as permission to sweep `research` into release. A second risk is another hidden dependency layer; mitigation is an explicit post-GO fail-closed requirement if focused tests reveal any additional out-of-envelope path.
