...
Work Item: WI-5059
target_paths: ["platform_tests/scripts/test_advisory_intake_scanner.py", "platform_tests/skills/test_advisory_proposal_skill.py", "platform_tests/skills/test_advisory_intake_skill.py", "platform_tests/skills/test_advisory_intake_profile_surfacing.py", "platform_tests/skills/test_skill_catalog_contract.py", "platform_tests/scripts/test_check_harness_parity.py", "bridge/gtkb-wi5059-advisory-intake-test-parity-*.md", "groundtruth.db"]
implementation_scope: Complete the test and parity coverage tying the child implementation reports to ADVISORY filtering, owner-grilling behavior, skill registration, adapter generation, activity profile surfacing, and harness parity expectations.
requires_review: true
requires_verification: true

# Implementation Proposal - Advisory Intake Filtering And Parity Tests

## Summary
Complete the test and parity coverage tying the child implementation reports to ADVISORY filtering, owner-grilling behavior, skill registration, adapter generation, activity profile surfacing, and harness parity expectations.
...
The parent proposal and child work items already encode the operative requirement: ADVISORY material can seed scoped proposals only after owner-grilling and explicit project/work-item approval, and advisory capture itself must not become implementation approval. The linked manual test TEST-11297 provides the acceptance anchor for this child item.

## Proposed Scope
- Add or extend tests for ADVISORY filtering, owner-grilling gate behavior, skill catalog registration, adapter generation, activity-profile surfacing, and parity expectations.
- Ensure each child implementation report maps its evidence back to the relevant TEST-1129x record.
- Keep parity expectations focused on governed skill/profile surfaces, not vendor-specific transient UI behavior.

## Out Of Scope
- No production behavior change is included except where required to expose testable registration or parity metadata.
- No release deployment is included.
...
- `python -m pytest platform_tests/scripts/test_advisory_intake_scanner.py platform_tests/skills/test_advisory_proposal_skill.py platform_tests/skills/test_advisory_intake_skill.py platform_tests/skills/test_advisory_intake_profile_surfacing.py platform_tests/skills/test_skill_catalog_contract.py platform_tests/scripts/test_check_harness_parity.py -q --tb=short` passes and maps to TEST-11297.
- Manual implementation-report review confirms each child report maps tests to ADVISORY filtering, owner-grilling gate behavior, skill catalog registration, adapter generation, activity-profile surfacing, and parity expectations.
- `python scripts/bridge_applicability_preflight.py --bridge-id gtkb-wi5059-advisory-intake-test-parity --json` returns no blocking applicability findings for the final bridge thread.

## Acceptance Criteria
- The test/parity suite gives Loyal Opposition enough evidence to verify all advisory-intake child work without relying on transcript memory.
- TEST-11297 is satisfied by test output and implementation-report evidence.
