...
Date: 2026-06-11
Responds-To: bridge/gtkb-fab-12-agent-red-residue-sweep-002.md
...
- **F1 (protected-narrative packet artifacts missing from target_paths):** added
  `.groundtruth/formal-artifact-approvals/*.json` so the per-file narrative-approval packet the
  CLAUDE.md L12 edit requires falls inside the GO'd path-scope.
- **F2 (`.github` template edits not in target_paths):** added `.github/pull_request_template.md` and
  `.github/ISSUE_TEMPLATE/**` to `target_paths`.
- **F3 (home-directory memory migration needs a root-boundary-safe method):** REMOVED the out-of-root
  home-cache lesson migration from this slice (Codex option B). Area 2 is now limited to **in-root**
  work only — retitling repo `memory/MEMORY.md` off "# Agent Red Memory" and amending CLAUDE.md L12 to
  classify the home-directory path as a non-authoritative harness cache. The implementation does NOT read
  or depend on `C:/Users/micha/.claude/...` content. The divergent home-cache reconciliation is deferred
  to a separate owner-approved in-root export/snapshot procedure (see `## Deferred Follow-On`), consistent
  with `DELIB-FAB12-REMEDIATION-20260610` HYG-016 (which makes the repo store authoritative — the home-dir
  is the non-authoritative side, so deferring its merge changes no decision).

No other substantive change; Areas 1, 3, and the rest of Area 4 are unchanged from -001. Constraints
preserved: no external Agent Red repository mutation; no full platform/application config split (reserved
for ISOLATION-018).
...
- `GOV-ARTIFACT-APPROVAL-001` — the CLAUDE.md L12 edit is a protected narrative artifact requiring a
  per-file approval packet recorded under `.groundtruth/formal-artifact-approvals/`.
...
2. **HYG-016 = Repo authoritative; home-dir = harness cache; amend CLAUDE.md L12.** Make repo
   `memory/MEMORY.md` the single authoritative ADR-0001 notepad (retitled) and amend CLAUDE.md L12 to
   classify the home-dir path as a non-authoritative harness cache. (REVISED-003: the divergent home-dir
   lesson migration is deferred to a root-boundary-safe in-root export procedure per NO-GO F3; see Deferred
   Follow-On.)
...
Agent Red spec corpus (separate); any deploy/push; the external Agent Red lifecycle repository (this
cluster touches only in-root `applications/Agent_Red/` surfaces); and (per F3) the out-of-root home-cache
lesson migration, now deferred to a separate owner-approved in-root export procedure. This proposal
**absorbs** the advisory's listed existing-WI overlap for FAB-12 — the sonar project-key item (3417,
partly repaired at S363), the `.github` template/workflow Agent-Red-residue items (the 3419/3430/3431/3466
set), and the memory index-template + topic-split items (4346/4347) — folding them into WI-4424's scope and
describing them here rather than re-filing. Backlog-state reconciliation for those items is a post-VERIFIED
operational step.
...
**Area 2 — HYG-016 memory authority (in-root only).** Retitle repo `memory/MEMORY.md` off "# Agent Red
Memory" to a platform title; amend CLAUDE.md L12 (narrative-artifact-approval packet at implementation
time) to designate repo `memory/MEMORY.md` authoritative and the home-directory path a non-authoritative
harness cache, with a wrap-time reconciliation note. Stays within the GOV-01 300-line CLAUDE.md limit
(small edit). The implementation reads NO out-of-root home-cache content; the divergent-lesson merge is
deferred (see Deferred Follow-On).
...
+ `.github/ISSUE_TEMPLATE/**` to platform-neutral (or GT-KB) vocabulary.
...
touches only in-root `applications/Agent_Red/` (not the external Agent Red repository), reads no out-of-root
home-cache content (F3 deferral), and the full platform/application config split is explicitly deferred to
ISOLATION-018.
...
| `ADR-0001` + `GOV-08` (HYG-016) | test: repo `memory/MEMORY.md` is not titled "Agent Red Memory"; CLAUDE.md L12 designates repo store authoritative + home-dir as cache; CLAUDE.md stays ≤300 lines (GOV-01); CLAUDE.md edit has a packet under `.groundtruth/formal-artifact-approvals/` |
| `GOV-RELEASE-READINESS-GOVERNED-TESTING-001` (HYG-024/043) | test: each of the four CI lanes passes on a `workflow_dispatch` run; root `pyproject` contains no path that does not exist; `dependabot.yml` references only extant dirs + covers `groundtruth-kb/`; `.github` templates are platform-neutral |
...
2. **Area 2:** repo `memory/MEMORY.md` retitled + authoritative; CLAUDE.md L12 amended (with packet under
   `.groundtruth/formal-artifact-approvals/`) and ≤300 lines; no out-of-root home-cache read.
3. **Area 3:** root `pyproject` is platform-identified with only extant paths; all four CI lanes green on
   `workflow_dispatch`; dependabot repaired; `.github` templates platform-neutral.
...
The divergent home-directory auto-memory lesson migration (HYG-016 Decision 2's "migrate divergent lessons
in") is deferred OUT of FAB-12 per NO-GO F3. The mandatory project-root boundary forbids treating
out-of-root `C:/Users/micha/.claude/...` content as a live project artifact, so reading it directly inside
FAB-12 implementation would not be reviewable through the bridge target set. The migration is routed to a
separate owner-approved procedure that produces an in-root export/snapshot artifact first (root-boundary-safe),
then reconciles it into the repo store. Deferring it changes no decision: HYG-016 already makes the repo
store authoritative and the home-dir the non-authoritative side.

## Bridge Protocol Compliance

Filed at `bridge/gtkb-fab-12-agent-red-residue-sweep-003.md` with a matching `REVISED` line inserted at the
top of this thread's entry in `bridge/INDEX.md`; append-only, no prior bridge version deleted or rewritten.
`GOV-FILE-BRIDGE-AUTHORITY-001` is honored — nothing implements until Loyal Opposition records `GO` on this
thread.

## Backlog Visibility

FAB-12 is WI-4424 under `GOV-STANDING-BACKLOG-001`; this REVISED performs no bulk backlog operation. The
absorbed overlapping items are described as an inventory in Scope and Boundaries; their backlog-state
reconciliation is a post-VERIFIED operational step (DECISION DEFERRED to that step).
