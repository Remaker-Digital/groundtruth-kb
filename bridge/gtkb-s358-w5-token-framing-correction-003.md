...
Version 003 (REVISED) supersedes the `-002` NO-GO. The `-002` review confirmed the correction is sound - both mandatory preflights pass, the target-file claims match current file state, and the owner-decision basis is real - and raised one finding, FINDING-P1-001: the implementation requires three narrative-artifact approval-packet writes under the formal-artifact-approvals directory, but the `-001` target_paths authorized only the three protected narrative files, leaving required governance evidence outside the implementation-start authorization envelope.

This revision addresses FINDING-P1-001's four required revisions: (1) target_paths is extended with three narrowly-scoped approval-packet globs, one per protected file; (2) config/governance/narrative-artifact-approval.toml is cited in Specification Links as the registry constraining packet location, protected-path matching, and packet schema; (3) a new Approval-Packet Plan section specifies the three packets; (4) the Specification-Derived Verification Plan adds the staged narrative-artifact evidence check. The Problem, Claim, IP-1 through IP-3 scope, and MemBase Scope are unchanged from `-001`.
...
- config/governance/narrative-artifact-approval.toml - the registry that constrains protected-path matching, the approval-packet directory and filename pattern, and the packet schema for the three narrative-artifact corrections; cited per the -002 NO-GO required revision.
- GOV-ARTIFACT-ORIENTED-GOVERNANCE-001 - the correction is preserved as durable artifacts: DELIB-S358-TOKEN-CONCERN-IS-WASTE-NOT-VOLUME, WI-3370, this proposal, the approval packets, and the post-implementation report.
...
## Approval-Packet Plan

Each of the three protected narrative-file corrections requires a narrative-artifact-approval packet under the formal-artifact-approvals directory, per config/governance/narrative-artifact-approval.toml. The implementation-start authorization envelope is extended (see target_paths) to cover the three packet writes. Planned packets:

| Target file | artifact_id | Planned packet path |
|---|---|---|
| CLAUDE.md | claude-md | .groundtruth/formal-artifact-approvals/{date}-claude-md.json |
| .claude/rules/bridge-essential.md | claude-rules-bridge-essential-md | .groundtruth/formal-artifact-approvals/{date}-claude-rules-bridge-essential-md.json |
| .claude/rules/canonical-terminology.md | claude-rules-canonical-terminology-md | .groundtruth/formal-artifact-approvals/{date}-claude-rules-canonical-terminology-md.json |

Each packet carries the schema fields required by the config/governance/narrative-artifact-approval.toml approval_packet block: artifact_type = "narrative_artifact"; artifact_id per the table; action = "update"; target_path = the protected file being corrected; source_ref = "gtkb-s358-w5-token-framing-correction"; full_content = the full corrected file content; full_content_sha256 = sha256 of full_content, computed at implementation time from the corrected content; approval_mode = "approve" (or "edit-and-approve" if the owner edits); presented_to_user = true; transcript_captured = true; explicit_change_request = the owner's verbatim approval text captured at implementation time; changed_by = the active harness identifier; change_reason citing this bridge thread. The {date} token is the UTC date of packet creation at implementation time; the target_paths globs match any date.

## Bridge INDEX Update Evidence

A REVISED entry for gtkb-s358-w5-token-framing-correction is inserted at the top of that document's version list in the bridge index, above the NO-GO: -002 and NEW: -001 lines. No prior bridge file and no prior index entry is deleted or rewritten; -001 and -002 remain on disk and in the index as the append-only audit trail.
...
| DCL-ARTIFACT-APPROVAL-HOOK-001 | Each of the three protected narrative-file changes clears the universal pre-commit narrative-artifact evidence floor only when its matching approval packet is staged | python scripts/check_narrative_artifact_evidence.py --staged - expecting PASS with the three protected files cleared, recorded in the post-implementation report |
...
Execution: the post-implementation report records the exact grep commands run over the three files, the diff confirming only framing and lesson text changed, the staged narrative-artifact evidence check result, and the three approval-packet paths.
...
- Each protected-file Write carries an owner-approved narrative-artifact-approval packet under the formal-artifact-approvals directory, matching the schema in config/governance/narrative-artifact-approval.toml.
- python scripts/check_narrative_artifact_evidence.py --staged reports PASS with the three protected narrative files cleared.
- Both bridge preflights pass on the post-implementation report.

## Option Rationale

Correcting the framing in place was selected over two alternatives. Deleting the incident history was rejected because the incident is real and its facts are valuable. Leaving the rule files unchanged and relying only on DELIB-S358-TOKEN-CONCERN-IS-WASTE-NOT-VOLUME to correct forward was rejected because the rule files are auto-loaded every session and the deliberation is not, so the distortion would keep reaching reasoning. Scoping to the three rule files, and deferring the MemBase specifications to a named follow-on, was selected over a single all-surfaces proposal because the rule files are the auto-loaded surface where the distortion actively misdirects reasoning; a bounded proposal lands the high-priority correction faster, and the MemBase specifications need individual per-spec reading that should not gate the rule-file fix. For FINDING-P1-001, extending target_paths with narrowly-scoped per-packet globs was selected over a single broad formal-artifact-approvals directory glob so the implementation-start envelope authorizes exactly the three packets this work creates and nothing else.
...
- Risk: an approval packet's full_content_sha256 does not match the staged blob (line-ending or BOM drift). Mitigation: the packet is generated from the exact corrected file content, and the staged narrative-artifact evidence check is run before commit to catch any mismatch.
