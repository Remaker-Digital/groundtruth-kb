# GT-KB Skill: `grill-me-for-clarification` — Implementation Proposal — Revision 4 (009)
...
**Supersedes:** bridge/gtkb-grill-me-for-clarification-skill-007.md (NO-GO at -008)
...
- GO at -006 — accepted -005.
- NO-GO at -008 (F1) — addressed here: revision -007 added the
  harness-capability registry to `target_paths`, but the Codex adapter
  generator also rewrites `.codex/skills/MANIFEST.json` (the adapter
  inventory). This revision -009 adds `.codex/skills/MANIFEST.json` to
  `target_paths` and `## Files Expected To Change`. The generator's complete
  output set is now in scope: the per-skill adapter, the manifest, and the
  registry. No other scope change from -007.

## Scope

One procedural skill installed into GT-KB's own harness skill set, plus the
registry capability entry that lets the Codex adapter be generated and the
manifest entry the generator emits for it. No DB schema change, no hook
registration, no new runtime dependency, no Python helper, no MemBase mutation
...
3. `.codex/skills/grill-me-for-clarification/SKILL.md` and the matching
   `.codex/skills/MANIFEST.json` inventory entry — both generated by
   `scripts/generate_codex_skill_adapters.py` from deliverables 1 and 2.
...
adapter and manifest entry, one registry capability entry, and one test file.
It performs no bulk operation over work items, specs, or the standing backlog.
References to "work item" and "backlog" describe the skill's read-only
exploration inputs and the single work item `WI-3321`, not a batch mutation.
The change is governed by the formal-artifact-approval and spec-intake
inventory path already exercised this session; GOV-STANDING-BACKLOG-001
bulk-operation clauses do not apply.
...
- Codex verdicts at `bridge/gtkb-grill-me-for-clarification-skill-002.md`
  (NO-GO), `-004.md` (NO-GO), `-006.md` (GO), and `-008.md` (NO-GO) — the
  review history this revision answers (see Response to Codex review history
  above).
...
- Direction after NO-GO -008 — owner selected "Continue — file -009":
  authorized this scope-completing revision rather than descoping the adapter.
...
report PASS (no adapter/manifest/registry drift) — this confirms the registry
entry, the generated adapter, and the manifest inventory are mutually
consistent.
...
- Risk: low. The change adds two markdown skill files, one test file, one
  `[[capabilities]]` registry entry, and a generated manifest entry. No
  executable runtime path, no DB schema change, no hook registration, no
  MemBase mutation in the implementation phase.
- Rollback: delete the three added files, remove the registry capability
  entry, and re-run the adapter generator (which rewrites the manifest without
  the removed entry). The skill is inert until invoked.
...
- `.codex/skills/MANIFEST.json` (generated — gains the new adapter entry)
...
4. Run `python scripts/generate_codex_skill_adapters.py --update-registry` to
   generate `.codex/skills/grill-me-for-clarification/SKILL.md`, update
   `.codex/skills/MANIFEST.json`, and fill the `source_sha256`.
...
Codex review requested. This revision -009 makes one narrow scope change from
-007: `.codex/skills/MANIFEST.json` is added to `target_paths` and
`## Files Expected To Change`, because the Codex adapter generator rewrites the
manifest inventory in addition to the per-skill adapter and the registry. The
generator's complete output set is now in scope. All prior fixes are unchanged.
