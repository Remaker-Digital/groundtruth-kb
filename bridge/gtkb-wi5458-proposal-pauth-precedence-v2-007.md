...
# WI-5458 v2 - Validated Proposal PAUTH Selection And Currentness - Third Revision

bridge_kind: prime_proposal
Document: gtkb-wi5458-proposal-pauth-precedence-v2
Version: 007
Responds to: bridge/gtkb-wi5458-proposal-pauth-precedence-v2-006.md
Date: 2026-07-29 UTC
...
evaluates work-item coverage with the full restrictive-inclusion truth table,
...
- The latest chain is `NEW -> NO-GO -> REVISED -> NO-GO -> REVISED -> NO-GO`; no protected implementation or implementation-start packet exists for this v2 thread.
...
## NO-GO v006 Finding Disposition

Version 006's sole blocker is accepted and closed in this design. Authorization
coverage is now evaluated before ranking and before any membership mutation by
the complete `DCL-PAUTH-INCLUDED-WORK-ITEM-IDS-RESTRICTIVE-001` truth table:

1. `excluded_work_item_ids` always denies.
2. A non-empty `included_work_item_ids` list is authoritative: the candidate
   covers a listed work item even without active membership and does not cover
   an unlisted work item even when membership is active.
3. Only an empty included list falls back to active membership in the
   candidate's project.

Automatic and explicit-selector paths share that one coverage predicate. The
requested project and work item are resolved read-only before selection; active
membership is an input only for empty-list fallback, not a prerequisite for an
explicitly listed work item. Every denied branch remains before membership,
PAUTH, claim, packet, preflight, writer, or filesystem mutation.
...
### 1a. Restrictive work-item coverage before ranking

Resolve the requested project record and work item without requiring or
creating membership. For each same-project authorization candidate, compute
coverage with one shared function in
`groundtruth_kb.bridge.proposal_filing`:

- reject immediately when the work item is excluded;
- when the included list is non-empty, cover exactly listed work items and do
  not consult membership;
- when the included list is empty, cover only work items with an active
  membership in that project.

Only covering candidates enter specificity ranking and currentness evaluation.
An explicit selector is validated by the same predicate before rank checks, so
it cannot convert a non-covering row into authority. A listed nonmember may use
an otherwise valid current authorization without a membership insert. An
unlisted member under a non-empty include list and a nonmember under an empty
include list both deny without falling back to another interpretation of the
same row.

The automatic and explicit matrices each cover: listed/nonmember allow,
unlisted/member deny, empty-list/member allow, empty-list/nonmember deny, and an
exclusion overriding both otherwise-allowing cases. Every denial asserts zero
membership, PAUTH, claim, packet, preflight, writer, and filesystem side
effects.
...
| Non-empty WI include list; requested WI listed; no active membership | Cover; rank normally; do not insert membership | Cover; continue selector validation; do not insert membership |
| Non-empty WI include list; requested WI unlisted; active membership exists | Not covering; deny if no other covering candidate | Deny selected row |
| Empty WI include list; active same-project membership exists | Cover through membership fallback; rank normally | Cover; continue selector validation |
| Empty WI include list; no active same-project membership | Not covering; deny if no other covering candidate | Deny selected row |
| WI exclusion in an otherwise-allowing listed or membership-fallback case | Deny whole request | Deny |
...
|---|---|
| `DCL-PROJECT-AUTHORIZATION-ENVELOPE-001`; `DCL-PAUTH-INCLUDED-WORK-ITEM-IDS-RESTRICTIVE-001`; `GOV-PROJECT-IMPLEMENTATION-AUTHORIZATION-001` | In both automatic and explicit modes, cross active membership with non-empty listed/unlisted and empty-list cases: listed nonmember allows; unlisted member denies; empty-list member allows; empty-list nonmember denies; exclusion overrides each otherwise-allowing case. Also run exact-singleton, bounded multi-WI, cross-project, unknown, equal-rank explicit override, and lower-rank explicit rejection fixtures; assert the selected envelope, decision evidence, and zero side effects for every denial. |
...
- Automatic and explicit listed-nonmember requests succeed without inserting membership.
- Automatic and explicit unlisted-member requests under a non-empty include list deny with zero side effects.
- Automatic and explicit empty-list-member requests succeed, while empty-list-nonmember requests deny with zero side effects.
- Explicit exclusion denies both the listed-nonmember and empty-list-member otherwise-allowing cases.
...
2. Automatic and explicit selection use one restrictive work-item coverage
   predicate: exclusion always denies; a non-empty included list covers exactly
   listed work items without requiring membership; only an empty included list
   falls back to active project membership.
3. Listed nonmembers are allowed without membership mutation, unlisted members
   under non-empty inclusion are denied, empty-list members are allowed,
   empty-list nonmembers are denied, and exclusions override otherwise-allowing
   cases in both automatic and explicit modes.
4. Explicit selection is limited to equally best-ranked current covering candidates; lower-rank selection denies.
5. The best specificity cohort is fixed before stale pruning. If it contains no current candidate, the request denies and never selects a broader rank.
6. `Z` and explicit-offset aware expiries normalize correctly; malformed or naive currentness denies globally.
7. Forbidden operation, disallowed target class, unresolvable owner authority, and invalidation defects deny globally without fallback.
8. `included_spec_ids` is non-restrictive at this gate; only `excluded_spec_ids` intersection denies. WI-5178 retains restrictive-inclusion ownership.
9. Create-missing-state never derives classes from requested paths, creates only bridge/metadata authority for bridge/metadata targets, and denies source/test targets with zero durable rows and a governed owner-PAUTH recovery route.
10. Every authorization denial leaves zero MemBase, claim, packet, preflight, writer, and filesystem side effects.
11. Generated metadata, JSON, and text expose complete decision evidence including exact session/role, fixed cohort, currentness, latest bridge status, reviewed proposal version, and invalidation inputs.
12. Existing deterministic specificity, insertion-order, root-boundary, and bridge/metadata create-missing behavior remains passing under strengthened assertions.
13. All 17 linked specifications map to the verification table; focused pytest, Ruff check, Ruff format check, diff check, and both preflights pass.
14. Only the three declared targets change. No live PAUTH/project/backlog/MemBase, dispatcher/TAFE, harness, credential, external, Git commit/push, deployment, release, or cleanup mutation is in implementation scope.
15. A fresh implementation report receives independent terminal VERIFIED before WI-5458 resolves or WI-5166 proceeds.
...
- `bridge/gtkb-wi5458-proposal-pauth-precedence-v2-004.md` - prior NO-GO; its F1/F2 closures are retained.
- `bridge/gtkb-wi5458-proposal-pauth-precedence-v2-006.md` - controlling NO-GO; its restrictive work-item coverage truth table is implemented by this revision.
...
The completed candidate is run through both mandatory commands immediately
before filing; the governed revision helper separately repeats the credential
scan and bridge-compliance audit on its metadata-enriched candidate:

```text
groundtruth-kb/.venv/Scripts/python.exe scripts/bridge_applicability_preflight.py --bridge-id gtkb-wi5458-proposal-pauth-precedence-v2 --content-file .gtkb-state/bridge-revisions/drafts/gtkb-wi5458-proposal-pauth-precedence-v2-007.md --json
groundtruth-kb/.venv/Scripts/python.exe scripts/adr_dcl_clause_preflight.py --bridge-id gtkb-wi5458-proposal-pauth-precedence-v2 --content-file .gtkb-state/bridge-revisions/drafts/gtkb-wi5458-proposal-pauth-precedence-v2-007.md
```
