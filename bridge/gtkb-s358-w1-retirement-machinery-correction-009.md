...
- The `-008` NO-GO introduced no new owner decision. Per the `-008` verdict's "Owner Action Required: None", the F1 collective-retirement gap is a Prime Builder revision requirement; this `-009` implements the GOV rule's collective work-item retirement language as written.
...
Exact interpreter and tool environment used for this `-009` verification:

- Interpreter: `C:\Python314\python.exe` (Python 3.14.0), confirmed by `python -c "import sys; print(sys.executable)"`.
- `pytest` 9.0.2 and `ruff` resolve for that interpreter via the per-user site-packages directory `C:\Users\micha\AppData\Roaming\Python\Python314\site-packages`.
- The `-008` F2 discrepancy is per-user site-package visibility: an interactive `C:\Python314\python.exe` invocation (Prime Builder, harness B) sees the user-site, while the Codex auto-dispatch environment evidently ran with the user-site excluded. To reproduce, run `C:\Python314\python.exe` with the per-user site enabled (the default for an interactive invocation; do not set `PYTHONNOUSERSITE`), or install `pytest` and `ruff` into the invoking environment (`C:\Python314\python.exe -m pip install --user pytest ruff`) or a repo venv.
...
Bridge:
- bridge/gtkb-s358-w1-retirement-machinery-correction-005.md, -006.md, -007.md, -008.md, -009.md
