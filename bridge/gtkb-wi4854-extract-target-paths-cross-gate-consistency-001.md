...
# Implementation Proposal: WI-4854 extract_target_paths cross-gate consistency (no post-GO dead-end)
...
Work Item: WI-4854
Project Authorization: PAUTH-PROJECT-GTKB-IMPLEMENTATION-START-GATE-HARDENING-001-WI-4854-EXTRACT-TARGET-PATHS-CONSISTENCY
...
- A GO'd proposal whose authorized-file list used a non-inline form was rejected
  by `begin` ("missing concrete target_paths") even though every pre-GO gate
  (both preflights + the bridge-compliance-gate) accepted it — a post-GO dead-end
  requiring a REVISED re-file plus re-GO.
- A later revision used the heading form correctly, but its revision narrative
  quoted the label-and-bracket pattern as prose; `search()` matched the prose
  occurrence first and raised on the placeholder, again before the heading branch
  ran. The independent LO `-005` NO-GO confirmed this.

The defect class: the pre-GO gate chain and `begin` disagree on which forms are
acceptable, and the reader fails closed on a leftmost partial match instead of
trying all forms. Result: a proposal can clear every gate and receive GO, then be
un-implementable.
...
- bridge/gtkb-wi4852-watchdog-dormancy-auto-restart-001.md / -003.md / -005.md /
  -006.md — the live evidence: -001 non-inline form blocked `begin`; -003 prose
  placeholder collision; -005 NO-GO confirmation; -006 inline-form workaround.
...
- WI-4624 (resolved) — `begin` false-positives on descriptive path tokens; same
  reader-robustness theme.
...
(`bridge/gtkb-wi4854-extract-target-paths-cross-gate-consistency-001.md`) under
...
- DELIB-20266194 — owner AUQ (2026-06-26) authorized the NEW-implementation-proposal
  ...
  PAUTH PAUTH-PROJECT-GTKB-IMPLEMENTATION-START-GATE-HARDENING-001-WI-4854-EXTRACT-TARGET-PATHS-CONSISTENCY
  ...
  decision is required to review this proposal.
