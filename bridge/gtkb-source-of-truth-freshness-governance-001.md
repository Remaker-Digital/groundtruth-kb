...
author_harness_id: B
author_session_context_id: S376-source-of-truth-freshness-governance-001
...
# Implementation Proposal — Source-of-Truth-Freshness Governance Formalization (WI-3501)
...
- Negative precedent / driving incident: the live demonstration that motivated this proposal — `scripts/session_self_initialization.py`'s startup rollup banners 106 / 20 / 33 / 13 (depending on definition), and the LO handoff itself surfaced still-different counts vs. my fresh re-read (210 / 120 / 133 / 14-or-15). The divergence-of-snapshots-over-time pattern is the failure mode the GOV/DCL forbid.
...
5. File the post-implementation report at
   `bridge/gtkb-source-of-truth-freshness-governance-002.md` carrying
   forward Specification Links, providing the spec-to-test mapping
   (`## Spec-Derived Tests`), the commands executed, the observed results,
   and the recommended commit type (`feat`).
...
and reported in the post-impl `-002.md`.
...
Two refinements I want Codex to scrutinize during review:

1. The `bridge_kind: governance_review` self-declaration. Rationale: this
   proposal scopes pure governance artifact authoring (DA/GOV/DCL inserts),
   and PROJECT-GTKB-SOURCE-OF-TRUTH-FRESHNESS has no PAUTH at filing time so
   the standard `Project Authorization:` / `Project:` / `Work Item:` triple
   cannot be populated. If Codex prefers an alternate path (e.g., file a
   sibling proposal that mints a PAUTH first; or treat this thread as
   `implementation_review` with `Work Item: WI-3501` and accept a temporary
   `Project Authorization: <NONE>` marker), please surface the preferred
   shape in NO-GO.

2. The fresh-read divergence captured above (106/20/33/13 in the handoff vs
   210/120/133/14-or-15 in my fresh re-read 16 minutes later) is itself the
   live demonstration the GOV/DCL forbid. The proposal text does NOT bind
   any specific count to the GOV/DCL acceptance — counts will be re-read at
   the WI-3500 / WI-3503 / WI-3502 implementation time. Codex should
   confirm this is the right framing rather than requiring the proposal to
   pin counts.
