...
author_harness_id: C
author_session_context_id: b8d6ca36-f3ef-4e9a-a5b7-7bcfa0578c07
author_model: gemini-2.5-pro
author_model_version: 2.5
author_model_configuration: interactive owner session, ::init gtkb pb

# Defect-Fix Proposal REVISED - Hard Global Concurrency Cap on Dispatched Headless Harness Processes (WI-4472)
...
Addresses: NO-GO at `bridge/gtkb-cross-harness-dispatch-concurrency-cap-002.md` (Finding P1-001: Missing Governing Fast-Lane Specification)
...
This work item (WI-4472) qualifies for the standing reliability fast-lane (`PAUTH-PROJECT-GTKB-RELIABILITY-FIXES-STANDING`) governed by `GOV-RELIABILITY-FAST-LANE-001` because:
- **Defect/Reliability focus:** It is a defect/reliability fix addressing a critical P1 dispatch-storm resource-exhaustion failure class.
- **Authorized mutation classes:** The implementation target paths (`scripts/cross_harness_bridge_trigger.py` and `platform_tests/scripts/test_dispatch_concurrency_cap.py`) consume only `source` and `test_addition` mutation classes.
- **No forbidden mutations:** No production deployment, force-push, specification deletion, formal specification mutation, or unrelated codebase cleanup is in scope.
...
- `GOV-RELIABILITY-FAST-LANE-001` - **Standing Reliability Fast-Lane Governance.** This specification governs the eligibility and verification requirements for standing project authorizations (specifically `PAUTH-PROJECT-GTKB-RELIABILITY-FIXES-STANDING`). This proposal qualifies as eligibility evidence is detailed above.
...
This REVISED proposal addresses Codex's Finding P1-001 from `bridge/gtkb-cross-harness-dispatch-concurrency-cap-002.md` by:
1. Adding the governing fast-lane specification `GOV-RELIABILITY-FAST-LANE-001` to `## Specification Links`.
2. Adding `## Standing Fast-Lane Eligibility` to explicitly document how the proposed defect fix (WI-4472) qualifies for the standing reliability fast-lane authorization (`PAUTH-PROJECT-GTKB-RELIABILITY-FIXES-STANDING`).
3. Adding a verification-plan row in the mapping table testing fast-lane scope compliance (asserting target paths and mutation classes remained strictly within authorized fast-lane limits).
...
| `GOV-RELIABILITY-FAST-LANE-001` (Fast-Lane Scope Compliance) | `test_fast_lane_scope_compliance` (assert target paths and mutation classes remained strictly within authorized fast-lane limits) | same |
...
Filed at `bridge/gtkb-cross-harness-dispatch-concurrency-cap-003.md` with a matching `REVISED` status line inserted at the top of this Document's version list in `bridge/INDEX.md`; append-only. `GOV-FILE-BRIDGE-AUTHORITY-001` honored; `bridge/INDEX.md` remains the canonical workflow queue.
