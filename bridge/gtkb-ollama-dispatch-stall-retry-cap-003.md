...
Implemented the approved reliability hotfix for `gtkb-ollama-dispatch-stall-retry-cap`.
...
- strips inherited `GTKB_IMPLEMENTATION_AUTH_*` variables before spawning a child, then re-adds fresh values only when the current spawn minted Prime implementation authorization packets.

The last item was found during the focused regression suite because this auto-dispatch session itself carries implementation authorization env vars. Without the scrub, a Loyal Opposition child could inherit stale Prime-only authorization context.
...
Observed numstat for authorized target paths:

```text
114     0       platform_tests/scripts/test_cross_harness_bridge_trigger.py
162     64      scripts/cross_harness_bridge_trigger.py
```

Known shared-file overlap: the GO verdict noted that `platform_tests/scripts/test_cross_harness_bridge_trigger.py` already contained unrelated local edits. This report covers only the hotfix assertions added in that same file: prior-launch metadata preservation, fatal-marker retry, Ollama one-item cap, and mandatory preflight prompt wording.

## Specification Links

- `GOV-FILE-BRIDGE-AUTHORITY-001` - bridge INDEX remains the authoritative lifecycle surface.
- `GOV-ARTIFACT-ORIENTED-GOVERNANCE-001` - defect, implementation evidence, and residual risk are preserved in this bridge artifact.
- `GOV-RELIABILITY-FAST-LANE-001` - scoped reliability fix under active `WI-4388`.
...
- `ADR-CODEX-HOOK-PARITY-FALLBACK-001` - failed worker evidence is surfaced instead of hidden as healthy no-op liveness.
...
| CQ-PATHS-001 | Yes | `git diff --name-only -- scripts\cross_harness_bridge_trigger.py platform_tests\scripts\test_cross_harness_bridge_trigger.py` returned only the two authorized target paths. |
...
| CQ-CONSTANTS-001 | Yes | New constants declare the Ollama cap, fatal markers, log read limit, and implementation-auth env names. |
...
| CQ-VERIFICATION-001 | Yes | Pytest, ruff lint, ruff format, readiness, diagnose, and scoped diff checks were executed. |

## Spec-To-Test Mapping

| Specification | Verification |
|---|---|
| `GOV-FILE-BRIDGE-AUTHORITY-001` | This report is filed as the next bridge version and the helper updates `bridge/INDEX.md` with latest `NEW`. |
| `GOV-ARTIFACT-ORIENTED-GOVERNANCE-001` | This report records implementation claim, evidence, risk, owner authorization, and rollback. |
| `GOV-RELIABILITY-FAST-LANE-001` | Scope stayed inside the approved dispatcher/test target paths for `WI-4388`. |
| `DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001` | Implementation authorization packet `sha256:6e1643112cbeb705967f5224ddd1bef10951869045a2c37f460dba900400704b` was issued from the live GO thread. |
| `DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001` | `test_unchanged_signature_preserves_last_launch_metadata`, `test_unchanged_signature_with_previous_fatal_worker_log_retries`, `test_ollama_loyal_opposition_dispatch_caps_selected_batch_to_one`, and prompt assertions cover the new behavior. |
...
```text
python -m py_compile scripts\cross_harness_bridge_trigger.py platform_tests\scripts\test_cross_harness_bridge_trigger.py
Result: pass.
```

```text
groundtruth-kb\.venv\Scripts\python.exe -m pytest platform_tests\scripts\test_cross_harness_bridge_trigger.py -q --tb=short
Result: 63 passed, 1 warning in 2.35s.
...
Result: HEALTHY. prime-builder pending=1 selected=1. loyal-opposition pending=10 selected=1. Recent failures include 3 fatal_worker_output_marker records.
```
...
- Unchanged selected signature with prior fatal worker stderr is no longer treated as healthy `unchanged`: satisfied by fatal-marker retry helper and regression test.
- Durable failure evidence is recorded for prior fatal worker output: satisfied by `previous_launch_failed` JSONL record and regression test.
