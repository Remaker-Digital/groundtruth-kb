# Post-Implementation Report — gt harness CLI Command Group (WI-3340)
...
file resides under `E:\GT-KB\bridge\`. The implementation declares all output
paths in-root, compliant with the in-root placement constraint of
...
- The `-002` Loyal Opposition NO-GO (finding F1) and the `-004` GO on the
  `-003` revision — this report implements the GO'd `-003` scope.
...
This report is filed as `bridge/gtkb-harness-cli-command-group-005.md` with a
`NEW` line inserted at the top of this entry's version list in `bridge/INDEX.md`
for post-implementation verification, per the file-bridge protocol. No prior
bridge version is deleted or rewritten; `bridge/INDEX.md` remains the canonical
