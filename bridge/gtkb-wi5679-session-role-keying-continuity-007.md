...
# WI-5679 Session-Role Keying Continuity - Second Revised Implementation Proposal

bridge_kind: prime_proposal
Document: gtkb-wi5679-session-role-keying-continuity
Version: 007
Date: 2026-07-28 UTC
Responds to: bridge/gtkb-wi5679-session-role-keying-continuity-006.md
...
This revision preserves every closure accepted by versions 004 and 006 and
closes both version-006 blockers without asking the owner to accept the
previously rejected continuity-chain-only outcome. The inert shared-projection
backstop is removed. In its place, bounded liveness recovery uses only an
explicit predecessor/successor relation created when two session ids are
co-observed in the same hook or continuation invocation. A shared projection,
an open-document scan, recency, and durable harness identity are never
sufficient role authority.

The canonical init parser is added as an eighth protected target because both
parsers must conform to the first-line requirement, not merely agree with one
another. The complete clean-HEAD baseline is stated for both mandated pytest
blocks: 152 passed / 6 failed and 18 passed / 3 failed, with all nine failures
named below.
...
006 remain immutable audit history.
...
implementation-start packet for this exact eight-path target set. If either
...
two-sided write can recover from the successor's recorded edge, including the
SessionStart path when UserPromptSubmit does not fire. It does not collapse to
continuity-chain-only and never borrows an unrelated peer. No new owner
...
The first mandated block is 158 collected, 152 passed, 6 failed. The second is
21 collected, 18 passed, 3 failed. All nine exact failures are listed in the
verification section. The two role-authority conformance failures retain their
legacy `WI-4781` ownership marker and are explicitly routed to WI-5718 for the
retired-authority purge/current-spec correction; this proposal neither hides
nor absorbs them.
...
may recover only from exactly one open same-harness keyed document whose
validated `predecessor_session_id` names the requested id and whose typed
binding proves the relation came from the current hook/continuation contract.
The candidate's own worker-role provenance must validate against its own
session id. Zero candidates, multiple candidates, a peer without that exact
edge, a cross-harness edge, or a closed candidate fails visibly.
...
`.claude/session/envelope.json`. A projection may locate a keyed candidate as
an optimization, but the candidate is reloaded from its exact authoritative
path and must pass the same reverse-edge checks. Concurrent projection
ownership remains separate work; this change does not claim to close WI-5086,
WI-5718, or WI-5721.

### C3 - Strict transcript-only restart inheritance

Before SessionStart creates a replacement document, inspect only the explicitly
named predecessor. Inherit its role only when validated
...
The only protected implementation targets are the eight paths declared above.
...
No `.codex/` file is changed; those files are read-only parity evidence. If
implementation proves any ninth path necessary, Prime Builder must stop and
...
| `GOV-PROJECT-IMPLEMENTATION-AUTHORIZATION-001` | Normal exact-session claim and implementation-start packet before any protected edit | Packet authorizes exactly the eight target paths and no emergency route is used |
...
2. A normal exact-session claim and implementation-start packet authorize
   exactly the eight declared paths. Failure of either gate leaves source and
...
11. No ninth protected implementation path, direct database mutation,
