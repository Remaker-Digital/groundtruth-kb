...
# Implementation Proposal: WI-4852 watchdog-dormancy auto-restart as a health_response remediation action
...
- No prior deliberation rejects adding a watchdog-liveness remediation action.
...
## Design (one design decision flagged for Loyal Opposition)
...
1. Dormancy detection (pure, in `dispatch_monitor.py`): a side-effect-free
   ...
   clock/IO inside) that returns a structured dormancy verdict
   (`dormant: bool`, `age_seconds`, `reason`) given the watchdog's last-run
   evidence epoch. `threshold_seconds` defaults to a configurable multiple of the
   watchdog cadence (proposed default: 2x cadence, fail-open below threshold).

2. Remediation action (in `dispatch_monitor.py`): a daemon-level (not per-role)
   `ResponseAction`-shaped record with `action = "remediate"` and
   `remediation_hint = "restart_storm_watchdog"` emitted when dormancy is
   detected. Per-role `health_response()` is unchanged; the watchdog check is a
   sibling daemon-level surface so the per-role contract is preserved.

3. Execution (in `gtkb_dispatcher_daemon.py`): on `restart_storm_watchdog`, the
   daemon re-runs/re-enables the watchdog scheduled task, fail-soft (a restart
   failure is recorded in status.json and never aborts the tick), mirroring the
   existing fail-soft monitoring contract (test_run_tick_monitoring_failsoft).

**Design decision for LO — the dormancy signal source:**

- Option B (RECOMMENDED): the watchdog writes a lightweight heartbeat artifact
  on each run (symmetric with the daemon's existing `heartbeat.txt`), and the
  daemon reads its age. Cross-platform, directly unit-testable via a pure
  age-vs-threshold function, and consistent with the daemon liveness model.
  Adds a small heartbeat write to `scripts/ops/harness_storm_watchdog.ps1`.
- Option A (alternative): query the Windows scheduled task's LastRunTime/State
  directly (schtasks / Get-ScheduledTaskInfo). No watchdog-side change, but
  platform-coupled, harder to unit-test, and silent if the task was
  deregistered entirely.

This proposal implements Option B unless LO directs Option A. The platform
binding for the restart step (scheduled-task name + re-run mechanism) follows the
DCL-SINGLE-HARNESS-DISPATCHER-DESKTOP-TASK-001 platform-bindings pattern.
...
- Rollback: changes are additive (a new pure function, a new daemon-level action,
  fail-soft execution, a watchdog heartbeat write). Reverting the five target
...
(`bridge/gtkb-wi4852-watchdog-dormancy-auto-restart-001.md`) under the canonical
append-only numbered-file chain. Prior versioned bridge files are never rewritten
or deleted; revisions and verdicts are added as new numbered files
(`-002.md`, `-003.md`, ...) so the numbered file chain remains the canonical
...
- DELIB-20266192 — owner AUQ (2026-06-26) selected "TIER-1 dispatcher: WI-4852"
  ...
  review this proposal. The dormancy-signal design decision (Option B vs A) is a
  technical choice for Loyal Opposition, not an owner decision.
