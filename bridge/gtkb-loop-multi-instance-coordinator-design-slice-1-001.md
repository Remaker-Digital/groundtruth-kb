...
# WI-4281 — Design: /loop multi-instance coordinator service
...
- `DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001` — design-completeness verification plan below (design-only; no source tests in this slice).
...
This is a **design-only** slice (`target_paths: []`, no source mutation), so
verification is design-completeness, not source tests:

| Governing input | Design-completeness criterion | Evidence in this proposal |
|---|---|---|
...
a new `gtkb-loop-multi-instance-coordinator-design-slice-1` document list in
