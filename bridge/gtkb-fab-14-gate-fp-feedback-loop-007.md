...
Date: 2026-06-12
Responds-To: bridge/gtkb-fab-14-gate-fp-feedback-loop-006.md (NO-GO)
...
It responds to the `-006` NO-GO, which found the scope expansion directionally justified but flagged ONE
blocking clause gap: the `-005` proposal omitted explicit `GOV-FILE-BRIDGE-AUTHORITY-001` /
CLAUSE-INDEX-IS-CANONICAL filing evidence. `-007` is identical to `-005` in substance and adds the
`## Bridge Protocol Compliance` section (below) to clear that clause. No target_paths, area intent, or
disposition changes from `-005`.
...
3. **`groundtruth-kb/templates/hooks/**`** — Area 4 (HYG-047) + Area 1 template parity. The tracked
   ...
   template in lockstep or the parity test fails.
...
All other content (the four areas, owner constraints, verification plan) is unchanged from `-003`/`-005`.

## Bridge Protocol Compliance

Filed at `bridge/gtkb-fab-14-gate-fp-feedback-loop-007.md` with a matching `REVISED` line inserted at the
top of the `gtkb-fab-14-gate-fp-feedback-loop` entry in `bridge/INDEX.md` (INDEX update: insert at the top
of the entry's version list, above the `-006` NO-GO line). `bridge/INDEX.md` remains the canonical
workflow state; this proposal does not alter that. Append-only discipline is preserved: all prior
versions (`-001` through `-006`) remain on disk and in the INDEX — none is deleted, renamed, or
rewritten. `GOV-FILE-BRIDGE-AUTHORITY-001` (CLAUSE-INDEX-IS-CANONICAL) is honored, and nothing
...
- Area 2: PowerShell matcher in `.claude/settings.json`; Codex directive adapter in
  `.codex/gtkb-hooks/` + registration in `.codex/hooks.json`; extend the parser FP corpus with
  PowerShell/Codex cases.
...
- `GOV-FILE-BRIDGE-AUTHORITY-001` — filed under `bridge/` with a matching `REVISED` INDEX entry; append-only (see Bridge Protocol Compliance above).
...
- `bridge/gtkb-fab-14-gate-fp-feedback-loop-005.md` / `-006.md` — the prior REVISE and the NO-GO this `-007` clears.
...
PowerShell/Codex coverage; HYG-046 three rigidities; HYG-047 auto-discovery in both gates). The
active `PAUTH-FAB14-20260610` (covering WI-4426) remains the substantive authority for the work.

## Requirement Sufficiency

**Existing requirements sufficient.** The dispositions are fixed by `DELIB-FAB14-REMEDIATION-20260610`
and unchanged. This REVISE is a pure `target_paths` scope correction (plus the `-007` clause-evidence
addition) so the already-decided work can be implemented within authorized scope; it introduces no new
requirement. The governing specifications (`SPEC-AUQ-POLICY-ENGINE-001`, `SPEC-AUQ-NO-LLM-CLASSIFIER-001`,
