...
# Implementation Proposal - WI-5361 Invalid Terminal VERIFIED Verdict Reissue Repair
...
Repair one malformed file-only terminal `VERIFIED` artifact in the `gtkb-wi5361-dispatch-cap-authority-precedence` bridge chain without touching source, tests, database state, dispatcher state, or unrelated bridge threads.

The current `bridge/gtkb-wi5361-dispatch-cap-authority-precedence-004.md` file is untracked, starts with `VERIFIED`, and substantively records Loyal Opposition terminal review, but it was written as a file-only verdict rather than through the canonical atomic finalizer. The current finalizer rejects its body because it lacks `Recommended commit type` evidence. It also lacks helper-generated `## Commit Finalization Evidence`, so it must not be swept into a direct pathspec commit.

This proposal applies the established invalid-terminal-verdict repair pattern: copy the exact failed verdict bytes to a durable in-root archive, verify byte/hash identity, remove only the untracked failed bridge copy, and let independent Loyal Opposition reissue `gtkb-wi5361-dispatch-cap-authority-precedence` version `004` through `.claude/skills/verify/helpers/write_verdict.py --finalize-verified` using a modern body that passes `validate_verified_body()` and receives helper-generated commit-finalization evidence.
...
- Failed verdict path: `bridge/gtkb-wi5361-dispatch-cap-authority-precedence-004.md`.
- Git status: untracked (`??`).
- Size: `2534` bytes.
- SHA-256: `FCD87706CE5BAA86F4D3B8655F8BBEFEE7162BDF8B9C197A7C475F7CBB12E814`.
- Git blob hash: `38d52aa53372a705a52329654adf065cb048bb99`.
...
1. Reconfirm that `bridge/gtkb-wi5361-dispatch-cap-authority-precedence-004.md` is still untracked, terminal `VERIFIED`, and matches the recorded size, SHA-256, and Git blob hash, or record fresh replacement evidence before mutation.
2. Copy that exact file to `independent-progress-assessments/WI-5370-wi5361-verdict-004.invalid-finalizer.md`.
...
5. Confirm the original `gtkb-wi5361-dispatch-cap-authority-precedence` thread returns to latest `NEW` at version `003`.
...
- The invalid version `004` bytes are preserved exactly at `independent-progress-assessments/WI-5370-wi5361-verdict-004.invalid-finalizer.md`.
- Only the failed untracked `bridge/gtkb-wi5361-dispatch-cap-authority-precedence-004.md` file is removed by this repair.
- The original `gtkb-wi5361-dispatch-cap-authority-precedence` thread becomes latest `NEW` at version `003` until LO reissues the terminal verdict.
- A later valid finalization transaction can write and commit a replacement version `004` through the canonical helper with same-transaction evidence.
...
Risk is low but provenance-sensitive. Until the replacement verdict is finalized, rollback is a byte-for-byte copy from the archive back to `bridge/gtkb-wi5361-dispatch-cap-authority-precedence-004.md`. After successful helper finalization, the committed replacement verdict and its helper-generated finalization evidence become authoritative. No Git history rewrite, hook bypass, dispatcher mutation, push, deployment, release, credential action, or unrelated cleanup is allowed.
