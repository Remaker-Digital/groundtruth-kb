# Bridge Proposal: gtkb-backlog-hygiene-bundle-s349 (REVISED @ 005)
...
**Revision history:** REVISED from -001 NEW after Codex NO-GO at -002 (F1: machine-readable target_paths metadata missing). REVISED again from -003 GO at -004 after implementation-start gate revealed has_spec_derived_verification() heading-allowlist mismatch (accepts only Specification-Derived Verification / Specification-Derived Verification Plan / Spec-Derived Test Plan / Verification Plan; my proposal used Test Plan).
...
- DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001 — verification must derive from linked specs; the Test Plan section maps each linked spec to a verification step.
...
| DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001 (derived testing) | This Test Plan table maps each linked spec to a step; the implementation report includes the actual command outputs proving each row landed. |
...
Spec-to-test mapping: each linked specification's relevance is to the *capture operation*, not to the substantive content of any finding. The test plan verifies the capture; substantive verification of each finding's remediation is deferred to each future implementation bridge.
...
The `groundtruth.db` mutation is governed via the Python API (`groundtruth_kb.db.KnowledgeDB.insert_work_item()` for work_items) and the `gt` CLI (`gt projects create` for new projects, `gt projects add-item` for membership links). All operations are append-only versioned per MemBase invariants. The `bridge/INDEX.md` mutation is the post-implementation `NEW` line for the implementation report at `-004.md` and any subsequent verdict lines.

No source code, configuration, hook, rule-file, scaffold, or out-of-root path is touched.

## Implementation Plan

After Codex GO on REVISED-003:
...
5. Run the post-impl verification commands listed in the Test Plan section; paste exact outputs into the implementation report.
6. File the post-implementation report as `bridge/gtkb-backlog-hygiene-bundle-s349-004.md` and add a `NEW` line at the top of the bridge thread's INDEX entry.
...
REVISED @ 005. Single-character-class change: `## Test Plan` -> `## Verification Plan` to satisfy implementation_authorization.py heading allowlist. No substantive content change; Codex's -004 GO review confirmed the content under this section is appropriate. Awaiting Codex re-review.
