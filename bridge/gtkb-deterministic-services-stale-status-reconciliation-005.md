...
Version: 005
Responds to: bridge/gtkb-deterministic-services-stale-status-reconciliation-004.md NO-GO
...
Codex NO-GO -004 confirmed the two -002 P1 findings are resolved (command
shapes dry-run clean, GOV-15 fail-closed proven, DELIB-2737 exists) and raised
one new P1: the verification plan + acceptance criterion 4 invoked the
`backlog list` command with a `--project` filter, which the current CLI does
not support (`backlog list` exposes only `--json` and `--all`).

## F1 (-004) — non-executable backlog-list verification command — CORRECTED

Reproduced: invoking `backlog list` with a `--project` filter errors
`No such option: --project`. Confirmed `backlog list` help exposes only
`--json` and `--all`.

Corrected: both the GOV-STANDING-BACKLOG-001 verification-plan row and
acceptance criterion 4 now use
`projects show PROJECT-GTKB-DETERMINISTIC-SERVICES-001 --json`. Verified live
that this command returns a `work_items` array in which each member row carries
`resolution_status`, `stage`, and `status_detail` — directly proving the 7
target rows' terminal states after reconciliation. (Example confirmed: the
returned array already shows WI-3264 `resolution_status: resolved`.)

No other content changed from -003. The corrected mutation commands, PAUTH
bootstrap with `--include-spec`, and DELIB-2737 owner-decision citation are
...
text are introduced or modified. It is a one-time batch reconciliation of seven
...
|---|---|---|
| `GOV-FILE-BRIDGE-AUTHORITY-001` | INDEX entry + Codex GO at -006 + post-impl -007 + VERIFIED -008 | Latest INDEX status per bridge protocol |
...
4. `projects show PROJECT-GTKB-DETERMINISTIC-SERVICES-001 --json` returns a
   `work_items` array in which the 7 target WIs show their new terminal
   `resolution_status` (6 `resolved`, 1 `wont_fix`) and none remain `open`.
...
- `bridge/gtkb-deterministic-services-stale-status-reconciliation-005.md` (this REVISED)
