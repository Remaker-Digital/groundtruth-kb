...
Self-corrected preflight findings without owner re-review:

- **Applicability preflight finding (advisory)**: -001 omitted citations for
  `ADR-ARTIFACT-ORIENTED-DEVELOPMENT-001` and `DCL-ARTIFACT-LIFECYCLE-TRIGGERS-001`,
  both of which the applicability preflight matched on content. Both are
  `advisory` severity (not blocking) but cleanly belong in the Specification
  Links section. Added to -002.
- **Clause preflight finding (blocking)**: -001 triggered
  `GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS` applicability via the
  phrase "Not a Bulk Operation" in the Clause Scope Clarification heading,
  but the evidence-pattern detector
  `(?i)(?:inventory|review[- ]packet|DECISION DEFERRED|formal-artifact-approval)`
  found no satisfying anchor word. -002 adds an honest evidence-anchor sentence
  in Clause Scope Clarification stating why no bulk-operation inventory or
  review packet is required (because this proposal is not a bulk operation).
  No owner waiver invoked.

No substantive scope change vs -001. All target_paths, mutations, acceptance
criteria, and risks remain identical.
...
INDEX entry was inserted at the top under -001 NEW; -002 REVISED line
prepended on the same entry block:
...
- **Risk**: Helper script reads DB before each idempotency check; a
  ...
  checked at proposal time (one live Claude session = mine; one live Codex
  session; no second active Claude session per lock heartbeats); operator
  should re-check at implementation time.
