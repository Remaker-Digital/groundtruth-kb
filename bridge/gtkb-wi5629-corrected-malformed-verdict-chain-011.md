...
# Revised Implementation Proposal - Exact-thread operation-neutral bridge lifecycle resolution

bridge_kind: prime_proposal
Document: gtkb-wi5629-corrected-malformed-verdict-chain
Version: 011
Responds to: bridge/gtkb-wi5629-corrected-malformed-verdict-chain-010.md
Date: 2026-07-19 UTC
...
Resolution is bounded to one exact bridge document. Only files named exactly
`<bridge-id>-NNN.md` participate. Every other filename is ignored, including a
valid bridge document whose longer slug begins with the selected bridge id.
Adding, changing, or removing such a prefix sibling cannot change the selected
thread's resolver result, diagnostics, implementation pair, or quarantine set.
Legacy no-suffix `<bridge-id>.md` files are not part of the new resolver.
...
### WI-5629 v007/v008 - Prefix siblings must not affect exact-thread authority

Accepted and corrected. Version 005's phrase "reject exact-prefix siblings"
could make one selected thread fail because a separate valid thread happened to
share its prefix. A live inventory found 110 such relationships. This revision
instead enumerates only exact `<bridge-id>-NNN.md` filenames and ignores every
nonmatching sibling.

Four sibling-invariance fixtures cover a longer valid slug, a malformed prefix
sibling, numeric-looking sibling suffixes, and sibling creation/removal after a
result is captured. Each fixture requires the selected thread's serialized
result to remain byte-equivalent. Duplicate versions and version gaps are still
errors only when they occur inside the selected exact chain.

Version 006's exact-numbered-chain observation is also incorporated: the new
resolver does not preserve legacy no-suffix `<bridge-id>.md` compatibility.
Existing authorization fixtures are updated to write canonical `-001.md`
version-one files.
...
- `DELIB-20260719-DISPATCHER-NEXT-MASTER-PB-AUTHORIZATION` accepts the isolated
  Dispatcher Next program, requires Master Prime Builder closure of derived
  prerequisites, and preserves independent review plus per-slice
  implementation-start gates.
- PAUTH `PAUTH-DISPATCHER-NEXT-PROGRAM-20260719` v4 is active and includes
  WI-5629 with the exact source/test mutation classes used here.
- No additional owner input is required. This revision narrows a shared
  lifecycle resolver to the canonical exact-thread contract and does not change
  product scope, runtime topology, activation, release, credentials, or
  external systems.
...
- `bridge/gtkb-wi5629-corrected-malformed-verdict-chain-007.md` through
  `bridge/gtkb-wi5629-corrected-malformed-verdict-chain-010.md`
- `bridge/gtkb-dispatcher-next-foundation-spike-001.md` through
  `bridge/gtkb-dispatcher-next-foundation-spike-004.md`
...
- `bridge/gtkb-wi5382-implementation-start-packet-contract-004.md`;
- `bridge/gtkb-wi5626-lifecycle-aware-clause-preflight-006.md`;
- `bridge/gtkb-wi5629-corrected-malformed-verdict-chain-006.md` through
...
2. Enumerate only exact `<slug>-NNN.md` files, require contiguous unique numeric
   versions, reject duplicate exact versions, and ignore every nonmatching
   prefix sibling. Do not accept the legacy no-suffix `<slug>.md` form.
...
12. Add four sibling-invariance fixtures and update legacy v1 fixtures to write
    canonical `-001.md` files.
13. Preserve WI-5382's 53 foreign test lines byte-for-byte.
...
```json
{
  ...
  "baseline": {
    ...
    "thread_boundary": "only exact <bridge-id>-NNN.md files participate; prefix siblings and no-suffix legacy files do not",
  ...
  "hard_invariants": [
    ...
    "Nonmatching prefix siblings can never change an exact thread's result, diagnostics, implementation pair, or quarantine set.",
  ...
  "fail_closed_conditions": [
    ...
    "Any exact-thread result changes because an unrelated prefix sibling is added, changed, or removed.",
...
| No stale fallback | Malformed NEW/REVISED, wrong token, missing NO-ACTION, malformed NO-ACTION/replacement, wrong role/document/link, non-adjacency, duplicate exact versions, and multiple malformed exact files | Stable structural error; no review/implementation artifact and no packet state. |
| Exact-thread sibling isolation | Add/remove a longer valid sibling slug, malformed prefix sibling, numeric-looking sibling suffix, and post-resolution sibling | Serialized selected-thread result remains byte-equivalent; sibling content is never audited, quarantined, or diagnosed. |
| Canonical numbering | Canonical `-001.md` fixture plus legacy no-suffix-only fixture | `-001.md` resolves as version one; no-suffix legacy file is ignored and cannot supply authority. |
...
- Prefix siblings are ignored and cannot perturb exact-thread authority.
- Only canonical `-NNN.md` files participate; legacy no-suffix files cannot
...
- Risk: prefix-related threads could become accidental shared failure domains.
  Mitigation: exact filename enumeration plus byte-equivalent sibling-invariance
  tests.
