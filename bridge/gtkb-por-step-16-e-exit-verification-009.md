REVISED
Responds-to: bridge/gtkb-por-step-16-e-exit-verification-008.md
...
target_paths: ["scripts/remediate_por_step_16e.py", "platform_tests/scripts/test_remediate_por_step_16e.py", "scripts/por_step_16_exit_verification.py", "groundtruth.db", "bridge/gtkb-por-step-16-e-exit-verification-manifest-009.json"]
...
We address the blockers in version 008 by:
1. Promoting the exact 69-adopt / 2,120-retire / 48-waiver manifest to a tracked bridge appendix file at `bridge/gtkb-por-step-16-e-exit-verification-manifest-009.json`.
2. Tracking the approved manifest in `target_paths` and verifying its SHA-256 hash (`8c1933322fe408599b61355a5d7441b834965007a62c78b49f4da59f0b6655fc`) on execution to ensure auditability and prevent local workstation state dependency.
3. Updating the remediation script `scripts/remediate_por_step_16e.py` to read from the tracked manifest path and fail closed if the manifest is missing or has a hash mismatch.
...
The exact list of adoptions (69 tests), retirements (2,120 tests), and waivers (48 specs) is defined in `bridge/gtkb-por-step-16-e-exit-verification-manifest-009.json` with SHA-256 content hash:
```
8c1933322fe408599b61355a5d7441b834965007a62c78b49f4da59f0b6655fc
```
...
| Committed manifest sha256 matches approved hash | Verify in implementation report via `sha256sum bridge/gtkb-por-step-16-e-exit-verification-manifest-009.json` | `GOV-RELEASE-READINESS-GOVERNED-TESTING-001` |

## Pre-Filing Preflight Subsection

### Applicability Preflight

- packet_hash: `sha256:6cea0e39db974855a8adc6f6f560150a5785b88ffbbd403b20e44ef8b98cd80e`
