...
# Harness diagnostic mode - provenance correction for WI-5179

bridge_kind: prime_proposal
Document: gtkb-wi5179-harness-diagnostic-mode
Version: 003
Responds to: bridge/gtkb-wi5179-harness-diagnostic-mode-002.md
...
Correct the sole Loyal Opposition provenance finding by citing the governed
owner-decision archive record `DELIB-202666086` for the already approved
`SPEC-HARNESS-DIAGNOSTIC-MODE-001`. This revision changes no behavior,
implementation scope, target path, PAUTH boundary, or verification requirement.
...
The projection will use the active registry as the parity inventory, read the role solely from the harness's validated session document, and treat dispatch as confirmation of dispatch intent rather than role authority. It will expose at most 50 recent privacy-safe records, including the allowed WI-5173 telemetry fields when present, while preserving `null` and coverage or unavailable reasons when evidence is missing or partial.
...
- `DELIB-202666086` is the governed archival record of the owner's explicit approval of `SPEC-HARNESS-DIAGNOSTIC-MODE-001`; it replaces the non-existent identifier cited in version 001.
- `DELIB-20260711-WI5179-PAUTH-APPROVAL` records owner approval of the bounded WI-5179 implementation authorization.
...
- `bridge/gtkb-wi5173-shim-dispatch-telemetry-008.md` is the independent VERIFIED evidence for the committed WI-5173 telemetry contract this diagnostic may safely project.

## Owner Decisions / Input

- `DELIB-202666086` records the owner's explicit `Approve diagnostic-mode child spec` answer and its bounded specification-only authority.
- `DELIB-20260711-WI5179-PAUTH-APPROVAL` authorizes `PAUTH-PROJECT-GTKB-DISPATCHER-COMPLEX-CLI-WI5179-HARNESS-DIAGNOSTIC-20260711`.
- The authorization permits this revised proposal only. Protected implementation remains blocked until an independent Loyal Opposition GO, a matching work-intent claim, and an implementation-start packet are live.

## Requirement Sufficiency

Existing requirements sufficient. `SPEC-HARNESS-DIAGNOSTIC-MODE-001` specifies
the diagnostic interface, semantics, privacy boundary, telemetry projection,
and acceptance matrix. The active PAUTH constrains implementation to the listed
targets and explicitly excludes provider requests, credentials, secrets,
environment values, prompt or message content, tool or provider bodies,
dispatcher configuration or selection changes, claims or production-state
mutation, metrics enrichment, automatic tuning, deployments, and a competing
report command.

## Findings Addressed

### [P2 -> blocking] Missing spec-approval owner-decision citation

Response: `DELIB-202666086` was created through the governed AUQ-backed
deliberation-record path, using the owner's exact historic answer and a formal
approval packet. The corrected decision links to
`SPEC-HARNESS-DIAGNOSTIC-MODE-001` and has `outcome: owner_decision` with the
authoring session recorded. The obsolete, non-existent decision identifier has
been removed from this proposal.

## Scope Changes

None. The implementation scope remains limited to the declared target paths.
Every declared target and this bridge artifact are located under `E:\GT-KB`;
the revision creates no external live dependency and retains the required
in-root placement boundary.

## Pre-Filing Preflight Subsection

Candidate-content applicability and ADR/DCL clause preflights will run against
this revision before its status-bearing bridge file is written. Filing is
permitted only when both report no blocking gap.
...
| Document-only role provenance | Supply a session document role that conflicts with dispatcher metadata; assert document role and source survive while invalid or missing documents report unavailable rather than infer a role. |
| Privacy and read-only operation | Assert local invocation makes no provider request and serialized output contains none of prompts, messages, generated text, tool arguments, results, credentials, secrets, environment values, or raw provider bodies. |
| Bounded evidence and null semantics | Assert no more than 50 recent records, unknown values are `null`, and every unavailable field has freshness, coverage, or unavailable-reason evidence. |
...
- Risk: a local command could trigger provider traffic or operational mutation. Mitigation: isolate local or cached collection and assert no provider call, claim mutation, configuration write, or dispatcher selection change.
...
This revision will be filed under `bridge/` as the next status-bearing numbered
bridge file for `gtkb-wi5179-harness-diagnostic-mode`; no prior version is
deleted or rewritten. Dispatcher or TAFE state plus the numbered file chain
remain the live workflow state.
