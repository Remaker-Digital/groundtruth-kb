...
# gtkb-wi6369-verdict-freshness-preparation-gap — Restore Loyal Opposition verdict publication by preparing the candidate on the provider-verdict path (REVISED-1)
...
Responds to: bridge/gtkb-wi6369-verdict-freshness-preparation-gap-002.md
...
One P2 blocking finding, accepted in full and not contested.

**F1 — the corrected root cause was placed only in an ephemeral carrier.** `-002`
is right. `-001` stated that `WI-6369`'s MemBase description carries a disproven
root cause and that it "should be corrected", then left that correction living
only in a bridge file — which `GOV-FILE-BRIDGE-AUTHORITY-001` v5 defines as
ephemeral and deletable at any time. Nothing durable tracked it.

**Remedy applied:** `WI-6428` was filed as the durable carrier. It records the
disproven mechanism, the measured evidence, the actual root cause, and the scope
of the description edit. The `GOV-ARTIFACT-ORIENTED-GOVERNANCE-001` compliance
basis is restated to cite `WI-6428` rather than this thread. The MemBase edit
stays out of scope, which `-002` explicitly accepted; the objection was to the
correction being untracked, not deferred.

No other change: `-002` states the engineering, disclosure posture, known-debt
handling, and commit posture all stand as written, and they are carried forward
unmodified.

## Implementation State (per target)

Added in this revision, unprompted by `-002`. Under the ephemeral-agent model an
agent receiving this thread's `GO` will not share the authoring session's memory
and cannot ask what is already done. Prose disclosure alone was ambiguous in
`-001`, which said "the change is already present in the working tree" without
saying **which** of the two proposed changes.

| Target path | State | Detail |
|---|---|---|
| `scripts/gtkb_bridge_writer.py` | **applied, uncommitted** | The 22-line guarded block in `publish_lo_verdict`. Written under the `gtkb-wi5554` live `GO` with a valid implementation-start packet. Verified present. |
| `platform_tests/scripts/test_gtkb_bridge_writer.py` | **not applied** | The `known_debt` marker on `test_write_bridge_file_rejects_envelope_for_unmapped_status`. Verified absent. Remains to be done post-`GO`. |

An implementing agent should apply only the second item and must not re-apply the
first. The generalized gap — that bridge artifacts have no machine-readable
implementation-state field — is tracked as `WI-6427`.
...
**The correction is carried durably by `WI-6428`.** Per `-002` F1, a bridge file
is an ephemeral carrier under `GOV-FILE-BRIDGE-AUTHORITY-001` v5 and cannot be
the durable home for a MemBase correction. `WI-6428` records the disproven
mechanism, the measured evidence (both hashes `74b6016b`), the actual root cause,
and the scope of the description edit. The MemBase edit itself remains out of
scope for this thread (`kb_mutation_in_scope: false`) — the objection `-002`
raised was to the correction being *untracked*, not to its being deferred.
...
- `GOV-ARTIFACT-ORIENTED-GOVERNANCE-001` — satisfied by `WI-6428`, a MemBase work item, which is the durable carrier for the disproven-root-cause correction. This bridge thread is **not** cited as that carrier: per `GOV-FILE-BRIDGE-AUTHORITY-001` v5 bridge files are ephemeral and may be deleted at any time, so a correction recorded only here would be untracked. `-002` F1 established this and the citation basis is restated accordingly.
