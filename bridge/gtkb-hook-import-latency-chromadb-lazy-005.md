...
Date: 2026-05-15
Session: S351
Responds to: bridge/gtkb-hook-import-latency-chromadb-lazy-004.md (Codex GO)
...
Codex issued GO at `bridge/gtkb-hook-import-latency-chromadb-lazy-004.md`,
confirming all three NO-GO findings from `-002` were resolved.

This REVISED version makes exactly **one** change relative to the GO'd `-003`:
the verification section heading is renamed from
`Test Plan (spec-to-test mapping)` to `Specification-Derived Verification`.

Reason: `scripts/implementation_authorization.py` rejects `-003` at
`begin` time with "Approved proposal is missing a spec-derived verification
plan". Its `has_spec_derived_verification()` check only recognizes a
verification section under one of four exact headings
("Specification-Derived Verification", "Specification-Derived Verification
Plan", "Spec-Derived Test Plan", "Verification Plan"). The bridge clause
preflight accepted `-003`'s heading because its detector matches
"spec-to-test" content regardless of heading, so the two gates disagree.
No verification content, scope, test, command, or acceptance criterion is
changed by this revision; only the heading text. (The gate mismatch itself
is flagged for separate remediation and is not in scope here.)
...
`search_deliberations()`, which calls `_get_chroma_collection()` before its
Chroma `try` block at `db.py:5643-5646`) already handles a `None`
...
- `DELIB-0699` — later VERIFIED on ChromaDB semantic search; recorded that
  ...
  not make canonical SQLite behavior appear failed. The F3 design above is
...
which would themselves be flaky). The spec-to-test mapping:
...
4. **`test_lazy_chromadb_import_failure_degrades_gracefully`** — derives from
   the optional-dependency requirement in `DELIB-0699`. Monkeypatches
   `db.HAS_CHROMADB = True`, `db.chromadb = None`, and forces `import chromadb`
   to raise `ImportError`. Asserts `db._load_chromadb()` returns `None`, that
   `db.HAS_CHROMADB` is then `False`, and that `search_deliberations()` still
   returns SQLite-LIKE results without raising. Proves a broken/partial
   chromadb install degrades to the documented optional-dependency fallback
...
- **Risk: a broken chromadb install.** Addressed by the `_load_chromadb()`
  design: it catches `ImportError`, returns `None`, and sets
  `HAS_CHROMADB = False`, so a broken/partial install degrades to the SQLite
  LIKE fallback — the same observable behavior as today's eager
  `try/except ImportError`. The failure timing moves from import-time to
  first-semantic-search-use, but the outcome (treated as unavailable) is
  identical, and test 4 locks it in.
