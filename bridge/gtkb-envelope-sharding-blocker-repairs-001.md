...
# Child Proposal - Transcript And CLI Audit Blocker Repairs
...
target_paths: ["independent-progress-assessments/CODEX-INSIGHT-DROPBOX/session-activity-envelope-sharding-blockers-*.md", ".claude/skills/bridge/helpers/scan_bridge.py", ".codex/skills/bridge/helpers/scan_bridge.py", ".claude/skills/bridge/helpers/impl_report_bridge.py", ".codex/skills/bridge/helpers/impl_report_bridge.py", "scripts/implementation_authorization.py", "scripts/startup_glossary_load.py", "groundtruth-kb/src/groundtruth_kb/bridge/read_commands.py", "groundtruth-kb/src/groundtruth_kb/session/handoff.py", "platform_tests/scripts/test_scan_bridge.py", "platform_tests/skills/test_bridge_impl_report_helper.py", "platform_tests/scripts/test_session_handoff.py"]

## Claim

Repair or disposition the concrete blocker cases that forced agents into direct manipulation or huge raw archival reads during the transcript/CLI audit.

## Scope

- File a blocker disposition inventory for bridge scan raw JSON, implementation authorization list verbosity, helper plan dirty-worktree noise, handoff/session marker drift, and broad generated/cache searches.
- Fix blockers that are narrow and already authorized by this project; route larger or unrelated blockers to narrower follow-on WIs/proposals.
- Update tests so routine focused workflows can answer current/actionable questions without direct DB/file manipulation or raw archival dumps.
...
- `PAUTH-PROJECT-GTKB-SESSION-ACTIVITY-ENVELOPE-SHARDING-WI-4952` - bounded implementation authorization for `WI-4952` only; allowed mutation classes are docs, CLI, tests, skills, and config; forbidden operations are unrelated cleanup, credential lifecycle, and production deployment.
...
- `independent-progress-assessments/CODEX-INSIGHT-DROPBOX/session-activity-envelope-sharding-blockers-*.md`
...
| --- | --- |
| `TEST-11257` linked to `SPEC-INTAKE-46594e` | PASS when every transcript/CLI blocker has a filed disposition, fix, or narrower child proposal, and routine focused workflows no longer require direct manipulation or huge raw object reads for those cases. |
...
- `groundtruth-kb/.venv/Scripts/python.exe -m pytest platform_tests/scripts/test_scan_bridge.py platform_tests/skills/test_bridge_impl_report_helper.py platform_tests/scripts/test_session_handoff.py -q --tb=short`
...
- packet_hash: sha256:48f1850f8e610958d49c6c4fda3742edfcaecec6062b2f68410c81376c4cad7b
...
- Operative file: `.gtkb-state\bridge-propose-drafts\session-activity-envelope-sharding\gtkb-envelope-sharding-blocker-repairs-001.md`
- Clauses evaluated: 5
...
- `WI-4952` is implemented only within the target paths listed above.
- `TEST-11257` has concrete PASS/FAIL evidence in the implementation report.
