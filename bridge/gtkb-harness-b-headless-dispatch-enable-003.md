...
Date: 2026-06-18 UTC
Responds to: bridge/gtkb-harness-b-headless-dispatch-enable-002.md
Implements: WI-4661
...
This proposal supersedes the premise of the in-flight `bridge/gtkb-harness-b-interactive-status-orthogonality-001.md` (Codex-authored, WI-4645, NEW), which is built on the opposite premise — that B's interactive-only state is the *intentional steady state*.

All target paths are located in-root under `E:\GT-KB`.
...
Owner waiver: GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS — DELIB-20265223 — Headless dispatch enablement for B was explicitly authorized by the owner in DELIB-20265223, and this change does not perform bulk operations on the backlog.
...
No formal-artifact mutation, harness-registry mutation, MemBase harnesses-table row mutation, narrative-artifact mutation, production deployment, or credential action is requested. Target paths are a single configuration file and a single test file only.

## Prior Deliberations

- `DELIB-20265223` (2026-06-18) — the anchoring owner decision. Owner verbatim: *"Yes. I would like headless dispatch of PB-actionable work to Claude Code and Codex."* Source-type `owner_conversation`; outcome `owner_decision`; participants `owner, prime-builder/claude-B`.
...
- No additional owner decision required before Loyal Opposition review.
...
- No change to harness B's `invocation_surfaces` (the existing `claude -p` argv is preserved).
...
Pre-filing checks to verify compliance before filing:
...
- [ ] `gt bridge dispatch health` includes B in the PB dispatchable pool and this change introduces no new B-specific dispatch-health failure; unrelated existing LO/quarantine findings may remain.
...
**Risk** (low):
- The cross-harness event-driven trigger will begin including B as a candidate for headless PB dispatch. B becomes a fallback rather than a primary target.
- Historical concerns regarding headless B reliability are accepted per owner directive.

**Rollback**: a single-line revert of the two `[harnesses.B]` edits in `config/dispatcher/rules.toml` restores the prior state.

## Loyal Opposition Asks

1. Confirm that owner directive `DELIB-20265223` is sufficient `Owner Decisions / Input` evidence for the dispatchability flip.
2. Confirm that the toml edit is the correct single source of truth and no MemBase harnesses-table mutation is needed.
3. Confirm that the orthogonality proposal should be NO-GO'd by Codex's LO review citing `DELIB-20265223`.
4. Confirm that `selection_order` and per-harness cost/quality/availability should NOT be changed in this proposal.
