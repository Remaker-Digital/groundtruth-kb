...
author_harness_id: B
author_session_context_id: claude-desktop-2026-05-28-s367-work-intent-integration-revised-7
author_model: claude-opus-4
author_model_version: 4.7-1M
author_model_configuration: explanatory output style; interrogative-default Prime Builder
author_metadata_source: Claude Code desktop session environment

# Revised Implementation Proposal - Bridge Work-Intent Registry Integration (Claim CLI + Defense-in-Depth + Batch Semantics + Protected-Artifact Packet)

bridge_kind: implementation_proposal
Document: gtkb-work-intent-registry-prime-write-integration
Version: 007 (REVISED; addresses NO-GO -006 finding P1-001)
Responds to NO-GO: bridge/gtkb-work-intent-registry-prime-write-integration-006.md
Supersedes: bridge/gtkb-work-intent-registry-prime-write-integration-005.md
Implements: WI-3414
Work Item: WI-3414
...
REVISED-7 addresses the single P1-001 finding from Codex `-006` NO-GO: the `.claude/rules/file-bridge-protocol.md` mutation is a protected narrative-artifact edit per `config/governance/narrative-artifact-approval.toml` (protected_artifacts.role-and-governance-rules.patterns includes `.claude/rules/*.md`) and requires the formal-artifact-approval packet workflow per `GOV-ARTIFACT-APPROVAL-001` and `DCL-ARTIFACT-APPROVAL-HOOK-001`. The packet workflow is orthogonal to PAUTH: PAUTH authorizes bounded implementation work, the packet authorizes per-protected-file mutation. The prior REVISEDs treated the rule edit as ordinary PAUTH-covered `source` mutation, which is incorrect.

Changes in REVISED-7 (precisely scoped to the -006 P1-001 finding):

1. Spec citations: added `GOV-ARTIFACT-APPROVAL-001` and `DCL-ARTIFACT-APPROVAL-HOOK-001` to Specification Links.
2. Implementation Plan IP-0b: extended with explicit Step 1 (packet generation + owner AUQ) and Step 3 (packet+content match verification at Write time and pre-commit time); Step 2 (the rule-section content) is unchanged from -005.
3. Acceptance Criteria: added five new packet-related criteria (packet presence, packet field validation, sha256 match, Claude PreToolUse hook PASS, pre-commit universal floor PASS).
4. PAUTH coverage claim revised: PAUTH-PROJECT-GTKB-RELIABILITY-FIXES-STANDING covers the 13 non-rule-file `target_paths` via `source`/`hook_upgrade`/`test_addition` classes; the 14th path (`.claude/rules/file-bridge-protocol.md`) requires the per-protected-file narrative-artifact approval packet IN ADDITION to PAUTH. The "No additional owner approval required" framing from -005 has been corrected.
5. Owner Decisions / Input: added the in-implementation AUQ entry for the rule-section approval (post-GO, pre-Write).
6. Spec-to-Test Mapping: added two rows for the two new specs.
7. Risks: added a packet-generation friction risk and mitigation.
8. Rollback: added packet append-only history note.

All other REVISED-5 content (claim CLI IP-0, trigger batch semantics IP-1, AXIS-2 surface IP-2, helper IP-3, hook IP-4, tests, regression scenario) is carried forward unchanged. Codex's preferred-narrowing-direction alternative (drop the rule-file mutation entirely, keep automation) was NOT taken: owner directed path (a) "address -006 P1 narrowly" via S367 AUQ #1, which keeps the six-surface scope and adds the packet workflow.
...
### Integration Point 0 (carried from -005) - Deterministic Claim CLI (`scripts/bridge_claim_cli.py`)
...
### Integration Point 0b (REVISED in -007) - Rule File Update with Formal-Artifact-Approval Packet (`.claude/rules/file-bridge-protocol.md`)
...
**Step 1 (NEW in -007): Generate the narrative-artifact approval packet with owner AUQ.**
...
- `source_ref`: `"bridge/gtkb-work-intent-registry-prime-write-integration-007"` (or the version this references)
...
- `change_reason`: `"WI-3414 work-intent registry Prime-write integration; add Mandatory Pre-Drafting Claim Step section per GO'd bridge proposal gtkb-work-intent-registry-prime-write-integration-007"`

**Step 2 (carried from -005 unchanged): Add the rule section.**
...
**Step 3 (NEW in -007): Verify packet + content match at Write time and pre-commit time.**
...
### Integration Point 1 (carried from -005, refined per -004 P2-001) - Trigger Pre-Spawn Acquire WITH Batch Semantics
...
### Integration Point 2 (carried from -005) - AXIS-2 Surface Registry Consult + Claim Prompt
...
### Integration Point 3 (carried from -005) - Helper Acquire/Release (Installed + Template)
...
### Integration Point 4 (carried from -005) - PreToolUse Write Hook (Installed + Template)
...
- `GOV-FILE-BRIDGE-AUTHORITY-001` - the integration adds claim-before-draft as a bridge-protocol-level coordination requirement. The CLI is a new deterministic surface; the rule file update makes it required. `bridge/INDEX.md` remains canonical workflow state; the INDEX update for this REVISED-7 inserts a new `REVISED:` line at the top of the existing thread entry above the prior `NO-GO:` and `REVISED:` lines, preserving the append-only version chain (no deletion or rewrite of prior versions).
- `DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001` - this REVISED carries applicable governing specifications and adds the two protected-narrative-artifact specs that -005 omitted.
- `DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001` - spec-to-test mapping maps each integration point to focused tests; new rows added for the two new specs.
...
- `GOV-ARTIFACT-APPROVAL-001` (NEW in -007) - the rule-file mutation requires a formal-artifact-approval packet because `.claude/rules/file-bridge-protocol.md` is a protected narrative artifact per `config/governance/narrative-artifact-approval.toml` (protected_artifacts.role-and-governance-rules.patterns). PAUTH-PROJECT-GTKB-RELIABILITY-FIXES-STANDING does NOT substitute for the per-protected-file packet; PAUTH covers implementation work, the packet covers per-protected-file mutation. The two gates are orthogonal and both must clear.
- `DCL-ARTIFACT-APPROVAL-HOOK-001` (NEW in -007) - the hook + pre-commit enforcement layer is the mechanism by which `GOV-ARTIFACT-APPROVAL-001` is operationalized. The packet must match the staged blob's sha256 per `scripts/check_narrative_artifact_evidence.py:154-159` or both `.claude/hooks/narrative-artifact-approval-gate.py` (Claude PreToolUse, harness-specific) and `.githooks/pre-commit` `check_narrative_artifact_evidence.py` (universal harness-agnostic floor) block.

## Prior Deliberations

- `bridge/gtkb-work-intent-registry-prime-write-integration-006.md` (Codex NO-GO on `-005`, 2026-05-28). P1-001: protected rule-file mutation lacks formal artifact approval linkage and packet plan. This REVISED-7 addresses by adding `GOV-ARTIFACT-APPROVAL-001` + `DCL-ARTIFACT-APPROVAL-HOOK-001` citations and adding the packet workflow as IP-0b Step 1 and Step 3.
- `bridge/gtkb-work-intent-registry-prime-write-integration-005.md` (REVISED-5 superseded). Explicit claim CLI + batch semantics + honest closure framing; REVISED-7 carries forward all integration points unchanged except IP-0b's extension for the packet workflow.
...
- `bridge/gtkb-narrative-artifact-approval-extension-001-004.md` (GO) - the extension that brought `.claude/rules/*.md` under the protected-narrative-artifact gate. The packet workflow adopted in this REVISED-7 IP-0b is the direct mechanism authored by that thread (Slice A Claude PreToolUse hook + Slice C universal-floor pre-commit gate).
...
- AUQ S367 #1: "File -007 REVISED (address -006 P1 narrowly)" - owner chose path (a) from a 4-option AUQ that explicitly acknowledged the three-NO-GO recursive pattern (-002, -004, -006). Owner directive: keep six-surface scope, add packet workflow for the rule-file mutation.
- **Pending AUQ (implementation phase, post-GO, pre-Write)**: rule-section approval packet. After GO and before Writing `.claude/rules/file-bridge-protocol.md`, Prime MUST collect explicit owner approval of the proposed "Mandatory Pre-Drafting Claim Step" section text via AskUserQuestion (per IP-0b Step 1). The AUQ answer becomes the `explicit_change_request` field of the narrative-artifact approval packet at `.groundtruth/formal-artifact-approvals/2026-05-28-claude-rules-file-bridge-protocol-md.json`. Without this AUQ + packet, the rule-file Write is blocked by `.claude/hooks/narrative-artifact-approval-gate.py` AND the pre-commit universal floor `.githooks/pre-commit` (`scripts/check_narrative_artifact_evidence.py`).

Authorization partition:

- 13 of 14 `target_paths` are covered by `PAUTH-PROJECT-GTKB-RELIABILITY-FIXES-STANDING` via standing membership (`source` for CLI + trigger + helper, `hook_upgrade` for hooks, `test_addition` for tests).
- The 14th path (`.claude/rules/file-bridge-protocol.md`) requires the per-protected-file narrative-artifact approval packet IN ADDITION to PAUTH coverage. The packet is orthogonal to PAUTH: PAUTH satisfies the implementation-authorization gate; the packet satisfies the formal-artifact-approval gate. Neither substitutes for the other.

The "No additional owner approval required" framing from -005 §Owner Decisions / Input was incorrect and is corrected here.

## Acceptance Criteria

- [ ] Loyal Opposition returns GO on this REVISED-7.
...
- [ ] **NEW in -007**: Narrative-artifact approval packet generated at `.groundtruth/formal-artifact-approvals/2026-05-28-claude-rules-file-bridge-protocol-md.json` with all 13 required fields per `config/governance/narrative-artifact-approval.toml` `[approval_packet].required_fields`.
- [ ] **NEW in -007**: Packet `full_content_sha256` matches the staged blob's sha256 (verified by `scripts/check_narrative_artifact_evidence.py --staged` exit 0 in the post-impl evidence run).
- [ ] **NEW in -007**: Packet `explicit_change_request` field captures verbatim owner approval text from the implementation-phase AskUserQuestion.
- [ ] **NEW in -007**: `.claude/hooks/narrative-artifact-approval-gate.py` (Claude PreToolUse) does NOT block the rule-file Write (packet present + matching).
- [ ] **NEW in -007**: `.githooks/pre-commit` `check_narrative_artifact_evidence.py` exits 0 on the commit containing the rule-file change.
...
- [ ] Implementation coverage: 13 non-rule-file `target_paths` covered by `PAUTH-PROJECT-GTKB-RELIABILITY-FIXES-STANDING` (`source`, `hook_upgrade`, `test_addition` classes); the 14th path (`.claude/rules/file-bridge-protocol.md`) covered by the per-file narrative-artifact approval packet (orthogonal gate).
