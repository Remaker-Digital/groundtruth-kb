...
Prime Builder proposes a narrow reliability fast-lane defect fix for WI-5068. The current Codex and Claude scan helper copies import a disposition matrix where NO-ACTION is a real latest-status disposition, but their status-recognition regexes omit that token. The managed template helper already recognizes NO-ACTION, and a focused preflight test currently fails on this exact parser drift.
...
- Origin is defect: current helper parsing contradicts the canonical status/disposition matrix.
...
All target paths are inside the GT-KB root E:/GT-KB. No Agent Red or external project files are in scope.

## Specification Links

- GOV-FILE-BRIDGE-AUTHORITY-001 - requires role-correct bridge status handling and prevents Prime Builder from treating latest LO/terminal dispositions as actionable implementation work.
- GOV-RELIABILITY-FAST-LANE-001 - authorizes small defect/reliability fixes under the standing reliability fast-lane project membership while preserving bridge review and verification.
...
- No prior deliberation was found for this exact helper-regex drift. The current owner directive during the WI-5033 auto-build explicitly restates that bridge/INDEX.md is obsolete and all live GT-KB references must be purged; this defect surfaced while following that directive.
...
| DCL-BRIDGE-PROPOSAL-PROJECT-LINKAGE-MANDATORY-001 | Re-run bridge applicability preflight for this proposal and confirm the live PAUTH/project/WI metadata remains valid. |
| DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001 | Re-run ADR/DCL clause preflight for this proposal and confirm no blocking linkage gaps. |
| DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001 | Implementation report must include the failing-then-passing pytest command for platform_tests/scripts/test_scan_bridge.py. |
...
| ADR-ISOLATION-APPLICATION-PLACEMENT-001 | Confirm all modified paths remain in-root platform/helper/test paths and no adopter application files are touched. |
...
Rollback is a revert of the source/test changes. Bridge files and MemBase records are append-only audit artifacts and must not be deleted by rollback.
