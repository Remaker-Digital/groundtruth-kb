...
Work Item: WI-5055
target_paths: [".claude/skills/advisory-proposal/SKILL.md", ".codex/skills/advisory-proposal/SKILL.md", ".codex/skills/MANIFEST.json", "config/agent-control/harness-capability-registry.toml", "platform_tests/skills/test_advisory_proposal_skill.py", "bridge/gtkb-wi5055-advisory-proposal-skill-*.md", "groundtruth.db"]
implementation_scope: Add the Loyal Opposition/advisory-side skill workflow for drafting ADVISORY bridge entries from reusable external or peer solutions while preserving owner confirmation and non-approval semantics.
requires_review: true
requires_verification: true

# Implementation Proposal - Deliberation-Side Advisory Proposal Skill

## Summary
Add the Loyal Opposition/advisory-side skill workflow for drafting ADVISORY bridge entries from reusable external or peer solutions while preserving owner confirmation and non-approval semantics.
...
The parent proposal and child work items already encode the operative requirement: ADVISORY material can seed scoped proposals only after owner-grilling and explicit project/work-item approval, and advisory capture itself must not become implementation approval. The linked manual test TEST-11293 provides the acceptance anchor for this child item.

## Proposed Scope
- Create the canonical advisory-proposal skill content and Codex adapter so advisory capture uses draft-first ADVISORY creation, final owner confirmation, source advisory preservation, and prior-deliberation capture.
- Register the skill in the skill manifest and harness capability registry using the managed-skill lifecycle surfaces already present in GT-KB.
- Add focused tests that prove advisory capture is not treated as implementation approval and that required owner-decision evidence is preserved.

## Out Of Scope
- No Prime Builder implementation intake workflow is included; that is WI-5056.
- No live scanner or promotion automation is included; that is WI-5057.
...
- `python -m pytest platform_tests/skills/test_advisory_proposal_skill.py -q --tb=short` passes and maps to TEST-11293.
- Manual skill review confirms draft-first ADVISORY creation, final owner confirmation before filing, source advisory/prior deliberation preservation, and explicit statement that advisory capture alone is not implementation approval.
- `python scripts/bridge_applicability_preflight.py --bridge-id gtkb-wi5055-advisory-proposal-skill --json` returns no blocking applicability findings for the final bridge thread.

## Acceptance Criteria
- The advisory-proposal skill is discoverable for the intended advisory authoring context.
- TEST-11293 is satisfied by test output and implementation-report evidence.
