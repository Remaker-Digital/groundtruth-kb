# gtkb-hook-strictness-p1-p2-remediation-001
...
- ADR-ISOLATION-APPLICATION-PLACEMENT-001 — application placement / project root boundary; this proposal modifies files only under `E:\GT-KB` and within `scripts/`, `.codex/gtkb-hooks/`, `.codex/hooks.json`, and `platform_tests/` (no application directory boundary crossings)
...
- .claude/rules/file-bridge-protocol.md — bridge-compliance gate authority
...
- Formal-artifact-approval: the WI insert into MemBase will carry the per-artifact formal-artifact-approval packet at MemBase write time per GOV-ARTIFACT-APPROVAL-001 + DCL-ARTIFACT-APPROVAL-HOOK-001 (collected post-GO, pre-insert, as part of the implementation phase).
...
- `python scripts/bridge_applicability_preflight.py --bridge-id gtkb-hook-strictness-p1-p2-remediation-001` reports `preflight_passed: true` with empty `missing_required_specs`.
- `python scripts/adr_dcl_clause_preflight.py --bridge-id gtkb-hook-strictness-p1-p2-remediation-001` exits 0, OR exits 5 with explicit owner waiver lines per blocking gap.
...
- packet_hash: `sha256:3d517376e303baaa362afec94d14d53d73c9dfb5b3589ac3c4b373a889f17090`
