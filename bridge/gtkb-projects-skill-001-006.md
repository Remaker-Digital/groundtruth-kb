...
# GT-KB Bridge Implementation Report - gtkb-projects-skill-001 - 006
...
Supersedes report: bridge/gtkb-projects-skill-001-005.md
Approved proposal: bridge/gtkb-projects-skill-001-003.md
Recommended commit type: feat:

## Self-Correction Note

`bridge/gtkb-projects-skill-001-005.md` was filed after implementation, then a
Prime-side sanity run of `python scripts/adr_dcl_clause_preflight.py --bridge-id
gtkb-projects-skill-001` found one report-evidence gap for
`GOV-STANDING-BACKLOG-001/CLAUSE-VISIBILITY-BULK-OPS`. This version preserves the
audit trail by superseding `-005` with a complete implementation report that
adds the explicit bulk-operation visibility evidence expected by the mandatory
clause detector.
...
The implementation adds `ProjectLifecycleService` as a deterministic service
over existing MemBase project, project-work-item membership, dependency, and
artifact-link tables. It extends the plural `gt projects` CLI group with
`create`, `show`, `list`, `update`, `add-item`, `reorder`, `retire`, and
`link-bridge`.
...
## Bulk-Operation Visibility Evidence

This implementation did not discover, implement, or execute a bulk backlog
operation. No command updates multiple projects, no command performs a broad
work-item rewrite, and no command creates a second backlog authority.

The implemented guard is:

- `add-item` accepts one explicit work item id.
- `reorder` accepts one project id and requires the caller to name that
  project's active membership set exactly.
- `link-bridge` records one project artifact link and does not edit
  `bridge/INDEX.md`.

If a future project lifecycle request requires multi-project or bulk work-item
updates, it remains outside this GO and must produce a dry-run inventory
artifact, a bridge review packet, and a `DECISION DEFERRED` marker before any
apply path unless a future explicit owner-approval packet authorizes that bulk
action.
...
| --- | --- |
| `GOV-FILE-BRIDGE-AUTHORITY-001` | This report is filed as a new bridge version and `bridge/INDEX.md` remains the queue authority. |
| `DCL-IMPLEMENTATION-PROPOSAL-SPEC-LINKAGE-MANDATORY-001` | GO proposal links are carried forward in this report; implementation authorization was created from the live latest GO before edits. |
...
| `GOV-STANDING-BACKLOG-001` | `test_projects_lifecycle_cli_preserves_append_only_versions` verifies project operations organize existing work items without replacing work-item authority; the Bulk-Operation Visibility Evidence section names inventory artifact, review packet, and `DECISION DEFERRED` handling for out-of-scope bulk work. |
...
| `DCL-CONCEPT-ON-CONTACT-001` | `test_projects_skill_adapter.py` verifies the skill surface preserves terminology deferral and no-second-backlog-authority language. |
...
| `.claude/rules/codex-review-gate.md` | Work began only after latest GO and local authorization packet; this report is filed for Loyal Opposition verification. |
...
- Applicability preflight on `-006`: pending post-file sanity check by Prime.
- Clause preflight on `-006`: pending post-file sanity check by Prime.
...
## Post-File Prime Sanity Check

After filing this `-006` report and inserting it as the latest `NEW` row in
`bridge/INDEX.md`, Prime Builder ran the mechanical bridge preflights against
the live indexed operative file:
