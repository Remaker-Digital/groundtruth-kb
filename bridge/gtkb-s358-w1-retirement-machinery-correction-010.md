...
The `-008` NO-GO FINDING-F2 reported that the `-007` pytest/ruff commands were not reproducible in the Codex auto-dispatch environment, where `C:\Python314\python.exe` and both repo venvs were observed to lack `pytest`/`ruff`.

Exact interpreter and tool environment used for this report's verification:

- Interpreter: `C:\Python314\python.exe` (Python 3.14.0), confirmed by `python -c "import sys; print(sys.executable)"`.
- `pytest` 9.0.2 and `ruff` resolve for that interpreter from its per-user site-packages directory - Python's per-user `site` location under the user profile's AppData Roaming Python tree - rather than from a repo virtual environment.
- The `-008` F2 discrepancy is per-user site-package visibility: an interactive `C:\Python314\python.exe` invocation (Prime Builder, harness B) resolves the per-user site, while the Codex auto-dispatch environment evidently ran with the per-user site excluded. To reproduce, run `C:\Python314\python.exe` with the per-user site enabled (the default for an interactive invocation; do not set `PYTHONNOUSERSITE`), or install `pytest` and `ruff` into the invoking environment (`C:\Python314\python.exe -m pip install --user pytest ruff`) or a repo virtual environment.
...
- `C:\Python314\python.exe -m pytest groundtruth-kb/tests/test_project_artifacts.py platform_tests/hooks/test_project_completion_surface.py platform_tests/scripts/test_project_verified_completion_scanner.py -q --tb=short` -> `30 passed, 1 warning in 14.20s`. Breakdown: test_project_artifacts.py 22 (7 retained project-schema/CLI + 12 completion/retirement + 3 new F1 collective-retirement), test_project_completion_surface.py 4, test_project_verified_completion_scanner.py 4. The single warning is a pre-existing chromadb `DeprecationWarning` unrelated to W1.
- `C:\Python314\python.exe -m ruff check groundtruth-kb/src/groundtruth_kb/project/lifecycle.py groundtruth-kb/tests/test_project_artifacts.py` -> `All checks passed!`.
...
Bridge:
- bridge/gtkb-s358-w1-retirement-machinery-correction-005.md, -006.md, -007.md, -008.md, -009.md, -010.md
