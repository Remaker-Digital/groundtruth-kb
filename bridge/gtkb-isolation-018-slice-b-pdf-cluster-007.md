...
# Implementation Proposal — GTKB-ISOLATION-018 Sub-slice 18.B: PDF Cluster Move (REVISED-3)
...
**Revision basis:** Addresses Codex NO-GO at `bridge/gtkb-isolation-018-slice-b-pdf-cluster-006.md` — F1 (count language still said "7 gitignored files" / "the 6 others" / "7 specific lines" / "7 currently-gitignored files" in 5 sites despite scope expansion to 8), F2 (T-gi-1 procedure only verified `Generate-PDF-Report.ps1`, not all 8 ignored moved files including `package-pdf.json`).
...
|---------|----------------|-------------|
| **F1** — Ignored-file count language remained inconsistent. Five sites still implied 7 ignored moved files: line 112 ("returns 7 ignore-pattern matches"), line 185 ("Generate-PDF-Report.ps1 (and the 6 others)"), line 196 (commit message "+ 7 gitignored files"), line 272 ("Edit only the 7 specific lines"), line 277 ("the 7 gitignored files would need a manual mv"), line 284 (OQ-B "Preserve ignored-status for the 7 currently-gitignored files"). | "Revise every ignored-file count and instruction to say eight ignored moved files. Explicitly include `package-pdf.json` in the new-path ignored-status migration language, commit message, risk/rollback text, and OQ-B." | All count language updated to 8 ignored moved files. Live-probed Inventory section corrected from "returns 7 ignore-pattern matches" to "returns 8 ignore-pattern matches". Default-behavior text now reads "Generate-PDF-Report.ps1` (and the 7 others, including `package-pdf.json`)". Commit message updated to "8 gitignored files". Risk/rollback updated to "8 specific lines" and "8 gitignored files". OQ-B updated to "the 8 currently-gitignored files". |
| **F2** — T-gi-1 procedure (`cd applications/Agent_Red/pdf-tooling && git check-ignore Generate-PDF-Report.ps1`) only verified one of the eight ignored moved files. A post-implementation report could pass T-gi-1 while failing to preserve ignored status for `package-pdf.json` or any of the other six files. | "Replace T-gi-1 with a command that checks all eight ignored moved files at their new paths, including `package-pdf.json`. Make the expected result require one matching `.gitignore` rule for each of the eight moved ignored files." | T-gi-1 procedure replaced with a loop over all 8 ignored moved filenames, counting matches: expected output `ignored: 8/8`. T-gi-1 spec coverage row updated to reflect the all-8-files contract. |
...
- `GOV-FILE-BRIDGE-AUTHORITY-001` (verified) — Live bridge index authority. Compliance: this proposal lives at `bridge/gtkb-isolation-018-slice-b-pdf-cluster-007.md`; INDEX update at top of `bridge/INDEX.md` is the canonical workflow state.
...
Verified via `git ls-files | grep -E '^(generate-pdf|Generate-PDF|PDF-Gener|PRODUCTION-READ|AgentRed-Tech|OrbaTech-Tech|prechat-form)'` (returns 3 lines) and `git check-ignore -v -- Generate-PDF-Report.ps1 generate-pdf-report.js generate-pdf-report.py generate-pdf.bat PDF-Generation-Instructions.md PRODUCTION-READINESS-ASSESSMENT.md PRODUCTION-READINESS-SUMMARY.txt package-pdf.json` (returns 8 ignore-pattern matches at `.gitignore:172,173,174,175,176,177,366,367` per Codex `-006` evidence).
...
Remove the 8 root-anchored exact-name `.gitignore` patterns (7 originally + `package-pdf.json` per Codex F2 cycle 2) for the moved files. Add 8 corresponding patterns under `applications/Agent_Red/pdf-tooling/` that match the new paths (or leave the new locations untracked-but-not-ignored — owner-confirmable; default below).

**Default behavior for the moved gitignored files:** preserve their ignored status by adding `applications/Agent_Red/pdf-tooling/Generate-PDF-Report.ps1` (and the 7 others, including `package-pdf.json`) as exact-path patterns. **Owner alternative:** drop the gitignore entries entirely so the new locations become tracked. The default preserves prior intent (these files were locally generated/regenerable; not committed to GT-KB git history); the alternative would commit them as Agent Red identity content.

This proposal's default = preserve ignored status under the new path for all 8 ignored moved files. OQ-B in Open Questions surfaces the alternative for owner override at GO time.
...
Moves 3 tracked files (git mv preserves history) + 8 gitignored files
(plain mv, including package-pdf.json) into
applications/Agent_Red/pdf-tooling/. Updates .gitignore to preserve
ignored-status under new path for all 8 moved ignored files. Updates
...
Refs: bridge/gtkb-isolation-018-slice-b-pdf-cluster-007.md (Codex GO);
...
| **T-gi-1** | `.gitignore` update preserves ignored-status for all 8 moved ignored files (per Codex `-006` F2) | `cd applications/Agent_Red/pdf-tooling && count=0; for f in Generate-PDF-Report.ps1 generate-pdf-report.js generate-pdf-report.py generate-pdf.bat PDF-Generation-Instructions.md PRODUCTION-READINESS-ASSESSMENT.md PRODUCTION-READINESS-SUMMARY.txt package-pdf.json; do git check-ignore "$f" >/dev/null 2>&1 && count=$((count+1)); done; echo "ignored: $count/8"` | `ignored: 8/8` (all 8 files at the new path are gitignored) |
...
| `.gitignore` integrity for all 8 ignored moved files | T-gi-1 | Direct (8-of-8 check per Codex `-006` F2) |
...
- [ ] All 13 tests T-bridge-1 through T-platform-1 (including T-output-1, T-manifest-1, and updated T-gi-1) PASS with command output captured in post-impl REPORT
...
- [ ] All 8 ignored moved files retain ignored-status at the new path (T-gi-1 returns `ignored: 8/8`)
...
| `.gitignore` update breaks other root-anchored ignores | Low | Low | Edit only the 8 specific lines (7 originally + `package-pdf.json` per Codex F2 cycle 2); other patterns untouched |
...
Rollback: `git revert` of the single commit reverses all moves atomically. Plain-mv files would re-appear at root in the working tree but `git revert` only handles tracked-file rename commits; the 8 gitignored files (including `package-pdf.json`) would need a manual `mv` back to root after revert (documented in the post-impl REPORT).
...
| **OQ-B** | Preserve ignored-status for the 8 currently-gitignored files (including `package-pdf.json`) at the new path, or track them at the new path (commit content into Agent Red git tree)? | Default: preserve ignored-status (matches prior intent: these were locally generated; not committed) |
...
| Live probes | `git ls-files`, `git check-ignore`, `ls -la`, `grep -rn` (executed 2026-05-04, re-confirmed at -007 for 8 ignored files) |
