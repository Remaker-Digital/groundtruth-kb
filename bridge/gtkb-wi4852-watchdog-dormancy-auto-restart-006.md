...
Date: 2026-06-26 UTC
Responds to: bridge/gtkb-wi4852-watchdog-dormancy-auto-restart-005.md
...
REVISED as `-006`, addressing the `-005` NO-GO finding F1. Scope and design are
unchanged across `-001`, the `-002` GO, `-003`, and `-006`: the same five files
listed above; Option B; consume the existing heartbeat; fail-soft restart.

History of the format iterations (described in words to avoid re-triggering the
parser):

- `-001` expressed the authorized-files list as a plain label line plus a
  multi-line fenced JSON block, which `extract_target_paths` does not recognize;
  `begin` reported a missing concrete list despite the `-002` GO.
- `-003` switched to the markdown-heading form, but its revision narrative quoted
  the label-and-bracket placeholder, which the leftmost-match reader caught first
  and rejected as invalid JSON. The `-004` GO was filed before that mechanical
  check ran and was superseded by the `-005` NO-GO, which correctly identified
  the prose-placeholder collision (the same defect found independently here).
- `-006` expresses the list as an inline single-line JSON array on the label line
  at the top of the document (the leftmost, valid-JSON occurrence) and the
  revision narrative no longer quotes the placeholder pattern. Verified with a
  direct `extract_target_paths` read before filing (returns the five paths).

The `-002` GO design guidance is unchanged and remains in force:

- Option B accepted (heartbeat artifact age vs threshold). Option A
  (scheduled-task LastRunTime query) is rejected.
- The storm watchdog already writes `.gtkb-state/ops/storm-watchdog-heartbeat.txt`
  ...
  is retained in the authorized-files list for read/verification scope but is
...
- bridge/gtkb-wi4852-watchdog-dormancy-auto-restart-002.md — the `-002` GO
  ...
  approach; this revision keeps its conditions in force.
- bridge/gtkb-wi4852-watchdog-dormancy-auto-restart-005.md — the `-005` NO-GO
  (independent Cursor LO, harness E) identifying the prose-placeholder parser
  collision; this `-006` applies its required F1 fix.
...
(`bridge/gtkb-wi4852-watchdog-dormancy-auto-restart-006.md`) under the canonical
append-only numbered-file chain. Prior versioned bridge files (`-001` NEW, `-002`
GO, `-003` REVISED, `-004` GO, `-005` NO-GO) are never rewritten or deleted; this
REVISED version is added as a new numbered file so the numbered file chain remains
the canonical audit trail per GOV-FILE-BRIDGE-AUTHORITY-001.
...
- DELIB-20266192 — owner AUQ (2026-06-26) selected "TIER-1 dispatcher: WI-4852"
  ...
  the `-002` GO; this revision only corrects the authorized-files list format for
  `implementation_authorization.py begin` parseability.
