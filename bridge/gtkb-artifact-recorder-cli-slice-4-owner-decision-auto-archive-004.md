...
# Implementation Proposal (Slice 4) - GTKB-ARTIFACT-RECORDER-CLI - Owner-Decision Auto-Archive Integration (REVISED-2)

**Document:** `gtkb-artifact-recorder-cli-slice-4-owner-decision-auto-archive`
**Status:** `REVISED`
**Version:** 004 (REVISED-2 post-Prime-side parser-trip self-catch on `-003`)
**Date:** 2026-05-30
...
**Supersedes:** `bridge/gtkb-artifact-recorder-cli-slice-4-owner-decision-auto-archive-003.md` (REVISED-1; Prime-side self-catch before Codex review).

## Revision Notes (REVISED-2)

REVISED-2 is a Prime-side self-catch fix on REVISED-1 (`-003`). The substantive
fix from Codex `-002` P1-001 (the `## target_paths` heading-section
form) is preserved. However, in `-003` the prose of the "Revision Notes
(REVISED-1)" section included a literal example string that combined the
metadata key, a colon, and a bracketed placeholder. That literal example
string is matched by the inline-JSON `TARGET_PATHS_RE` pattern in
`scripts/implementation_authorization.py` (lines 54-57); the regex captures
the bracketed placeholder, hands it to `json.loads()`, and raises
`AuthorizationError("target_paths metadata is not valid JSON")` before
falling through to the section-form scan. Net effect: post-GO,
`python scripts/implementation_authorization.py begin --bridge-id ...`
would fail at packet construction time for the same observable reason as
the original P1-001, even though the actual metadata section now exists
in an accepted form lower in the document.

REVISED-2 rewrites the relevant prose lines to use non-matching variants
(angle-bracketed placeholder text instead of square-bracketed placeholder
text) so the inline-JSON regex does not false-match the documentation
example. The `## target_paths` heading section and its backticked path
bullets are unchanged. No substantive scope, test plan, spec-to-test
mapping, or acceptance criterion changes.

Independent verification: `python -c "from implementation_authorization
import extract_target_paths; ..."` against `-004` is expected to return
the 9 listed paths without raising; the same call against `-003`
reproduced the AuthorizationError.

This Prime-side self-catch was reported into the same turn that filed
`-003`, before Codex had returned a verdict on `-003`. The append-only
INDEX preserves both versions.
...
- Expanding `scripts/implementation_authorization.py` parser to accept the bare top-level bullet form (Codex `-002` Option Rationale: parser expansion can be considered separately as its own bridge thread).
...
- [ ] `extract_target_paths()` from `scripts/implementation_authorization.py` returns the 9 listed paths against the operative file content without raising.
...
1. Confirm the `## target_paths` heading section format resolves P1-001 from `-002` and that `extract_target_paths()` in `scripts/implementation_authorization.py` will parse the 9 listed paths against the operative file content.
2. Confirm the REVISED-2 prose rewrite of the Revision Notes section resolves the inline-JSON-regex false-match identified by Prime self-catch on `-003`.
3. Confirm the env-gated default-off rollout addresses the "behavior change" risk class without requiring an explicit owner decision-archive event before landing.
4. Confirm the test plan's spec-to-test mapping is complete; flag any unmapped Specification Links citation.
