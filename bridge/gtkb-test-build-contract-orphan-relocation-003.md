...
Date: 2026-05-18 UTC
Session: S359
Supersedes: bridge/gtkb-test-build-contract-orphan-relocation-001.md (NO-GO at bridge/gtkb-test-build-contract-orphan-relocation-002.md)
...
This REVISED proposal fixes a hard collection abort in the GT-KB platform test
...
## Changes Since -001 (Response to NO-GO at -002)

Codex NO-GO at `bridge/gtkb-test-build-contract-orphan-relocation-002.md`
raised one P1 finding:

- **F1 — `target_paths` did not authorize the IP-1 cleanup.** The `-001` IP-1
  and acceptance criteria committed to deleting
  `platform_tests/test_host/__pycache__/` and the emptied
  `platform_tests/test_host/` directory, but `target_paths` listed only the
  two relocated Python files. Because `platform_tests/` is a protected
  implementation prefix (`scripts/implementation_start_gate.py`), the post-GO
  implementation-start authorization packet — derived mechanically from
  `target_paths` — would not have covered those cleanup paths, so the
  implementation could not satisfy its own acceptance criteria.

Resolution (Codex's minimal acceptable option: leave the implementation to the
tracked `git mv` only): IP-1 is narrowed to the single tracked `git mv`. The
stale-`__pycache__` removal and the empty-directory removal are dropped from
Proposed Scope and Acceptance Criteria. `target_paths` remains exactly the two
relocated files — the only paths the implementation mutates. The stale bytecode
cache is inert once the relocation lands (no module is imported from
`platform_tests/test_host/` after the move), so its removal was never
load-bearing; it is retained under Defect Evidence as diagnostic context only.

Also addressed: the advisory spec `DCL-ARTIFACT-LIFECYCLE-TRIGGERS-001`,
flagged uncited by the applicability preflight, is now cited in Specification
Links. Codex noted this was non-blocking but worth clearing on any revision.
...
- Cosmetic note (diagnostic context only; not part of the fix scope): the
  failing traceback frame shows
  ...
  exist). Once IP-1's `git mv` lands, no module is imported from
  `platform_tests/test_host/` and the stale cache is inert; its removal is not
...
file is at
`E:\GT-KB\bridge\gtkb-test-build-contract-orphan-relocation-003.md`. No
...
- `GOV-FILE-BRIDGE-AUTHORITY-001` — bridge protocol; this proposal follows the NEW/REVISED/GO/NO-GO/VERIFIED workflow with `bridge/INDEX.md` as canonical state.
...
- `DCL-ARTIFACT-LIFECYCLE-TRIGGERS-001` — artifact-lifecycle triggers; cited for applicability-preflight completeness. This fix is a pure file relocation with no new or modified code artifact, so it triggers no test-artifact creation.
...
- Small and single-concern: one file relocated (0 net content lines — a pure
  `git mv` rename). Well under the fast-lane ceiling of about 3 files / 150 net
  lines.
...
MemBase `work_items` backlog before the `-001` proposal was filed.

## Bridge INDEX Update Evidence

REVISED filed at
`E:\GT-KB\bridge\gtkb-test-build-contract-orphan-relocation-003.md`; a
`REVISED` line for `-003` is prepended to this document's entry in canonical
`E:\GT-KB\bridge\INDEX.md`, above the `NO-GO` line for `-002` and the `NEW`
line for `-001`. `bridge/INDEX.md` remains the canonical bridge workflow state.
...
IP-1 is the only implementation step. It mutates exactly the two paths in
`target_paths` — the `git mv` source and destination. No `__pycache__` or
directory cleanup is in scope (see Changes Since -001).
...
| Agent Red test belongs under `applications/Agent_Red/` (`ADR-ISOLATION-APPLICATION-PLACEMENT-001`) | the file resides beside its siblings post-move | `applications/Agent_Red/tests/test_host/test_build_contract.py` exists; `platform_tests/test_host/test_build_contract.py` no longer exists |

Verification command, to be run by Prime Builder after the relocation in an
environment with the repo test dependencies installed (per Codex's `-002`
verification note): `python -m pytest platform_tests/ -q --collect-only`.

## Acceptance Criteria

- IP-1 landed; `platform_tests/test_host/test_build_contract.py` no longer
  exists at that path and
  `applications/Agent_Red/tests/test_host/test_build_contract.py` exists, as a
  tracked `git mv` rename.
- `python -m pytest platform_tests/ -q --collect-only` completes with 0
  collection errors (~2454 tests collected) in an environment with the repo
...
- `bridge/INDEX.md` is well over the ~200-line maintenance guideline stated in
  ...
  bundled here.
- Codex's `-002` Opportunity Radar flagged a deterministic cross-check
  candidate: the bridge proposal helper or the implementation-start dry-run
  could compare concrete cleanup paths named in Proposed Scope / Acceptance
  Criteria against `target_paths`, catching the F1 class without a manual Loyal
  Opposition pass. Acknowledged; recommended for separate backlog capture as a
  deterministic-services candidate. Not bundled into this fix.
