...
implementation_scope: source_code, tests, rule_files, narrative_artifact_approval_packets, hook_scripts
...
# Invisible Interactive Role Switch Hardening

## Claim

Fix a role-continuity defect where an interactive Codex session initialized by the owner as Prime Builder can be silently pulled toward Loyal Opposition behavior after heartbeat, compaction, resume, or SessionStart-like cached context when harness A's durable registry role is Loyal Opposition.

The intended authority rule is already decided: a transcript-defined interactive role from `::init gtkb (pb|lo)` persists across compaction, resume, and contiguous SessionStart-like boundaries within the same interactive context until the owner explicitly changes it. The durable registry role remains authoritative for headless dispatch routing, but it is only a default or hint for interactive agent behavior when no transcript-defined role is present.

## Current-State Diagnosis

No suitable live bridge thread currently owns this exact defect.

Live bridge scans found active Loyal Opposition work, but the actionable items are unrelated to this runtime-continuity hardening. One nearby item, `gtkb-test-workstream-focus-stale-relay-cache-fixtures`, is a post-implementation report about removing stale comments in `platform_tests/hooks/test_workstream_focus.py`; it does not change runtime role authority, startup relay semantics, heartbeat payloads, or session-envelope behavior.
...
Backlog and authorization state:

- `PROJECT-GTKB-INTERACTIVE-SESSION-ROLE-OVERRIDE` has an active authorization `PAUTH-PROJECT-GTKB-INTERACTIVE-SESSION-ROLE-OVERRIDE-001`, including `WI-4663` and allowing `source_code`, `tests`, `rule_files`, `doctor_checks`, `parity_checks`, and `hook_scripts`.
- `WI-4663` is marked resolved in MemBase, but it is the closest active PAUTH-linked work item for durable session-envelope role authority. This proposal uses it as a follow-on regression-hardening bridge under the still-active PAUTH. If Loyal Opposition considers the resolved WI status disqualifying, return NO-GO with the required replacement WI or authorization packaging.
- A separate open intake `WI-AUTO-SPEC-INTAKE-A3CDEF` exists for interactive transcript-defined session-role authority, but the substance has already been formalized by the role-authority persistence ADR/DCL work. This proposal does not mutate that intake.
...
No new owner AskUserQuestion is required to file this proposal. The only requested review adjudication is technical: whether Loyal Opposition accepts using `WI-4663` under the active PAUTH for this follow-on hardening, or requires a fresh work item and revised proposal.
...
- No MemBase formal spec mutation.
- No production deployment, credential action, or external service action.

## Implementation Guardrails

Implementation must not start until Loyal Opposition returns GO on this proposal and Prime Builder obtains a matching work-intent claim and implementation-start authorization. Protected source, tests, rule files, hooks, and config are not to be mutated before GO.
...
Draft preflights run against `.gtkb-state/bridge-propose-drafts/gtkb-invisible-interactive-role-switch-hardening-001.md` before filing:

```text
python scripts/bridge_applicability_preflight.py --bridge-id gtkb-invisible-interactive-role-switch-hardening --content-file .gtkb-state/bridge-propose-drafts/gtkb-invisible-interactive-role-switch-hardening-001.md --json
python scripts/adr_dcl_clause_preflight.py --bridge-id gtkb-invisible-interactive-role-switch-hardening --content-file .gtkb-state/bridge-propose-drafts/gtkb-invisible-interactive-role-switch-hardening-001.md
python scripts/proposal_target_paths_coverage_preflight.py --content-file .gtkb-state/bridge-propose-drafts/gtkb-invisible-interactive-role-switch-hardening-001.md --json --strict
```

Observed draft results:

- Applicability preflight: `preflight_passed: true`; `missing_required_specs: []`.
- Clause applicability preflight: blocking gaps `0`; exit code `0`.
- Target-path coverage preflight: `verdict: clean`; no uncovered implied generator or verification paths.
...
groundtruth-kb/.venv/Scripts/python.exe scripts/check_narrative_artifact_evidence.py --paths .claude/rules/canonical-terminology.md
```
...
Rollback is a single bridge-scoped revert of source/test/narrative changes, plus removal or supersession of any generated narrative approval packet. No database schema, durable role registry, credential, or deployment state changes are in scope.

## Loyal Opposition Asks

1. Confirm that `WI-4663` plus `PAUTH-PROJECT-GTKB-INTERACTIVE-SESSION-ROLE-OVERRIDE-001` is acceptable for this follow-on hardening despite the WI's resolved status, or return NO-GO requiring fresh WI/PAUTH packaging.
...
4. Return GO if the proposal is implementation-ready; otherwise return NO-GO with specific scope or requirement fixes.
