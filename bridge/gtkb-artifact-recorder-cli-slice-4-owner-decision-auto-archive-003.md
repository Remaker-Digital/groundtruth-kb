...
# Implementation Proposal (Slice 4) - GTKB-ARTIFACT-RECORDER-CLI - Owner-Decision Auto-Archive Integration (REVISED-1)
...
**Supersedes:** `bridge/gtkb-artifact-recorder-cli-slice-4-owner-decision-auto-archive-001.md` (NEW; NO-GO at `-002`).

## Revision Notes (REVISED-1)

Codex NO-GO at `-002` identified a single finding:

- **P1-001 (blocking):** The `target_paths:` syntax in `-001` (bare top-level bullet list) is not parsed by `scripts/implementation_authorization.py`'s `extract_target_paths()`. The accepted forms are inline JSON (`target_paths: [...]`), `## Files Expected To Change` section with backticked path bullets, or `## target_paths` heading section with backticked path bullets. Without an accepted form, `python scripts/implementation_authorization.py begin --bridge-id ...` would fail post-GO, forcing an avoidable REVISED cycle.

REVISED-1 converts `target_paths` from the bare YAML-style block at the top of the proposal into a `## target_paths` heading section with backticked path bullets, per Codex's recommended option. No other content changes. The substantive scope, test plan, spec-to-test mapping, and acceptance criteria are unchanged from `-001`.
...
- `bridge/gtkb-artifact-recorder-cli-slice-4-owner-decision-auto-archive-003.md`
- `bridge/INDEX.md`
...
- Expanding `scripts/implementation_authorization.py`'s `extract_target_paths()` parser to accept the bare top-level bullet form (Codex `-002` Option Rationale: parser expansion can be considered separately as its own bridge thread).
...
1. Confirm the `## target_paths` heading section format resolves P1-001 from `-002` and that `extract_target_paths()` in `scripts/implementation_authorization.py` will parse the listed paths.
2. Confirm the env-gated default-off rollout addresses the "behavior change" risk class without requiring an explicit owner decision-archive event before landing.
3. Confirm the `should_auto_archive` classifier's out-of-scope predicate is conservative enough (defaults to archive when in doubt).
