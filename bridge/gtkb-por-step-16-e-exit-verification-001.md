...
target_paths: ["scripts/remediate_por_step_16e.py", "platform_tests/scripts/test_remediate_por_step_16e.py"]
...
This proposal details the remediation plan to satisfy the exit criteria of POR Step 16.E.
...
We propose:
1. Creating a database remediation script `scripts/remediate_por_step_16e.py` that:
   - Maps 69 adoptable orphan tests to their named specifications in `groundtruth.db`.
   - Links the 84 untested specifications to test stubs or actual tests from `platform_tests/`.
   - Retires the remaining 2,120 legacy visual, layout, and adopter tests that belong to the reference adopter application (Agent Red) by deleting them from the database `groundtruth.db` (since they do not correspond to any platform files or code in the GT-KB platform repository).
2. Implementing regression tests to verify that the remediation script executes correctly and that the exit verification script `scripts/por_step_16_exit_verification.py` passes successfully after the run.
...
- `DELIB-0823` — POR 16.D Phase 2 complete, which classified the 2,189 orphan baseline.
...
| Behavior | Test |
|---|---|
| Remediation script resolves all orphans and untested specs | `platform_tests/scripts/test_remediate_por_step_16e.py` |
...
Rollback: A SQLite backup of `groundtruth.db` is captured before the mutation. The changes can be rolled back via git restore or restoring the database file.
