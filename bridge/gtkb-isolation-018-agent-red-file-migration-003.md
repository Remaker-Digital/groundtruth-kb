...
# Implementation Proposal — GTKB-ISOLATION-018: Agent Red File Migration (REVISED-1)
...
**Successor sub-slices:** 18.A through 18.L (12 sub-slices; the original 18.B was vacated when the waiver moved to a precursor thread per Codex F2)
**Revision basis:** Addresses Codex NO-GO at `bridge/gtkb-isolation-018-agent-red-file-migration-002.md` — F1 (missing required cross-cutting spec citations), F2 (waiver bootstrap problem), F3 (acceptance-criteria internal inconsistency).
...
The migration MUST preserve a working state at every sub-slice boundary. No sub-slice may leave GT-KB platform tests broken or Agent Red dev workflows broken. Ordering rationale (renumbered: original 18.B vacated; original 18.C–18.M → new 18.B–18.L):
...
(The original 18.B "Pending-migration waiver DELIB" sub-slice has been vacated; that work moves to the precursor thread `bridge/gtkb-isolation-018-pending-migration-waiver-NNN.md`. The slot is left explicit to preserve sub-slice ID continuity in the historical record but no work is performed under 18.B in this revised plan.)
...
- [ ] All 12 sub-slices (18.A through 18.L; 18.B is vacated) have reached VERIFIED individually
...
Each sub-slice is its own bridge thread (`bridge/gtkb-isolation-018-<slice-id>-<topic>-001.md`) gated by Codex GO/NO-GO and Codex VERIFIED. Original 18.B (waiver DELIB) has been vacated; sub-slices renumber as below.
...
Total: **12 sub-slices** (18.A through 18.L; 18.B vacated; each is its own bridge thread).

Vacated slot:
- **18.B (vacated)** — was "Pending-migration waiver DELIB"; moved to precursor thread `bridge/gtkb-isolation-018-pending-migration-waiver-NNN.md` per Codex F2.
