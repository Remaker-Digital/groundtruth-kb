...
# Implementation Proposal — Write-Time + Impl-Start Self-Review Verdict Gate (WI-4829)
...
Mutation classes used (within the PAUTH `allowed_mutation_classes`
[source, hook_upgrade, test_addition]):
- `hook_upgrade` — the compliance-gate hook (canonical template + re-activated copy).
- `source` — the shared comparator module, the verify finalization helper, the impl-start authorizer, the dispatch-trigger delegation refactor.
- `test_addition` — the new spec-derived tests.

`.claude/hooks/bridge-compliance-gate.py` is the re-activated byte-identical copy
of the tracked template `groundtruth-kb/templates/hooks/bridge-compliance-gate.py`
(the committed change is the template; re-activation is a local runtime step).
No protected narrative-authority file (`.claude/rules/*.md`, `AGENTS.md`,
`CLAUDE*.md`) and no MemBase/spec mutation is in scope.
...
- `self_review_reason(reviewer_session_context_id, target_author_session_context_id) -> str | None`
  — pure comparator: empty/missing target → `"author_session_context_missing"`;
  equal → `"author_meets_reviewer_refused"`; otherwise `None`.
- `parse_author_session_context_id(content) -> str | None` — read the
  `author_session_context_id:` header line.
- `reviewed_artifact_path(verdict_content, bridge_id, project_root) -> Path | None`
  — resolve the artifact a verdict reviews via its `Responds to:` /
  `Reviewed report:` / `Approved proposal:` reference, falling back to the latest
  prior versioned file. (Using the latest prior file alone is insufficient for
  `VERIFIED`, where an intermediate `GO` version can be newer than the reviewed
  implementation report — e.g., `-005` reviews report `-003`, not `GO -004`.)
...
- **`groundtruth-kb/templates/hooks/bridge-compliance-gate.py`** (+ re-activate to
  ...
  content and **hard-block** the Write on any refusal reason, with a message naming
  the reviewed artifact and the equal session ids. Imported the same way the gate
  already imports `scripts.bridge_author_metadata`.
- **`.claude/skills/verify/helpers/write_verdict.py`**: the verify helper writes
  ...
  check immediately before the write; raise a clear error (mirroring the
  credential/compliance fail-closed pattern) so the helper never emits a
  self-review verdict.
...
refuse authorization (`authorized: false`, reason `author_meets_reviewer_refused`)
when equal or when author metadata is missing/unreadable. This catches a
self-review `GO` even if it reached disk through a path that bypassed both
write-time surfaces.
...
|---|---|
| Comparator: equal sessions refused; distinct pass; missing fails closed (GOV-DOCUMENT-AUTHOR-PROVENANCE-001) | `test_comparator_*` (refused / passes / missing / unreadable) |
...
| Dispatch trigger behavior preserved after delegation refactor | existing `test_dispatch_author_meets_reviewer.py` still passes |

Execution commands (run on touched files before filing the report):
...
1. A `GO`/`NO-GO`/`VERIFIED` verdict whose `author_session_context_id` equals the
   reviewed artifact's author session is **blocked** at both write-time surfaces
   (compliance gate + verify helper) and refused at impl-start.
2. A self-review verdict that names a missing/unreadable reviewed-artifact author
   fails **closed**.
3. A genuinely independent verdict (distinct sessions) passes unchanged.
4. The dispatch-time `author_meets_reviewer_refused` behavior is unchanged
   (existing test green).
5. New tests PASS; `ruff check` + `ruff format --check` clean on touched files.

## Risk / Rollback

- Risk: medium. Touches a load-bearing PreToolUse hook (compliance gate) and the
  impl-start authorizer. Mitigation: the new check fires only on LO verdict bridge
  files; fail-closed only on genuine self-review or unreadable author metadata, so
  legitimate independent verdicts are unaffected. The dispatch-trigger refactor is
  behavior-preserving and test-guarded.
- A false block would prevent a legitimate verdict; mitigated by the precise
  `author_session_context_id` equality condition and the `Responds to:`-anchored
  target resolution.
- Rollback: revert the seven target paths and re-activate the prior gate template;
  no data, state, or migration.

## Recommended Commit Type

`fix:` — closes a review-independence enforcement gap (a defect: an invalid
self-review verdict could be created and could authorize implementation). Adds
enforcement + tests, not a new user-facing capability.
