...
# Implementation Proposal: Dispatcher Config CLI Whole-Candidate Validation

## Summary

Correct the role/configuration CLI semantics after the owner's 2026-06-06 dispatcher clarification. The bridge is now a dispatcher: dispatch target selection is based on harness dispatch availability plus the role and subject in the session envelope and first topic envelope. The default role recorded on a harness is informative fallback metadata, not a self-service selector that should cause the CLI to choose complementary role holders, pick topics, or rewrite unrelated harnesses.
...
No additional owner decision is required before review. This is a scoped correction to the deterministic transaction component and its operating-role documentation, not a production deployment, credential action, or MemBase mutation.
...
Manual preflight before filing: all target paths are under `E:\GT-KB`; no credential values are included; no MemBase or production-deploy mutation is proposed; this is a correction to the deterministic transaction component under existing active project authorization and owner clarification.
