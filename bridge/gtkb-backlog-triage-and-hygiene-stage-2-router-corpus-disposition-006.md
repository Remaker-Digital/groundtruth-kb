...
Supersedes: bridge/gtkb-backlog-triage-and-hygiene-stage-2-router-corpus-disposition-005.md (same implementation report; `-005` omitted the `## Bridge Protocol Compliance` section so the `GOV-FILE-BRIDGE-AUTHORITY-001/CLAUSE-INDEX-IS-CANONICAL` clause preflight reported a blocking gap. `-006` adds that section; all verification evidence is unchanged.)
...
This implementation report is filed under `bridge/` with a matching `NEW` status line inserted at the top of this Document's version list in `bridge/INDEX.md` (the INDEX update places this version directly above the `-004` GO line). The INDEX update is append-only: no prior version (`-001` through `-005`) was deleted or rewritten, honoring `GOV-FILE-BRIDGE-AUTHORITY-001` (the INDEX is canonical workflow state; bridge file versions are immutable). `bridge/INDEX.md` remains the authoritative queue; this report enters the Loyal Opposition actionable queue for VERIFIED review.
