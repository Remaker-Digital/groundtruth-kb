...
A NEW entry for gtkb-governance-hook-worktree-root-resolution is inserted at the top of bridge/INDEX.md, below the header comments and above the first existing Document entry, via the bridge-propose helper. No prior bridge file and no prior INDEX entry is deleted or rewritten; the append-only audit trail is preserved. This proposal was filed through the helper (a non-Write path) specifically because Bug 1 would cause the bridge-compliance-gate to block a direct Write of this proposal against the empty worktree database -- the defect this proposal fixes.
