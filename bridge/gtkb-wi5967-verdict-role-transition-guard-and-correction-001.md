...
target_paths: ["scripts/bridge_lifecycle_resolver.py", ".claude/hooks/bridge-compliance-gate.py", "groundtruth-kb/templates/hooks/bridge-compliance-gate.py", "platform_tests/scripts/test_bridge_lifecycle_role_invalid_correction.py", "platform_tests/hooks/test_bridge_compliance_gate_verdict_role_transition.py"]
...
## Post-Implementation Consequence (stated, not performed here)

Once C2 lands, `gtkb-dispatcher-next-foundation-spike` becomes recoverable: a Prime
`NO-ACTION` at v014 followed by an independent, role-correct Loyal Opposition verdict at
v015 would resolve the chain and unwedge the WI-5617 foundation thread. That corrective
chain is **not** authored by this proposal - it is separate bridge work requiring its own
independent review, and it cannot be attempted until this implementation is VERIFIED.
