...
# Ollama Qwen Full Loyal Opposition Route Gate-Compliant Proposal
...
Work Item: WI-4385
target_paths: [".ollama/routing.toml", "scripts/ollama_harness.py", "scripts/verify_ollama_dispatch.py", "platform_tests/scripts/test_ollama_harness.py", "platform_tests/scripts/test_ollama_routing_config.py", "platform_tests/scripts/test_verify_ollama_dispatch.py", "bridge/gtkb-ollama-qwen-full-lo-route-gate-compliant-*.md", "bridge/INDEX.md"]
Owner Decision: DELIB-20260606-OLLAMA-QWEN-FULL-LO-DIRECTIVE
Prior Bridge Evidence: bridge/gtkb-ollama-qwen-full-lo-route-001.md and bridge/gtkb-ollama-qwen-full-lo-route-002.md
Date: 2026-06-06 UTC
...
This proposal supersedes the earlier `gtkb-ollama-qwen-full-lo-route` proposal for implementation-start purposes. Qwen successfully acted through `scripts/ollama_harness.py` to file `bridge/gtkb-ollama-qwen-full-lo-route-002.md`, which proved that the full-tool route can write guarded bridge artifacts after a bridge work-intent claim is active. However, `scripts/implementation_authorization.py begin --bridge-id gtkb-ollama-qwen-full-lo-route` correctly refused implementation because the approved proposal lacked exact `target_paths` metadata and the required `## Requirement Sufficiency` section. This gate-compliant proposal carries forward the same owner-approved implementation scope with the mandatory metadata shape.

## Requirement Sufficiency

Existing requirements sufficient.

The owner directive, active work item `WI-4385`, active project authorization `PAUTH-PROJECT-GTKB-OLLAMA-LO-OPERATIONS-QWEN-FULL-LO`, and linked Ollama harness specifications are sufficient for the scoped route, harness-context, readiness, and focused-test changes. No new formal requirement is required before implementation.
...
- `DCL-OLLAMA-TOOL-PARITY-GATE-001` - dispatch readiness must ensure the route exposes tools required for assigned bridge work.
...
- `.ollama/routing.toml` - route `bridge-review` and `verification` to `qwen3-coder-next-cloud`.
- `scripts/ollama_harness.py` - inject compact Loyal Opposition bridge context for bridge-review and verification runs while preserving guarded tools.
- `scripts/verify_ollama_dispatch.py` - require the complete LO tool set for readiness.
- `platform_tests/scripts/test_ollama_harness.py` - cover context injection and guarded tool behavior.
- `platform_tests/scripts/test_ollama_routing_config.py` - cover Qwen active route selection.
- `platform_tests/scripts/test_verify_ollama_dispatch.py` - cover full-tool readiness requirements.
- `bridge/gtkb-ollama-qwen-full-lo-route-gate-compliant-*.md` and `bridge/INDEX.md` - record bridge proposal, verdict, implementation report, and verification state for this thread.
...
| CQ-VERIFICATION-001 | Yes | Run bridge preflights, focused pytest, ruff check, ruff format check, live readiness verification, and a Qwen smoke bridge action. | Capture exact commands and results in the post-implementation report. |  |
...
- A Qwen smoke review can use `scripts/ollama_harness.py --model qwen3-coder-next-cloud --skill bridge-review` to write a bridge verdict artifact through the governed tool path after a bridge claim is active.
- `python scripts/implementation_authorization.py begin --bridge-id gtkb-ollama-qwen-full-lo-route-gate-compliant` succeeds before implementation starts.
