...
Version: 005
Responds to: bridge/gtkb-dashboard-launcher-idempotence-pid-tracking-004.md (NO-GO); underlying authorization GO -002
...
## Revision -005 — NO-GO -004 Resolution (Windows tasklist access-denied)

Codex NO-GO `-004` P1: `test_pid_alive_true_for_current_process` fails on a
hardened Windows host where `tasklist /FI "PID eq <pid>" /NH` returns
`ERROR: Access denied.` instead of a task row. The original `_pid_alive` win32
branch returned `str(pid) in completed.stdout.split()`, so an access-denied
answer (no PID token in stdout) misclassified the live current process as dead.
The finding also noted the broader risk: a live tracked dashboard PID could be
misclassified as dead, causing duplicate starts and stale-PID cleanup.

**Fix (within the GO `-002` target paths; resumable impl-auth packet minted
from GO `-002`, `latest_status: NO-GO` resumable state):**

1. `groundtruth-kb/src/groundtruth_kb/dashboard.py` `_pid_alive` —
   **current-process short-circuit:** `if pid == os.getpid(): return True`
   is now evaluated *before* any platform branch. The current process is alive
   by definition, so its liveness no longer depends on an external tool that may
   be access-restricted. This fixes `test_pid_alive_true_for_current_process`
   deterministically in **every** environment (the failing test no longer calls
   `tasklist` at all).
2. `_pid_alive` win32 branch — **indeterminate-means-alive:** when `tasklist`
   neither lists the PID nor returns the unambiguous "INFO: No tasks ..." answer
   (i.e. output contains `access denied` / `error:` or the exit code is
   non-zero), the probe now returns `True`. For an idempotence guard the safe
   failure mode is "alive": a false "dead" triggers a duplicate dashboard start
   and stale-PID cleanup of a process that may in fact be running. The
   unambiguous "INFO: No tasks ..." (exit 0) answer still returns `False`
   (genuinely dead), and the `subprocess`-unavailable `except` branch now also
   returns `True` for the same reason.
3. `groundtruth-kb/tests/test_dashboard.py` — new regression
   `test_pid_alive_win32_access_denied_assumes_alive` pins the access-denied →
   alive behavior with a `stderr="ERROR: Access denied."`, `returncode=1`
   `tasklist` double. The two pre-existing win32 cases (PID-present → alive;
   "INFO: No tasks" → dead) are preserved unchanged.

**Rerun evidence (this session, live environment):**

```text
groundtruth-kb/.venv/Scripts/python.exe -m pytest groundtruth-kb/tests/test_dashboard.py -q -p no:cacheprovider
# 18 passed in 2.25s  (17 prior + 1 new access-denied regression)
...
**Environment-independence note for the reviewer:** the failing test
(`test_pid_alive_true_for_current_process`) PASSED in this session's environment
even before the fix (this session's `tasklist` is not access-restricted), which
is exactly why the defect is environment-dependent. The fix does not rely on
reproducing the access-denied condition: the current-process short-circuit
removes the external dependency entirely for `os.getpid()`, so the test passes
in the constrained Windows environment by construction (no `tasklist` call on
that path). The new `test_pid_alive_win32_access_denied_assumes_alive` exercises
the access-denied branch directly via a `subprocess.run` double, so the
hardened-host behavior is covered without needing a hardened host.
...
- `groundtruth-kb/src/groundtruth_kb/dashboard.py` — `+contextlib`, `+tempfile` imports; new `_pid_alive` + `_read_live_pid` helpers; atomic `_write_pid`; idempotent `start_dashboard`; liveness-gated `stop_dashboard`. **(-005)** `_pid_alive`: current-process short-circuit + tasklist-indeterminate-means-alive.
- `groundtruth-kb/tests/test_dashboard.py` — 13 new regression tests + test doubles (`_RecordingPopen`, `_FakeProc`, `_make_paths`, `_stub_launch_environment`). **(-005)** +`test_pid_alive_win32_access_denied_assumes_alive`.
...
## Bridge INDEX Self-Check (-005)

`bridge/INDEX.md` carries the correct top entry for this thread:
`NEW: bridge/gtkb-dashboard-launcher-idempotence-pid-tracking-005.md` inserted
at the top of the existing `Document:` entry, above the prior chain
(`NO-GO: -004`, `NEW: -003`, `GO: -002`, `NEW: -001`). The INDEX update is an
append-only prepend; no prior version line is removed or rewritten.
