...
Version: 003
Date: 2026-07-28 UTC
Responds to: bridge/gtkb-wi5718-retired-session-role-authority-purge-002.md
...
| --- | --- |
| `FINDING-P0-001` | Adds the tracked benchmark as path 26, admits it to the registry, adds the tracked-file control scan, and aligns its probe with surviving DCL/ADR authority. |
...
it atomically before transforming it. Other nonregistered hits are disposable
or immutable local history under the owner's registry-authority rule and are
not retained, corrected, or used as proof.

The proposal and its approval packets add allowed historical occurrences; no
fixed post-filing aggregate total is authoritative. Revalidation aborts only on
drift in the operative registered class (25 paths / 43 hits), the tracked
benchmark (one path / one hit), their preimage hashes, or the exact current-row
manifest. Growth confined to immutable history is informational.
...
4. `DCL-ACTIVITY-CONTEXT-MANIFEST-001` v1: remove the retired `affected_by`
   link while preserving activity/context separation.
5. `DCL-INIT-KEYWORD-CONSISTENT-ASSERTION-001` v3: remove the retired GOV from
   its authority list; retain init syntax and deterministic resolution links.
6. `DCL-ROLE-RESOLUTION-DECLARED-AUTHORITY-001` v1: remove the retired GOV from
   the relationship paragraph and preserve warn-not-override semantics.
7. `DCL-SESSION-ROLE-RESOLUTION-001` v6: remove the retired authority bullet and
...
8. `GOV-HARNESS-ONBOARDING-CONTRACT-001` v1: remove the retired GOV from the
   related-artifact list; preserve identity/capability/dispatch eligibility.
9. `SPEC-CANONICAL-INIT-KEYWORD-SYNTAX-001` v3: remove the retired GOV from its
   authority list; preserve current syntax and resolution links.
10. `SPEC-DISPATCH-ENVELOPE-ELEMENT-001` v1: replace the retired parenthetical
   with surviving dispatched-packet/session-separation authority.
11. `SPEC-INTERACTIVE-GO-IMPLEMENTATION-CLAIM-DOCUMENT-AUTHORITY-001` v1:
...
exemption is accepted. The update service preserves every unspecified field.
Direct SQL and bulk status mutation are forbidden.
...
Every active row receives one exact owner-approved append-only amendment that
removes the retired identifier from `included_spec_ids` and, where present,
`scope_summary`, `change_reason`, or `allowed_mutation_classes`. Replacement
spec IDs must already be included and may not widen work items, targets,
mutation classes, or operations. Status remains active. The completed Ollama
authorization is immutable historical evidence and is the sole inactive PAUTH
allowlist entry.
...
Packet `WI5718-PAUTH-NN` maps one-to-one to this list. The exact 37 postimages
must be owner-presented and packet-validated before mutation. The direct owner
...
4. Runs a tracked-file control scan. Any tracked unregistered hit fails with an
...
5. Audits every surfaced field of current specification, work-item, test,
   ...
   and named immutable audit row/field pairs above.
...
No new sequencing decision is required. `DELIB-202667220` expressly requires
removal from every active reference, so the 37 active PAUTH amendments remain in
this WI rather than being deferred while active authority envelopes stay dirty.
...
2. Re-run the coherent registry scan, tracked-file control scan, and
   field-complete five-table current-record audit. Compare the exact operative
   paths, counts, hashes, IDs, and fields to this revision. Ignore aggregate
   history growth; stop on operative drift.
3. Admit the one benchmark through `gt registry register --batch-file` and
   prove declaration/mirror/projection parity. This admission is independently
   correct and idempotent if a later phase stops.
4. Produce all 26 exact file postimages in a transaction-local staging area
   from the preimage-bound map. Parse TOML, YAML, and JSON with structured
   parsers. Do not recursively edit files by agent judgment.
5. Produce eleven specification, four narrative, and 37 PAUTH postimages;
   owner-present and validate all 52 postimage packets. Recheck the existing
   PAUTH evidence packet as packet 53.
...
9. Run both zero-reference scans, the field-complete record guard, focused
   tests, parsers, packet validators,
...
| Registry file coverage | Registry scanner: zero operative registered hit outside the three historical classes; tracked-file control: zero unregistered tracked hit after benchmark admission |
| Current records clean | Field-complete scan of five current tables: zero non-allowlisted hit; exact retired identities, completed PAUTH, and named audit row/field pairs only |
...
| Role behavior preserved | Ten-module group becomes exactly 70/4; current adjacent group remains exactly 40/4; no new residual |
| Zero guard works | New test fails on synthetic registered, tracked-unregistered, and current-record operative hits and passes only the exact history/audit allowlist |
...
2. The registry-backed scan reports zero operative registered hit outside the
   three historical classes, and the tracked-file control reports zero
   unregistered tracked hit after the benchmark is admitted.
3. A field-complete audit across current specs, work items, tests, projects, and
   PAUTHs reports zero non-allowlisted hit. Only the exact retired identities,
   completed PAUTH, and named immutable audit row/field pairs remain.
...
5. Eleven specifications, nineteen work items, ten tests, one project, and 37
   ...
   fields and lifecycle states preserved.
...
7. The executable zero-reference guard contains no full literal identifier,
   rejects synthetic registered, tracked-unregistered, and current-record
   recurrence, and permits only the exact historical classes and audit fields.
...
constrain every PAUTH to citation removal only.
