...
The NO-GO at `bridge/gtkb-bridge-stop-drain-deference-repair-002.md` raised one P1 finding, F1: IP-2 adds wrap-up-command deference, but the `-001` proposal omitted the verified session-lifecycle / wrap-up specifications that govern the owner-wrap-up command surface. Codex scoped the required revision to specification linkage and spec-derived test mapping only — the defect evidence, project authorization, `target_paths`, IPs, test plan, and mechanical preflights were accepted.

This REVISED makes exactly that change and nothing else:

- `## Specification Links` now cites the three governing session-lifecycle/wrap-up specifications: `GOV-SESSION-LIFECYCLE-PROACTIVE-ENGAGEMENT-001`, `PB-SESSION-WRAP-UP-PROACTIVE-001`, and `DCL-SESSION-WRAP-UP-AUTOMATION-SAFETY-001`.
- `## Requirement Sufficiency` and `## Fast-Lane Eligibility` criterion 3 now substantiate "no new requirement" against those existing wrap-up records — IP-2 implements them rather than creating a requirement.
- `## Spec-To-Test Mapping` adds a row mapping the IP-2 wrap-up-command tests to those three specifications.

The `Claim`, IPs, `Scope`, `Out Of Scope`, `target_paths`, `Files Expected To Change`, the test plan, the acceptance criteria, and the risk/rollback section are unchanged from `-001`.
...
- PB-SESSION-WRAP-UP-PROACTIVE-001 — sessions proactively initiate wrap-up guidance before ending. IP-2 governs `bridge-stop-drain.py` so a drain-block cannot interrupt the wrap-up path when the owner has issued a wrap-up command; the proactive wrap-up emitter and the drain hook are sibling Stop-array hooks and must not work against each other.
- DCL-SESSION-WRAP-UP-AUTOMATION-SAFETY-001 — automatic session lifecycle hooks are non-mutating unless separately authorized. IP-2's wrap-up deference is itself non-mutating: on a wrap-up command the drain hook returns the empty allow-stop result and takes no action, consistent with this constraint.
...
3. **No new requirement** — met. Gap 1 deference is governed by `ADR-SMART-POLLER-OWNER-OUT-OF-LOOP-001` and the `-005` design's deference intent. Gap 2 (wrap-up-command deference) is governed by the existing verified session-lifecycle specifications `GOV-SESSION-LIFECYCLE-PROACTIVE-ENGAGEMENT-001`, `PB-SESSION-WRAP-UP-PROACTIVE-001`, and `DCL-SESSION-WRAP-UP-AUTOMATION-SAFETY-001` — IP-2 makes `bridge-stop-drain.py` honor those existing wrap-up requirements rather than introducing a new one. The owner 2026-05-17 directive clarifies the existing deference requirements; no new GOV/SPEC/PB/ADR/DCL artifact is created.
...
Existing requirements sufficient. Gap 1's owner-decision deference is governed by `ADR-SMART-POLLER-OWNER-OUT-OF-LOOP-001` and the approved `-005` proposal's deference design. Gap 2's wrap-up-command deference is governed by the existing verified session-lifecycle specifications `GOV-SESSION-LIFECYCLE-PROACTIVE-ENGAGEMENT-001`, `PB-SESSION-WRAP-UP-PROACTIVE-001`, and `DCL-SESSION-WRAP-UP-AUTOMATION-SAFETY-001`: those records already require sessions to wrap up proactively and constrain lifecycle hooks to non-mutating behavior, so IP-2 implements existing requirements rather than creating one. The owner 2026-05-17 directive clarifies the existing deference requirements. No new or revised GOV/SPEC/PB/ADR/DCL artifact is required before implementation.
...
| --- | --- |
| ADR-SMART-POLLER-OWNER-OUT-OF-LOOP-001 / DCL-SMART-POLLER-AUTO-TRIGGER-001 | Tests: a pending owner decision of any age suppresses the drain — the drain yields to the owner instead of blocking turn-end, so owner-out-of-loop dispatch does not override active owner engagement. |
| GOV-SESSION-LIFECYCLE-PROACTIVE-ENGAGEMENT-001 / PB-SESSION-WRAP-UP-PROACTIVE-001 / DCL-SESSION-WRAP-UP-AUTOMATION-SAFETY-001 | Tests (IP-2): a wrap-up-command turn-end makes the drain return the empty allow-stop result — proving the Stop-drain yields to an owner-invoked wrap-up (proactive lifecycle engagement / proactive wrap-up) and takes no action on that turn (lifecycle-hook automation safety: non-mutating). The non-wrap-up negative case, the normalization-tolerance cases, and the absent/unparseable-transcript case confirm the deference is scoped to genuine wrap-up commands and fails open. |
...
The applicability preflight (`scripts/bridge_applicability_preflight.py`) and the ADR/DCL clause preflight (`scripts/adr_dcl_clause_preflight.py`) are run against this revised content with `--content-file` before the live `REVISED:` `bridge/INDEX.md` entry is inserted. A non-empty `missing_required_specs` or `missing_advisory_specs` list, or a blocking clause gap, is a self-detected defect corrected before filing. Observed results (run against this `-003` revised content prior to filing): the applicability preflight reported `preflight_passed: true` with `missing_required_specs: []` and `missing_advisory_specs: []` (6 specs matched, all cited); the clause preflight evaluated 5 `must_apply` clauses with 0 evidence gaps and 0 blocking gaps (gate exit 0).
...
1. Confirm the F1 resolution — the three session-lifecycle/wrap-up specifications are now cited, and `Requirement Sufficiency` plus Fast-Lane criterion 3 substantiate "no new requirement" against them.
