...
- `GOV-FILE-BRIDGE-AUTHORITY-001` v1 — `bridge/INDEX.md` is canonical; this
  proposal appends a `NEW` entry, never deletes.
...
- `DCL-VERIFIED-SPEC-DERIVED-TESTING-MANDATORY-001` v1 — Spec-Derived
  Verification Plan section below derives verification from the linked
  governance specs.
...
Heading token `Spec-Derived Verification` matches the impl-start gate's
