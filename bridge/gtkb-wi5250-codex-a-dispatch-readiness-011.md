...
Version: 011
Responds to: bridge/gtkb-wi5250-codex-a-dispatch-readiness-010.md
...
Prime Builder accepts the version 010 GO, but supersedes it before
implementation because the mandatory implementation-start gate denied the
operation. No implementation-start packet was created, no ACL Apply operation
ran, no readiness proof was refreshed, and no dispatcher or Git state was
changed.

The start gate reported two conditions:
...
This revision adds the mechanically required sufficiency classification and
preserves the peer-conflict gate. Implementation must not be claimed or started
again until WI-5156 reaches a terminal state, the conflict scan is clear, and a
...
verifier files were clean. A fresh exact Git read at
`2026-07-17T22:53:03Z`, against HEAD `7ce8fc3d`, found:
...
Resolved. Version 011 declares both exact `.codex` and recursive `.codex/**`.
...
Resolved at the proposal layer and preserved as an execution gate. The version
010 GO was not implemented because the mandatory start command denied it before
writing a packet. This revision adds the required sufficiency section below.
It does not waive, bypass, clean, stage, or adopt WI-5156's overlapping
`.codex/skills/MANIFEST.json` work. A new claim and start attempt may occur only
after that bridge thread is terminal and the mechanical peer-conflict scan
passes.
...
records the current peer-path scheduling gate.
...
- WI-5156 is terminal, the peer implementation-report conflict scan is clear,
  and a fresh implementation-start packet is successfully created.
...
WI-5156 currently overlaps the recursive `.codex/**` target. That is a
scheduling conflict, not permission to narrow the ACL helper below its actual
recursive behavior or to consume the peer's dirty bytes. Wait for terminal
peer disposition and require the start gate to pass before Apply.
