...
# Child Proposal - Session/Activity Envelope Sharding Taxonomy And Global Baseline
...
target_paths: ["config/agent-control/activity-envelope-sharding.toml", "config/agent-control/activity-disposition-profiles.toml", "config/agent-control/SESSION-STARTUP-INDEX.md", "config/agent-control/SESSION-STARTUP-CONTROL-MAP.md", "groundtruth-kb/src/groundtruth_kb/activity/profiles.py", "platform_tests/scripts/test_activity_disposition_profiles.py"]

## Claim

Define the enforceable taxonomy for what belongs in the light global session envelope, what belongs only in an opened activity envelope, and what remains explicit-query/archive-only.

## Scope

- Add a canonical sharding taxonomy/config surface for global baseline, activity-only, explicit-query, and never-startup content classes.
- Make the six existing activity profiles declare enough classification metadata for skills, terminology, history_state, and direction to be audited.
- Update startup control documentation so base startup points to the bounded primer plus activity-profile loading rather than broad rule/skill reads.
...
- `PAUTH-PROJECT-GTKB-SESSION-ACTIVITY-ENVELOPE-SHARDING-WI-4946` - bounded implementation authorization for `WI-4946` only; allowed mutation classes are docs, CLI, tests, skills, and config; forbidden operations are unrelated cleanup, credential lifecycle, and production deployment.
...
| --- | --- |
| `TEST-11251` linked to `SPEC-INTAKE-46594e` | PASS when a classification table or machine-readable equivalent identifies the global baseline, activity-only content, and explicit exclusion criteria for extraneous startup content. |
...
- packet_hash: sha256:8a3f55ea2543a3825a93de734b623bdd4d026c740321bd02ccbbbb91ac396ebf
...
- Operative file: `.gtkb-state\bridge-propose-drafts\session-activity-envelope-sharding\gtkb-envelope-sharding-taxonomy-baseline-001.md`
- Clauses evaluated: 5
...
- `WI-4946` is implemented only within the target paths listed above.
- `TEST-11251` has concrete PASS/FAIL evidence in the implementation report.
