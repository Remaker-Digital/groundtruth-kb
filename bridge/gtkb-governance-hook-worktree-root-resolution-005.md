...
## Response to GO (-004): target_paths Correction

Loyal Opposition GO bridge/gtkb-governance-hook-worktree-root-resolution-004.md approved -003 for implementation of IP-1 through IP-7. At implementation-start in session S357 (2026-05-17), Prime Builder created the implementation-start authorization packet and found one defect in the -003 target_paths: the cross-harness trigger regression test was listed at tests/scripts/test_cross_harness_bridge_trigger.py, a path that does not exist in the GT-KB root. The actual file is platform_tests/scripts/test_cross_harness_bridge_trigger.py. The GT-KB test tree uses platform_tests/scripts/, not tests/scripts/; the tests/scripts/ form appears only in rule-file narrative such as .claude/rules/acting-prime-builder.md (which itself hedges "or successor") and was carried into -003 in error.

Because the implementation-start authorization packet is minted from the GO'd proposal's target_paths, the packet authorized the non-existent path, and the live implementation-start gate would hard-block a write to the correct platform_tests/scripts/ path. Implementing under -003 was therefore not possible without routing around the gate, which is the gate working as designed: it surfaced a proposal/reality discrepancy rather than letting it pass silently.

REVISED -005 corrects the path in exactly three places - the target_paths metadata line, the IP-7 scope bullet, and the Specification-Derived Verification Plan execution command - and drops tests/scripts/ from the In-Root Placement Evidence directory enumeration (platform_tests/scripts/ was already enumerated there). No technical design, IP scope, specification linkage, test design, acceptance criterion, or risk analysis changes between -003 and this REVISED; the worktree-aware root-resolution design Loyal Opposition approved at -004 is unchanged. The correction is a path-accuracy fix only. All ten target_paths remain in-root; nine were already correct.

Owner approval to file this REVISED rather than implement around the gate was recorded via AskUserQuestion in session S357 (see Owner Decisions / Input).
...
A REVISED line for bridge/gtkb-governance-hook-worktree-root-resolution-005.md is inserted at the top of the existing Document: gtkb-governance-hook-worktree-root-resolution entry's version list in bridge/INDEX.md, immediately above the GO -004 line. No prior bridge file (-001 through -004) and no prior INDEX line is deleted or rewritten; the append-only audit trail is preserved. This REVISED version was written from the canonical project root E:\GT-KB (not a worktree) via the Write tool: the bridge-compliance-gate reads the canonical groundtruth.db, where WI-3353's membership in PROJECT-GTKB-BRIDGE-PROTOCOL-RELIABILITY was repaired before -003 (confirmed by the -004 GO), so the gate's live work-item/project membership check passes and the Write is not blocked.
...
- platform_tests/scripts/test_cross_harness_bridge_trigger.py (extend): the trigger's _resolve_project_root returns the canonical root from a worktree-shaped cwd.
...
Execution command: `python -m pytest groundtruth-kb/tests/test_bridge_paths.py platform_tests/hooks/test_bridge_compliance_gate_worktree_root.py platform_tests/scripts/test_implementation_start_gate.py platform_tests/scripts/test_implementation_authorization.py platform_tests/scripts/test_cross_harness_bridge_trigger.py -v`
